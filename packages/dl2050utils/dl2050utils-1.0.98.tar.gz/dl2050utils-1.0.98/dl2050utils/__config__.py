...
def pump_version(version):
      res = get_version_parts(version)
      version2 = f'{res[0]}.{res[1]}.{str(int(res[2])+1)}'
      with open(f'./{package}/__version__.py', 'w') as f: f.write(f'version = "{version2}"')
      return version2