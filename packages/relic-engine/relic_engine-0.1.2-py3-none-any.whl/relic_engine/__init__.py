...
def build_set_data(relic_list):
    ...
    for i in range(len(set_list)):
        ...
        for j in range(len(set_list[i]) - 1):
            parts['parts'][set_list[i][j + 1]] = {'plat': price_info(fix_name(set_list[i][j + 1]), price_data),
        ...
        set_data[set_list[i][0]]['vaulted'] = vaulted_info(set_list[i][0], wfcd_api)
        set_data[set_list[i][0]]['type'] = category_info(set_list[i][0], wfcd_api)
        set_data[set_list[i][0]]['plat'] = price_info(set_list[i][0] + " Set", price_data)
