...
__all__ = ["Array", "SquareArray", "HoneycombArray", "TriangularArray"]
...
class Array:
    ...
    def copy(self):
        n1, n2 = self.get_junction_nodes()
        return Array(EmbeddedGraph(self.graph.x, self.graph.y, n1, n2),
    ...
    def add_nodes_and_junctions(self, x, y, node1, node2,
        ...
        return Array(EmbeddedGraph(new_x, new_y, new_node1, new_node2),
    ...
    def remove_junctions(self, junctions_to_remove):
        ...
        return Array(EmbeddedGraph(self.graph.x, self.graph.y, new_node1, new_node2),
                     critical_current_factors=new_Ic, resistance_factors=new_R,
                     capacitance_factors=new_C, inductance_factors=new_L,
                     matrix_format=self.matrix_format)
    ...
    def set_inductance_factors(self, inductance_factors):
        ...
        from static_problem import lobpcg_test_negative_definite
        self.inductance_factors = inductance_factors
        L = Matrix(inductance_factors, assert_square=True, assert_symmetric=True)
        if not L.is_zero():
            is_positive_definite, _, _ = lobpcg_test_negative_definite(-L.matrix(self._Nj()), None, accept_ratio=50)
    ...
    def _MT(self) -> Matrix:
        ...
        return Matrix(self.cut_matrix_transposed)

    def _MrT(self) -> Matrix:
        if self.cut_matrix_reduced_transposed is None:
            self.cut_matrix_reduced_transposed = self.cut_matrix[:-1, :].T
        return Matrix(self.cut_matrix_reduced_transposed)

    def _Msq(self) -> Matrix:
        if self.cut_square is None:
            self.cut_square = self.cut_matrix @ self.cut_matrix.T
        return Matrix(self.cut_square, assert_square=True)

    def _Mrsq(self) -> Matrix:
        if self.cut_reduced_square is None:
            self.cut_reduced_square = self.cut_matrix[:-1, :] @ self.cut_matrix[:-1, :].T
        return Matrix(self.cut_reduced_square, assert_square=True)

    def _A(self) -> Matrix:
        return Matrix(self.cycle_matrix)

    def _AT(self) -> Matrix:
        if self.cycle_matrix_transposed is None:
            self.cycle_matrix_transposed = self.cycle_matrix.T
        return Matrix(self.cycle_matrix_transposed)

    def _Asq(self) -> Matrix:
        if self.cycle_square is None:
            self.cycle_square = self.cycle_matrix @ self.cycle_matrix.T
        return Matrix(self.cycle_square, assert_square=True)
    ...
    def _has_capacitance(self):
        ...
        C = Matrix(self.capacitance_factors)
        if C.is_scalar:
            if C.A == 0:
                return False
        return True
    ...
    def _has_inductance(self):
        ...
        L = Matrix(self.inductance_factors)
        if L.is_scalar:
            if L.A == 0:
                return False
        return True
