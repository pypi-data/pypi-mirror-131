...
__all__ = ["Plot", "ArrayPlot", "ArrayMovie"]
...
class Plot():
    ...
    def _get_lims(self):
        x, y = self.config.get_array().get_node_coordinates()
    ...
    def _plot_grid(self):
        x1, y1, x2, y2 = self.config.get_array().get_juncion_coordinates()
    ...
    def _plot_faces(self, face_quantity):
        nodes = self.config.get_array().get_faces()
        x, y = self.config.get_array().get_node_coordinates()
    ...
    def _plot_vortices(self, n):
        ...
        xc, yc = self.config.get_array().get_face_centroids()
        ns = np.unique(n)
        vort_handles = []
        for ni in ns:
            if ni != 0:
