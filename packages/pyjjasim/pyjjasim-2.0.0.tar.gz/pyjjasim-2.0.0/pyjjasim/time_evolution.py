...
class TimeEvolutionProblem:
    """
    Define multiple time evolution problems with varying parameters in a Josephson Junction Array circuit.
    ...
    Algorithm parameters:
     - voltage_scheme     "forward"  "forward", "central" or "backward"
    ...
    def __init__(self, array: Array, time_step=0.05, time_step_count=1000,
        ...
        Nj, Nf, W, Nt = self.array._Nj(), self.array._Nf(), self.get_problem_count(), self.get_time_step_count()
        ...
        self.voltage_scheme_name = voltage_scheme
        self.voltage_scheme = self._set_voltage_scheme(self.voltage_scheme_name)
        self.config_at_minus_1 = np.zeros((Nj, W), dtype=np.double) if config_at_minus_1 is None else config_at_minus_1
        if self.array._has_capacitance():
    ...
    def get_static_problem(self, vortex_configuration, problem_nr=0, time_step=0) -> StaticProblem:
        return StaticProblem(self.array, current_sources=self._Is(time_step)[:, problem_nr].copy(),
    ...
    def get_net_sourced_current(self, time_step):
        return 0.5 * np.sum(np.abs((self.get_array()._M().A @ self._Is(time_step))), axis=0)
    ...
    def get_voltage_scheme(self):
        return self.voltage_scheme_name
    ...
    def compute(self) -> TimeEvolutionResult:
        ...
        out = TimeEvolutionResult(self)

        array = self.array
        Nj = array._Nj()
        dt, scheme = self._dt(), self.voltage_scheme
        sm, s0, sp = scheme[0], scheme[1], scheme[2]

        store_th, store_I, store_V = self.store_theta, self.store_current, self.store_voltage

        shape = (array._Nj(), self.get_problem_count())
        A, AT = array._A().A, array._AT().A
        Rv = 1 / (dt * array._R().diag(force_as_vector=True, vector_length=Nj)[:, None])
        Cv = array._C().diag(force_as_vector=True, vector_length=Nj)[:, None] / (dt ** 2)

        theta_next = self.config_at_minus_1

        if array._has_inductance():
            L = self.array._L().matrix(Nj)
            L_sw_fact = scipy.sparse.linalg.factorized(A @ L @ AT)

        if array._has_capacitance():
            theta = self.config_at_minus_2
            Cprev, C0, Cnext = Cv + sm * Rv, -2.0 * Cv + s0 * Rv, Cv + sp * Rv
            Asq_fact = scipy.sparse.linalg.factorized(A @ scipy.sparse.diags(1.0 / Cnext[:, 0], 0) @ AT)
            for i in range(self._Nt()):
                Is, T, theta_s, df = self._Is(i), self._T(i), self._theta_s(i), self._df(i)

                fluctuations = ((2.0 * T * Rv) ** 0.5) * np.random.randn(*shape)
                theta_prev = theta.copy()
                theta = theta_next.copy()
                if array._has_inductance():
                    y = AT @ L_sw_fact(A @ (theta + theta_s + L @ Is) - df)
                    theta_next = -(self._cp(theta) + fluctuations - Is + C0 * theta + Cprev * theta_prev + y) / Cnext
                    if self.store_time_steps[i]:
                        out._update([theta_next if self.store_theta else None,
                                     (theta * s0 + theta_next * sp + theta_prev * sm) / dt if store_V else None,
                                     Is - y if store_I else None])
                else:
                    x = (self._cp(theta) + fluctuations - Is + C0 * theta + Cprev * theta_prev) / Cnext
                    theta_next = -x + (AT @ Asq_fact(A @ (x + theta_s) - df)) / Cnext
                    if self.store_time_steps[i]:
                        out._update([theta_next if self.store_theta else None,
                                     (theta * s0 + theta_next * sp + theta_prev * sm) / dt if store_V else None,
    ...
    def _f(self, time_step) -> np.ndarray:  # (Nf, W), read-only
        return np.broadcast_to(self.frustration(time_step), (self.array._Nf(), self.get_problem_count())) \
            if hasattr(self.frustration, "__call__") else self.frustration[:, :, time_step]

    def _Is(self, time_step) -> np.ndarray: # (Nj, W), read-only
        return np.broadcast_to(self.current_sources(time_step), (self.array._Nj(), self.get_problem_count()))\
    ...
    def _Vs(self, time_step) -> np.ndarray: # (Nj, W), read-only
        return np.broadcast_to(self.voltage_sources(time_step), (self.array._Nj(), self.get_problem_count()))\
    ...
    def _T(self, time_step) -> np.ndarray:  # (Nj, W), read-only
        return np.broadcast_to(self.temperature(time_step), (self.array._Nj(), self.get_problem_count()))\
            if hasattr(self.temperature, "__call__") else self.temperature[:, :, time_step]

    def _df(self, time_step) -> np.ndarray:  # (Nf, W), read-only
        Nf = self.get_array()._Nf()
        area = self.array._area().diag(force_as_vector=True, vector_length=Nf)[:, None]
        if time_step < 0:
            f = self.config_at_minus_2.problem._f() if time_step == -2 else self.config_at_minus_1.problem._f()
            return 2 * np.pi * (-area * f)
        else:
            f = self._f(time_step)
        return 2 * np.pi * (- area * f)
    ...
    def _cp(self, theta) -> np.ndarray:   # (Nj, W)
        # theta -> (Nj, W)
        Ic = self.get_array()._Ic().diag(force_as_vector=True, vector_length=self.get_array()._Nj())[:, None]
...
class TimeEvolutionResult:
    """
    Represents data of simulated time evolution(s) on a Josephson array.

    It is defined by a problem, array and any of the quantities theta, current and voltage.
    ...
    def __init__(self, problem: TimeEvolutionProblem):
        ...
        Nj, W, Nt_s = problem.array._Nj(), self.get_problem_count(), problem._Nt_s()
    ...
    def select_static_configuration(self, prob_nr, time_step) -> StaticConfiguration:
        ...
        problem = StaticProblem(self.get_array(), current_sources=self.problem._Is(time_step)[:, prob_nr],
    ...
    def get_phi(self, select_time_points=None) -> np.ndarray:
        M, Mrsq = self.get_array()._Mr().A, self.get_array()._Mrsq().A
        ...
        return self._select(select_time_points, self.get_array()._Nn(), func)

    def get_theta(self, select_time_points=None) -> np.ndarray:
        return self._select(select_time_points, self.get_array()._Nj(), self._th)

    def get_n(self, select_time_points=None) -> np.ndarray:
        func = lambda tp:  -self.get_array()._A().A @ np.round(self._th(tp) / (2.0 * np.pi))
        return self._select(select_time_points, self.get_array()._Nf(), func).astype(int)

    def get_EJ(self, select_time_points=None) -> np.ndarray:
        func = lambda tp: self.problem._icp(self._th(tp))
        return self._select(select_time_points, self.get_array()._Nj(), func)

    def get_I(self, select_time_points=None) -> np.ndarray:
        return self._select(select_time_points, self.get_array()._Nj(), self._I)
    ...
    def get_J(self, select_time_points=None) -> np.ndarray:
        func = lambda tp: scipy.sparse.linalg.spsolve(self.get_array()._Asq().A,  self.get_array()._A().A @ self._I(tp))
        return self._select(select_time_points, self.get_array()._Nf(), func)

    def get_flux(self, select_time_points=None) -> np.ndarray:
        Nj, Nf = self.get_array()._Nj(), self.get_array()._Nf()
        func = lambda tp: self.get_array()._A().A @ (self.get_array()._L().matrix(Nj) @ self._I(tp))
        return self._select(select_time_points, Nf, func)

    def get_EM(self, select_time_points=None) -> np.ndarray:
        Nj = self.get_array()._Nj()
        func = lambda tp: 0.5 * self.get_array()._L().matrix(Nj) @ (self._I(tp) ** 2)
        return self._select(select_time_points, Nj, func)

    def get_V(self, select_time_points=None):
        return self._select(select_time_points, self.get_array()._Nj(), self._V)
    ...
    def get_EC(self, select_time_points=None):
        Nj = self.get_array()._Nj()
        C = self.get_array()._C().diag(force_as_vector=True, vector_length=Nj)
    ...
    def animate(self, problem_nr=0, time_points=None, show_vortices=True,
        ...
        return ArrayMovie(self, problem_nr=problem_nr, time_points=time_points, show_vortices=show_vortices,
...
class AnnealingProblem:
    ...
    def __init__(self, array: Array, time_step=0.5, time_step_count=10**4,
    ...
    def compute(self):
        ...
        prob = TimeEvolutionProblem(self.array, time_step_count=self.time_step_count, time_step=self.time_step,
...
class AnnealingProblem2:
    ...
    def __init__(self, array: Array, time_step=0.5, interval_steps=10,
    ...
    def compute(self):
        ...
        f = self.frustration * np.ones((1, self.problem_count), dtype=np.double)
        th = np.zeros((self.array.junction_count(), self.problem_count))
        prob = TimeEvolutionProblem(self.array, time_step_count=self.interval_steps, time_step=self.time_step,
