...
from pxpy.functions import StatisticsType
...
class ModelQUBOmixin:
	...
	def export_qubo(self):
		dim = int(self.ysum)
		W = numpy.copy(self.weights)
		
		assert self.stats_type == StatisticsType.overcomplete or self.stats_type is None
		
		W -= numpy.min(W) # shift into positive orthant, requires overcomplete MRF

		penalty = numpy.sum(numpy.sort(W)[-self.graph.edges:])
		Q = numpy.zeros(dim**2).reshape(dim,dim)

		for v in range(self.graph.nodes):
			vi = self.offsets[v]
			for x in range(self.states[v]):
				for y in range(self.states[v]):
					if x != y:
						Q[int(vi+x),int(vi+y)] = penalty
