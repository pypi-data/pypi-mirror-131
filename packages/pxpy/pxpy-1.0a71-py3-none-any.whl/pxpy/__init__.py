...
from pxpy.tools import ModelONNXmixin, ModelQUBOmixin
...
class Graph_incomplete(ctypes.Structure):
	...
	def draw(self,filename,nodenames=None,model=None):
		if self.destroyed:
			return
		import os
		...
		for j in range(len(self.edgelist)):
			...
			if model is not None:
				W = model.slice_edge(j,model.weights)
				w = numpy.linalg.norm(W,ord=numpy.inf)
			dot.edge(str(e[0]), str(e[1]), len=str(w))
...
class Model_incomplete(ModelONNXmixin,ModelQUBOmixin,ctypes.Structure):
	...
	def predict(self, observed, progress_hook = 0, iterations = None, inference = InferenceType.belief_propagation):
		...
		if ext_lib is not None and (inference == InferenceType.external or inference == InferenceType.belief_propagation_gpu):
			EXTINF = ext_lib.external(itype,vtype)
	...
	def sample(self, observed = None, num_samples = None, sampler = SamplerType.gibbs, progress_hook = 0, iterations = None, temperature = 1.0, burn = 100, inference = InferenceType.belief_propagation):
		if self.destroyed:
			return numpy.array([])
		...
		if sampler == SamplerType.perturb_and_map:
			write_register("PAM", integer_from_float(temperature))
			write_register("MIL", iterations)
			write_register("INF", int(inference))
	...
	def infer(self, observed = None, inference = InferenceType.belief_propagation, iterations = None, k = 3, epsilon = 0.000001):
		...
		res = numpy.array(numpy.ctypeslib.as_array(P, shape = (int(self.dimension + self.ysum), )))
		#res.flags.writeable = False
...
def train(data=None, graph=None, mode = ModelType.mrf, inference = InferenceType.belief_propagation, iters = 100, infer_iters=None, seed = 0, k = 3, input_model = None, initial_stepsize = 0.1, opt_progress_hook = 0, progress_hook = 0, regularization = 0, proximal_operator = 0, T = 1, threshold = 0.1, lambda_proximal = 0, lambda_regularization = 0, shared_states = False, optimizer = Optimizer.accelerated_proximal_gradient, zero_init=False, clique_size = 3, states = None, counts = None, num_instances = None, vtype=default_vtype, itype=default_itype, infer_epsilon=0.000001):
	...
	if states is not None:
		...
		L.append("GVX " + str(len(states)) + ";")
		
	if counts is not None and num_instances is not None:
		assert vtype == counts.dtype
		counts_ptr = ctypes.c_uint64(counts.ctypes.data)
		L.append("SSS " + str(counts_ptr.value) + ";")
		L.append("NXX " + str(num_instances) + ";")
