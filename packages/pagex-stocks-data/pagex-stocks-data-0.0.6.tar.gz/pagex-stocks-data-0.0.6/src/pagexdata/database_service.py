...
def read_all_by(entity_class_, query_filter, query_order_by, order_by_desc=False):
    from pagexdata.database import db_session
    if order_by_desc:
        result = db_session.query(entity_class_).filter(*query_filter).order_by(query_order_by.desc()).all()
    else:
        result = db_session.query(entity_class_).filter(*query_filter).order_by(query_order_by).all()
    db_session.close()
    return result
