...
class HBDB:
    ...
    def db2df(self,sql,db='readonly'):
        ...
        data = hbs.db_data_query(db, sql, page_size=5000)
        pages=data['pages']
        data = pd.DataFrame(data['data'])
        if(pages>1):
            for page in range(2,pages+1):
                temp_data= hbs.db_data_query(db, sql, page_size=5000,page_num=page)
    ...
    def extract_factors(self,sec_code,date_list):
        ...
        """.format(','.join(sec_code),"'" + "','".join(date_list) + "'")
        raw_factors = self.db2df(sql, db='alluser')


        return raw_factors

    def extract_benchmark_return(self,date_list):

        sql="""
        select ZQDM,TJYF,HB from funddb.T_FU_GG_ZSHB_M   
        where (ZQDM='000001' or ZQDM='000300' or ZQDM='000012') and TJYF in ({0})
        """.format(','.join([ x[0:6] for x in [''.join(x.split('-')) for x in date_list.astype(str)]]))
        factors_benchmark=self.db2df(sql,'readonly')
