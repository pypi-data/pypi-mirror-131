...
def copy_documents(
    ...
    """The simplest way to copy and escriptorium document.
...
def copy_documents_monitored(
    ...
    while not progress.done:
        print(
            ...
            + Style.BRIGHT
            + Fore.GREEN
            + f"Performing Document Copy, step {progress.main_progress} of {progress.main_complete_total}: {progress.main_msg}"
            + Style.RESET_ALL
        )
...
def copy_documents_generator(
    ...
    dest_region_type_names = [
        x.name for x in (destination_server.get_region_types()).results
    ]
    dest_line_type_names = [
        x.name for x in (destination_server.get_line_types()).results
    ]
    missing_region_types = [
        y for x in src_region_types for y in x if y.name not in dest_region_type_names
    ]
    missing_line_types = [
        y for x in src_line_types for y in x if y.name not in dest_line_type_names
    ]
    current_step = current_step + 1
    yield CopyProgress(
        current_step,
        total_steps,
        0,
        len(missing_region_types) + len(missing_line_types),
        "Creating region and line types on destination",
    )
    for idx, region_type in enumerate(missing_region_types):
        logging.debug(
            f"""Creating region type {region_type.name} on {destination_server.base_url}"""
        )
        new_region_data = PostRegionType(name=region_type.name)
        destination_server.create_region_type(new_region_data)
        yield CopyProgress(
            current_step,
            total_steps,
            idx,
    ...
    for idx, line_type in enumerate(missing_line_types):
        logging.debug(
            f"""Creating line type {line_type.name} on {destination_server.base_url}"""
        )
        new_line_data = PostLineType(name=line_type.name)
        destination_server.create_line_type(new_line_data)
        yield CopyProgress(
            current_step,
            total_steps,
            len(missing_region_types) + idx,
            len(missing_region_types) + len(missing_line_types),
            None,
        )
    ...
    yield CopyProgress(
        ...
        total_steps,
        0,
        sum([len(x) for x in src_region_types]) + sum([len(x) for x in src_line_types]),
        "Linking region/line types to destination documents",
    )
    dest_region_types = (destination_server.get_region_types()).results
    dest_line_types = (destination_server.get_line_types()).results
    tracked_parts_for_deletion: List[Tuple[int, int]] = []
    count = 0
    for src_doc, region_type, line_type in zip(
        source_docs, src_region_types, src_line_types
    ):
        dest_doc = [
            x for x in dest_docs if x.name == src_doc.name and x.project == project_name
        ]
        ...
        dummy_image = Image.new(
            mode="RGB", size=(image_width, image_height), color="black"
        )
        dummy_image_byte_arr = BytesIO()
        dummy_image.save(dummy_image_byte_arr, format="JPEG")
        dummy_image_bytes = dummy_image_byte_arr.getvalue()
        dummy_part_data = PostPart(
            name="dummy", typology=None, source="document copy function"
        )
        dummy_part = destination_server.create_document_part(
            dest_doc.pk, dummy_part_data, "dummy.jpg", dummy_image_bytes
        )
        logging.debug(
            f"""Created dummy part {dummy_part.pk} on destination document {dest_doc.pk}"""
        )
        tracked_parts_for_deletion.append((dest_doc.pk, dummy_part.pk))

        matching_regions = [
            x for x in dest_region_types if x.name in [y.name for y in region_type]
        ]
        for matching_region in matching_regions:
            dummy_region_data = PostRegion(
                document_part=dummy_part.pk,
                external_id="",
                box=[[10, 10], [10, 20], [20, 20], [20, 10], [10, 10]],
                typology=matching_region.pk,
            )
            destination_server.create_document_part_region(
                dest_doc.pk, dummy_part.pk, dummy_region_data
            )
            logging.debug(
                f"""Created region {matching_region.name}, pk {matching_region.pk} on dummy part {dummy_part.pk} on destination document {dest_doc.pk}"""
            )
            yield CopyProgress(
                current_step,
                total_steps,
                count,
                sum([len(x) for x in src_region_types])
                + sum([len(x) for x in src_line_types]),
                None,
            )
            count = count + 1

        blank_region_data = PostRegion(
            document_part=dummy_part.pk,
            external_id="",
            box=[[10, 10], [10, 20], [20, 20], [20, 10], [10, 10]],
            typology=1,
        )
        blank_region = destination_server.create_document_part_region(
            dest_doc.pk, dummy_part.pk, blank_region_data
        )
        matching_lines = [
            x for x in dest_line_types if x.name in [y.name for y in line_type]
        ]
        for matching_line in matching_lines:
            dummy_line_data = PostLine(
                document_part=dummy_part.pk,
                external_id="",
                region=blank_region.pk,
                baseline=[[10, 10], [20, 10]],
                mask=[[10, 5], [10, 15], [20, 15], [20, 5], [10, 5]],
                typology=matching_line.pk,
            )
            destination_server.create_document_part_line(
                dest_doc.pk, dummy_part.pk, dummy_line_data
            )
            logging.debug(
                f"""Created line {matching_line.name}, pk {matching_line.pk} on dummy part {dummy_part.pk} on destination document {dest_doc.pk}"""
            )
            yield CopyProgress(
                current_step,
                total_steps,
    ...
    yield CopyProgress(
        ...
        total_steps,
        0,
        len(tracked_parts_for_deletion),
        "Deleting dummy parts",
    )
    for idx, tracked_part_for_deletion in enumerate(tracked_parts_for_deletion):
        destination_server.delete_document_part(
            tracked_part_for_deletion[0], tracked_part_for_deletion[1]
        )
        logging.debug(
            f"""Deleted part {tracked_part_for_deletion[1]} from document {tracked_part_for_deletion[0]} on destination"""
        )
        yield CopyProgress(
            current_step, total_steps, idx, len(tracked_parts_for_deletion), None
        )
