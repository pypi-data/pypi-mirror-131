...
def execute_expectation_process(
    ...
    try:
        ...
        # First pass to check a failure
        #
        error_occured = False

        for key in global_results.keys():
            if global_results[key]["status"] is False:
                message = "Technical error => {}".format(global_results[key]["result"])
                error_occured = True
            else:
        ...
        if error_occured is True:
            ...
            #
            logging.info("{} Expectation : {}".format(caller_task_id_for_logging, message))
            logging.info("{} Setting task status : failed".format(caller_task_id_for_logging))
            task_infos = {}
            task_infos[task_id] = "failed"
            status_doc_id = firestore_com_id
