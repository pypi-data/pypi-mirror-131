...
from .. import DUAL_HNDLR, HNDLR, LOGS, SUDO_HNDLR, asst, udB, beastx_bot
...
# Inspiration of beastx_cmd decor is from RaphielGang/Telegram-Paperlane
# https://github.com/RaphielGang/Telegram-Paperplane/blob/625875a9ecdfd267a53067b3c1580000f5006973/userbot/events.py#L22


def beastx_cmd(allow_sudo=should_allow_sudo(), **args):
    ...
    def decorator(func):
        ...
        if "official" in type:
            args["outgoing"] = True
            beastx_bot.add_event_handler(doit("official"), events.NewMessage(**args))
            if TAKE_EDITS:
                args["func"] = (
                    lambda x: not (
            ...
            if allow_sudo:
                ...
                beastx_bot.add_event_handler(doit("sudo"), events.NewMessage(**args))
                del args["outgoing"]
