...
def overlay_images(background, foreground, x_position="left", y_position="up", resize=True):
    if resize:
        foreground = background.resize(
            (round(foreground.size[0]), round(foreground.size[1])))
...
def telegram_eval_image_overlay(update: Update, context: CallbackContext, picture, x_position="left", y_position="up", resize=True):
    if resize in ["False", "flase"]:
        resize = False
