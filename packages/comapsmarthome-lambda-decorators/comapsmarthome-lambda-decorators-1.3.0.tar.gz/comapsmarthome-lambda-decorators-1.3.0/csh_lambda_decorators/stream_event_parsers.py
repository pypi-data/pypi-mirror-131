...
def frame_sent_event_parser(event):
    ...
    return serial_number, request_frame, response_frame, {'timestamp': timestamp, 'breadcrumb_id': breadcrumb_id, 'ip_address': ip_address}


def _extract_transmission_data(transmission):
    request_frame = transmission['request_frame']
    frame_type = request_frame['type']
    serial_number = request_frame['t5_frame']['serial_number'] if frame_type == 4 else request_frame['serial_number']
