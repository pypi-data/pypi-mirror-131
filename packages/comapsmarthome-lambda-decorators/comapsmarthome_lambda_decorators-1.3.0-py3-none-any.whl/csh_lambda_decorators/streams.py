...
def process_sns_event(_func=None, *, event_parser=noop_event_parser, batch_records=False):
    def decorator(func):
...
def process_sqs_event(_func=None, *, event_parser=noop_event_parser, batch_records=False):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(event, _context):
            ...
            if batch_records:
                ...
                if len(parsed_commands[0]) == 1:
                    parsed_commands = [parsed_event[0] for parsed_event in parsed_commands]
