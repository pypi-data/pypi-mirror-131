...
def MLAS_with_bias_and_alpha_scaling(
    ...
    accumulated = Array(
        role=Array.Role.TEMP, element_type=A.element_type, shape=(1,))
    ...
    f0, stack_f0, i_f0, j_f0, k_f0 = idxs_f0[0], tuple(
        idxs_f0[1:-3]), *idxs_f0[-3:]
    fused0.reorder(*stack_f0, i_f0, j_f0, f0, k_f0)

    fused1 = fuse((bias_schedule, fused0), partial=len(stack) + 2)
    idxs_f1 = fused1.get_indices()
    f1, stack_f1, i_f1, j_f1, f0_f1, k_f1 = idxs_f1[0], tuple(
        idxs_f1[1:-4]), *idxs_f1[-4:]
...
def MLAS(
        ...
        transA=False, transB=False,
        alpha=1.0, beta=1.0,
        zero_C=False, bias: Array = None, opts=Options(),
        target=Target.HOST):
