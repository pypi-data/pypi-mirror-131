...
def computeOnce(model: Callable[..., Dict], inputs: Dict[str, Any],
    ...
    # TODO attraper une exception si la variable n'appartient pas aux sorties
    #  du modèle
    out = [dico[vars] for vars in outputs]
...
def computeParametric(model: Callable[..., Dict], variable: str,
    ...
    for x in range:
        ...
        dico = {k: v for k, v in res.__iter__()}  # conversion en dictionnaire
        # TODO attraper une exception si la variable n'appartient pas aux
        #  sorties du modèle
        out = [dico[vars] for vars in outputs]
