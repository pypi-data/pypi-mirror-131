...
from bhfutils.crawler.items import RawResponseItem, MenuResponseItem
...
class LoggingBeforePipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if isinstance(item, RawResponseItem):
            ...
            item_copy = dict(item)
            del item_copy['encoding']
            del item_copy['attrs']
            if isinstance(item, MenuResponseItem) and item_copy['status_code'] == 200:
                del item_copy['groupCategoryName']
                del item_copy['groupName']
                del item_copy['groupUrl']
