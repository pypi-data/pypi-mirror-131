# AUTOGENERATED! DO NOT EDIT! File to edit: nbs/00_tests/core/utils.ipynb (unless otherwise specified).

__all__ = ['test_data_io_factory', 'test_profiler', 'test_comparator']
...
def test_comparator ():
    ...
    tr2= MyTransform (different=True)
    result = comp.compare (tr(), tr2())
    print (result)
    assert len(result)>0

    tr2= MyTransform (noise=1e-10)
    result = comp.compare (tr(), tr2())
    assert len(result)==0

    tr2= MyTransform (noise=1e-1)
    result = comp.compare (tr(), tr2())
    print(result)
    assert len(result)>0