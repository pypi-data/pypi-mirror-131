...
public:
  ...
  // Returns a copy of the underlying data_t data as a complex vector
  APP::Vector<std::complex<data_t>> copy_to_vector() const;

  // Moves the data to a complex vector
  APP::Vector<std::complex<data_t>> move_to_vector();
  ...
  void initialize_from_vector(APP::Vector<std::complex<data_t>> &&vec);
...
cdict_t<data_t> QubitVector<data_t>::vector_ket(double epsilon) const {
    return APP::Utils::vec2ket(data_, size(), epsilon, 16);
}

template <typename data_t>
APP::Vector<std::complex<data_t>> QubitVector<data_t>::copy_to_vector() const {
  return APP::Vector<std::complex<data_t>>::copy_from_buffer(data_size_, data_);
}

template <typename data_t>
APP::Vector<std::complex<data_t>> QubitVector<data_t>::move_to_vector() {
  const auto vec = APP::Vector<std::complex<data_t>>::move_from_buffer(data_size_, data_);
  data_ = nullptr;
  return vec;
}
...
template <typename data_t>
void QubitVector<data_t>::initialize_from_vector(APP::Vector<std::complex<data_t>> &&vec) {
  if (data_size_ != vec.size()) {
    std::string error = "QubitVector::initialize input vector is incorrect length (" +
                        std::to_string(data_size_) + "!=" +
                        std::to_string(vec.size()) + ")";
...
double QubitVector<data_t>::expval_pauli(const reg_t &qubits,
  ...
  if (!x_mask) {
    auto lambda = [&](const int_t i, double &val_re, double &val_im)->void {
      ...
      if (z_mask && (APP::Utils::popcount(i & z_mask) & 1)) {
        val = -val;
      }
      val_re += val;
    };
  ...
  auto lambda = [&](const int_t i, double &val_re, double &val_im)->void {
    ...
    for (int_t j = 0; j < 2; ++j) {
      if (z_mask && (APP::Utils::popcount(idxs[j] & z_mask) & 1)) {
        val_re -= vals[j];
      } else {
        val_re += vals[j];
      }
                                         ...
                                         const uint_t z_count,const uint_t z_count_pair,const complex_t initial_phase) const 
{
  ...
  auto lambda = [&](const int_t i, double &val_re, double &val_im)->void {
    ...
    if( (APP::Utils::popcount(idxs[0] & z_mask)+z_count) & 1)
      vals[0] = -vals[0];
    if( (APP::Utils::popcount(idxs[1] & z_mask)+z_count_pair) & 1)
      vals[1] = -vals[1];
...
template <typename data_t>
inline std::ostream &operator<<(std::ostream &out, const APP::QV::QubitVector<data_t>&qv) {
