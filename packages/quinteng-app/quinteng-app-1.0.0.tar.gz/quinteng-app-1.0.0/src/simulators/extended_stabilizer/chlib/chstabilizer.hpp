...
void StabilizerState::X(unsigned q)
{
  ...
  phase += 4*(APP::Utils::hamming_parity((z_string & ~(v)) & s));
  ...
  phase += 4*(APP::Utils::hamming_parity((x_string & v) & s));
...
scalar_t StabilizerState::Amplitude(uint_fast64_t x)
{
  ...
  scalar_t amp;
  amp.e=2*P.e;
  int p = (int) APP::Utils::popcount(v);
...
scalar_t StabilizerState::ProposeFlip(unsigned flip_pos)
{
  ...
  scalar_t amp;
  amp.e=2*Q.e;
  amp.p=-1*(APP::Utils::popcount(v));// each Hadamard gate contributes 1/sqrt(2)
  bool isNonZero=true;
...
void StabilizerState::H(unsigned q)
{
    ...
    unsigned alpha =  APP::Utils::popcount( rowG & (~v) & s );
    unsigned beta =  APP::Utils::popcount( (rowM & (~v) & s) ^ (rowF & v & (rowM ^ s)) );
...
void StabilizerState::MeasurePauli(pauli_t PP)
{
    ...
    for (unsigned j=0; j<n; j++)
        if ((PP.X>>j) & one)
        {
            ...
            R.e+= 2*APP::Utils::popcount(R.Z & rowF); // extra sign from Pauli commutation
            R.Z^=rowM;
            R.X^=rowF;
            R.e+= ((gamma1>>j) & one) + 2*((gamma2>>j) & one);
        }
    for (unsigned q=0; q<n; q++)
          R.Z^=(one<<q)*(APP::Utils::popcount( PP.Z & G[q]) % 2);
    ...
    R.e=(R.e + 2*APP::Utils::popcount(v & R.X & R.Z)) % 4;
    R.X=tempX;
    R.Z=tempZ;

    // now the initial state |s> becomes 0.5*(|s> + R |s>) = 0.5*(|s> + i^b |s ^ R.X>)
    unsigned b = (R.e + 2*APP::Utils::popcount(R.Z & s) ) % 4;
                                       ...
                                       const std::vector<uint_fast64_t>& A)
{
    ...
    for (size_t i=0; i<n; i++)
    {
        ...
        {
            if (APP::Utils::hamming_parity(MT[i] & FT[j]))
            {
                J[i] |= (one << j);
                J[j] |= (one << i);
            }
    ...
    for (size_t i=0; i<n; i++)
    {
        ...
        {
            if (APP::Utils::hamming_parity(col_i & G[j]))
            {
                placeholder[j] |= (one<<i);
            }
        }
    ...
    for (size_t i=0; i<n; i++)
    {
        ...
        for (size_t j=i; j<n; j++)
        {
            if(APP::Utils::hamming_parity(col_i & placeholder[j]))
            {
                K[j] |= shift;
                K[i] |= (one << j);
            }
    ...
    unsigned col=0;
    QuadraticForm q(APP::Utils::popcount(v));
    ...
    for (size_t i=0; i<n; i++)
    {
        if ((v>>i) & one)
        {
            ...
            q.D2 ^= (( ((K_diag2 >> i) ^ (s >> i)) & one)
                     ^ APP::Utils::hamming_parity(K[i] & s)) * shift;
            // q.D2 ^= ((s >> i) & one) * shift;
            // q.D2 ^= APP::Utils::hamming_parity(K[i] & s) * shift;
            unsigned row=0;
            for (size_t j=0; j<n; j++)
            {
                if((v>>j) & one)
    ...
    // Q = 4* (s.v) + sKs
    q.Q = APP::Utils::hamming_parity(s&v)*4;
    for (size_t i=0; i<n; i++)
    {
        if ((s>>i) & one)
        {
