...
"""This module implements the job class used for AppBackend objects."""

import logging

from quinteng.providers import JobV1 as Job
from quinteng.providers import JobStatus, JobError
...
class AppJob(Job):
    """AppJob class for Quinteng App Simulators."""

    def __init__(self, backend, job_id, fn, qobj, executor=None):
        """ Initializes the asynchronous job.

        Args:
            backend(AppBackend): the backend used to run the job.
            ...
            fn(function): a callable function to execute qobj on backend.
                This should usually be a bound :meth:`AppBackend._run()` method,
    ...
    def submit(self):
        ...
        if self._future is not None:
            raise JobError("App job has already been submitted.")
