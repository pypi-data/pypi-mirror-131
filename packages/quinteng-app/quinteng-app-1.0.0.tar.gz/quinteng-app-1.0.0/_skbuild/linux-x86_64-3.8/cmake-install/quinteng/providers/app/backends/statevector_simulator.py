...
"""
Quinteng App statevector simulator backend.
"""
...
class StatevectorSimulator(AppBackend):
    ...
    * ``max_job_size`` (int or None): If the number of run circuits
      ...
      on the executor. If ``None`` simulation of all circuits app submitted
    ...
    * ``fusion_threshold`` (int): Threshold that number of qubits must be greater
    ...
    def __init__(self,
        ...
        warn('The `StatevectorSimulator` backend will be deprecated in the'
             ' future. It has been superseded by the `AppSimulator`'
             ' backend. To obtain legacy functionality initialize with'
             ' `AppSimulator(method="statevector")` and append run circuits'
    ...
    def set_option(self, key, value):
        if key == "method":
            ...
            warn("The method option of the `StatevectorSimulator` has been"
                 " deprecated as of quinteng-app 0.9.0. To run a GPU statevector"
    ...
    def available_methods(self):
        ...
        warn("The `available_methods` method of the StatevectorSimulator"
             " is deprecated as of quinteng-app 0.9.0 as this simulator only"
    ...
    def _validate(self, qobj):
        ...
        if getattr(qobj.config, 'noise_model', None) is not None:
            raise AppError("{} does not support noise.".format(name))
