...
void build_from(const inputdata_t& input, Clifford& clif){
  bool has_keys = APP::Parser<inputdata_t>::check_keys({"stabilizer", "destabilizer"}, input);
  if (!has_keys)
    throw std::invalid_argument("Invalid Clifford JSON.");

  std::vector<std::string> stab, destab;
  APP::Parser<inputdata_t>::get_value(stab, "stabilizer", input);
  APP::Parser<inputdata_t>::get_value(destab, "destabilizer", input);
