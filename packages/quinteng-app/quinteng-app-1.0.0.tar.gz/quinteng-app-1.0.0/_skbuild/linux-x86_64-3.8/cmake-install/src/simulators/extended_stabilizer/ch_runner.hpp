...
private:
  ...
  void init_metropolis(APP::RngEngine &rng);
  void metropolis_step(APP::RngEngine &rng);
  //
...
public:
  ...
  //Routine for Norm Estimation, thin wrapper for the CHSimulator method that uses APP::RngEngine
  //to set up the estimation routine.
  double norm_estimation(uint_t n_samples, uint_t n_repetitions, APP::RngEngine &rng);
  double norm_estimation(uint_t n_samples, uint_t n_repetitions, std::vector<pauli_t> generators, APP::RngEngine &rng);

  //Metropolis Estimation for sampling from the output distribution
  uint_t metropolis_estimation(uint_t n_steps, APP::RngEngine &rng);
  std::vector<uint_t> metropolis_estimation(uint_t n_steps, uint_t n_shots, APP::RngEngine &rng);
  // NE Sampling Routines
  uint_t ne_single_sample(uint_t default_samples, uint_t repetitions, bool preserve_states,
                        const APP::reg_t &qubits, APP::RngEngine &rng);
  ...
  uint_t stabilizer_sampler(APP::RngEngine &rng);
  std::vector<uint_t> stabilizer_sampler(uint_t n_shots, APP::RngEngine &rng);
  //Utilities for the state-vector snapshot.
  complex_t amplitude(uint_t x_measure);
  APP::Vector<complex_t> statevector();
...
double Runner::norm_estimation(uint_t n_samples, uint_t repetitions, APP::RngEngine &rng)
{
...
double Runner::norm_estimation(uint_t n_samples, uint_t repetitions, std::vector<pauli_t> generators, APP::RngEngine &rng)
{
...
uint_t Runner::metropolis_estimation(uint_t n_steps, APP::RngEngine &rng)
{
  init_metropolis(rng);
...
std::vector<uint_t> Runner::metropolis_estimation(uint_t n_steps, uint_t n_shots, APP::RngEngine &rng)
{
...
void Runner::init_metropolis(APP::RngEngine &rng)
{
  accept_ = false;
...
void Runner::metropolis_step(APP::RngEngine &rng)
{
...
uint_t Runner::stabilizer_sampler(APP::RngEngine &rng)
{
  uint_t max = (1ULL << n_qubits_) -1;
  return states_[0].Sample(rng.rand_int(ZERO, max));
}

std::vector<uint_t> Runner::stabilizer_sampler(uint_t n_shots, APP::RngEngine &rng)
{
  if(num_states_ > 1)
  {
...
APP::Vector<complex_t> Runner::statevector()
{
  uint_t ceil = 1ULL << n_qubits_;
  APP::Vector<complex_t> svector(ceil, false);
