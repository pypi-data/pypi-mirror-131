...
void DensityMatrix<data_t>::initialize_from_vector(const list_t &vec) {
  if (BaseVector::data_size_ == vec.size()) {
    ...
    BaseVector::initialize_from_vector(
      APP::Utils::tensor_product(APP::Utils::conjugate(vec), vec));
  } else {
...
void DensityMatrix<data_t>::apply_unitary_matrix(const reg_t &qubits,
  ...
  if (qubits.size() > apply_unitary_threshold_) {
    ...
    BaseVector::apply_matrix(conj_qubits, APP::Utils::conjugate(mat));
  } else {
...
void DensityMatrix<data_t>::apply_diagonal_unitary_matrix(const reg_t &qubits,
                                                          const cvector_t<double> &diag) {
  // Apply as single 2N-qubit matrix mult.
  apply_diagonal_superop_matrix(qubits, APP::Utils::tensor_product(APP::Utils::conjugate(diag), diag));
}
...
double DensityMatrix<data_t>::expval_pauli(const reg_t &qubits,
  ...
  auto lambda = [&](const int_t i, double &val_re, double &val_im)->void {
    ...
    if (z_mask && (APP::Utils::popcount(idx_vec & z_mask) & 1)) {
      val = - val;
    }
    val_re += val;
  };
...
template <typename data_t>
inline std::ostream &operator<<(std::ostream &out, const APP::QV::DensityMatrix<data_t>&m) {
