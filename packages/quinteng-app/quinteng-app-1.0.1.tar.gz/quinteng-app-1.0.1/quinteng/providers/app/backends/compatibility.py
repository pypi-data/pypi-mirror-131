...
def _deprecation_warning(instance, instances_name):
    ...
    warnings.warn(
        ...
        f" a `numpy.ndarray` to a `quinteng.quantum_info.{class_name}` as"
        "of quinteng-app 0.10. Accessing numpy array attributes is deprecated"
...
class Statevector(qi.Statevector):
    """App result backwards compatibility wrapper for quinteng Statevector."""
...
class DensityMatrix(qi.DensityMatrix):
    """App result backwards compatibility wrapper for quinteng DensityMatrix."""
...
class Operator(qi.Operator):
    """App result backwards compatibility wrapper for quinteng Operator."""
...
class SuperOp(qi.SuperOp):
    """App result backwards compatibility wrapper for quinteng SuperOp."""
...
class StabilizerState(qi.StabilizerState):
    """App result backwards compatibility wrapper for quinteng StabilizerState."""

    def __deprecation_warning(self):
        warnings.warn(
            "The return type of saved stabilizers has been changed from"
            " a `dict` to a `quinteng.quantum_info.StabilizerState` as of quinteng-app 0.10."
