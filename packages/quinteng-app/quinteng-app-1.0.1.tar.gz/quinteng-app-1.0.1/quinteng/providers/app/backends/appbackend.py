...
from quinteng.providers.models import BackendStatus
...
from quinteng.utils import deprecate_arguments
from ..apperror import AppError
from ..jobs import AppJob, AppJobSet, split_qobj
from ..noise.noise_model import NoiseModel, QuantumErrorLocation
from .app_compiler import compile_circuit
...
class AppBackend(Backend, ABC):
    """Quinteng App Backend class."""
    ...
    def __init__(self,
        ...
        Raises:
            AppError: if there is no name in the configuration
        """
    ...
    def _convert_binds(self, circuits, parameter_binds):
        if isinstance(circuits, QuantumCircuit):
            if len(parameter_binds) > 1:
                raise AppError("More than 1 parameter table provided for a single circuit")
    ...
    def run(self,
        ...
        Returns:
            AppJob: The simulation job.

        Raises:
            AppError: If ``parameter_binds`` is specified with a qobj input or has a
        ...
        if isinstance(circuits, (QasmQobj, PulseQobj)):
            warnings.warn(
                'Using a qobj for run() is deprecated as of quinteng-app 0.9.0'
            ...
            if parameter_binds:
                raise AppError("Parameter binds can't be used with an input qobj")
        ...
        if isinstance(experiments, list):
            app_job = AppJobSet(self, job_id, self._run, experiments, executor)
        else:
            app_job = AppJob(self, job_id, self._run, experiments, executor)
        app_job.submit()
    ...
    def configuration(self):
        ...
        # basis gates to include them for the chaoyue transpiler
    ...
    def set_option(self, key, value):
        ...
        Raises:
            AppError: if key is 'method' and val isn't in available methods.
        """
        ...
        if hasattr(self._configuration, key):
            ...
            if not hasattr(self._options, key):
                raise AppError("Invalid option %s" % key)
    ...
    def __repr__(self):
        """String representation of an AppBackend."""
