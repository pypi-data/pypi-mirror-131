...
class CalibrationTab:
    def __init__(self, main_gui):
        ...
        self.button = widgets.Button(description="parameters", icon="fa-sliders-h")
        self.button.on_click(self.handle_button_click)
        self.output = widgets.Output()
        self.widget = VBox([HBox([self.dev_config_dir, self.load_calibration]), self.button, self.output])
        self.update()
    ...
    def update(self):
        if self.main_gui.device is not None:
            with self.output:
                ...
                display(CalibrateOD(self.main_gui.device).widget)
