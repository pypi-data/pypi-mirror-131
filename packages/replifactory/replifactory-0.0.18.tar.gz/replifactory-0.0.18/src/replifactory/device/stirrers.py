...
class Stirrers:
    ...
    def connect(self):
        ...
        if self.pwm_controller.port is not None:
            self.multiplexer_port = self.device.i2c.get_port(self.device.PORT_GPIO_MULTIPLEXER_STIRRERS)  # for 3-pin fan speed feedback
            self.multiplexer_port.write_to(6, [0x00])  # all GPIO pins output
            self.multiplexer_port.write_to(7, [0x00])  # all GPIO pins output
    ...
    def get_speed(self, vial):  # TODO
        ...
        duty_cycle = self.pwm_controller.get_duty_cycle(led_number=self.led_numbers[vial])
        ...
        try:
            for vial in range(1, 8):
                if speed == 0:
                    ...
                    duty_cycle = self.device.calibration_fan_speed_to_duty_cycle[vial][speed]
                if duty_cycle < 0.2 and accelerate:
