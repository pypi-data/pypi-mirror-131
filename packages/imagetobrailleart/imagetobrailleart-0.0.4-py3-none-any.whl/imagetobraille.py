...
# Read the image
def brailletoart(image_location = "", size =100, inverse = "n"):
    ...
    for i in range(len(image)):
        ...
        for j in range(len(image[0])):
            # outputart += "   " if image[i, j] < 200 else ".  "
            if b==0:
                if image[i, j] < 200:
                    pixellist1.append(0)
                else:
...
while True:
    try:
        command = input("Enter command (t if text none if not text, image/text name, size, y for inverse and n for non inverse, j for jpg and p for png,): ")
        ...
        if commandlist[0] == "t":
            ...
            textedtoimage(texttoprint)

            brailletoart(texttoprint, int(commandlist[-2]), commandlist[-1])    
            os.remove("images\\{}.jpg".format(texttoprint))
        else:
            brailletoart(commandlist[0], int(commandlist[1]), commandlist[2], commandlist[3])    
