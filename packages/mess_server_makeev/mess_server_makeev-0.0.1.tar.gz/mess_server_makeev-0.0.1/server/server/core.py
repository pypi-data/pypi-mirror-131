...
class MessageProcessor(threading.Thread):
            ...
            if recv_data_lst:
                for client_with_message in recv_data_lst:
                    try:
                        ...
                        logger.debug(
                            'Getting data from client exception.', exc_info=err)
    ...
    def autorize_user(self, message, sock):
        ...
        logger.debug(f'Старт авторизации пользователя {message[USER]}')
        ...
        # Проверяем что пользователь зарегистрирован на сервере.
            ...
            try:
                logger.debug(f'Неизвестный пользователь, посылаем {response}')
                send_message(sock, response)
            except OSError:
                pass
            self.clients.remove(sock)
            ...
            logger.debug('ПКорректный пользователь, Начата проверка пароля.')
