...
from .. import version
from ..settings import get_settings, get_ui_tweaks, CHANGEFORM_TEMPLATES, INLINE_TEMPLATES
from ..utils import (
...
@register.simple_tag
def get_mazer_settings(request: WSGIRequest) -> Dict:
    """
    Get Mazer settings, update any defaults from the request, and return
    """
...
@register.simple_tag
def get_mazer_ui_tweaks() -> Dict:
    """
    Return Mazer ui tweaks
    """
    return get_ui_tweaks()


@register.simple_tag
def mazer_paginator_number(change_list: ChangeList, i: int) -> SafeText:
    """
...
@register.simple_tag
def mazer_list_filter(cl: ChangeList, spec: ListFilter) -> SafeText:
    """
...
@register.simple_tag
def mazer_admin_url(value: Union[str, ModelBase], admin_site: str = "admin") -> str:
    """
...
@register.filter
def has_mazer_setting(settings: Dict[str, Any], key: str) -> bool:
...
def get_sections(
    ...
    if hasattr(admin_form.model_admin, "mazer_section_order"):
        fieldsets = order_with_respect_to(
            fieldsets, admin_form.model_admin.mazer_section_order, getter=lambda x: x.name
        )
...
@register.simple_tag
def get_inline_template(adminform: AdminForm) -> str:
    """
    Go get the correct change form template based on the modeladmin being used,
    the default template, or the overridden one for this modeladmin
    """
    ...
    inline_format = options.get("inline_format", "")
    if model_name in options.get("inline_format_overrides", {}):
        inline_format = options["inline_format_overrides"][model_name]

    if not has_fieldsets and not has_inlines:
        return INLINE_TEMPLATES["single"]

    if not inline_format or inline_format not in INLINE_TEMPLATES.keys():
        return INLINE_TEMPLATES["single"]

    return INLINE_TEMPLATES[inline_format]
