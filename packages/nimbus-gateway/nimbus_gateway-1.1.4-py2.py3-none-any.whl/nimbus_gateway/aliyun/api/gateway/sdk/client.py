...
from .constant import HttpHeader, HttpMethod, SystemHeader, ContentType, HTTP_METHOD_MAP, DEFAULT_TIMEOUT
from .utils import message, signature
...
class DefaultClient:
    ...
    def _build_headers(self, req: Request = None):
        ...
        if http_method in [HttpMethod.POST, HttpMethod.PUT]:
            headers[HttpHeader.HTTP_HEADER_CONTENT_MD5] = message.base64_data(body)
        sign = signature.sign(secret=self.__app_secret, method=http_method, path=req.get_path(),
