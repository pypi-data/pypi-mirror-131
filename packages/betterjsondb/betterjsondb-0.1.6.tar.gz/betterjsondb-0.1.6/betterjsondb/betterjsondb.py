...
class connect:
    ...
    def get(self, key: str):
        ...
        result = {}
        if key == "all":
            exec(f"{get_standarts_read(self.file)}"
    ...
    def push(self, key: str, value, callback: bool = None):
        ...
        exec(f"{get_standarts_read(self.file)}"
             f"data{splitter(key, self.prefix)}={value_class}({value})"
             f"{get_standarts_write(self.file)}")
        if callback:
            return True
    ...
    def update(self, key: str, operator: str, value, callback: bool = None):
        ...
        exec(f"{get_standarts_read(self.file)}"
             f"data{splitter(key, self.prefix)}{operator}{value_class}({value})"
             f"{get_standarts_write(self.file)}")
        if callback:
            return True
