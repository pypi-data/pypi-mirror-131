...
def get_http_session(retry_strategy=None, verify_tls=True):
    # type: (Optional[Retry], bool) -> Session
    session = Session()
...
def get_thread_session(retry, verify_tls):
    # type: (bool, bool) -> Session
    ...
    if retry:
        ...
        new_retry_session = get_http_session(
        ...
        new_session = get_http_session(verify_tls=verify_tls)
...
def get_rest_api_client(
    ...
    low_level_api = LowLevelHTTPClient(
        server_address=server_url,
        default_timeout=config["comet.timeout.api"],
        headers=headers,
        verify_tls=config.get_bool(None, "comet.internal.check_tls_certificate"),
    )
...
def get_optimizer_api_client(api_key, server_address=None, headers=None):
    ...
    low_level_api = LowLevelHTTPClient(
        server_address=server_url,
        default_timeout=config["comet.timeout.predictor"],
        headers=headers,
        verify_tls=config.get_bool(None, "comet.internal.check_tls_certificate"),
    )
