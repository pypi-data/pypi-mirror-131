...
class Task(FriendlyRepr, DynamicClass):
    ...
    def _dynamic_initialization(self, task_parameters: List[ParamValuePair]):
        ...
        class TaskParameters(object):
            ...
            def __setattr__(zelf, k, v):
                if k != "_modified_keys" and getattr(zelf, k, v) != v:
    ...
    def _minified_task_parameters(self) -> TaskParametersDict:
        ...
        for k, v in self._task_parameters_dict.items():
            ...
            def equivalent_to_default(value, default=default_value):
                if any(
                    [value == default, value is default, str(value) == str(default)]
                ):
                    return True

            if equivalent_to_default(v):
                continue
            ...
            if definition.type in ["select", "selectgrid"]:
                ...
                try:
                    ...
                    if index < len(values):
                        value = values[index]
                        if equivalent_to_default(value):
                            continue
                ...
                try:
                    ...
                    if index < len(values):
                        value = values[index]
                        if equivalent_to_default(v, default=value):
                            continue
                except (ValueError, TypeError):
                    pass
