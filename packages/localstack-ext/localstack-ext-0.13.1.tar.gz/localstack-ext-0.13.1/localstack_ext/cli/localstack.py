...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return AOEDW if e else AOEDo
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->AOEDv:
...
@click.option("--revert",is_flag=AOEDW,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:AOEDr):
...
def _cpvcs_initialized(pod_name:AOEDh)->AOEDr:
...
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_init(name:AOEDh):
...
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=AOEDW,default=AOEDo)
def cmd_cpvcs_delete(name:AOEDh,remote:AOEDr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_register_remote(name:AOEDh):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=AOEDW)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=AOEDW)
def cmd_cpvcs_rename(name:AOEDh,new_name:AOEDh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_commit(message:AOEDh,name:AOEDh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=AOEDW,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=AOEDW,default=AOEDo,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_push(squash:AOEDr,message:AOEDh,name:AOEDh,three_way:AOEDr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
@click.option("-v","--version",type=AOEDM)
@click.option("-m","--message",required=AOEDo)
def cmd_cpvcs_push_overwrite(version:AOEDM,message:AOEDh,name:AOEDh):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=AOEDM,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=AOEDW,default=AOEDo,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_inject(version:AOEDM,reset:AOEDr,name:AOEDh):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--inject/--no-inject",default=AOEDW,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=AOEDW,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
@click.option("--lazy/--eager",default=AOEDW,help="Will only fetch references to existing versions, i.e. version state is only downloaded when required")
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:AOEDh,inject:AOEDr,reset:AOEDr,lazy:AOEDr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=AOEDW,default=AOEDo)
def cmd_cpvcs_list_pods(remote:AOEDr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_versions(name:AOEDh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=AOEDW,type=AOEDM)
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_version_info(version:AOEDM,name:AOEDh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=AOEDM,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_version_metamodel(version:AOEDM,name:AOEDh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=AOEDW,type=AOEDM,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=AOEDW,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=AOEDW,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=AOEDo,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_set_version(version:AOEDM,inject:AOEDr,reset:AOEDr,commit_before:AOEDr,name:AOEDh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_commits(version:AOEDM,name:AOEDh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=AOEDW)
@click.option("--commit","-c",required=AOEDW)
@click.option("-n","--name",help="Name of the cloud pod",required=AOEDW)
def cmd_cpvcs_commit_diff(version:AOEDM,commit:AOEDM,name:AOEDh):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(AOEDv)
  status.stop()
...
def print_table(columns:Dict[AOEDh,AOEDh],rows:List[Dict[AOEDh,Any]],formatters:Dict[AOEDh,Callable[[Any],AOEDh]]=AOEDv):
 from rich.table import Table
 if formatters is AOEDv:
  formatters=AOEDC()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  AOEDX(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->AOEDv:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->AOEDv:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->AOEDv:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->AOEDv:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
