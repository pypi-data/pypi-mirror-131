...
SENT_EMAILS=[]
EMAIL_BLACKLIST=fVKUo()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",fVKUw(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=fVKUT,from_name=fVKUT,smtp_host=fVKUT,smtp_user=fVKUT,smtp_pass=fVKUT,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if fVKUi(recipients,fVKUs)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=fVKUT):
 ...
 result["From"]=formataddr((fVKUw(Header(from_name,"utf-8")),from_email))
 return result
