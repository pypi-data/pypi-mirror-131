...
class StateFileLocator(abc.ABC):
 location="_none_"
 active_instance=tlsae
 @abc.abstractmethod
 def get_state_file_location_by_key(self,key:tlsaj)->tlsaj:
  pass
 @tlsaJ
 def get(cls,requested_file_locator:tlsaj):
...
class ObjectStorageProvider(abc.ABC):
 ...
 active_instance=tlsae
 @tlsaJ
 def get(cls,state_file_locator:tlsaj,requested_storage:tlsaj,serializers:Dict[tlsaj,CPVCSSerializer]):
  ...
  return cls.active_instance
 def __init__(self,state_file_locator:StateFileLocator,serializers:Dict[tlsaj,CPVCSSerializer]):
 ...
 def get_state_file_location_by_key(self,key:tlsaj)->tlsaj:
  pass
 @abc.abstractmethod
 def get_version_by_key(self,key:tlsaj)->Version:
  pass
 @abc.abstractmethod
 def get_revision_by_key(self,key:tlsaj)->Revision:
  pass
 @abc.abstractmethod
 def get_revision_or_version_by_key(self,key:tlsaj)->Union[Revision,Version]:
  pass
 @abc.abstractmethod
 def get_delta_file_by_key(self,key:tlsaj)->tlsaj:
  pass
 @abc.abstractmethod
 def upsert_objects(self,*args:CPVCSObj)->List[tlsaj]:
  pass
 @abc.abstractmethod
 def update_revision_key(self,old_key:tlsaj,new_key:tlsaj,referenced_by_version:tlsaj=tlsae):
  pass
 @abc.abstractmethod
 def version_exists(self,key:tlsaj)->tlsaT:
  pass
 @abc.abstractmethod
 def merge_remote_into_local_version(self,remote_location:tlsaj,key:tlsaj):
  pass
 @abc.abstractmethod
 def _update_key(self,old_key:tlsaj,new_key:tlsaj,base_path:tlsaj)->tlsaT:
  pass
 @abc.abstractmethod
 def _serialize(self,*args:CPVCSObj)->List[tlsaj]:
  pass
 @abc.abstractmethod
 def _deserialize(self,key_serializer:Dict[tlsaj,tlsaj],remote_location:tlsaj=tlsae)->List[CPVCSObj]:
  pass
class ObjectStorageLocal(ObjectStorageProvider):
 ...
 def _serialize(self,*args:CPVCSObj)->List[tlsaj]:
  ...
  for cpvcs_obj in args:
   if tlsaU(cpvcs_obj,Version):
    hash_refs.append(self.serializers["version"].store_obj(cpvcs_obj))
   elif tlsaU(cpvcs_obj,Revision):
    hash_refs.append(self.serializers["revision"].store_obj(cpvcs_obj))
  return hash_refs
 def _deserialize(self,key_serializer:Dict[tlsaj,tlsaj],remote_location:tlsaj=tlsae)->List[Union[Revision,Version]]:
