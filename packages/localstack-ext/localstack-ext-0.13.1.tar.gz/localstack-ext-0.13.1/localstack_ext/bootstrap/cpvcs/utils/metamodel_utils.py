...
CAPTURE_METAMODEL_SEPARATELY=suWkt
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=suWkE(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=suWkE(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->suWkc:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[suWkc,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,suWkj)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(suWkj,state_files_to)
...
class CommitMetamodelUtils:
 @suWkC
 def create_metadata_archive(cls,version:Version,delete_reference:bool=suWkt,overwrite:bool=suWkt,metamodels_file:suWkc=suWkj):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @suWkC
 def create_delta_metamodel_file(cls,version:suWka,revision:suWka,store_to_zip=suWkt)->Tuple[Dict,Dict]:
 ...
 @suWkC
 def reconstruct_metamodel(cls,version:suWka,revision:suWka)->Dict:
  result={}
  for rev in suWki(1,revision+1):
  ...
  return result
 @suWkC
 def get_version_metamodel(cls,version:suWka,revision:suWka=suWkj)->Dict:
 ...
 @suWkC
 def get_commit_diff(cls,version_no:suWka,commit_no:suWka)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->suWkg:
 if suWkh(clazz,suWkr):
  return{}
 constructor=suWko(clazz,"__init__",suWkj)
...
def _create_metamodel_helper(obj,width=25,visited:set=suWkj):
 if obj is suWkj:
  return obj
