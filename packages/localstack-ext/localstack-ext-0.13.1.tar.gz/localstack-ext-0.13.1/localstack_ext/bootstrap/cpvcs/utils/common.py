...
class CPVCSConfigContext:
 ...
 def get_version_state_archive_path(self,version,with_format:mlQaD=mlQaY)->mlQaO:
 ...
 def update_ver_log(self,author:mlQaO,ver_no:mlQae,rev_id:mlQaO,rev_no:mlQae):
  with mlQaM(self.get_ver_log_path(),"a")as fp:
   fp.write(f"{VER_LOG_STRUCTURE.format(author=author, ver_no=ver_no, rev_rid_no=f'{rev_id}_{rev_no}')}\n")
 def create_version_symlink(self,name:mlQaO,key:mlQaO=mlQaJ)->mlQaO:
  return self._create_symlink(name,key,self.get_ver_refs_path())
 def create_revision_symlink(self,name:mlQaO,key:mlQaO=mlQaJ)->mlQaO:
  return self._create_symlink(name,key,self.get_rev_refs_path())
 def is_initialized(self)->mlQaD:
  return self.pod_root_dir and os.path.isdir(self.pod_root_dir)
 def _create_symlink(self,name:mlQaO,key:mlQaO,path:mlQaO)->mlQaO:
...
def zip_directories(zip_dest:mlQaO,directories:List[mlQaO])->mlQaO:
 ...
 return zip_dest
def add_file_to_archive(archive:mlQaO,entry_name:mlQaO,content:Union[mlQaO,mlQaI]):
 with zipfile.ZipFile(archive,"a")as zip_file:
  zip_file.writestr(entry_name,content)
def read_file_from_archive(archive_path:mlQaO,file_name:mlQaO)->mlQaO:
 try:
