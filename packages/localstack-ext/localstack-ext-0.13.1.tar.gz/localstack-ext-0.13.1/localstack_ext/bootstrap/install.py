...
def install_package(packages:Union[List[IizDN],IizDN],api_name:IizDN):
 if not(is_debian()and in_docker()):
  ...
  return
 if IizDU(packages,IizDN):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=IizDm)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=IizDm)
def check_or_install(command:IizDN,packages:Union[List[IizDN],IizDN],api:IizDN):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:IizDN,target_file:IizDN,timeout:int=IizDQ):
 def do_download(url,print_error=IizDm):
  try:
