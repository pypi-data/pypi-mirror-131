...
def api_key_configured()->XUCPn:
 return XUCPt if os.environ.get("LOCALSTACK_API_KEY")else XUCPJ
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if XUCPe([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
