...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or tLEkj
 @tLEkM
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return tLEkC(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @tLEkA
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if tLEkJ(origins,tLEkV)else origins.get("Items")
  origins_to_create=tLEks([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if tLEkJ(aliases,tLEkV)else aliases.get("Items")
  aliases_to_create=tLEks(aliases)
  for distr in distrs:
   origins=tLEks([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=tLEks(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",tLEkg)
   config["Enabled"]=tLEkv(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if tLEkJ(custom_errors,tLEkU)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=tLEkv(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":tLEkg}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=tLEkv(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=tLEkj):
    parent=config if parent is tLEkj else parent
    items=parent.get(key)or[]
    if tLEkJ(items,tLEkU)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":tLEkT(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if tLEkJ(cache_behavs,tLEkU)else cache_behavs)
