...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=VYPkx(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not VYPkH
   return result
  except VYPkQ:
   props=self.props
   ...
   if parts[0]in candidates and VYPkG(parts)>1:
