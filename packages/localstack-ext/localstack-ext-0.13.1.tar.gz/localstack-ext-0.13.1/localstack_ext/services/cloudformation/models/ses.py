...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[XvCHf])[0]
 def get_physical_resource_id(self,attribute=XvCHf,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if XvCHq(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return XvCHe(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @XvCHr
 def add_defaults(resource,stack_name:XvCHk):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return XvCHe(SESReceiptRule,self).get_cfn_attribute(attribute)
 @XvCHT
