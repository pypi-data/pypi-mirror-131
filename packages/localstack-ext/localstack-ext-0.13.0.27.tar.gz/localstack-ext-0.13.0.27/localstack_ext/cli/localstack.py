...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return coIAm if e else coIAj
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->coIAi:
...
@click.option("--revert",is_flag=coIAm,help="Revert systemd-resolved settings for the docker interface")
def cmd_dns_systemd(revert:coIAt):
...
def _cpvcs_initialized(pod_name:coIAr)->coIAt:
...
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_init(name:coIAr):
...
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
@click.option("-r","--remote",help="Whether the Pod should also be deleted remotely.",is_flag=coIAm,default=coIAj)
def cmd_cpvcs_delete(name:coIAr,remote:coIAt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_register_remote(name:coIAr):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Current Name of the cloud pod",required=coIAm)
@click.option("-nn","--new-name",help="New name of the cloud pod",required=coIAm)
def cmd_cpvcs_rename(name:coIAr,new_name:coIAr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_commit(message:coIAr,name:coIAr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("--squash",is_flag=coIAm,help="Squashes commits together, so only the latest commit is stored in the revision graph")
@click.option("--three-way",is_flag=coIAm,default=coIAj,help="")
@click.option("-m","--message",help="Add a comment describing the version")
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_push(squash:coIAt,message:coIAr,name:coIAr,three_way:coIAt):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
@click.option("-v","--version",type=coIAz)
@click.option("-m","--message",required=coIAj)
def cmd_cpvcs_push_overwrite(version:coIAz,message:coIAr,name:coIAr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
...
@click.option("-v","--version",default="-1",type=coIAz,help="Loads the state of the specified version - Most recent one by default")
@click.option("--reset",is_flag=coIAm,default=coIAj,help="Will reset the application state before injecting")
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_inject(version:coIAz,reset:coIAt,name:coIAr):
 from localstack_ext.bootstrap import pods_client
 if _cpvcs_initialized(pod_name=name):
  pods_client.inject_state(pod_name=name,version=version,reset_state=reset,pre_config={"backend":"cpvcs"})
@click.option("--inject/--no-inject",default=coIAm,help="Whether the latest version of the pulled pod should be injected")
@click.option("--reset/--no-reset",default=coIAm,help="Whether the current application state should be reset after the pod has been pulled")
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
@cpvcs.command(name="pull",help="Injects the state from a version into the application runtime")
def cmd_cpvcs_pull(name:coIAr,inject:coIAt,reset:coIAt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("--remote","-r",is_flag=coIAm,default=coIAj)
def cmd_cpvcs_list_pods(remote:coIAt):
 from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_versions(name:coIAr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=coIAm,type=coIAz)
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_version_info(version:coIAz,name:coIAr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",type=coIAz,default=-1,help="Latest version by default")
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_version_metamodel(version:coIAz,name:coIAr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-v","--version",required=coIAm,type=coIAz,help="The version the state should be set to")
@click.option("--inject/--no-inject",default=coIAm,help="Whether the state should be directly injected into the application runtime after changing version")
@click.option("--reset/--no-reset",default=coIAm,help="Whether the current application state should be reset before changing version")
@click.option("--commit-before",is_flag=coIAj,help="Whether the current application state should be commited to the currently selected version before changing version")
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_set_version(version:coIAz,inject:coIAt,reset:coIAt,commit_before:coIAt,name:coIAr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_commits(version:coIAz,name:coIAr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
@click.option("--version","-v",required=coIAm)
@click.option("--commit","-c",required=coIAm)
@click.option("-n","--name",help="Name of the cloud pod",required=coIAm)
def cmd_cpvcs_commit_diff(version:coIAz,commit:coIAz,name:coIAr):
 if _cpvcs_initialized(pod_name=name):
  from localstack_ext.bootstrap import pods_client
...
def cmd_pod_list():
 ...
 from localstack_ext.bootstrap import pods_client
 try:
  result=pods_client.list_pods(coIAi)
  status.stop()
...
def print_table(columns:Dict[coIAr,coIAr],rows:List[Dict[coIAr,Any]],formatters:Dict[coIAr,Callable[[Any],coIAr]]=coIAi):
 from rich.table import Table
 if formatters is coIAi:
  formatters=coIAD()
 t=Table()
...
class ConsoleLogger(logging.Logger):
 def __init__(self,name):
  coIAk(ConsoleLogger,self).__init__(name)
 def info(self,msg:Any,*args:Any,**kwargs:Any)->coIAi:
  console.print(msg%args)
 def warning(self,msg:Any,*args:Any,**kwargs:Any)->coIAi:
  console.print("[red]Warning:[/red] ",msg%args)
 def error(self,msg:Any,*args:Any,**kwargs:Any)->coIAi:
  console.print("[red]Error:[/red] ",msg%args)
 def exception(self,msg:Any,*args:Any,**kwargs:Any)->coIAi:
  console.print("[red]Error:[/red] ",msg%args)
  console.print_exception()
