...
def api_key_configured()->YMtnx:
 return YMtnT if os.environ.get("LOCALSTACK_API_KEY")else YMtnu
...
def configure_pro_container(container:LocalstackContainer):
 try:
  ...
  if YMtnz([is_api_enabled(api)for api in EXTERNAL_PORT_APIS]):
