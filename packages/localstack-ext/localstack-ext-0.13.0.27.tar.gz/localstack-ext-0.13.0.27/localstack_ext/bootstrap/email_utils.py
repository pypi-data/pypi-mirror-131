...
SENT_EMAILS=[]
EMAIL_BLACKLIST=FRCSb()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",FRCSf(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=FRCSl,from_name=FRCSl,smtp_host=FRCSl,smtp_user=FRCSl,smtp_pass=FRCSl,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if FRCSg(recipients,FRCSI)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=FRCSl):
 ...
 result["From"]=formataddr((FRCSf(Header(from_name,"utf-8")),from_email))
 return result
