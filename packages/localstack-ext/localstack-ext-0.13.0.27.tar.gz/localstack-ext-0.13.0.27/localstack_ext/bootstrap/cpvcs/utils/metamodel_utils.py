...
CAPTURE_METAMODEL_SEPARATELY=pLfUs
...
class MetamodelDelta(SubtypesInstanceManager):
 def create_delta_for_state_files(self,sf1:StateFileRef,sf2:StateFileRef):
  ...
  if not sf1:
   backend2=load_persisted_object(config_context.get_obj_file_path(sf2.hash_ref))
   clazz=pLfUc(backend2)
   backend1=_infer_backend_init(clazz,sf2)
  else:
   backend1=load_persisted_object(config_context.get_obj_file_path(sf1.hash_ref))
   clazz=pLfUc(backend1)
   backend2=_infer_backend_init(clazz,sf1)
  diff=self.create_delta_json(backend1,backend2)
  return diff
 def create_delta_log(self,state_from:Set[StateFileRef],state_to:Set[StateFileRef])->pLfUz:
  ...
  def _create_sf_lookup(state_files:Set[StateFileRef])->Dict[pLfUz,StateFileRef]:
  ...
  for state_file_from in state_files_from_regular:
   ...
   if state_file_from.any_congruence(state_files_to_regular):
    ...
    diff_json=self.create_delta_for_state_files(state_file_from,pLfUt)
  ...
  for state_files_to in state_files_to_lookup.values():
   ...
   diff_json=self.create_delta_for_state_files(pLfUt,state_files_to)
...
class CommitMetamodelUtils:
 @pLfUX
 def create_metadata_archive(cls,version:Version,delete_reference:bool=pLfUs,overwrite:bool=pLfUs,metamodels_file:pLfUz=pLfUt):
 ...
 def get_metamodel_delta(cls,prev_metamodel:Dict,this_metamodel:Dict)->Dict:
  ...
  return result
 @pLfUX
 def create_delta_metamodel_file(cls,version:pLfUJ,revision:pLfUJ,store_to_zip=pLfUs)->Tuple[Dict,Dict]:
 ...
 @pLfUX
 def reconstruct_metamodel(cls,version:pLfUJ,revision:pLfUJ)->Dict:
  result={}
  for rev in pLfUE(1,revision+1):
  ...
  return result
 @pLfUX
 def get_version_metamodel(cls,version:pLfUJ,revision:pLfUJ=pLfUt)->Dict:
 ...
 @pLfUX
 def get_commit_diff(cls,version_no:pLfUJ,commit_no:pLfUJ)->Optional[Dict]:
  ...
  return result
def _infer_backend_init(clazz:Type,sf:StateFileRef)->pLfUh:
 if pLfUQ(clazz,pLfUg):
  return{}
 constructor=pLfUr(clazz,"__init__",pLfUt)
...
def _create_metamodel_helper(obj,width=25,visited:set=pLfUt):
 if obj is pLfUt:
  return obj
