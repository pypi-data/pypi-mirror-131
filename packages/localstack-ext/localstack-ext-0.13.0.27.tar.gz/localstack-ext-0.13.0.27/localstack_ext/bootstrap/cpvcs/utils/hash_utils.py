...
def random_hash()->MXOSa:
 return MXOSL(random.getrandbits(160))
def compute_file_hash(file_path:MXOSa,accum=MXOSv)->Optional[MXOSa]:
 try:
  with MXOSj(file_path,"rb")as fp:
   if accum:
...
def compute_version_archive_hash(version_no:MXOSf,state_archive:MXOSa)->MXOSa:
 ...
 m.update(MXOSa(version_no).encode("utf-8"))
 rm_rf(tmp_state_dir)
 return m.hexdigest()
def compute_revision_hash(cpvcs_node:Revision)->MXOSa:
 if not cpvcs_node.state_files:
  return random_hash()
 state_file_keys=MXOSR(lambda state_file:state_file.hash_ref,cpvcs_node.state_files)
 m=hashlib.sha1()
 for key in state_file_keys:
  try:
   with MXOSj(os.path.join(config_context.get_obj_store_path(),key),"rb")as fp:
    m.update(fp.read())
  except MXOSp as e:
   LOG.warning(f"Failed to open file and compute hash for {key}: {e}")
 if MXOSG(cpvcs_node,Revision):
  m.update(cpvcs_node.rid.encode("utf-8"))
  m.update(MXOSa(cpvcs_node.revision_number).encode("utf-8"))
 elif MXOSG(cpvcs_node,Version):
  m.update(MXOSa(cpvcs_node.version_number).encode("utf-8"))
 return m.hexdigest()
