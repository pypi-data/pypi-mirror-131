...
class PodInfo:
 def __init__(self,name=KVTkt,pod_size=0):
  self.name=name
  self.pod_size=pod_size
  self.pod_size_compressed=0
  self.persisted_resource_names=[]
class CloudPodManager(KVTkD):
 BACKEND="_none_"
 def __init__(self,pod_name=KVTkt,config=KVTkt):
  self.pod_name=pod_name
  self._pod_config=config
 def init(self):
  raise KVTkx("Not implemented")
 def delete(self,remote:KVTkv)->KVTkv:
  raise KVTkx("Not implemented")
 def push(self,comment:KVTkC=KVTkt,three_way:KVTkv=KVTkM)->PodInfo:
  raise KVTkx("Not implemented")
 def push_overwrite(self,version:KVTkX,comment:KVTkC=KVTkt):
  pass
 def pull(self,inject_version_state:KVTkv=KVTkM,reset_state_before:KVTkv=KVTkM,fetch_all:KVTkv=KVTkt):
  raise KVTkx("Not implemented")
 def commit(self,message:KVTkC=KVTkt):
  raise KVTkx("Not implemented")
 def inject(self,version:KVTkX,reset_state:KVTkv):
  raise KVTkx("Not implemented")
 def list_versions(self)->List[KVTkC]:
  raise KVTkx("Not implemented")
 def version_info(self,version:KVTkX):
  raise KVTkx("Not implemented")
 def version_metamodel(self,version:KVTkX):
  raise KVTkx("Not implemented")
 def set_version(self,version:KVTkX,inject_version_state:KVTkv,reset_state:KVTkv,commit_before:KVTkv):
  raise KVTkx("Not implemented")
 def list_version_commits(self,version:KVTkX)->List[KVTkC]:
  raise KVTkx("Not implemented")
 def get_commit_diff(self,version:KVTkX,commit:KVTkX)->Dict:
  raise KVTkx("Not implemented")
 def register_remote(self,pod_name:KVTkC,ci_pod:KVTkv)->KVTkv:
  raise KVTkx("Not implemented")
 def rename_pod(self,current_pod_name,new_pod_name)->KVTkv:
  raise KVTkx("Not implemented")
 def list_pods(self,fetch_remote:KVTkv)->Set[KVTkC]:
  raise KVTkx("Not implemented")
 ...
 def get(cls,pod_name,pre_config=KVTkt):
  ...
  raise KVTkx('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 def deploy_pod_into_instance(self,pod_path):
  delete_pod_zip=KVTkM
  if os.path.isdir(pod_path):
   ...
   for folder in PERSISTED_FOLDERS:
    ...
    cp_r(src_folder,tgt_folder,rm_dest_on_conflict=KVTkn)
   pod_path=create_zip_file(tmpdir)
   rm_rf(tmpdir)
   delete_pod_zip=KVTkn
  zip_content=load_file(pod_path,mode="rb")
  url=get_pods_endpoint()
  result=requests.post(url,data=zip_content)
  if result.status_code>=400:
   raise KVTkx("Unable to restore pod state via local pods management API %s (code %s): %s"%(url,result.status_code,result.content))
  if delete_pod_zip:
   rm_rf(pod_path)
  else:
   return pod_path
 @KVTkf
 def get_state_zip_from_instance(get_content=KVTkM):
  url=f"{get_pods_endpoint()}/state"
  result=requests.get(url)
  if result.status_code>=400:
   raise KVTkx("Unable to get local pod state via management API %s (code %s): %s"%(url,result.status_code,result.content))
  if get_content:
   return result.content
  zip_file=f"{new_tmp_file()}.zip"
  save_file(zip_file,result.content)
  return zip_file
 def get_pod_info(self,pod_data_dir:KVTkC=KVTkt):
...
class CloudPodManagerCPVCS(CloudPodManager):
 BACKEND="cpvcs"
 @KVTkf
 def parse_pod_name_from_qualifying_name(qualifying_name:KVTkC)->KVTkC:
 ...
 @KVTkf
 def _get_max_version_for_pod_from_platform(pod_name:KVTkC,auth_headers):
  ...
  remote_max_ver=KVTkX(content["max_ver"])
 ...
 def _upload_version_and_product_space(self,presigned_urls):
  ...
  with KVTkU(version_space_archive,"rb")as version_space_content:
  ...
  for version_no,urls in presigned_meta_state_urls.items():
   ...
   with KVTkU(meta_archive,"rb")as meta_archive_content:
   ...
   with KVTkU(state_archive,"rb")as state_archive_content:
 ...
 def _add_state_to_cpvs_store(self):
  ...
  with ZipFile(zip_file,"r")as state_zip:
   ...
   api_states_traverse(api_states_path=api_states_path,side_effect=CloudPodManagerCPVCS._add_state_files_func,mutables=KVTkt)
  rm_rf(zip_file)
  rm_rf(tmp_dir)
 def _pull_versions(self,auth_headers,required_versions:KVTkC):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def delete(self,remote:KVTkv)->KVTkv:
  ...
  if remote:
   pass
  return KVTkM
 def _push_to_remote(self,url:KVTkC):
 ...
 def push_overwrite(self,version:KVTkX,comment:KVTkC=KVTkt):
  ...
  if cpvcs_api.is_remotely_managed():
   ...
   self._push_to_remote(url=url)
  return KVTkn
 def pull(self,inject_version_state:KVTkv=KVTkM,reset_state_before:KVTkv=KVTkM,fetch_all:KVTkv=KVTkn):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  auth_headers=get_auth_headers()
  if self.pod_name in cpvcs_api.list_locally_available_pods(show_remote_or_local=KVTkM):
   cpvcs_api.set_pod_context(self.pod_name)
   ...
   if remote_max_ver==current_max_ver:
    ...
    return
   if fetch_all:
    required_versions=",".join(KVTkG(lambda ver:KVTkC(ver),KVTkw(current_max_ver+1,remote_max_ver+1)))
   else:
 ...
 def inject(self,version:KVTkX,reset_state:KVTkv):
  ...
  tmp_pod_path=cpvcs_api.PodFilePaths.get_version_state_archive(version)
  if not tmp_pod_path:
   LOG.warning(f"Could not find state for pod with version {version}")
   return
  if reset_state:
   reset_local_state(reset_data_dir=KVTkn,exclude_from_reset=["dynamodb","kinesis","stepfunctions"])
 ...
 def version_info(self,version:KVTkX):
  ...
  return version_info
 def version_metamodel(self,version:KVTkX)->KVTkC:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
  if version==-1:
   version=cpvcs_api.get_max_version_no()
  return cpvcs_api.CommitMetamodelUtils.get_version_metamodel(version=version)
 def set_version(self,version:KVTkX,inject_version_state:KVTkv,reset_state:KVTkv,commit_before:KVTkv):
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
  cpvcs_api.set_pod_context(self.pod_name)
 ...
 def get_commit_diff(self,version:KVTkX,commit:KVTkX)->Dict:
  ...
  return commit_diff
 def register_remote(self,pod_name:KVTkC,ci_pod:KVTkv=KVTkM)->KVTkv:
  from localstack_ext.bootstrap.cpvcs import cpvcs_api
 ...
 def list_pods(self,fetch_remote:KVTkv)->Set[KVTkC]:
  ...
  return result
 @KVTkr
 def upload_content(cls,presigned_url:KVTkC,zip_data_content):
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise KVTkx("Unable to upload pod state to S3 (code %s): %s"%(res.status_code,res.content))
  return res
 @KVTkf
 def create_platform_url(request:KVTkC)->KVTkC:
...
class CloudPodManagerManaged(CloudPodManager):
 BACKEND="managed"
 def push(self,comment:KVTkC=KVTkt,three_way:KVTkv=KVTkM)->PodInfo:
  zip_data_content=self.get_state_zip_from_instance(get_content=KVTkn)
  print('Pushing state of cloud pod "%s" to backend server (%s KB)'%(self.pod_name,format_number(KVTkj(zip_data_content)/1000.0)))
  self.push_content(self.pod_name,zip_data_content)
  print("Done.")
  result=self.get_pod_info()
  result.pod_size_compressed=KVTkj(zip_data_content)
  return result
 def pull(self,inject_version_state:KVTkv=KVTkM,reset_state_before:KVTkv=KVTkM,fetch_all:KVTkv=KVTkt):
  ...
  rm_rf(zip_path)
 @KVTkf
 def presigned_url(pod_name:KVTkC,mode:KVTkC)->KVTkC:
  ...
  if response.status_code>=400:
   raise KVTkx("Unable to get cloud pod presigned URL (code %s): %s"%(response.status_code,content))
 ...
 def push_content(cls,pod_name,zip_data_content):
  ...
  res=safe_requests.put(presigned_url,data=zip_data_content)
  if res.status_code>=400:
   raise KVTkx("Unable to push pod state to API (code %s): %s"%(res.status_code,res.content))
  return res
class CloudPodManagerGit(CloudPodManager):
 BACKEND="git"
 def push(self,comment:KVTkC=KVTkt,three_way:KVTkv=KVTkM):
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=KVTkn)
 ...
 def pull(self,inject_version_state:KVTkv=KVTkM,reset_state_before:KVTkv=KVTkM,fetch_all:KVTkv=KVTkt):
  ...
  if remote_ref not in remote_refs:
   raise KVTkx('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get("url")
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=KVTkW(self,"clone_dir",KVTkt)
...
class PodConfigManagerMeta(KVTkp):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=KVTkt
   for manager in cls.CHAIN:
    try:
     tmp=KVTkW(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif KVTkd(tmp,KVTkE)and KVTkd(result,KVTkE):
       result.extend(tmp)
    except KVTkx:
     if LOG.isEnabledFor(logging.DEBUG):
      LOG.exception("error during PodConfigManager call chain")
   if result is not KVTkt:
    return result
   raise KVTkx('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(KVTkD,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @KVTkr
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise KVTkx('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(KVTkD):
 CONFIG_FILE=".localstack.yml"
 def list_pods(self):
  local_pods=self._load_config(safe=KVTkn).get("pods",{})
  local_pods=[{"pod_name":k,"state":"Local Only",**v}for k,v in local_pods.items()]
  existing_names=KVTkh([pod["pod_name"]for pod in local_pods])
  result=[pod for pod in local_pods if pod["pod_name"]not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=KVTkM):
  try:
   return yaml.safe_load(to_str(load_file(self.CONFIG_FILE)))
  except KVTkx:
   if safe:
    return{}
   raise KVTkx('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(KVTkD):
 def list_pods(self):
  result=[]
  auth_headers=get_auth_headers()
  ...
  content=response.content
  if response.status_code>=400:
   raise KVTkx("Unable to fetch list of pods from API (code %s): %s"%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise KVTkx("Unable to store pod metadata in API (code %s): %s"%(response.status_code,content))
...
def init_cpvcs(pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.init()
def delete_pod(pod_name:KVTkC,remote:KVTkv,pre_config:Dict[KVTkC,KVTkC])->KVTkv:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.delete(remote=remote)
 return result
def register_remote(pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],**kwargs)->KVTkv:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 result=backend.register_remote(pod_name=pod_name,ci_pod=pre_config.get("ci_pod",KVTkM))
 return result
def rename_pod(current_pod_name:KVTkC,new_pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],**kwargs):
 ...
 return result
def list_pods_cpvcs(remote:KVTkv,pre_config:Dict[KVTkC,KVTkC],**kwargs)->List[KVTkC]:
 backend=CloudPodManager.get(pod_name="",pre_config=pre_config)
 result=backend.list_pods(fetch_remote=remote)
 return result
def commit_state(pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],message:KVTkC=KVTkt,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.commit(message=message)
def inject_state(pod_name:KVTkC,version:KVTkX,reset_state:KVTkv,pre_config:Dict[KVTkC,KVTkC],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.inject(version=version,reset_state=reset_state)
def list_versions(pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],**kwargs)->List[KVTkC]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 versions=backend.list_versions()
 return versions
def get_version_info(version:KVTkX,pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 info=backend.version_info(version=version)
 return info
def get_version_metamodel(version:KVTkX,pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],**kwargs)->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 metamodel=backend.version_metamodel(version=version)
 return metamodel
def set_version(version:KVTkX,inject_version_state:KVTkv,reset_state:KVTkv,commit_before:KVTkv,pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC],**kwargs)->KVTkv:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 ...
 return success
def list_version_commits(version:KVTkX,pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC])->List[KVTkC]:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commits=backend.list_version_commits(version=version)
 return commits
def get_commit_diff(version:KVTkX,commit:KVTkX,pod_name:KVTkC,pre_config:Dict[KVTkC,KVTkC])->Dict:
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 commit_diff=backend.get_commit_diff(version=version,commit=commit)
 return commit_diff
def push_overwrite(version:KVTkX,pod_name:KVTkC,comment:KVTkC,pre_config:Dict[KVTkC,KVTkC]):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.push_overwrite(version=version,comment=comment)
def push_state(pod_name,pre_config=KVTkt,squash_commits=KVTkM,comment=KVTkt,three_way=KVTkM,**kwargs):
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
...
def pull_state(pod_name,inject_version_state=KVTkM,reset_state_before=KVTkM,**kwargs):
 pre_config=kwargs.get("pre_config",KVTkt)
 if not pod_name:
  raise KVTkx("Need to specify a pod name")
 backend=CloudPodManager.get(pod_name=pod_name,pre_config=pre_config)
 backend.pull(inject_version_state=inject_version_state,reset_state_before=reset_state_before)
 print("Done.")
def reset_local_state(reset_data_dir=KVTkM,exclude_from_reset:List[KVTkC]=KVTkt):
 url=f"{get_pods_endpoint()}/state"
 if reset_data_dir:
  url+="/datadir"
 ...
 if result.status_code>=400:
  raise KVTkx("Unable to reset service state via local management API %s (code %s): %s"%(url,result.status_code,result.content))
 print("Done.")
def list_pods(args):
 return PodConfigManager.list_pods()
def get_data_dir_from_container()->KVTkC:
 try:
...
def get_persisted_resource_names(data_dir)->List[KVTkC]:
 names=[]
 ...
 with os.scandir(os.path.join(data_dir,"api_states"))as entries:
  for entry in entries:
   if entry.is_dir()and KVTkj(os.listdir(entry.path))>0:
    names.append(entry.name)
