...
def install_package(packages:Union[List[GnIaf],GnIaf],api_name:GnIaf):
 if not(is_debian()and in_docker()):
  ...
  return
 if GnIaP(packages,GnIaf):
  packages=packages.split()
 LOG.info("Downloading dependencies for %s API. This may take a while."%api_name)
 with INSTALL_LOCK:
  run(["apt-get","update"],shell=GnIaE)
  run(["apt-get","install","-y","--no-install-recommends"]+packages,shell=GnIaE)
def check_or_install(command:GnIaf,packages:Union[List[GnIaf],GnIaf],api:GnIaf):
...
def setup_ssl_cert():
 ...
 try:
  ...
  try:
   ...
   raise
def download_github_artifact(url:GnIaf,target_file:GnIaf,timeout:int=GnIag):
 def do_download(url,print_error=GnIaE):
  try:
