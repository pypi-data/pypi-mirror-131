...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=HnDPu(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not HnDPW
   return result
  except HnDPE:
   props=self.props
   ...
   if parts[0]in candidates and HnDPJ(parts)>1:
