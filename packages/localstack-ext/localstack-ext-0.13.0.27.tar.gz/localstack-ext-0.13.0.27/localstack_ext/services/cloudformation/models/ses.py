...
class SESTemplate(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return(template or[RwcJu])[0]
 def get_physical_resource_id(self,attribute=RwcJu,**kwargs):
 ...
 def update_resource(self,new_resource,stack_name,resources):
  ...
  if RwcJS(template.get(attr,"")==new_template.get(attr,"")for attr in["SubjectPart","TextPart","HtmlPart"]):
   return
...
class SESReceiptRuleSet(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return RwcJT(SESReceiptRuleSet,self).get_cfn_attribute(attribute)
 @RwcJs
 def add_defaults(resource,stack_name:RwcJg):
...
class SESReceiptRule(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute):
  return RwcJT(SESReceiptRule,self).get_cfn_attribute(attribute)
 @RwcJI
