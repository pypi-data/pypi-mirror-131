...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or qaPxX
 @qaPxe
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return qaPxg(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @qaPxv
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if qaPxN(origins,qaPxp)else origins.get("Items")
  origins_to_create=qaPxI([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if qaPxN(aliases,qaPxp)else aliases.get("Items")
  aliases_to_create=qaPxI(aliases)
  for distr in distrs:
   origins=qaPxI([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=qaPxI(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",qaPxt)
   config["Enabled"]=qaPxo(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if qaPxN(custom_errors,qaPxS)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=qaPxo(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":qaPxt}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=qaPxo(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=qaPxX):
    parent=config if parent is qaPxX else parent
    items=parent.get(key)or[]
    if qaPxN(items,qaPxS)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":qaPxK(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if qaPxN(cache_behavs,qaPxS)else cache_behavs)
