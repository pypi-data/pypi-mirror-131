...
async def chrome_page(url, browser=None):
    if browser is None:
...
class Chrome:
    ...
    async def cart(self, url):
        page, browser = await chrome_page(url)
        await self.login(page)
        await asyncio.sleep(2)
        slider_again = await page.Jeval('div.order-button-children-list > div:nth-child(2) > div', 'node => node.textContent')
        print(slider_again)
        await asyncio.sleep(2)
        await page.click("#sku-count-widget-wrapper > div:nth-child(1) > div:nth-child(2) > span > span > span.next-input-group-addon.next-after > button")
        await asyncio.sleep(2)
        ...
        print(cg)
        # document.querySelector(" div.order-cart-logo > h4 > em")
        time.sleep(30)
    ...
    def start(self, url):
        ...
        loop.run_until_complete(self.cart(url))
