...
class Cookies:
    def __init__(self):
        self.proxies = ProxyGetter(
            'http://svip.kdlapi.com/api/getproxy/?orderid=923885343231575&num=30&protocol=2&method=1&an_ha=1&quality=2&format=json&sep=1',
            'kuaidaili_ip',
        )
    ...
    async def get_content(self, us, url):
        # {
        #         "url": 'http://svip.kdlapi.com/api/getproxy/?orderid=923885343231575&num=30&protocol=2&method=1&an_ha=1&quality=2&format=json&sep=1',
        #         "name": 'kuaidaili_ip',
        #     }

        proxy = [i.replace("https://", '') for i in self.proxies.get_proxies()]
        ...
        browser = await launch({
            ...
            'handleSIGHUP': False,
            'args': {
                f"--proxy-server={random.choice(proxy)}"
            }
