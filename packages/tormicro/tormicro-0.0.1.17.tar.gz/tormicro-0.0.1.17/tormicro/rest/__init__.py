...
from ..doc import cache_rest_doc
...
def config(func, method, *params, **kwparams):
    ...
    if not operation._produces in [mediatypes.APPLICATION_JSON, mediatypes.APPLICATION_XML, mediatypes.TEXT_XML,
                                   mediatypes.TEXT_PLAIN, None]:
        raise PyRestfulException('The media type used do not exist : ' + operation.func_name)

    if operation._func_doc:
        cache_rest_doc(operation._path, operation._method.lower(), operation._func_doc)
...
class RestHandler(tornado.web.RequestHandler):
    ...
    async def _exe(self, method):
        ...
        functions = list(filter(lambda op: hasattr(getattr(self, op), '_service_name') and inspect.ismethod(
            getattr(self, op)), dir(self)))
        ...
        for operation in list(map(lambda op: getattr(self, op), functions)):
            ...
            if operation._method == self.request.method and service_name == services_from_request and len(
                ...
                response = await operation(*p_values)
