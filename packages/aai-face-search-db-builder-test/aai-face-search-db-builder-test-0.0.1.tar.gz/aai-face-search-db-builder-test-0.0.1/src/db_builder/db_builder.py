...
class BaseDbBuilder(metaclass=abc.ABCMeta):
    def __init__(self, mode: str, location: str, db_build_url: str):
        self.mode = mode
...
class DirDbBuilder(BaseDbBuilder):
    def __init__(self, location: str, db_build_url: str):
        super().__init__(MODE_DIRECTORY, location, db_build_url)
...
class S3DbBuilder(BaseDbBuilder):
    def __init__(self, location: str, db_build_url: str, access_key: str, secret: str, bucket: str):
        super().__init__(MODE_S3, location, db_build_url)
...
class OssDbBuilder(BaseDbBuilder):
    def __init__(self, location: str, db_build_url: str, access_key: str, secret: str, bucket: str, endpoint: str):
        super().__init__(MODE_OSS, location, db_build_url)
...
def run():
    ...
    if args.mode == MODE_DIRECTORY:
        db_builder = DirDbBuilder(args.location, args.db_build_url)
    elif args.mode == MODE_S3:
        db_builder = S3DbBuilder(args.location, args.db_build_url, args.access_key, args.secret, args.bucket)
    elif args.mode == MODE_OSS:
        db_builder = OssDbBuilder(
            args.location, args.db_build_url, args.access_key, args.secret, args.bucket, args.endpoint
        )
