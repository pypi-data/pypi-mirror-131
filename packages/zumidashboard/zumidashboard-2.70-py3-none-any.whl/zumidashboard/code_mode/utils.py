...
def is_lesson_completed(title):
    ...
    if not os.path.exists(current_user_dir + '.code_editor_lesson.json'):
        print('no json file')
        generate_code_editor_lesson_json(current_user_dir)
        return False
    with open(current_user_dir + '.code_editor_lesson.json', 'r') as json_file:
...
def generate_code_editor_lesson_json(current_user_dir):
    ...
    if not os.path.isdir(lesson_dir_path):
        ...
        return
    with open("{}/.code_editor_lesson.json".format(current_user_dir), "w") as f:
        json_data = dict()
...
def mark_lesson_as(title, completed):
    current_user_dir = usr_dir + current_app.config['USER'] + '/'
    complete_data = {}
    with open("{}/.code_editor_lesson.json".format(current_user_dir), "r") as f:
...
def get_lesson_image(file_name):
    file_path = '/home/pi/Dashboard/user/' + current_app.config['USER'] + '/Zumi_Content_en/Data/images/' + file_name
    try:
        with open(file_path, 'rb') as f:
            b = base64.b64encode(f.read())
            blob_image = b.decode('utf-8')
