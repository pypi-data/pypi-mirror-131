...
class KeyDiscoveryAgent(Agent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._store_path = os.path.join(
            os.environ["VOLTTRON_HOME"], "external_platform_discovery.json"
        )
    ...
    def startup(self, sender, **kwargs):
        ...
        if self._setup_mode:
            ...
            delay = get_aware_utc_now() + timedelta(seconds=sec)
    ...
    def _send_to_router(self, op, platform_info):
        ...
        try:
            ...
            # Try sending later
            _log.error(
                "ZMQ error while sending external platform info to router: {}".format(
                    ex
                )
            )
    ...
    def _get_platform_discovery(self, web_address):
        ...
        try:
            ...
            raise DiscoveryError("Invalid discovery response from {}".format(real_url))
        except requests.exceptions.Timeout:
            raise DiscoveryError("Timeout error from {}".format(real_url))
        except AttributeError as e:
            raise DiscoveryError("Invalid web_address passed {}".format(web_address))
        except (ConnectionError, NewConnectionError) as e:
            raise DiscoveryError("Connection to {} not available".format(real_url))
        except Exception as e:
            raise DiscoveryError("Unknown Exception: {}".format(e))
