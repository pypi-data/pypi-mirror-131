...
def test_model(filepath):
    ...
    if torch.cuda.is_available():
        net = torch.load('model.py', map_location='cuda')
        net = net.to(device)
        torch.no_grad()
        img = Im.open(filepath)
        ...
        net = torch.load('model.py', map_location='cpu')
        net = net.to(device)
        torch.no_grad()
        img = Im.open(filepath)
        img = transform(img).unsqueeze(0)
...
a_file=os.path.exists('model.py')
if a==0:
    if a_file == True:
        a=a+1
    else:
        try:
            os.rename('model.py','model.py')
            a=a+1
        except:
            tkinter.messagebox.showerror('错误', 'model.py文件缺失，重新安装本程序可能会解决问题')
            a=a+1
