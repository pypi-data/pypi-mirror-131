...
'''
def check_files():
    a=os.path.exists('model.dll')
    if a == True:
        tkinter.messagebox.showinfo('提示','所有文件都可以被找到')
    else:
        tkinter.messagebox.showerror('错误','model.dll文件缺失，重新安装本程序可能会解决问题')
...
def fix_files():
    ...
    if a == True:
        ...
        tkinter.messagebox.showerror('错误', 'model.dll文件缺失,并且无法被修复，也可以点击下方的按钮训练，重新安装本程序可能会解决问题')
...
def Test_model_old():
    ...
    if a == True:
        ...
        tkinter.messagebox.showerror('错误', 'model.dll文件缺失,并且无法被修复，重新安装本程序可能会解决问题，或者是使用下方的训练按钮训练，点击确定以继续')
    if b==1 :
...
'''
@ the code below is about the the visualize tools
@ this code was writen by lry
@ the version of this function is version 1.0.0
@ time : 2021.11.10 
'''
def visualize_tools():
