...
'''
def check_files():
    a=os.path.exists('C:\\model_lry\\model.py')
    if a == True:
...
def fix_files():
    ...
    if a == True:
        ...
        tkinter.messagebox.showerror('错误', 'C:\\model_lry\\model.py文件缺失,并且无法被修复，也可以点击下方的按钮训练，重新安装本程序可能会解决问题')
...
def Test_model_old():
    ...
    if a == True:
        ...
        tkinter.messagebox.showerror('错误', 'C:\\model_lry\\model.py文件缺失,并且无法被修复，重新安装本程序可能会解决问题，或者是使用下方的训练按钮训练，点击确定以继续')
    if b==1 :
...
def test_model(filepath):
    ...
    if torch.cuda.is_available():
        net = torch.load('C:\\model_lry\\model.py', map_location='cuda')
        net = net.to(device)
        torch.no_grad()
        img = Im.open(filepath)
        ...
        net = torch.load('C:\\model_lry\\model.py', map_location='cpu')
        net = net.to(device)
        torch.no_grad()
        img = Im.open(filepath)
        img = transform(img).unsqueeze(0)
...
def train():
    ...
    torch.save(model, 'C:\\model_lry\\model.py')
    print('loading model......')
...
a_file=os.path.exists('C:\\model_lry\\model.py')
if a==0:
    if a_file == True:
        a=a+1
    else:
