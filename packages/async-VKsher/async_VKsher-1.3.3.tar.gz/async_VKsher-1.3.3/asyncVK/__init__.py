...
class Bot:
    ...
    async def send_event(self, event):
        tasks = [asyncio.create_task(middleware.pre(get_dispatcher_by_event(self.token, event, event["type"]), self.reject))
        ...
        tasks = [asyncio.create_task(middleware.post(get_dispatcher_by_event(self.token, event, event["type"])))
                 for middleware in self.middlewares if middleware.event_type in (event["type"], None)]
        await asyncio.gather(*tasks)
