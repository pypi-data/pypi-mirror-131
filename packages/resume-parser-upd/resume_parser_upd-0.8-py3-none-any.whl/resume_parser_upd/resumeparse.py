...
class resumeparse(object):
    ...
    def calculate_experience(resume_text):
        ...
        while regex_result:
          ...
          try:
            ...
            #print(re.search('\n.{4}' + date_range_1,resume_text_1))
            #print(bool(re.search('\n.{4,}' + date_range_1,resume_text_1)))
            if(re.search('\n.{4,}' + date_range_1,resume_text_1)):
                job.append(re.search('(?<=\n).*(?=' + date_range_1 + ')',resume_text_1).group())
            else:
                job.append(re.search('^([^._^%$#!~@,-]+)', resume_text).group())

            job.append(date_range_1)
        ...
        return [end_year - start_year ,job] # Use the obtained month attribute
    ...
    def get_experience(resume_segments):
        total_exp = 0
     #   #print(resume_segments['work_and_employment'])
    ...
    def get_uni(resume_segments):
        total_exp = 0
        #   #print(resume_segments['work_and_employment'])
        if len(resume_segments['education_and_training'].keys()):
            text = ""
            for key, values in resume_segments['education_and_training'].items():

                #values.remove('EDUCATION')
                #values.remove('ADDITIONAL')
                text += "\n ".join(values) + " "

            total_exp = resumeparse.extract_uni(text)
    ...
    def extract_uni(resume_text):
        ...
        if(regex_result):
           a = 1
            ...
            regex_result = re.search('\d{4}', resume_text)
            date_range = regex_result.group()
            replace = False
        job = []
        elems = []
        ...
        if(replace):
            while regex_result:
                ...
                try:
                    ...
                    if (re.search('\n.{4}' + date_range_1, resume_text_1)):
                        ...
                        for val in resStr:
                            ...
                            if([ele for ele in cs.EDUCATION if (ele in val)]):
                                degree.append(val)

                            if ([ele for ele in cs.UNIVERSITY if (ele in val)]):
                                university.append(val)


                            #res2 = [ele for ele in cs.EDUCATION if (ele in val)]
                            ##print(res2)
                    job.append(university)
                    job.append(degree)
             ...
             degree = []
             elems = []
             job.append(date_range)

             if (re.search('\n.{4,}' + date_range, resume_text)):

                 elems.append(re.search('(?<=\n).*(?=' + date_range + ')', resume_text).group())

                 elems.append(re.search('(?<=' + date_range + ')' + '.*?[._^%$#!~@,-]', resume_text,re.DOTALL).group())
...
#
     ...
     custom_nlp_text = custom_nlp(nlp_text)
    ...
    def read_file(file,docx_parser = "tika"):
       ...
       # #print(resume_segments)
        ...
        university = resumeparse.get_uni(resume_segments)
        ...
        languages = []
        for val in skills:
            if ([ele for ele in cs.LANGUAGES if (ele in val)]):
