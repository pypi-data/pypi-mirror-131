...
class TestCompat(unittest.TestCase):
    def test_compat_getenv(self):
        test_str = 'тест'
        compat_setenv('nextdl_COMPAT_GETENV', test_str)
        self.assertEqual(compat_getenv('nextdl_COMPAT_GETENV'), test_str)

    def test_compat_setenv(self):
        test_var = 'nextdl_COMPAT_SETENV'
        test_str = 'тест'
    ...
    def test_all_present(self):
        import nextdl.compat
        all_names = nextdl.compat.__all__
        present_names = set(filter(
            lambda c: '_' in c and not c.startswith('_'),
            dir(nextdl.compat))) - set(['unicode_literals'])
