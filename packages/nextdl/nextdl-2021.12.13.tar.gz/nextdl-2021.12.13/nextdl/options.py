...
def parseOpts(overrideArguments=None):
    def _readOptions(filename_bytes, default=[]):
        ...
        try:
            # FIXME: https://github.com/ytdl-org/nextdl/commit/dfe5fa49aed02cf36ba9f743b11b0903554b5e56
    ...
    def _readUserConf():
        ...
        if xdg_config_home:
            userConfFile = os.path.join(xdg_config_home, 'nextdl', 'config')
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(xdg_config_home, 'nextdl.conf')
        else:
            userConfFile = os.path.join(compat_expanduser('~'), '.config', 'nextdl', 'config')
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(compat_expanduser('~'), '.config', 'nextdl.conf')
        ...
        if userConf is None:
            ...
            if appdata_dir:
                userConf = _readOptions(
                    os.path.join(appdata_dir, 'nextdl', 'config'),
                    default=None)
                if userConf is None:
                    userConf = _readOptions(
                        os.path.join(appdata_dir, 'nextdl', 'config.txt'),
                        default=None)

        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser('~'), 'nextdl.conf'),
                default=None)
        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser('~'), 'nextdl.conf.txt'),
                default=None)
    ...
    general.add_option(
        ...
        help='Use this prefix for unqualified URLs. For example "gvsearch2:" downloads two videos from google videos for nextdl "large apple". Use the value "auto" to let nextdl guess ("auto_warning" to emit a warning when guessing). "error" just throws an error. The default value "fixup_error" repairs broken URLs, but emits an error if this is not possible instead of searching.')
    general.add_option(
        '--ignore-config',
        action='store_true',
        help='Do not read configuration files. '
        'When given in the global configuration file /etc/nextdl.conf: '
        'Do not read the user configuration in ~/.config/nextdl/config '
        '(%APPDATA%/nextdl/config.txt on Windows)')
    general.add_option(
        '--config-location',
    ...
    authentication.add_option(
        ...
        help='Account password. If this option is left out, nextdl will ask interactively.')
    authentication.add_option(
        '-2', '--twofactor',
    ...
    adobe_pass.add_option(
        ...
        help='Multiple-system operator account password. If this option is left out, nextdl will ask interactively.')
    adobe_pass.add_option(
        '--ap-list-mso',
    ...
    verbosity.add_option(
        ...
        help='Contact the nextdl server for debugging')
    verbosity.add_option(
        '--no-call-home',
        dest='call_home', action='store_false', default=False,
        help='Do NOT contact the nextdl server for debugging')
    ...
    filesystem.add_option(
        ...
        help='Force resume of partially downloaded files. By default, nextdl will resume downloads if possible.')
    filesystem.add_option(
        '--no-continue',
    ...
    filesystem.add_option(
        ...
        help='Location in the filesystem where nextdl can store some downloaded information permanently. By default $XDG_CACHE_HOME/nextdl or ~/.cache/nextdl . At the moment, only YouTube player files (for videos with obfuscated signatures) are cached, but that may change.')
    filesystem.add_option(
    ...
    if overrideArguments is not None:
        ...
        if '--config-location' in command_line_conf:
            ...
            if os.path.isdir(location):
                location = os.path.join(location, 'nextdl.conf')
            ...
            system_conf = _readOptions('/etc/nextdl.conf')
