...
from .utils import (
    ...
    write_string,
    nextdlCookieJar,
    nextdlCookieProcessor,
    nextdlHandler,
    nextdlRedirectHandler,
)
...
class nextdl(object):
    ...
    nextdl objects are the ones responsible of downloading the
    ...
    For this, nextdl objects have a method that allows
    InfoExtractors to be registered in a given order. When it is passed
    a URL, the nextdl object handles it to the first InfoExtractor it
    finds that reports being able to handle it. The InfoExtractor extracts
    all the information about the video or videos the URL refers to, and
    nextdl process the extracted information, possibly using a File
    Downloader to download the video.

    nextdl objects accept a lot of parameters. In order not to saturate
    the object constructor with arguments, it receives a dictionary of
    options instead. These options are available through the params
    attribute for the InfoExtractors to use. The nextdl also
    ...
    The following parameters are not used by nextdl itself, they are used by
    ...
    def print_debug_header(self):
        ...
        self._write_string('[debug] nextdl version ' + __version__ + '\n')
        if _LAZY_LOADER:
    ...
    def _setup_opener(self):
        ...
        if opts_cookiefile is None:
            ...
            self.cookiejar = nextdlCookieJar(opts_cookiefile)
            if os.access(opts_cookiefile, os.R_OK):
                self.cookiejar.load(ignore_discard=True, ignore_expires=True)

        cookie_processor = nextdlCookieProcessor(self.cookiejar)
        ...
        def file_open(*args, **kwargs):
            raise compat_urllib_error.URLError('file:// scheme is explicitly disabled in nextdl for security reasons')
