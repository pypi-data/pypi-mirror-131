...
def __metafile(filename):
    here = os.path.abspath(os.path.dirname(__file__))
    filepath = os.path.join(here, 'meta', filename)
    with open(filepath) as f:
        return f.read().strip()


name = __metafile('PACKAGE_NAME')
version = __metafile('PACKAGE_VERSION')
python_requires = __metafile('PYTHON_VERSION')
