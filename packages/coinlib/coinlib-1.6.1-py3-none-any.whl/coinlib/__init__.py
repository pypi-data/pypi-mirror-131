...
def connectCoinlib(hostname=None, workspaceId=None, worker_modules=["chart", "stats", "data", "statsMethod", "spreadsheet", "notification"], reconnect=False):
    if registrar.connected and not reconnect:
        return

    if not registrar.hasEnvironment() or registrar.environment == "dev":
