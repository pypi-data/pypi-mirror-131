...
class Simulator:
    ...
    def __createChannel(self):
        ...
        return grpc.insecure_channel(self._registrar.get_coinlib_backend_grpc(), options=chan_ops,
    ...
    def testBroker(self, broker_id: str, broker_login_data: BrokerTestLoginData):
        ...
        print("We are testing your symbol broker - but this can take 4-5 minutes - we´ll send you some infos when finished.");
        ...
        exit = False
        if info.success:
            print("Your symbol Broker does work, read message after!")

            print(info.simulatorResponseText+"\r\n")

            iframe_src = 'http://localhost:3000/simulator/broker/' + info.workspaceId + "/" + get_current_kernel(
            ...
            if in_ipynb():
                ...
                # printing iframe:
                print("Broker works - here is your Testframe: " + iframe_src)

        else:
            exit = True
            print("ERROR! Your symbol Broker does not work, read message after!")
            print(info.simulatorResponseText)
    ...
    def simulateFeature(self, identifier, options=None):
        ...
        print("We are testing your Feature Class. When finished we will report the data to you.")
        print("  => Please beware: When you have a lake Feature - we will start it and then please trigger some data inputs. ")
        print("  => This process will finish when any data is inserted into the data storage and we will return the info to you")
        ...
        exit = False
        if info.success:
            print("Your Feature system works fine!!")
        else:
            exit = True
            print("ERROR! Your Feature service does not work, read message after!")
    ...
    def testNotification(self, identifier, channels, inputs):
        ...
        print("We are testing your Notification and sending two messages to it. Please click on the second one to finish the test")
        ...
        exit = False
        if info.success:
            print("Your Notification system works fine!!")
        else:
            exit = True
            print("ERROR! Your Notification service does not work, read message after!")
    ...
    def statsMethod(self, methods, name="simulatorDefaultWorkspace"):
        ...
        iframe_src = 'http://localhost:3000/simulator/statsMethod/' + info.workspaceId + "/" + get_current_kernel(self.workerJobs.workerId)
        if in_ipynb():
            ...
            # we are in anything of?????
            # printing iframe:
            print("Your Testframe: "+iframe_src)
    ...
    def statsRule(self, name="simulatorDefaultWorkspace"):
        ...
        iframe_src = 'http://localhost:3000/simulator/stats/' + info.workspaceId + "/" + get_current_kernel(self.workerJobs.workerId)
        if in_ipynb():
    ...
    def chart(self, broker, symbol, timeframe, indicators):
        ...
        iframe_src = 'http://localhost:3000/simulator/chart/' + info.workspaceId + "/" + get_current_kernel(self.workerJobs.workerId)
        if in_ipynb():
