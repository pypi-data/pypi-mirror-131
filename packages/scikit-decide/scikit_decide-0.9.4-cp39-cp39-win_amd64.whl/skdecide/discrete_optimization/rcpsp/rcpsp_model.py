...
from skdecide.discrete_optimization.generic_tools.graph_api import Graph
...
class RCPSPSolution(Solution):
    ...
    def generate_schedule_from_permutation_serial_sgs(self):
        ...
        for res in list(self.problem.resources.keys()):
            if isinstance(self.problem, RCPSPModelCalendar):
                ...
                resource_avail_in_time[res] = np.full(
                    new_horizon, self.problem.resources[res], dtype=int
                ).tolist()
        # 3
        ...
        while len(perm_extended) > 0 and not unfeasible_non_renewable_resources:
            ...
            while not valid:  # 7
                ...
                for t in range(
                    current_min_time,
                    current_min_time
                    + self.problem.mode_details[act_id][modes_extended[act_id - 1]][
                        "duration"
                    ],
                ):  # 9
                    for res in resource_avail_in_time.keys():  # 10
                        if t < new_horizon:
                            if (
                                ...
                                < self.problem.mode_details[act_id][
                                    modes_extended[act_id - 1]
                                ][res]
                            ):  # 11
                                valid = False  # 12
                        else:
        ...
        for act_id in activity_end_times:
            ...
            self.rcpsp_schedule[act_id]["start_time"] = (
                activity_end_times[act_id]
                - self.problem.mode_details[act_id][modes_extended[act_id - 1]][
                    "duration"
                ]
    ...
    def generate_schedule_from_permutation_serial_sgs_2(
        self, current_t, completed_tasks, scheduled_tasks_start_times
    ):
        perm = deepcopy(self.rcpsp_permutation)
        activity_end_times = {}
        ...
        all_activities = [
            x
            for x in list(self.problem.resources.keys())
            if x not in list(completed_tasks)
        ]
        ...
        if rcpsp_sol.rcpsp_schedule_feasible is False:
            ...
            for t in range(makespan):
                ...
                for act_id in list(self.successors.keys()):
                    ...
                    for (
                        res
                    ) in self.resources.keys():  # self.mode_details[act_id][mode]:
    ...
    def compute_resource_consumption(self, rcpsp_sol: RCPSPSolution):
        ...
        for act_id in rcpsp_sol.rcpsp_schedule:
            for ir in range(len(self.resources)):
                use_ir = self.mode_details[act_id][modes_extended[act_id - 1]][
...
class MultiModeRCPSPModel(RCPSPModel):
    ...
    fixed_permutation: Union[
        List[int], np.array
    ]  # - Used to fix temporarily the permutation while an algorithm is searching the space of modes
