...
def plot_task_gantt(
    rcpsp_model: RCPSPModel, rcpsp_sol: RCPSPSolution, fig=None, ax=None, current_t=None
):
    ...
    for j in range(nb_task):
        ...
        patches.append(
            pp(xy=[(xx[1], xx[0]) for xx in box], facecolor=colors((j - 1) % nb_colors))
        )
    ...
    ax.set_yticklabels(
        tuple(["Task " + str(tasks[j]) for j in range(nb_task)]), fontdict={"size": 7}
    )
    return fig
