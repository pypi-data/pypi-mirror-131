...
class PostProMSRCPSP(PostProcessSolution):
    def __init__(
        self, problem: MS_RCPSPModel, params_objective_function: ParamsObjectiveFunction
    ):
...
def sgs_variant(solution: MS_RCPSPSolution, problem: MS_RCPSPModel, predecessors_dict):
    ...
    new_horizon = min(
        solution.schedule[problem.sink_task]["end_time"] * 4, problem.horizon
    )
    ...
    sorted_tasks = sorted(
        solution.schedule.keys(), key=lambda x: (solution.schedule[x]["start_time"], x)
    )
