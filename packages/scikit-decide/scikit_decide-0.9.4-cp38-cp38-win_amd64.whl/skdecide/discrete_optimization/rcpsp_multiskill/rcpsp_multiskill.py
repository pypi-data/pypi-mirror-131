...
class MS_RCPSPSolution(Solution):
    def __init__(
        ...
        employee_usage: Dict[int, Dict[int, Set[str]]],
...
def schedule_solution_to_variant(solution: MS_RCPSPSolution):
    ...
    solution = MS_RCPSPSolution_Variant(
        ...
        modes_vector=[s.modes[i] for i in sorted(s.problem.tasks)[1:-1]],
    )
    return solution
...
class MS_RCPSPSolution_Variant(MS_RCPSPSolution):
    ...
    def do_recompute(self):
        rcpsp_schedule, modes_extended, employee_usage, modes_dict = sgs_multi_skill(
            solution=self
        )
...
def sgs_multi_skill(solution: MS_RCPSPSolution_Variant):
    ...
    for res in problem.resources_set:
        resource_avail_in_time[res] = problem.resources_availability[res][
            : new_horizon + 1
        ]
        ...
        if not unfeasible_non_renewable_resources and not unfeasible_in_horizon:
            ...
            for t in range(
                ...
                for res in resource_avail_in_time.keys():  # 16
                    resource_avail_in_time[res][t] -= problem.mode_details[act_id][
                    ...
                    if res in problem.non_renewable_resources and t == end_t:
                        for tt in range(end_t + 1, new_horizon):
                            resource_avail_in_time[res][tt] -= problem.mode_details[
                                act_id
                            ][modes_extended[act_id - 1]][res]
...
class MS_RCPSPModel(Problem):
    ...
    def build_multimode_rcpsp_calendar_representative(self):
        ...
        from skdecide.discrete_optimization.rcpsp.rcpsp_model import RCPSPModelCalendar
