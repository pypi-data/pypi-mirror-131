...
def generate_rcpsp_with_helper_tasks_data(
    ...
    n_renewables = len(list(the_model.resources.keys())) - len(
        the_model.non_renewable_resources
    )
    ...
    for act_id in the_model.mode_details.keys():
        for mode_id in the_model.mode_details[act_id].keys():
            the_model.mode_details[act_id][mode_id][
                "duration"
            ] *= original_duration_multiplier
    ...
    assisted_activities = np.random.permutation(non_dummy_act_ids)[
        :n_assisted_activities
    ].tolist()
    ...
    for act_id in the_model.successors.keys():
        if old_sink_activity_id in the_model.successors[act_id]:
            del the_model.successors[act_id][
                the_model.successors[act_id].index(old_sink_activity_id)
            ]
    ...
    for i in range(n_new_activities):
        ...
        the_model.mode_details[old_sink_activity_id + i][1][
            "duration"
        ] = fixed_helper_duration  # TODO: Change this fixed duration
    ...
    for i in range(n_assisted_activities):
        ...
        if new_sink_activity_id in the_model.successors[assisted_activities[i]]:
            ...
            del the_model.successors[assisted_activities[i]][
                the_model.successors[assisted_activities[i]].index(new_sink_activity_id)
            ]
            the_model.successors[
                (old_sink_activity_id + n_assisted_activities + i)
            ].append(new_sink_activity_id)
    ...
    for main_act_id in assisted_activities:
        ...
        for main_act_id_2 in assisted_activities[the_index + 1 :]:
            if random.random() < probability_of_cross_helper_precedences:
                ...
                print(
                    "adding cross helper precedence between helper activities: ",
                    post_act_id,
                    "and",
                    pre_act_id,
                )
    ...
    if isinstance(the_model, SingleModeRCPSPModel):
        rcpsp_h = RCPSP_H_Model(
            base_rcpsp_model=the_model,
            pre_helper_activities=pre_helper_activities,
            post_helper_activities=post_helper_activities,
        )
    elif isinstance(
        the_model, MultiModeRCPSPModel
    ):  # Need to check the multi-mode works
        rcpsp_h = MRCPSP_H_Model(
            base_rcpsp_model=the_model,
            pre_helper_activities=pre_helper_activities,
            post_helper_activities=post_helper_activities,
        )
    ...
    if len(predecessors_sink) != rcpsp_h.n_jobs + 1:
        ...
        missing = [
            i for i in range(1, rcpsp_h.n_jobs + 2) if i not in predecessors_sink
        ]
        for m in missing:
            rcpsp_h.successors[m].append(
                new_sink_activity_id
            )  # this will insure the sink is the last activity...
    return rcpsp_h
