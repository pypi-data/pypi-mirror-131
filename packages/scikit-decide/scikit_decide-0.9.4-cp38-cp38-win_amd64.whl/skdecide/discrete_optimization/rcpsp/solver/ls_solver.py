...
from skdecide.discrete_optimization.generic_tools.ls.hill_climber import HillClimber
...
class LS_RCPSP_Solver(SolverDO):
    ...
    def solve(self, **kwargs):
        ...
        res = RestartHandlerLimit(
            200, cur_solution=dummy, cur_objective=model.evaluate(dummy)
        )
        if self.ls_solver == LS_SOLVER.SA:
            ls = SimulatedAnnealing(
                ...
                temperature_handler=TemperatureSchedulingFactor(
                    temperature=2, restart_handler=res, coefficient=0.9999
                ),
