...
That's why we have `teachable_scheduler` which gets configured by setting up the `scheduler.ini` config file (there is an example no worries), for instance you can run `echo test` every minute just by setting the following config section in that file:
