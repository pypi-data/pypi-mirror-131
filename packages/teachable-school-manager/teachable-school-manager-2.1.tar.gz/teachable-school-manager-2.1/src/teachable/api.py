...
class Teachable:
    ...
    def __init__(self, config_file=None):
        ...
        if self.config:
            ...
            self.notif_status = self._open_safe_shelve(notif_path)
            self.logger.debug('Using {} as statements_db'.format(statement_path))
            self.statement_status = self._open_safe_shelve(statement_path)
    ...
    def _open_safe_shelve(self, path: str) -> shelve:
        """
        A method to safely opening shelves.

        It gracefully waits until the cache file becomes available before returning.

        :return: a shelve object to the cache
        :param path: the path to the shelve cache file
        """
        opened = False
        waittime = 5
        while opened is False:
            try:
                ...
                self.logger.error('Shelve {} is not available. Waiting {} secs'.format(path, waittime))
                time.sleep(waittime)
                waittime *= 2


    def __del__(self):
        self.logger.debug('Releasing API. Deleting api object.')
        if self.config:
            try:
                if self.cached_data:
                        self.logger.debug('Syncing cache data')
            ...
            try:
                if self.notif_status:
                        ...
                        self.logger.debug('Syncing notification status data')
            ...
            try:
                if self.statement_status:
                        ...
                        self.logger.debug('Syncing statements data')
    ...
    @staticmethod
    def setup_logging(logconf: str) -> logging.Logger:
        """
        A static method to setup the logging facilities

        :return: the logger
        :param logconf: the path to the log config file
        """
    ...
    def prepare_cache(self, cache_path):
        ...
        # TODO: handle conflict situations when different instances of the API want to access cache.db
        self.cached_data = self._open_safe_shelve(cache_path)
    ...
    def find_user(self, email: str) -> User or None:
        """Searches for a specific user the email needs to be exact.

        :param email: email address of the user you are searching
        :return: the user you are searching
        """
        user_list = self._get_json_at(self.URL_GET_USER + email).get('users')
        if len(user_list) == 0:
            return None
        else:
    ...
    def get_user_info(self, email: str) -> json:
        """Gets the info for a specific user, the email needs to be exact.
    ...
    def get_all_users(self) -> list[User]:
        ...
        :rtype: [User]
        """
        response = self._get_json_at(self.URL_GET_ALL_USERS)
        # the API returns at least one page, at most N pages
        pages = response.get('meta')['number_of_pages']
        result_list = []
        user_list = response.get('users')
        if len(user_list) > 0:
            result_list += [User(self, user['email']) for user in user_list]

        # cycling through all pages after the first one if existing
        if pages > 1:
            self.logger.debug('There are {} pages of results, gonna take a while'.format(pages))
            for page_no in range(2,pages+1):
                self.logger.debug('Querying page #{}'.format(page_no))
                response = self._get_json_at(self.URL_GET_ALL_USERS+'?page='+str(page_no))
                user_list = response.get('users')
                if len(user_list) > 0:
                    result_list += [User(self, user['email']) for user in user_list]
        return result_list
    ...
    def get_user_report_card(self, user_id: str):
    ...
    def change_user_password(self, user: User, password: str) -> str:
        """
        Change password for the specific User

        :param user: the user we want to change
        ...
        path = self.URL_USERS.replace('USER_ID', str(user.id))
        return self._put_json_at(path, json.dumps(payload))
    ...
    def enroll_user_to_course(self, user: User, course: Course) -> str:
        ...
        # while it might seem that redundant Teachable keeps track of enrolled courses in 2 places:
        # reportcard and enrollment APIs
        # and it's very important that we keep the cache consistent across these places
        enrolled_courses = []
        delay = 1
        max_retry = 4
        retry = 0
        while int(course.id) not in enrolled_courses:
            self.usecache = False
            self.logger.debug('Checking if cache update was successful, waiting for {} secs'.format(delay))
            time.sleep(delay)
            # seems that Teachable backend takes a while to refresh :/
            enrolled_courses = [int(c.id) for c in user.courses]
            delay *= 2
            self.usecache = True
            retry += 1
            if retry > max_retry:
                self.logger.error("Something is wrong, we can't update the cache. :/")
                sys.exit(9)
        enrolled_courses = []
        delay = 1
        retry = 0
        while str(course.id) not in enrolled_courses:
            self.usecache = False
            self.logger.debug('Checking if cache update was successful, waiting for {} secs'.format(delay))
            time.sleep(delay)
            enrolled_courses = [i for i in user.reportcard if i != 'meta']
            delay *= 2
            self.usecache = True
            retry += 1
            if retry > max_retry:
    ...
    def unenroll_user_from_course(self, user: User, course: Course) -> str:
        ...
        # reportcard and enrollment APIs
        # and it's very important that we keep the cache consistent across these places
        enrolled_courses = [str(course.id)]
        delay = 1
        max_retry = 4
        retry = 0
        while str(course.id) in enrolled_courses:
            self.usecache = False
            self.logger.debug('Checking if cache update was successful, waiting for {} secs'.format(delay))
            time.sleep(delay)
            enrolled_courses = [i for i in user.reportcard if i != 'meta']
            ...
            if retry > max_retry:
                self.logger.error("Something is wrong, we can't update the cache. :/")
                sys.exit(9)
        enrolled_courses = [int(course.id)]
        delay = 1
        retry = 0
        while int(course.id) in enrolled_courses:
            self.usecache = False
            self.logger.debug('Checking if cache update was successful, waiting for {} secs'.format(delay))
            time.sleep(delay)
            # seems that Teachable backend takes a while to refresh :/
