    ...
    for s_file in sorted(os.listdir(s_segpath)):
        ...
        if not (o_match is None):
            ...
            if (s_seg_markers is None):
                ...
                #s_seg_markers = None  # bue 20211218: this is input!
                ...
                #s_seg_markers = o_match[config.d_nconv['di_regex_tiff_celllabel_nuccellmatched']['s_seg_markers']]  # bue 20211218: this is input!
            ...
            # it might actually be that more then one times same marker are merged properly by adding a suffix.
...
def load_cellpose_features_df(
    ...
    input:
        ...
        s_thresh_marker: string which specifies the marker to be used for cytoplasm detection.
        s_afsubdir: auto fluorescence subtracted registered images directory or registered image directory.
    ...
    description:
        function loads for all slides listed in es_slide the raw feature csv files into one dataframe.
    '''
    # handle input
    s_input = s_afsubdir.split('/')[-2].lower()
    ...
    df_mi = pd.DataFrame()
    for s_slide in sorted(es_slide):
        s_ifile = config.d_nconv['s_format_csv_raw_centroid_shape_meanintenisty'].format(s_slide, s_thresh_marker, s_input)
...
def filter_cellpose_xy(
        ...
        s_thresh_marker,
        ds_centroid = {
            'DAPI1_nuclei_centroid-0': 'DAPI_Y',
            'DAPI1_nuclei_centroid-1': 'DAPI_X',
        },
    ...
    input:
        es_slide: set of side ids that should be processed
        s_thresh_marker: string which specifies the marker to be used for cytoplasm detection.
    ...
    for s_slide in sorted(es_slide):
        ...
        # drop NA
        print(f'feat.filter_cellpose_xy {s_slide}: for quality control, make sure centroids don`t have too many NAs: {round(df_xy_slide.isna().sum().sum() / (df_xy_slide.shape[0] * df_xy_slide.shape[1]), 3)}[fraction]')
        ...
        # output
        df_xy_slide.index = df_xy.slide_scene + '_cell' + df_xy.cellid.astype(str)
        df_xy_slide.index.name = 'index'
        print('df_xy_slide info:', df_xy_slide.info())
        print('write file:', s_ofile)
...
def _thresh_cytoplasm(
    ...
    input:
        df_mi: cellpose segmented feature dataframe.
        s_thresh_marker: string which specifies the marker to be used for cytoplasm detection.
        i_thresh_manual: integer to specify the s_thresh_marker threshold value.

    output:
        df_mi: updated input dataframe.

    description:
        function detects s_thresh_marker cytoplasm positive cells ({s_thresh_marker}_negative False).
        cells without cytoplasm (nan) or s_thresh_marker cytoplasm mean values below i_thresh_manual are set to {s_thresh_marker}_negative True.
    '''
    ...
    if (s_thresh_marker is None):
        ls_neg_cells = df_mi.index.tolist()
    else:
        #ls_neg_cells = df_mi.loc[(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] < i_thresh_manual), :].index.tolist() #jenny 20211029: this is wrong because of nas
...
def _patch_nucleus_without_cytoplasm(
    ...
    input:
        df_mi: data frame with segmentation feature mean intensity values,
            ...
            which introduces the {s_thresh_marker}_negative column.
        s_thresh_marker: string which specifies the marker to be used for cytoplasm detection.
        i_exp: integer to specify the doughnut size used for cytoplasm for s_thresh_marker negative cells.
...
def _patch_weak_membrane_marker(
    ...
    input:
        df_mi: data frame with segmentation feature mean intensity values,
            already processed with _thresh_cytoplasm,
            which introduces the {s_thresh_marker}_negative column.
        s_thresh_marker: column specifies cytoplasm marker positive cells (e.g. tumor cells).
...
def _patch_bleedthrough(
    ...
    description:
        ...
        jenny thinks this function only helps bleed trough a little.
...
def _filter_cellpartition_and_shape(
    ...
    input:
        ...
        es_cytoplasm_marker: set of strings to specify cytoplasm marker.
            these are markers that are expressed in the cytoplasm in s_thresh_marker positive cells.
        ...
        b_filter_na: boolean to specify if any rows with an NA segmentation feature mean intensity values should be dropped.
            NAs occur, if from a slide_pxscene rounds are missing.
            default is False.
    ...
    description:
        ...
        cytoplasm (es_cytoplasm_marker), additional (es_custom_markerpartition) cell partitions,
...
def _plot_thresh_result_dapiecad(
    ...
    input:
        ...
        dfb_thresh: boolean dataframe with DAPI and s_thresh_marker_partition on/off values for each cell,
            e.g. generated with thresh.auto_thresh.
        df_img_thresh: parsed filename and threshold data frame.
        s_thresh_marker: strong (tumor) cell cytoplasm marker used for cytoplasm detection.
    ...
    description:
        generate marker positive cell location in tissue plots for DAPI and the s_thresh_marker (e.g. Ecad),
...
def filter_features(
        ...
        es_cytoplasm_marker = config.es_cytoplasmmarker_standard,  # optional, other cytoplasm marker then s_thresh_marker
        s_tissue_dapi = 'DAPI1',
    ...
    print('loaded feature extraction mean intensity raw dataframe:')
    print('df_mi (mean intensity) shape:', df_mi.shape)

    # detect cytoplasm negative cells
    if not (s_thresh_marker is None):
        print(f'{s_thresh_marker}_cytoplasm > {i_thresh_manual}:', sum(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] > i_thresh_manual))
        print(f'{s_thresh_marker}_cytoplasm < {i_thresh_manual}:', sum(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] < i_thresh_manual))
        print(f'{s_thresh_marker}_cytoplasm is NA:', sum(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'].isna()))
        print(f'not {s_thresh_marker}_cytoplasm > {i_thresh_manual}:', sum(~(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] > i_thresh_manual)))
    #_fill_cellpose_nas(
    _thresh_cytoplasm(
        df_mi = df_mi,
        s_thresh_marker = s_thresh_marker,
    ...
    print('after patched cells without cytoplasm:')
    print('df_mi (mean intensity) shape:', df_mi.shape)
    if not (s_thresh_marker is None):
        print(f'{s_thresh_marker}_cytoplasm > {i_thresh_manual}:', sum(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] > i_thresh_manual))
        print(f'{s_thresh_marker}_cytoplasm < {i_thresh_manual}:', sum(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] < i_thresh_manual))
    ...
    print('after filter dapi positive:')
    print('df_mi (mean intensity) shape:', df_mi.shape)
    if not (s_thresh_marker is None):
        print(f'{s_thresh_marker}_cytoplasm > {i_thresh_manual}:', sum(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] > i_thresh_manual))
        print(f'{s_thresh_marker}_cytoplasm < {i_thresh_manual}:', sum(df_mi.loc[:,f'{s_thresh_marker}_cytoplasm'] < i_thresh_manual))
    ...
    for s_slide in sorted(df_mi.slide.unique()):
        ...
        s_ofile = config.d_nconv['s_format_csv_patched_shape_meanintenisty'].format(s_slide, s_thresh_marker, s_filter_dapi, df_mi.index.name)
...
def filter_features_spawn(
        ...
        es_shrink_marker = set(), # optional against bleed through list of shrunken marker that should replace nucleus, exp{i_exp}, perinuc{i_exp}, or cytoplasm.
        es_custom_markerpartition = set(),  # optional
        es_cytoplasm_marker = config.es_cytoplasmmarker_standard,  # optional cancer marker other than s_thresh_marker
        s_tissue_dapi = 'DAPI1',
    ...
    input:
        ...
        i_tissue_area_thresh: specify pixel area threshold to use to fill tissue gaps between dapi nuclei.
        b_filter_na: boolean to specify, if any rows with an NA segmentation feature mean intensity values should be dropped.
            NAs occur, if from a slide_pxscene in the batch rounds are missing.
            default is False.
        ds_shape: dictionary which defines which original feature to take (key)
            for nucleus, cell, and cytoplasm shape values,
            and how the features should be named by standard (value).
...
def feature_correct_labels(
    ...
    # load mean intensity segmentation feature data frame
    ...
    for s_file in sorted(os.listdir(s_ipath)):
        ...
        if not (o_match is None):
            ...
            es_seg_marker = s_seg_markers.split('.')
            if (s_thresh_marker in es_seg_marker):
    ...
    with open(s_path_seg + config.d_nconv['s_format_json_celltouch_segmentation'].format(s_slide, s_thresh_marker), 'w') as f:
