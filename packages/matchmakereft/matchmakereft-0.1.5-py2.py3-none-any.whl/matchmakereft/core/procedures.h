...
id gam(y1?,FIVE,?a,mua?,mua?,?b,y1?) = gam(y1,FIVE, ?a, ?b, y1) * (D + [MM.s1] * (D-4)) ;
*<<<<<<<<
...
* We also use iCPV=epsM_{0,1,2,3}, note that this is -1 for Peskin, Schwartz, ..., but +1 for Warsaw, Manohar, ...
* -------> >   Tr((gmu gnu grho gsigma) x g5) = 4i iCPV \eps(mu,nu,rho,sigma)   <-------
 
* JSP *
id gam(y1?,FIVE,mu1?,mu2?,mu3?,mu4?,y1?) = 4*I*iCPV*epsM(mu1,mu2,mu3,mu4);
*id gam(y1?,FIVE,mu1?,mu2?,mu3?,mu4?,y1?) = -4*I*e_(mu1,mu2,mu3,mu4);
...
*: this is for assigning each trace with a different unknown coefficient
*: if, in the end, we only need one coefficient per diagram we can remove this and add set [MM.s1]->samb('i') above.
if ( count([MM.s1],1) > 0 );
*: we don't care how many contractions of gamma matrices happened; [MM.s1]^n = [MM.s1]
id [MM.s1]^[MM.s2]? = [MM.s1];
*: increment the dollar  variable that counts interesting traces
$counter = $counter+1;
id [MM.s1] = samb($counter);
endif;
.sort
...
***                 -d_(mu1,mu3)*gam(mu2)
***                 -iCPV*I*epsM(mu1,mu2,mu3,mu4)*gam(mu4,five)
*** This is only valid in 4D therefore only for the RGEMaker mode
**** Recall that iCPV=eps_{0123}=-1 for (peskin ...), and +1 for warsaw, ...
*** note that we write gam(five,mu4) so we change the sign

repeat;
* JSP *
id, once gam(y1?,?a,mu1?,mu2?,mu3?,?b,y2?)=d_(mu1,mu2)*gam(y1,?a,mu3,?b,y2)+d_(mu2,mu3)*gam(y1,?a,mu1,?b,y2)-d_(mu1,mu3)*gam(y1,?a,mu2,?b,y2)+iCPV*I*epsM(mu1,mu2,mu3,mu79)*gam(y1,?a,FIVE,mu79,?b,y2);
...
******************+
*** We now introduce ga(mu1,mu2)*eps(mu1,mu2,mu3,mu4)=iCPV*(I*gam(FIVE,mu3,mu4)-I*gam(FIVE,mu4,mu3))
...
repeat id,once gam(y1?,?a,mu1?,mu2?,?b,y2?)*epsM(mu1?,mu2?,mu3?,mu4?)=iCPV*(I*gam(y1,?a,FIVE,mu3,mu4,?b,y2)-I*gam(y1,?a,FIVE,mu4,mu3,?b,y2));
