...
from test_framework.engine.perses_power_cycle_engine_old import PowerCycleEngine
...
class PersesDownload(TestBase):
    ...
    def __init__(self):
        ...
        self.power_engine = PowerCycleEngine()
    ...
    def update_fw_path(self, parameters):
        if parameters["auto_build"] == "True":
            ...
            win_fw_bin, lnx_fw_bin = self.fw_path_manage.get_auto_build_fw_path(commit, volume)
            parameters["fw_path"] = lnx_fw_bin
    ...
    def run(self, parameters):
        ...
        if test_path:
            for index in range(3):
                ...
                result = self.power_engine.run(self.download_testcase, test_path[0], parameters, queue)
                logs = result["log"]
