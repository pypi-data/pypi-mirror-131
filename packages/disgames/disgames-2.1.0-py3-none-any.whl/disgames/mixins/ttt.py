...
class TicTacToe(commands.Cog):
    ...
    def make_bot_move_ttt(self, board, difficulty):
        if difficulty == 1:
            return random.choice([str(x)+str(y) for x, y in self.get_empty_ttt(board)])
        else:
            bestScore = -800
            bestMove = 0
            for x, y in self.get_empty_ttt(board):
    ...
    def make_ttt_move(self, answer, board, turn):
        if answer not in [
            ...
            if board[int(answer[0])-1][int(answer[1])-1] != ' ':
                return (
                    False,
                    f"Invalid Syntax: {answer} has already been chosen, Please try again",
                )
            else:
                board[int(answer[0])-1][int(answer[1])-1] = turn
                return True, board
    ...
    async def tictactoe(self, ctx: commands.Context, member: discord.Member=None):
        ...
        if member == None:
            ...
            msg = await self.bot.wait_for('message', check=lambda m: m.author == ctx.author and m.channel == ctx.channel and m.content.lower() in ['easy', 'hard'])
