...
class TrackingBase(ABC):
    ...
    def set_tracking_connection(
        ...
        if flight_tracking_id is None:
            if self.part_of_flight:
                print("getting flight tracking id")
                self.set_flight_tracking_id()
                print(f"Flight tracking id: {self.flight_tracking_id}")
    ...
    def _set_storage_location(self):
        ...
        if self.tracker_type == "model":
            tracker_keyword = "model_registry"
        elif self.tracker_type == "data":
            tracker_keyword = "data_registry"
        else:
            tracker_keyword = self.tracker_type
        ...
        if self.s3_bucket is not None:
            ...
            if main_tracking_id is not None:
                if bucket_prefix is not None:
                    bucket_prefix = (
                        f"{bucket_prefix}/{main_tracking_id}/{tracker_keyword}"
                    )
                else:
