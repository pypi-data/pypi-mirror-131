...
class CommandsTestCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.runner = Runner(config=config)

        headless_config = config.copy()
        headless_config["driver"]["tool_config"]["webdriver_config"]["headless"] = True
        self.headless_runner = Runner(config=headless_config)
    ...
    def test_chrome_headless_current_pos(self):
        self.headless_runner.execute_scenario("command-current-pos")
