...
class Action(object):
    ...
    def __str__(self):
        s =  f"nodename={str(self.get_node_name())} "
        s += f"type={str(self.getElementType())} "
        s += f"xpath={str(self.getXpathStr())} "
        s += f"value={str(self.getValue())} "
        s += f"key={str(self.get_key())} "
        s += f"waitafter={str(self.get_wait_after())}"
        return s
...
class Graph( object ):
    ...
    def build(self , fileName ):
        ...
        for action in soup.find_all('action'):
            ...
            a.set_node_name(action['nodename'])
