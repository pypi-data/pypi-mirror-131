        ...
        while True:
            ...
            if state != previous_state:
                if state in waiting_states:
                    ...
                    plan = self.status.get("plan", None)
...
class BuildSet(HeadersMixin):
    ...
    def build(self):
        ...
        for build_object in self.builds:
            build_entry, patch = build_object.generate_build_request()
            data["builds"].append(build_entry)
            data["patches"].update(patch)
