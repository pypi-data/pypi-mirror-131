...
class IdGenerator:
    @staticmethod
    def generate_request_id(data, guid=None):
        if not guid:
            guid = IdGenerator.generate_id()
        data_to_hash = data + str(uuid.uuid4())
...
class MetadataRepository:
    ...
    def insert_raw_metadata(self, data: List[str], params) -> str:
        ...
        guid, request_hash, request_id = IdGenerator.generate_request_id(serialized_body)
    ...
    def insert_blobs_metadata(self, blob_names: List[str], params, guid) -> List[str]:
        ...
        for blobs_chunk in chunks(blob_names, 99):
            ...
            for blob_name in blobs_chunk:
                guid, request_hash, request_id = IdGenerator.generate_request_id(blob_name, guid)
...
class Worker:
    def __init__(self, worker_name: str, 
        ...
        self.reader = BlobReader(connection_string, return_only_source)
        self.metadata_repository = MetadataRepository(connection_string, worker_name + "Data", create_if_not_exist)
        self.message_queue_client = MessageQueueClient(worker_name.lower(), worker_name.lower() + "-poison", connection_string, create_if_not_exist)
