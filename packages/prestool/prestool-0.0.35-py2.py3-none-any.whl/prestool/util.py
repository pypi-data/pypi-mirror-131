...
def make_sql_str_util(sql_type, table, select_target=None, where=None, update_target=None, insert_target=None,
    ...
    # null not null语句
    where_info += " and " + " and ".join(
        [f'{k} is {v}' if v in ['null', 'not null'] else f'{k}={format_quot_str_util(v)}' for k, v in
    ...
    # like语句
    where_info += " and " + " and ".join(
        [f'{k} like {format_quot_str_util(v)}' for k, v in like.items()]) if like else ''
    # 大于 小于 语句
    where_info += " and " + " and ".join(
        [' and '.join([f'{k} {i} {format_quot_str_util(j)}' for i, j in v.items()]) for k, v in compare.items()]) if compare else ''
    # 插入语句
    if sql_type == 'insert':
