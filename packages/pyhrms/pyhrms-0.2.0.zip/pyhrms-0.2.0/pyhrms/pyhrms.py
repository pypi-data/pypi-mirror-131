...
def peak_alignment(files_excel, rt_error=0.1, mz_error=0.015):
    ...
    pd.DataFrame(peak_ref, columns=['rt', 'mz']).to_excel(os.path.join(os.path.split(files_excel[0])[0], 'peak_ref.xlsx'))
    j = 1
...
def gen_lib_result_DIA(lib_df, mzml_file, unique_excel, i_min=5000, company='Waters'):
    '''
    ...
    for i in tqdm(range(len(unique_df)), desc='lib matching'):
        ...
        ms2_frag = gen_frag_DIA(ms1, ms2, rt, mode='profile')
        try:
            result = str(ms2_search(precursor, ms2_frag, lib_df))
        except:
            result = None
