...
from v2.presets.base import BaseProperty, PropertyContainer
...
class WJFConfig(LossBase):
    ...
    def parameter_block(self):
        ...
        ) for i, name in enumerate(SegmentationModelsConfig().CLASSES.CLASS_NAMES)}
    ...
    def get(self):
        if self.CLASS_WEIGHT is not None:
            ...
            # TODO fix seg limitation
            from v2.presets.semantic_segmentation import SegmentationModelsConfig
            class_weights_normalized = np.ones(len(SegmentationModelsConfig().CLASSES.CLASS_NAMES))
