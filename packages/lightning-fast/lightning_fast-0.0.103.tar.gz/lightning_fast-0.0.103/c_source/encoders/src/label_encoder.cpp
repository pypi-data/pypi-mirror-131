...
void encoders::LabelEncoder::syncEncode1D(const std::vector<std::string> &string_vector) {
  boost::progress_display bar(string_vector.size());
  auto start_end_point = std::make_pair(0, string_vector.size() - 1);
  encoders::LabelEncoder::singleThreadEncode1d(string_vector, start_end_point, this->encoder_map);
}

void encoders::LabelEncoder::encode1D(const std::vector<std::string> &string_vector, int n_worker) {
  if (n_worker == 1) {
    this->syncEncode1D(string_vector);
    return;
  }
  boost::progress_display bar(string_vector.size());
  u_long start, end;
