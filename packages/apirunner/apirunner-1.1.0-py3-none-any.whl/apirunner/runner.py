...
from apirunner import utils, exceptions
from apirunner.client import HttpSession
from apirunner.exceptions import ValidationFailure, ParamsError
from apirunner.ext.uploader import prepare_upload_step
from apirunner.loader import load_project_meta, load_testcase_file
from apirunner.parser import build_url, parse_data, parse_variables_mapping
from apirunner.response import ResponseObject
from apirunner.testcase import Config, Step
from apirunner.utils import merge_variables
from apirunner.models import (
    TConfig,
    TStep,
    VariablesMapping,
    StepData,
...
global_extracted_variables = {}    # *** DIY优化：优化支持导出参数支持跨用例执行
...
class HttpRunner(object):
    ...
    def __run_step_request(self, step: TStep) -> StepData:
        ...
        try:
            ...
            session_success = False
            # ***DIY修改：优化用例执行失败后重复打印请求与响应信息，不再在控制台和报告中输出
            # log_req_resp_details()# 原代码
            # ***DIY修改结束
    ...
    def __run_step(self, step: TStep) -> Dict:
        ...
        if step.request:
            # ***DIY修改：优化logger打印出当前的运行的summary
            if step.variables.get("summary"):
                logger.info(f"run step param: {step.variables.get('summary')} >>>>>>")
            # *** 优化结束
    ...
    def run_testcase(self, testcase: TestCase) -> "HttpRunner":
        ...
        # ***DIY修改：优化支持导出参数支持跨用例执行
        global global_extracted_variables
        global_extracted_variables.update(extracted_variables)
        extracted_variables = global_extracted_variables
        # ***DIY结束，上述新增代码
    ...
    def test_start(self, param: Dict = None) -> "HttpRunner":
        ...
        # ***DIY修改：修改日志输出文件名
        self.__case_id = self.__case_id or time.strftime("%Y%m%d%H") + "_log"
        self.__log_path = self.__log_path or os.path.join(
            self.__project_meta.RootDir, "logs", f"log_{self.__case_id}.txt"
        )
        # ***DIY修改结束
        # 原代码
        ...
        # ***DIY修改：优化将接口请求和返回详细信息同时输出到控制台与报告中
        import sys
        logger.remove()
        log_handler = logger.add(sys.stdout, colorize=True)
        # ***DIY修改结束
        ...
        logger.info(
            f"Start to run testcase: {self.__config.name}, TestCase ID: {self.__case_id.replace('log_','')}"
        )
