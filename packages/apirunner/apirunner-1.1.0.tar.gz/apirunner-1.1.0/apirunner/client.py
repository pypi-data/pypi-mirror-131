...
from apirunner.models import RequestData, ResponseData
from apirunner.models import SessionData, ReqRespData
from apirunner.utils import lower_dict_keys, omit_long_data
...
def get_req_resp_record(resp_obj: Response) -> ReqRespData:
    ...
    # ***DIY修改：优化删减请求与响应中header的不常见属性
    def del_header_key(header):

        del_key = ["HRUN-Request-ID", "Content-Length", "Access-Control-Expose-Headers", "X-Api-ID",
                   "X-Api-RequestId", "x-content-type-options", "x-envoy-upstream-service-time",
                   "x-xss-protection", "Vary", "Transfer-Encoding", "server"]
        for key in del_key:
            if key in header:
                header.pop(key)
        return header
    ...
    def log_print(req_or_resp, r_type):
        ...
        for key, value in req_or_resp.dict().items():
            if isinstance(value, dict):
                # ***DIY修改：优化因编码显示中文为异常字符，以json的ensure_ascii属性解决
                value = json.dumps(value, indent=4, ensure_ascii=False)
                # ***DIY修改结束
                # value = json.dumps(value, indent=4) # 原代码

            msg += "{:<8} : {}\n".format(key, value)
        logger.debug(msg)

    # record actual request info
    request_headers = del_header_key(dict(resp_obj.request.headers))  # ***DIY修改：优化将header中不常见参数删除
    ...
    resp_headers = del_header_key(dict(resp_obj.headers))  # ***DIY修改：优化将header中不常见参数删除
