...
class StepRequestValidation(object):
    ...
    # ***DIY修改：优化assert中新增一种if_equal断言（结果存在此字段才断言）
    def assert_if_equal(
        self, jmes_path: Text, expected_value: Any, message: Text = ""
    ) -> "StepRequestValidation":
        self.__step_context.validators.append(
            {"if_equal": [jmes_path, expected_value, message]}
        )
        return self
    ...
    # ***DIY修改：优化assert中新增一种if_greater_or_equals断言（结果存在此字段才断言）
    def assert_if_greater_or_equals(
        self, jmes_path: Text, expected_value: Union[int, float], message: Text = ""
    ) -> "StepRequestValidation":
        self.__step_context.validators.append(
            {"if_greater_or_equals": [jmes_path, expected_value, message]}
        )
        return self
    ...
    # ***DIY修改：优化assert中新增一种if_contains断言（结果存在此字段才断言）
    def assert_if_contains(
        self, jmes_path: Text, expected_value: Any, message: Text = ""
    ) -> "StepRequestValidation":
        self.__step_context.validators.append(
            {"if_contains": [jmes_path, expected_value, message]}
        )
        return self

    # ***DIY修改：优化assert中新增一种not_contains断言（即不包含）
    ...
    # ***DIY修改：优化assert中新增一种if_type_match断言（结果存在此字段才断言）
    def assert_if_type_match(
        self, jmes_path: Text, expected_value: Any, message: Text = ""
    ) -> "StepRequestValidation":
        self.__step_context.validators.append(
            {"if_type_match": [jmes_path, expected_value, message]}
        )
        return self
