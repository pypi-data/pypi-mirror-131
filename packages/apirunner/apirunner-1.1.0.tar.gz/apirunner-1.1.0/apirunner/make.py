...
from apirunner import exceptions, __version__
from apirunner.compat import (
    ensure_testcase_v3_api,
    ensure_testcase_v3,
    convert_variables,
    ensure_path_sep,
)
from apirunner.loader import (
    load_folder_files,
    load_test_file,
    load_testcase,
    load_testsuite,
...
from apirunner.response import uniform_validator
from apirunner.utils import merge_variables, is_support_multiprocessing
...
# ***DIY修改：优化提取summary作为报告中的用例名与支持skip以summary作判断跳过testcase中的teststep
...
{% if parameters %}
import pytest
from apirunner import Parameters
{% endif %}

from apirunner import HttpRunner, Config, Step, RunRequest, RunTestCase
...
class {{ class_name }}(HttpRunner):
    ...
    ids = [row["summary"] for row in values if "summary" in row] or None
    
    @pytest.mark.parametrize("param", values, ids=ids)
    def test_start(self, param):
        steps = list([])
        for i in range(len(self.teststeps)):
            for step in self.teststeps:
                variables = step.perform().variables
                if "skip" in variables and param["summary"] not in variables["skip"]:
                    steps.append((self.teststeps.index(step), step))
                    self.teststeps.remove(step)

        super().test_start(param)

        for i in range(len(steps)):
            self.teststeps.append(steps[i][1]) if i > 0 else self.teststeps.insert(steps[i][0], steps[i][1])
    {% endif %}
...
def convert_testcase_path(testcase_abs_path: Text) -> Tuple[Text, Text]:
    ...
    # ***DIY修改：优化将用例文件yml和py文件分层，方便查看和操作
    src_path = os.path.join(dir_path, f"src")
    if not os.path.exists(src_path):
        import pathlib
        pathlib.Path(os.path.join(dir_path, f"__init__.py")).touch()
        os.mkdir(src_path)
    testcase_python_abs_path = os.path.join(src_path, f"{file_name}_test.py")
    # testcase_python_abs_path = os.path.join(dir_path, f"{file_name}_test.py") #原代码
...
def make_testcase(testcase: Dict, dir_path: Text = None) -> Text:
    ...
    # ***DIY修改：优化在config的vavariables中做引用公共变量文件功能
    interface_to_request(testcase)
    # ***DIY修改：优化在teststeps的以reference引用api文件功能
    reference_global_parameters(testcase)
    # ***DIY修改结束，上述为新增代码
...
def main_make(tests_paths: List[Text]) -> List[Text]:
    ...
    # ***DIY修改：优化测试用例执行时按默认排序顺序执行
    pytest_files_format_list = list(pytest_files_format_list)
    for pytest_file in pytest_files_format_list:
        if pytest_file not in pytest_files_run_set:
            pytest_files_format_list.remove(pytest_file)
    return pytest_files_format_list
    # ***DIY结束，上述新增代码
    # 原代码如下
...
# ***DIY修改：优化在config和teststeps的variables中做引用公共变量文件功能
def reference_global_parameters(testcase: Dict) -> Dict:
    """ make testcases: parse global.yml variables and write to testcase config variables
    """
    config = testcase["config"]
    teststeps = testcase["teststeps"]
    reference_variables, reference_dict, parameters_dict = {}, {}, {}

    try:
        project_path = __ensure_absolute(testcase["config"]["path"]).split("testcases")[0]
        parameters_dict = load_test_file(__ensure_absolute(project_path + "/global.yml"))

        def replace_param(reference, parameters):
            variables = {}
            for key in reference.keys():
                if reference[key] == "all":
                    variables.update(parameters[key])
                else:
                    param_list = reference[key].split("-")
                    variables.update({param: parameters[key][param] for param in param_list})
            return variables

        if "variables" in config and "reference" in config["variables"]:
            reference_dict = config["variables"]["reference"]
            reference_variables = replace_param(reference_dict, parameters_dict)
            config["variables"].update(reference_variables)
            config["variables"].pop("reference")

        for i in range(len(teststeps)):
            if "variables" in teststeps[i] and "reference" in teststeps[i]["variables"]:
                reference_dict = teststeps[i]["variables"]["reference"]
                reference_variables = replace_param(reference_dict, parameters_dict)
                teststeps[i]["variables"].update(reference_variables)
                teststeps[i]["variables"].pop("reference")

    except Exception as ex:
        raise exceptions.ParamsError(
            f"ERROR reference key , value should be in global, and reference value type should be str！"
...
# ***DIY修改：优化在teststeps的以reference引用api文件功能
def interface_to_request(testcase: Dict) -> Dict:
    """ make testcases: parse interface file and write to request
    """
    teststeps = testcase["teststeps"]
    try:
        for i in range(len(teststeps)):
            if "interface" in teststeps[i]:
                project_path = __ensure_absolute(testcase["config"]["path"]).split("testcases")[0]
                ref_interface_path = __ensure_absolute(project_path + teststeps[i]["interface"])
                teststeps[i]["request"] = load_test_file(ref_interface_path)
    except Exception as ex:
        raise exceptions.ParamsError(
            f"ERROR parse interface file failed！\ninterface_: {testcase}"
        )
    return testcase
