...
from apirunner import __version__
...
"""
    ...
    # ***DIY修改：新增一些默认目录或文件
    globa_content = """env_variables:
    host_url: "www.host.com"
    token: "token"

api_variables:
    traceId: "traceId"
"""
    win_run_content = """set filename=%date:~0,4%%date:~5,2%%date:~8,2%%time:~0,2%%time:~3,2%%time:~6,2%
echo "测试开始：时间-"
set "filename=%filename: =0%"
set test_file=testcases/demo_testcase_ref.yml
set report_arg=--pytest_report
set "report_file=reports/report-%filename%.html"

D:
cd "%~dp0"
hrun %test_file% %report_arg% %report_file%
echo %test_file% %report_arg% %report_file%
echo "测试结束：请查看报告-%report_file%"
"""
    linux_run_content = """now_time=$(date +%Y-%m-%d_%H:%M:%S)

echo "测试开始：时间为 ${now_time}"

now_time=$(date +%Y%m%d%H%M%S)
test_file="testcases/demo_testcase_ref.yml"
report_arg="--pytest_report"
report_file="reports/report-${now_time}.html"
hrun "${test_file}" "${report_arg}" "${report_file}"

echo "测试结束：请查看报告-""${report_file##*t }"
    """
    ...
    create_file(
        os.path.join(project_name, "global.yml"),
        globa_content,
    )
    ...
    if platform.system().lower() == 'windows':
        run_file = "runner.bat"
        run_content = win_run_content
    elif platform.system().lower() == 'linux':
        run_file = "runner.sh"
        run_content = linux_run_content

    create_file(
        os.path.join(project_name, run_file),
        run_content,
    )
