...
from apirunner import loader, utils, exceptions
from apirunner.models import VariablesMapping, FunctionsMapping
...
# ***DIY修改：优化变量引用函数时可以传带有单引号，双引号，星号的参数
function_regex_compile = re.compile(r"\$\{(\w+)\(([\$\w\.\-/\s=\"\'\*,]*)\)\}")
# function_regex_compile = re.compile(r"\$\{(\w+)\(([\$\w\.\-/\s=,]*)\)\}") # 源代码
...
def get_mapping_function(
    ...
    if function_name in functions_mapping:
        ...
        from apirunner.ext import uploader
...
def parse_data(
    ...
    if isinstance(raw_data, str):
        ...
        # ***DIY修改：优化请求参数值为空格时自动变为空
        raw_data = raw_data.strip("\t")
        # raw_data = raw_data.strip(" \t") # 原代码
        ...
        if isinstance(parameter_content, List):
            ...
            for parameter_item in parsed_parameter_content:
                if isinstance(parameter_item, Dict):
                    ...
                    # ***DIY修改：优化参数化字段名支持以all表示引入所有参数，同时处理参数文件中的null,NA
                    if parameter_name == "all":
                        parameter_dict: Dict = parameter_item
                    ...
                    def null_to_none(param_dict, null_item="null"):
                        for key, value in param_dict.items():
                            if value == null_item:
                                param_dict[key] = None
                        return param_dict

                    def remove_null(param_dict, delete_item="NA"):
                        return {key: value for key, value in param_dict.items() if value != delete_item}

                    parameter_dict = null_to_none(parameter_dict)
                    parameter_dict = remove_null(parameter_dict)
                    # ***DIY修改结束
