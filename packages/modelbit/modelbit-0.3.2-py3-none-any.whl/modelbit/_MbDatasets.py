class _MbDatasets:
  ...
  def _makeDatasetsMkTable(self):
    ...
    for d in self._datasets:
      ...
      formatStr += f'| <pre>{ d["name"] }</pre> | { dataTimeVal } | { sqlTimeVal } |' + \
        f' { self._fmt_num(numRows) } | {self._mbMain._sizeof_fmt(numBytes)} |\n'
    return formatStr
  ...
  def get(self, dsName):
    ...
    try:
      ...
      self._mbMain._printMk(f'_Error fetching dataset. Please try again. ({err})_')
  ...
  def _storeDatasetResultIfMissing(self, dsName, dsId, url):
    ...
    self._mbMain._printMk(f'_Downloading "{dsName}"..._')
