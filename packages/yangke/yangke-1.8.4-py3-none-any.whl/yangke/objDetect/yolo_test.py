...
def test(cfg,
    ...
    data = {  # data_dict = parse_data_cfg(data)
        "classes": getSettings('aiTrain.darknet.class'),
        "names": getSettings('aiTrain.darknet.names'),
        "train": getSettings('aiTrain.darknet.train'),
        "valid": getSettings('aiTrain.darknet.valid'),
        "trainLabelFolder": getSettings('aiTrain.darknet.trainLabelFolder') or None,  # 替换默认Label文件中的路径，保留图片名不变
        "validLabelFolder": getSettings('aiTrain.darknet.validLabelFolder') or None,
    }
    nc = 1 if single_cls else int(data['classes'])  # number of classes
    path = data['valid']  # path to test images
    names = load_classes(data['names'])  # class names
    iouv = torch.linspace(start=0.5, end=0.95, steps=10).to(device)  # iou vector for mAP@0.5:0.95，steps是步数，不是步长
    iouv = iouv[0].view(1)  # comment for mAP@0.5:0.95
    niou = iouv.numel()

    # Dataloader
    if dataloader is None:  # 如果测试数据集的Dataloader
        dataset = DarknetDataset(path, label_folder=getSettings('aiTrain.darknet.validLabelFolder'), img_size=img_size,
                                 batch_size=batch_size, rect=True, single_cls=opt.single_cls)
    ...
    seen = 0
    model.eval()
    # 如果不是cpu训练，就是用批大小为1的单张图片作为输入，是用model(input)进行测试计算
    ...
    if save_json and map and len(jdict):
        ...
        cocoGt = COCO(glob.glob(r'D:\Users\pycharm\face\data\annotations/instances_val*.json')[0])  # initialize COCO ground truth api
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='win_x64.py')
