...
import os
from yangke.base import getSettings, plot_3d, getArgs
...
from yangke.common.qt import YkItem, layout_to_widget, set_menu_bar, YkDataTableWidget, YkInputPanel
...
class YkWindow(QMainWindow):
    def __init__(self):
        ...
        # self.toolBar: QToolBar = self.addToolBar("界面副标题")
        # self.toolBar.setStyleSheet("font-size: 20px;letter-spacing: 10px;horizontal-align:center")
        # self.toolBar.addWidget(QLabel("                      西安热工院"))
        ...
        # title_label.setFixedSize(500,16)  # 这条语句会导致标签上字体被裁剪
    ...
    def get_result_panel(self):
        ...
        if len(self.result) == 0:
            return QLabel("待求解")
        v_box = QVBoxLayout()
        size = getSettings("size", setting_file=os.path.join(os.path.dirname(__file__), "ui/ui_data.yaml"))
        v_box.addStretch()
    ...
    def init_model(self, debug: bool = True):
        ...
        try:
            ...
            from yangke.base import interpolate_2d
            if modified_time == modified_time_1:
    ...
    def set_font(self, font_size=10):
        font_size, okPressed = QInputDialog.getText(self, "设置界面字体大小", "字体大小（px）:", QLineEdit.Normal, "")
        if okPressed and font_size.isnumeric():
