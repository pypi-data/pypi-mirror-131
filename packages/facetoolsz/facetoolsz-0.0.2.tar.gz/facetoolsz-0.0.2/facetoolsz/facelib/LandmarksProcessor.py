...
from facetoolsz.core import imagelib
from facetoolsz.core import mathlib
from facetoolsz.facelib import FaceType
from facetoolsz.core.mathlib.umeyama import umeyama
...
def get_transform_mat (image_landmarks, output_size, face_type, scale=1.0):
    '''
        1. 先得到从global到local的变换矩阵mat
        2. 再利用该矩阵得到从local到global的角点（4个角和一个center）
        3. 根据FaceType得到padding系数，算出四边的padding
        4. 根据padding计算新的四个角点，（实际上只需要三个）
        5. 根据新的三个角点，以及最终的output size的角点对应关系，算出最终的变换矩阵；
        这个矩阵可以将原图的人脸，变换到对齐后的图片上，同时也可以将align的人脸反向映射回原始图像上
    ...
    ### 计算global图像中的角点和中心点的坐标，也就是经过变换前的坐标；通过将变换后的坐标反变换回去的方法实现。
    ...
    if not remove_align:
        ...
        # get area of face square in global space （脸部面积？）
        area = mathlib.polygon_area(l_t[:,0], l_t[:,1] )

        # calc side of square （计算边长）
