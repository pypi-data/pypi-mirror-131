...
class CVisualPyvistaBilayer(IVisualizer):
    ...
    def _make_cylinder(self):
        ...
        """
        l_cyl = pv.Cylinder(center=self.lattice.midpoint + np.array([0,0,self.dist_betweenlay / 2]) -  np.array([0,0,self.lattice.midpoint[2]]), direction=np.array([0,0,1]),radius=self.filter_around_layercenter,
