...
class KDJStrategy(StrategyBase):
    ...
    """
    def __init__(self):
        super(KDJStrategy, self).__init__()
        self.name = 'KDJStrategy'
    ...
    def set_params(self, params_dict=None):
        super(KDJStrategy, self).set_params(params_dict)
    ...
    def on_tick(self, k):
        ...
        if new_5min_bar:     # calc KDJ. cidx_5min-1 bar is complete
            ...
            _logger.info(f'{self.name} tick time {k.timestamp}, tick price {k.price}, bar time {self.df_5min_bar.index[self.cidx_5min-1]}, bar OHLC {self.df_5min_bar.Open[self.cidx_5min-1]}, {self.df_5min_bar.High[self.cidx_5min-1]}, {self.df_5min_bar.Low[self.cidx_5min-1]}, {self.df_5min_bar.Close[self.cidx_5min-1]}, Ln {Ln}, Hn {Hn}, Cn {Cn}, RSV {self.df_5min_KDJ.RSV[self.cidx_5min-1]}, K {self.df_5min_KDJ.K[self.cidx_5min-1]}, D {self.df_5min_KDJ.D[self.cidx_5min-1]}, J {self.df_5min_KDJ.J[self.cidx_5min-1]}')
            ...
            if long & (current_pos <= 0) & (len(self._order_manager.standing_order_set) == 0):
                o = OrderEvent()
                o.full_symbol = self.symbols[0]
                o.order_type = OrderType.MARKET
                o.order_size = 1 - current_pos
                _logger.info(f'{self.name} long order placed, current price {k.price}, current size {current_pos}, order size {o.order_size}')
                self.place_order(o)
            elif short & (current_pos >= 0) & (len(self._order_manager.standing_order_set) == 0):
                o = OrderEvent()
                o.full_symbol = self.symbols[0]
                o.order_type = OrderType.MARKET
                o.order_size = -1 - current_pos
                _logger.info(f'{self.name} short order placed, current price {k.price}, current size {current_pos}, order size {o.order_size}')
                self.place_order(o)
