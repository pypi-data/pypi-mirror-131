...
class MovingAverageCrossStrategy(StrategyBase):
    ...
    def on_fill(self, fill_event):
        ...
        _logger.info(f'{self.name} order filled. oid {fill_event.order_id}, filled price {fill_event.fill_price} size {fill_event.fill_size}')
    ...
    def on_tick(self, k):
        ...
        if k.tick_type == TickType.ASK:
            ...
            _logger.info(f'{self.name}: {self.last_trade} {self.ema}')
            if k.price > self.ema:    # (flip to) long
                ...
                if len(standing_oids) > 0:
                    _logger.info(f"{self.name} standing orders: {','.join(map(str, standing_oids))}")
                ...
                for oid in standing_oids:
                    if self._order_manager.retrieve_order(oid).order_size > 0:
                        _logger.info(f'{self.name} long order already in place. ema {self.ema}, last {k.price}, bid {self.last_bid}')
                        return
                    elif self._order_manager.retrieve_order(oid).order_size < 0:
                        _logger.info(f'{self.name} cancel existing short. ema {self.ema}, last {k.price}, bid {self.last_bid}')
                ...
                if current_pos not in [-1, 0]:
                    # _logger.error(f'{self.name} current size exceeds. {current_pos}')
                    return
                o = OrderEvent()
                ...
                _logger.info(f'{self.name} long order placed, current size {current_pos}, order size {o.order_size}. ema {self.ema}, last {k.price}, bid {self.last_bid}')
                self.place_order(o)
            else:   # (flip to) short
                ...
                for oid in standing_oids:
                    if self._order_manager.retrieve_order(oid).order_size < 0:
                        _logger.info(f'{self.name} short order already in place. ema {self.ema}, last {k.price}, bid {self.last_bid}')
                        return
                    elif self._order_manager.retrieve_order(oid).order_size > 0:
                        _logger.info(f'{self.name} cancel existing long. ema {self.ema}, last {k.price}, bid {self.last_bid}')
                        self.cancel_order(oid)
                ...
                o.order_size = -1 - current_pos
                _logger.info(f'{self.name} short order placed, current size {current_pos}, order size {o.order_size}, ema {self.ema}, last {k.price}, ask {self.last_ask}')
                self.place_order(o)
