...
class LinkerDescriptors:
    ...
    def num_rings(self, labeled_mol: Mol) -> int:
        linker_mol = self._bond_breaker.get_linker_fragment(labeled_mol)
        # the attachment atom at this point has a single lone electron which causes errors when computing rings
        # transform the linker Mol object to SMILES and back to Mol --> RDKit takes care of implicit Hs this way
        linker_mol = self._remove_explicit_hydrogens(linker_mol)
    ...
    def _remove_explicit_hydrogens(self, linker_mol: Mol) -> Mol:
        return self._conversions.smile_to_mol(self._conversions.mol_to_smiles(linker_mol))
