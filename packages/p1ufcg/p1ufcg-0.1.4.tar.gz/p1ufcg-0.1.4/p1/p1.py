...
def read_specification(filename=None, verbose=False):
    ...
    p1yaml_exists = os.path.exists('p1.yaml')
    p1json_exists = os.path.exists('p1.json')
    if verbose and p1yaml_exists and p1json_exists:
        cprint(YELLOW, "Found both p1.yaml and p1.json: using p1.yaml")

    if p1yaml_exists:
        try:
            specification = JsonFile('p1.yaml', array2map="tests")
        ...
        try:
            specification = JsonFile('p1.json', array2map="tests")
        except CorruptedJsonFile:
            _assert(False, "Corrupted specification file")
        return specification

    # neither p1.yaml, nor p1.json exist
...
def save_assignment(activity, dir_name, etag, url, repo):
    ...
    with io.open('./.p1/activity.json', mode='w', encoding='utf-8') as f:
        f.write(data2json({
            "url": url,
    ...
    for file in files:
        ...
        try:
            ...
            print("p1: fatal: Can't save file '%s'" % file['name'], file=sys.stderr)
            sys.exit(1)
