...
from . import json_requests
import json
import pandas as pd
import numpy
from .tools import is_string_type, tz_replacer, get_default_session
...
def get_data_frame(json_data):
    ...
    first_created = [
        tz_replacer(headline["firstCreated"]) for headline in json_headlines_array
    ]
    headlines = [
        [headline[field] for field in Headline_Selected_Fields]
    ...
    if len(headlines):
        ...
        headlines_dataframe = pd.DataFrame(
            [], numpy.array(first_created, dtype="datetime64"), Headline_Selected_Fields
        )

    headlines_dataframe["versionCreated"] = headlines_dataframe.versionCreated.apply(
        pd.to_datetime
    )
