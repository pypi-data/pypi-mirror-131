...
class _keys:  # noqa
    ...
    def stream_connects_locations(streaming_name, endpoint_name) -> str:
        return "apis.streaming.%s.endpoints.%s.locations" % (
            streaming_name,
            endpoint_name,
        )
    ...
    def stream_protocols(streaming_name, endpoint_name) -> str:
        return "apis.streaming.%s.endpoints.%s.protocols" % (
            streaming_name,
            endpoint_name,
        )

    @staticmethod
    def get_stream_websocket_url(streaming_name, endpoint_name) -> str:
        return "apis.streaming.%s.endpoints.%s.websocket-url" % (
            streaming_name,
            endpoint_name,
        )

    @staticmethod
    def stream_connects_type(session_name) -> str:
        return "streaming.connections.%s.type" % session_name

    @staticmethod
    def stream_connects_format(session_name) -> str:
