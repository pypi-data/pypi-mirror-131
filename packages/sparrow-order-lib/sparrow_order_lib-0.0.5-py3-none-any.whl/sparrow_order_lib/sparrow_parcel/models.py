...
class Parcel(BaseModelTimeAndDeleted):
    ...
    parcel_status = models.CharField("包裹状态", max_length=32, db_index=True)
    in_time = models.DateTimeField("包裹入系统时间")
    in_user_id = models.CharField("包裹入系统操作人", max_length=32)
    in_user_name = models.CharField("包裹入系统操作人", max_length=32)
    in_user_role = models.CharField("包裹入系统操作人角色", null=True, max_length=16)
    out_time = models.DateTimeField("包裹出系统时间")
    out_user_id = models.CharField("包裹出系统操作人", max_length=32)
    out_user_name = models.CharField("包裹出系统操作人", max_length=32)
...
class Transfer(BaseModelTimeAndDeleted):
    ...
    transfer_status = models.CharField("交接状态", max_length=16, db_index=True)
    recall_user_id = models.CharField("召回人信息", max_length=32, db_index=True)
    recall_user_name = models.CharField("召回人信息", max_length=32)
    recall_time = models.DateTimeField("召回时间")
    shelf_id = models.CharField("货架ID", max_length=32)
    shelf_number = models.CharField("货架编号", max_length=32)
    parcel_cnt_initial = models.PositiveIntegerField("包裹初始数量", default=0)
    parcel_cnt_checked = models.PositiveIntegerField("包裹揽收数量", default=0)
