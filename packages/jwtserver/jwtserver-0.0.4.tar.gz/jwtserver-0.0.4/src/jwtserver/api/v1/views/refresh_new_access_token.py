...
async def get_current_active_user(Authorization: str = Header(...),
    ...
    user = await session.get(User, token.payload_access_token_untested()['uuid'])
...
async def refresh_new_access_token(
    ...
    payload_access_untested = token.payload_access_token_untested()
    payload_refresh_token = token.payload_refresh_token()

    try:
        _secret_full = payload_access_untested['secret'] + \
                       payload_refresh_token['secret']

        if not _secret_full:
            return {"error": "invalid pair of tokens"}

        if _secret_full == secret(
                payload_access_untested['uuid'],
                payload_access_untested['exp'],
        ):
            ...
            response.set_cookie(
                ...
                httponly=True,
                secure=True,
                max_age=KEYS.REFRESH_TOKEN_EXPIRE_DAYS * 24 * 60 * 60)
                # max_age=60)
