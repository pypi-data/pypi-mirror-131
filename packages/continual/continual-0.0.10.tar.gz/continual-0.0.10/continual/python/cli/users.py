...
def list(
    ...
    for user in c.users.list_all():
        typer.echo("%s - %s" % (user.id, user.display_name))
...
def get(
    ...
    if json:
        ...
        typer.echo("Name: %s" % user.full_name)
...
@utils.exit_on_error
def register(
    full_name: str = typer.Argument(..., help="Full name.", show_default=False),
    ...
    """Create user."""
    user = c.register(full_name, email, password)
    print("Created user %s with id %s." % (full_name, user.id))
