...
# We just need to add a field to property so we can set a default value
class _property(property):
    default_attr = MISSING
    default_factory_attr = MISSING


def get_return_type(
    default: Any = MISSING, default_factory: Callable[[], Any] = MISSING
):
    ...
    if isinstance(return_type, units.UnitBase):
...
class field_property(property):
    ...
    def getter(self, fget: Callable[[Any], Any]) -> "field_property":
        return type(self)(
            ...
            self.fdel,
            self.__doc__,
            default=self.default_attr,
            default_factory=self.default_factory_attr,
        )
    ...
    def default_factory(
        self, default_factory: Callable[[Any], None]
    ) -> "field_property":
        self.default_factory_attr = default_factory
        return self
...
def create_fset(name, unit):

    if isinstance(unit, units.UnitBase):

        @units.quantity_input
        def fset(self, value: Quantity[unit]):
            setattr(self, name, value)

    else:

        def fset(self, value: unit):
            if value is not None:
                if type(unit) == type and isinstance(value, unit):
                    pass
                else:
                    value = unit(value)
            setattr(self, name, value)
...
def create_fget(name):
    def fget(self) -> Quantity:
...
def dataclass(
    ...
    *,
    ...
    def wrap(cls):
        ...
        # Turn certain values into properties
        for name, attr in cls.__annotations__.items():
            if isinstance(attr, InitVar):
                continue
            # For quantity we add the quantity_inputs decorator
            __name = f"__dataclass_{name}__"
            value = getattr(cls, name)
            # We need to copy it, so its not the same object
            ...
            if isinstance(attr, units.UnitBase):
                ...
                converter = attr
            fset = create_fset(__name, converter)
            fget = create_fget(__name)

            # The default value of the property is defined via the
            # default_attr value, which is not available in the
            # default class, thus we use a subclass
            prop = _property(fget, fset)
            if hasattr(value, "default_factory"):
                prop.default_factory_attr = value.default_factory
                setattr(cls, __name, None)
            else:
                prop.default_attr = value
                setattr(cls, __name, value)

            setattr(cls, name, prop)
            cls.__annotations__[name] = attr
...
def _get_field(cls, a_name, a_type):
    ...
    if isinstance(default, Field):
        ...
        if isinstance(default, types.MemberDescriptorType):
            ...
            if default_factory_attr is not MISSING:
            ...
            elif default_attr is not MISSING:
