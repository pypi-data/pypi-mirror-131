...
def RunRFTestSuite(project_dir, ts_name, ts_subfolders, browser_var, lang_var, run_type_var, tc_row_index_var):
    ...
    c = "_reports.html -o "
    d = "import win32com.client
#other libraries to be used in this script
import os
from datetime import datetime, timedelta

def read_outlook_email(username, mailbox_folder, sender_email_address, email_subject, start_hours, end_hours):
    outlook = win32com.client.Dispatch('outlook.application')
    mapi = outlook.GetNamespace("MAPI")
    """
    Retrieve the start and end date and time range to retrieve outlook emails
    for the values passes in to the following parameters
    Mailbox parameter:  mailbox_folder ... mailbox to which the email was sent
    Sender Email Address parameter: sender_email_address ... the mailbox from which the email was sent
    Email subject parameter: email_subject ... the subject of the email
    Start hours parameter: start_hours ... number of hours before current date and time
    End hours parameter: end_hours ... number of hours before current date and time
    """

    start_hours = int(start_hours)
    end_hours = int(end_hours)

    received_dt_start = datetime.now() - timedelta(hours=start_hours)
    received_dt_start = received_dt_start.strftime('%m/%d/%Y %H:%M %p')

    received_dt_end = datetime.now() + timedelta(hours=end_hours)
    received_dt_end = received_dt_end.strftime('%m/%d/%Y %H:%M %p')

    print("start date: ", received_dt_start)
    print("start end: ", received_dt_end)

    messages = mapi.Folders(mailbox_folder).Folders("Inbox").Items
    messages = messages.Restrict("[ReceivedTime] >= '" + received_dt_start
                                 + "' And [ReceivedTime] <= '" + received_dt_end + "'")

    messages = messages.Restrict("[SenderEmailAddress] = '" + sender_email_address + "'")

    messages = messages.Restrict("@SQL=(urn:schemas:httpmail:subject LIKE '%" + email_subject + "%')")

    for msg in list(messages):

        msg_body_text = msg.Body

        #Retrieve the temporary password for an organization username, based on the position of the
        # organization username
        if username in msg.Body:
            password_label = "Password: "
            password_label_len = len(password_label)
            start_pos_pwd_label = msg_body_text.find(password_label)

            start_pos_temp_pwd =  start_pos_pwd_label + password_label_len
            end_pos_temp_pwd = start_pos_temp_pwd + 10
            temp_pwd_text = msg_body_text[start_pos_temp_pwd:end_pos_temp_pwd]

            print("Email Subject: ", msg.Subject)
            print("Email Received Time: ", msg.ReceivedTime)
            print("Email Body: ", msg.Body)
            print("Email Sender: ", msg.SenderEmailAddress)
            print("temp password:", temp_pwd_text)

            return (temp_pwd_text)_output.xml -l "
    e = "_log.html "
