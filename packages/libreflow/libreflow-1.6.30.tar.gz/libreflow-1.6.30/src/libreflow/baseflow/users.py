...
class User(flow.Object):
    ...
    kitsu_id = flow.Param("").ui(label="Kitsu login")
    kitsu_uid = flow.Param("").ui(label="Kitsu ID")
...
class Users(flow.Map):
    ...
    def columns(self):
        return ["ID", "Kitsu login", "Kitsu ID", "Status"]
    ...
    def get_user(self, kitsu_id):
        for u in self.mapped_items():
            if u.kitsu_uid.get() == kitsu_id:
                return u
    ...
    def _fill_row_cells(self, row, item):
        ...
        row["Kitsu ID"] = item.kitsu_uid.get()
