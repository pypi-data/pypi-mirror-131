...
import asyncio
import ast
from .__defines__ import embeds_error
          ...
          for commandinfo in commandloader.commandsToLoad:
            ...
            if str(trigger).lower() == str(prefix + commandname):
              ...
              class BlueButton(discord.ui.Button):
                def __init__(self, label=None, ephemeral="false", usecode=None):
              ...
              if "$createNormalButton[" in commandcode:
                ...
                if codestatus['onlyForIDs'] == True:
                  ...
                  is_code = button_config[2]
                  button = BlueButton(f"{is_name}", f"{is_ephe}", f"{is_code}")
                  view.add_item(button)
                  codestatus['iters'] = True
                  commandcode = commandcode.replace(f"$createNormalButton[{button_config[0]};{button_config[1]};{button_config[2]}]", str(f""))
              ...
              while "$ban[" in commandcode:
                ...
                user = discord.utils.get(message.guild.members, iid=int(args[0]))
                reason = args[1]
                try:
                  commandcode = commandcode.replace(f"$ban[{user};{reason}]", str(""))
                  ...
                  commandcode = commandcode.replace(f"$ban[{user};{reason}]", "ban: Failed to ban user")
                  break
              ...
              try:
                try:
                  msg = await mesage.channel.send(commandcode, embed=embed, view=view)
                except Exception:
                  try:
                ...
                try:
                  msg = await message.channel.send(commandcode, view=view)
                except Exception:
                  try:
                    msg = await message.channel.send(commandcode, view=view)
                  except Exception:
