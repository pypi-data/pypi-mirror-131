...
class setupBot(commandloader):
  def __init__(self, intents: bool=False, token: str=None, prefix: str="", sharding: bool=False, shardsAmount: int=2):
    "Create bot"
    self.__token = token
      ...
      for file in os.listdir(path):
        with open("{}/{}".format(path, file), "r") as file:
          ...
          for cmd in comds:
            ...
            print(f'{text_to_load(self.__text_to_load, "done")}')
    except Exception as error:
        print(f'{text_to_load(self.__text_to_load, "failed")}')
        ...
        if not message.author.bot:
          ...
          for commandinfo in commandloader.commandsToLoad:
            ...
            if str(trigger).lower() == str(prefix + commandname):
              ...
              try:
                try:
                  msg = await message.channel.send(commandcode, embed=embed, view=view)
                except Exception:
