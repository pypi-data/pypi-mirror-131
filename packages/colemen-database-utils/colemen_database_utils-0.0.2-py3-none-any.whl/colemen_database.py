...
class ColemenDatabase:
    ...
    def run(self, sql, args=False):
        ...
        '''
        statements = sql
        # if the sql is a string, split it into a list separate statements
    ...
    def execute_single_statement(self, sql, args=False):
        ...
        try:
            ...
            print(f"{traceback.format_exc()}", PRESET="FATAL_ERROR_INVERT")
            print(f"SQL: {sql}", PRESET="FATAL_ERROR_INVERT")
            ...
            print(f"{traceback.format_exc()}", PRESET="FATAL_ERROR_INVERT")
            print("")
            print(f"{print(sys.exc_info()[2])}", PRESET="FATAL_ERROR_INVERT")
            print("")
            print("")
    ...
    def executeSqlFromFile(self, filePath, **kwargs):
        ...
        if 'DISABLE_KEY_RESTRAINTS' in kwargs:
            if kwargs['DISABLE_KEY_RESTRAINTS'] is False:
                disableForeignKeyRestraints = False
        self.runFromList(statements, DISABLE_KEY_RESTRAINTS=disableForeignKeyRestraints)
        # self.run("SET FOREIGN_KEY_CHECKS=1;")
    ...
    def filter_dict_by_columns(self, data_dict, table_name):
        ...
        '''
        column_array = self.table.get_column_array(table_name)
        finalDict = {}
