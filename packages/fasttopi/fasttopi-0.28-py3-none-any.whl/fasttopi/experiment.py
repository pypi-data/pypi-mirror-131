...
def main():
    ...
    if args.train:
        ...
        if args.baseline:
            ...
            c, max_df = get_best_hyper_parameter()
            model = create_model(c, max_df)
    ...
    if args.crossvalidate:
        ...
        if args.baseline:
            ...
            all_cs,all_max_dfs = get_hyper_parameter()
            for max_df in all_max_dfs:
                for c in all_cs:
                    print(f"### Hyperparameter C {c}    max df {max_df }    ###")
                    model = create_model(c, max_df)
                    metrics = cross_validate(model, training_set, splits_count)
                    print_metrics(metrics)
                    print("####\n")
