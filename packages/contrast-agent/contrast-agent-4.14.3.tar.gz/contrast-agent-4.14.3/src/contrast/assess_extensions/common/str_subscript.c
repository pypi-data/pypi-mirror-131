...
static inline PyObject *build_slice(PyObject *a, Py_ssize_t i, Py_ssize_t j) {
    /* Copied from bytes_slice in Objects/stringobject.c in Py2.7 */
    if (i < 0)
        i = 0;
    if (j < 0)
...
ssizessizeargfunc bytes_slice_orig;
HOOK_SLICE(unicode);
HOOK_SLICE(bytes);

#if defined(UBUNTU_XENIAL) || defined(UBUNTU_BIONIC_PY2)
static void propagate_sequence(
    const char *name, PyObject *a, PyObject *result, Py_ssize_t i1, Py_ssize_t i2) {
    ...
    /* Record input and result */
    if (result != NULL)
        call_string_propagator(name, a, result, args, NULL);
...
PyObject *seq_slice_new(PyObject *a, Py_ssize_t i1, Py_ssize_t i2) {
    ...
    /* We're hooking this because we need it as a string propagator, but it gets
     * called with other types as well. We want to gate our propagation logic so
     * that we don't hurt performance and/or create extraenous events.
     */
    if (PyUnicode_Check(a)) {
        propagate_sequence("propagate_unicode_subscript", a, result, i1, i2);
    } else if (PyBytes_Check(a)) {
        propagate_sequence("propagate_bytes_subscript", a, result, i1, i2);
    } else if (PyByteArray_Check(a)) {
        propagate_sequence("propagate_bytearray_subscript", a, result, i1, i2);
    }
...
#ifdef PY2
    bytes_slice_orig = PyString_Type.tp_as_sequence->sq_slice;
    unicode_slice_orig = PyUnicode_Type.tp_as_sequence->sq_slice;

    funchook_prep_wrapper(funchook, &bytes_slice_orig, bytes_slice_new);
