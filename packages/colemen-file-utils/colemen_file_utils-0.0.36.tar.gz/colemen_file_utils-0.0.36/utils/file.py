...
def _copy_files_from_array(file_list, attempted_delete=False):
    for file in file_list:
        try:
            ...
            if attempted_delete is False:
                ...
                _copy_files_from_array([file], True)
            else:
...
def _delete_single_file(file_path, shred=False, ftp=None):
    '''
        ...
        [`ftp`=None] {ftputil object}
            The ftp object to use.
...
# def get_modtime_size(file_path, **kwargs):
...
def get_modified_time(file_path, ftp=None):
    ...
    '''
    if ftp is not None:
        return datetime.timestamp(datetime.utcfromtimestamp(ftp.path.getmtime(file_path)))

    mod_time = os.path.getmtime(file_path)
    mod_time = int(datetime.fromtimestamp(mod_time).replace(tzinfo=timezone.utc).timestamp())
    return datetime.timestamp(datetime.fromtimestamp(mod_time))
...
# src = r"Z:\Structure\Archive\Images\Personal\Other\desktop.ini"
# dst = r"X:\Structure\Archive\Images\Personal\Other\desktop.ini"
