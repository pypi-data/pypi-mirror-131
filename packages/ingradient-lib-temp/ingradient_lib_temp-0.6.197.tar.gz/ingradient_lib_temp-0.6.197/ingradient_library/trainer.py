...
class Trainer(object):
    ...
    def calculate_loss(self, output, target, writer, epoch, is_val = False):
        ...
        for i in range(len(self.losses)):
            ...
            for _ in range(len(temp.shape)):
                temp = temp.mean(-1)
           
            if is_val:
                writer.add_scalar('Iteration\Val_Loss_'+ str(type(loss)), temp.item(), epoch) #(batch, -1) 로 나오게끔 resize
            else:
                writer.add_scalar('Iteration\Train_Loss_'+ str(type(loss)), temp.item(), epoch) #(batch, -1) 로 나오게끔 resize
            if self.is_deep_supervision == True:
