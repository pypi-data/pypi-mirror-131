...
class MonitoringValidator:
    ...
    def _basic_accuracy_checks(self, model_info):
        ...
        if model_info.model_task == ModelTask.BINARY_CLASSIFICATION:
            if model_info.targets[0].data_type not in [DataType.FLOAT, DataType.INTEGER, DataType.BOOLEAN, DataType.CATEGORY]:
            ...
            if model_info.targets[0].possible_values is not None:
                for value in model_info.targets[0].possible_values:
                    if not isinstance(value, str) and not isinstance(value, float) and not isinstance(value, bool) and not isinstance(value, int):
