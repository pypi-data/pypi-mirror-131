...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "relation_and_filename", nargs="+", help="Format: relation:filename, example: is_partner:partner.csv.gz"
        )
        parser.add_argument("--debug", action="store_true")
        parser.add_argument("-p", "--sample-size", type=int, default=3000)
        parser.add_argument("-d", "--no-drop-table", action="store_true")
        parser.add_argument("-c", "--chunk-size", type=int, default=8388608)
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for relation_filename in options["relation_and_filename"]:
                relation, filename = relation_filename.split("--")
                temp_table_name = "temp_import_relations_{}".format(random_name())
                try:
                    self.execute_import_data(relation, filename, temp_table_name, *args, **options)
                except:  # noqa
                    traceback.print_exc()
                    ok = False
    ...
    def create_table(self, schema, table_name):
        ...
        sql = sql.replace("id BIGINT", "id SERIAL")
    ...
    def import_relations(self, schema, relation_name, temp_table_name, n_rows, batch_size):
        ...
        statement_name = f"urlid_add_relations_{random_name()}"
