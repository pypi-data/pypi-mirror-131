...
class DrbNode(DrbItem, NodeImpl, abc.ABC):
    ...
    def has_child(self) -> bool:
        ...
        Returns:
            bool: ``True`` if current node has at least one child, otherwise
            ``False``
        """
        raise NotImplementedError
    ...
    def __getitem__(self, item):
        ...
        * ``int``: the item-th children node will be returned (>= 0)
        * ``str``: the first child node with the item as name is retrieved
          from its children
        * ``tuple``: the following tuple must be supported per each node
          implementation:
            * tuple(name: ``str``, namespace: ``str``): retrieves children by
              is name and namespace_uri.
            * tuple(name: ``str``, occurrence: ``int``): retrieves a child by
              its name, occurrence and having its namespace_uri to ``None``
            * tuple(name: ``str``, namespace: ``str``, occurrence: ``int``):
              retrieves a child by its name, namespace_uri and its occurrence
        * ``Predicate``: children nodes matching this predicate are
        ...
        .. code-block:: python
            ...
            child = node['child_name', 3]

            # get child by is name and namespace
            children = node['child_name', 'child_ns']
            child = node['child_name', 'child_ns', 2]
