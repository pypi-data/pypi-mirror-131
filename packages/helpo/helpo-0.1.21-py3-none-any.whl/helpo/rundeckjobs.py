...
class RundeckJobs(object):
    ...
    def terraform_cloud_workspace(
        ...
        try:
            ...
            self.run_command(["terraform", "workspace", "select", f"{fqdn}.{deployment_environment}.{datacenter}"],
                             cwd=config_dir_location, check=True)
        except:
            # zx.run_shell_print(
            #     f"cd {config_dir_location} && \
            #     terraform workspace new {fqdn}.{deployment_environment}.{datacenter}")
            self.run_command(["terraform", "workspace", "new", f"{fqdn}.{deployment_environment}.{datacenter}"],
                             cwd=config_dir_location, check=True)
        ...
        if deployment_action == "apply":
            ...
            self.run_command(["terraform", "apply", "-input=false", "-auto-approve", plan_file_location],
                             cwd=config_dir_location, check=True)
            self.run_command(["terraform", "workspace", "select", "default"], cwd=config_dir_location, check=True)
