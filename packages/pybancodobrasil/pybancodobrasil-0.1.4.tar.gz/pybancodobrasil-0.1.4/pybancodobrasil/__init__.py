    ...
    table2Json: (table) => {
        ...
        for (var i = 1; i < table.rows.length; i++) {
            ...
            for (var j = 0; j < tableRow.cells.length; j++) {
                var element = tableRow.cells[j]
                for (var k = 0; k < element.childNodes.length; ++k) {
                    if (element.childNodes[k].nodeType === Node.TEXT_NODE)
                        column = window.pybancodobrasil.clearStr(element.childNodes[k].textContent)
                    else
                        column = window.pybancodobrasil.clearStr(element.childNodes[k].innerText)
                    if (column)
                        rowData.push(column)
                }
            }
                ...
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        ...
                        window.pybancodobrasil.faturas.methods.buscaExtrato(indice, 0, len, () => {
                            for (var cartao in window.pybancodobrasil.faturas.cartoes){
                                for (var i in window.pybancodobrasil.faturas.cartoes[cartao]){
                                    var temp = [];
                                    var tempi = [];
                                    for (var t of window.pybancodobrasil.faturas.cartoes[cartao][i]){
                                        var text = t.join('')
                                    ...
                                    window.pybancodobrasil.faturas.cartoes[cartao][i] = tempi
                                }
                            }
                    ...
                    req.onreadystatechange = function () {
                        if (req.readyState == 4) {
                            ...
                            if (cardNumber) {
                                ...
                                var lines = $(req.responseText).find('.lancamentos tr').toArray()
                                lines = lines.filter(line => window.pybancodobrasil.clearStr(line.innerText))
                                lines = lines.filter(line => window.pybancodobrasil.clearStr(line.children[0].innerText))
                                var columns_by_line = lines.map(line => line.querySelectorAll('td'))
                                var table = document.createElement('table')
                                for (var line of columns_by_line) {
