...
async def buy_forex(symbol):
    ...
    try:
        ...
        #trade
        print(await connection.create_stop_buy_order(symbol=symbol,volume=0.0,stop_loss=0.9,take_profit=0.2,open_price=1.0))
        return True
...
async def sell_forex(symbol):
    ...
    try:
        ...
        a  = await connection.get_positions()
        print(a)

        print(await connection.create_stop_sell_order(symbol=symbol,volume=0.1,stop_loss=1.2,open_price=1.13))
