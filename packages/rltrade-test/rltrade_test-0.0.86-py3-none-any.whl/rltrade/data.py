...
class OandaDownloader:
    ...
    async def fetch_min_data(self):
            
        df = pd.DataFrame()
        for tic in self.ticker_list:
            for date in self.date_range:
                if date.weekday() < 5:
                    new_data = await self.account.get_historical_candles(tic,'5m',date,limit=self.mins_per_day)
    ...
    async def fetch_todays_min_data(self,time_now,duration):
        df = pd.DataFrame()
        for tic in self.ticker_list:
            if time_now.weekday() < 5:
                new_data = await self.account.get_historical_candles(tic,'5m',time_now,limit=duration)
                print(f'Downloaded {len(new_data) if new_data else 0} historical data for {tic}')
                temp_df = pd.DataFrame(new_data)
                temp_df.columns = ['date','open','high','low','close','volume']
                temp_df['date'] = temp_df['date'].dt.tz_convert('EST')
