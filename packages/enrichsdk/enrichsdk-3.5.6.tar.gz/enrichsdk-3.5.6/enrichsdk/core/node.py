...
class Node(metaclass=NodeMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.args_schema = {}
        """JSON Schema to validate the input args
        ...
        """
        data_root = self.get_file("%(data_root)s")
    ...
    def validate_args(self, what, state):
        ...
        try:
            ...
            logger.exception("Invalid/insufficient arguments. Doesnt match schema",
                             extra={
                                 'transform': self.name
                             })
...
class Transform(Node):
    ...
    def get_column_metadata(self, name, df):
        ...
        for c in list(df.columns):
            sample = list(df[c].sample(min(3, df.shape[0])))
            sample = [flatten(c) for c in sample]
            columns[c] = {
