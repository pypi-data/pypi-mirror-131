...
from odrive.enums import EncoderId, AxisState, ComponentStatus, ProcedureResult
...
class EncoderRef(EntityRef):
    def __init__(self, axis_ref, enc_num, config: 'EncoderConfig'):
        EntityRef.__init__(self, axis_ref, f'encoders[{enc_num}]', enc_num, EntityType.AXIS_ENCODER, None)
        self.num = enc_num
        self.config = config
...
#        self._rs485_connections.append([enc_ref, rs485_intf_ref])
    ...
    def get_status(self, odrives):
        ...
        output_configs = [{'config': {},
            **{
                f'axis{i}': {'motor': {'config': {}}, 'controller': {'config': {}}, 'config': {}}
        ...
        for net in self.get_nets(NetType.THREE_PHASE):
            ...
            if motor_config.phase_resistance != "":
                axis_output_config['motor']['config']['phase_resistance'] = motor_config.parse_phase_resistance('Ohm')
            else:
                axis_output_config['motor']['config']['phase_resistance'] = motor_data['phase_resistance']

            if motor_config.phase_inductance != "":
                axis_output_config['motor']['config']['phase_inductance'] = motor_config.parse_phase_inductance('H')
            else:
        ...
        for net in self.get_nets(NetType.THERMISTOR):
            ...
            odrv_data = _db.get_odrive(odrv_config['board_version'])
        ...
        for dev_num, dev_config in enumerate(self.devices):
            for inv_num in range(len(_db.get_product(dev_config.product)['inverters'])):
                if len(dev_config.shunt_conductances) > inv_num:
                    ...
                    axis_output_config['motor']['config']['shunt_conductance'] = dev_config.shunt_conductances[inv_num]
        ...
        for axis_ref in ref.axes():
            ...
            if not axis_config.parse_calib_torque('Nm') is None:
                axis_output_config['motor']['config']['calibration_current'] = axis_config.parse_calib_torque('Nm') / motor_data['torque_constant']
...
def _run_state(axis, state):
    ...
    if result != ProcedureResult.SUCCESS:
        codes = {v: k for k, v in odrive.enums.__dict__ .items() if k.startswith("ProcedureResult.")}
