...
def _traverse_all_imported_python_files(file_full_path):
    ...
    def __traverse_all_imported_python_files(file_full_path):
        ...
        for imports in utilities.find_imports(file_full_path):
            try:
                file_path = imports[3]
            except IndexError:
                continue
    ...
    return all_imported_python_file
...
def check_idx_binary_compatibility(app, reporter):
    ...
    for executable_file in set(executable_files):
        ...
        if binary_status["error"]:
            ...
            reporter.warn(
                "Found non-ARM aarch64 binary file. " f"File: {executable_file}",
                file_name=executable_file,
            )

    def check_report_binary_file_in_code(ast_node, file_iname):
        ...
        if (
            ...
            reporter.warn(
                ...
                f"File: {file_iname}. Line: {ast_node.lineno}",
                file_name=file_iname,
        ...
        if app.file_exists(inspect_file_name):
            ...
            if binary_status["error"]:
                ...
                # Change from warning to fail 3 months after deploying
                reporter.warn(
                    f"Found non-ARM aarch64 binary file. "
                    f"File: {file_iname}. Line: {ast_node.lineno}",
                    file_name=file_iname,
                    line_number=ast_node.lineno,
                )
