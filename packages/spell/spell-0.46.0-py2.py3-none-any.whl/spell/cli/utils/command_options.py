...
def machine_config_params(f):
    ...
    @click.option(
        "--node-group",
        hidden=True,
        help="Node group to schedule the run to (K8s-only). Cannot be used in conjunction with --machine-type.",
    )
    @click.option(
        "--kubernetes",
        is_flag=True,
    ...
    def wrapper(*args, **kwargs):
        ...
        if kwargs.get("machine_type") and kwargs.get("node_group"):
            raise ExitException("--machine-type and --node-group options are mutually exclusive", SPELL_INVALID_CONFIG)
