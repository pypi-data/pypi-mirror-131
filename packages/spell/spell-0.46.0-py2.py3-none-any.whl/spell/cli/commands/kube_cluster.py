...
def perform_kube_cluster_upgrade(
    ...
    if support_runs:
        cluster_utils.create_namespace("spell-run")
        cluster_utils.create_build_run_configmap("spell-run")
        cluster_utils.create_build_artifacts_pvc("spell-run")
        kube_runs.add_argo()
...
def node_group_add(
    ...
    # NOTE(waldo): To be perfectly honest, I'm not sure why we generate a node-group config
    # on the server to pass back to the client, rather than just passing back flags to
    # eksctl or gcloud, respectively
    ...
    with KubeClusterContext(cluster, aws_session.profile_name if aws_session is not None else None):
        if cluster["cloud_provider"] == "AWS":
            ...
            cluster_aws.eks_add_nodegroup(ng_name, config, aws_session.profile_name)
        elif cluster["cloud_provider"] == "GCP":
