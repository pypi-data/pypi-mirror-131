...
### Install
```text
pip3 install generalindex==0.0.4
```
...
- Corrections: get information about corrections to the data. Available options are 'true', 'false', 'only'
- Delta: return the data with dates defined by data insertion time and not dataset timestamp
...
df = pd.read_csv(fileIO)
```
### How to read data from Timeseries for specific upload dates?
It is possible to get the data using a timeframe when the data point were inserted into the database using *delta* parameter.
...
# retrieve only the data inserted between 2021-01-04 and 2021-02-05
# and stream to memory as BytesIO object
fileIO = ts.retrieve_data_as_csv(file_name=None,
                                 symbols=symbols,
                                 columns=columns,
...
### How to get the corrections along with the data ? 
It is possible to get the history of corrections to the queried data using *corrections* parameter.
Available options for correction are *true*, *false* and *only*
By default it is set to *false* and no corrections are retrieved
...
# retrieve only the corrections from group according to keys & columns 
# and stream to memory as BytesIO object
fileIO = ts.retrieve_data_as_csv(file_name=None,
                                 symbols=symbols,
                                 columns=columns,
...
# retrieve the data and corrections from group according to keys & columns 
# and stream to memory as BytesIO object
fileIO = ts.retrieve_data_as_csv(file_name=None,
                                 symbols=symbols,
                                 columns=columns,
