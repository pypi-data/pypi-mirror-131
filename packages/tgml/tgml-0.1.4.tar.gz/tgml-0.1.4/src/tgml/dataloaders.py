...
class GraphLoader(BaseLoader):
    def __init__(self,
        ...
        There are two ways to use the data loader.
        * First, it can be used as an iterator, which means you can loop through
          it to get every batch of data. If you load the whole graph at once, 
          there will be only one batch of data (of the whole graph) in the iterator.
        * Second, you can access the data property of the class directly. If there is 
          only one batch of data, it will give you the batch directly instead of an 
          iterator, which might make more sense in that case. If there are multiple 
          batches of data to load, it will return the loader again.

        Args:
            graph (TigerGraph): Connection to the TigerGraph database.
            v_index (str): Attribute that contains indices of the vertices. The indices
                should be integers from 1 to the number of vertices. 
            v_in_feats (str, optional): Attributes to be used as input features. 
                Defaults to "".
            v_out_labels (str, optional): Attributes to be used as labels for prediction. 
                Defaults to "".
            v_extra_feats (str, optional): Other attributes to get such as indicators of
                train/test data. Defaults to "".
            local_storage_path (str, optional): Place to store data locally. 
                Defaults to "./tmp".
            cloud_storage_path (str, optional): S3 or GCP path used for cloud caching. 
                Defaults to None.
