...
class PubSub(object):
    ...
    def create_kafka_topics(self):
        ...
        for topic in self.kafka_topics:
            for topic_prefix in self.kafka_receive_prefixes:
                custom_topic = "%s%s" % (topic_prefix, topic)
                if custom_topic not in already_topics:
    ...
    def receive_kafka(self, listeners):
        ...
        consumer.subscribe(list(new_listeners.keys()))
        while True:
