...
from .types import NonTextElement, Argument_T
...
class Option(OptionInterface):
    ...
    def help(self, help_string: str):
        argument_string = f"{self.separator}".join([f"<{{{i}}}>" for i in range(len(self.args))]).format(*list(self.args.keys()))
        setattr(self, "help_doc", f"# {help_string}\n"
                                  f":  {self.name}{self.separator}{argument_string}\n")
        return self
...
class Arpamar(Representation):
    ...
    __slots__ = ["current_index", "is_str", "results", "elements", "raw_texts", "need_marg", "matched", "head_matched",
                 "_options", "_args"]
