...
def get_string_columns(df: pd.DataFrame):
    """
    Returns all the string column_headers in the
    passed dataframe
    """
    string_columns = []
    for column_header in df.columns:
        if is_string_dtype(str(df[column_header].dtype)):
            string_columns.append(column_header)
