...
from mitosheet.api.get_dateframe_as_excel import get_dataframe_as_excel
...
from mitosheet.api.get_starting_formula import get_starting_formula
...
def handle_api_event(send, event, steps_manager):
    ...
    elif event['type'] == 'get_cell_values':
        result = get_sheet_data(event, steps_manager)
    elif event['type'] == 'get_dataframe_as_excel':
        result = get_dataframe_as_excel(event, steps_manager)
    elif event['type'] == 'get_starting_formula':
        result = get_starting_formula(event, steps_manager)
    else:
