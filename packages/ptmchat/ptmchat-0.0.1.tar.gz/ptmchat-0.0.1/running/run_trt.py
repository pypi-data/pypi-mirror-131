...
from model.onnx_helper import ONNXClassifierWrapper
...
def get_final():
    ...
    if not responses:
        ...
        query = '。'.join(us)
        input_ids, input_mask, segment_ids = convert_list_to_features(query, 30, tokenizer, responses)
        t11 = time.time()
        trt_result = trt_model.predict(input_ids.astype(np.int32), input_mask.astype(np.int32), segment_ids.astype(np.int32))
        t12 = time.time()
        logits = sigmoid(trt_result)
...
if __name__ == '__main__':
    ...
    trt_model = ONNXClassifierWrapper("model/ernie_match.trt", 20)

    app.run(host='0.0.0.0', port=6997, threaded=False)
