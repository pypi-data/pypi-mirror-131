...
HDDF = 'holidays.csv'
HOLIDAYS = []
def __update_holidays__(proxy=False):
    try:
        if proxy:
            proxys = { 
            ...
            r = requests.get('https://openapi.piasset.net/public/@holidays', proxies=proxys)
        else:
            r = requests.get('https://openapi.piasset.net/public/@holidays')
        hddf = pd.DataFrame()
        hddf['holidays'] = list(r.json())
        hddf.to_csv(HDDF, index=False)
    except Exception as e:
        print(e.args)
        print(traceback.format_exc())
try:
    __update_holidays__()
    HOLIDAYS = list(pd.read_csv(HDDF)['holidays'])    
except Exception as e:
    print(e.args)
    print(traceback.format_exc())
    HOLIDAYS = [
...
class Holidays:
    ...
    @classmethod
    def range_exp(self, start: str, end: str) -> pd.DataFrame:
        """
