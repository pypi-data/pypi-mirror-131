...
def two_consecutive_payments_made(_, payload):
    ...
    return payload["loc_record"]["current_cons_full_pmt"] >= consecutive_payment_required
...
def two_missed_payments(_, payload):
    ...
    for missed_pmt in payload["loc_record"]["missed_pmt"]:
        if datetime.strptime(missed_pmt["payment_date"],'%Y-%m-%dT%H:%M:%S.%fZ') >= (datetime.today() - timedelta(days=90)):
...
def two_payment_reductions_in_180_days(_, payload):
    ...
    payment_reductions_list = [x for x in payload["loc_record"]["modification"] \
                               if datetime.strptime(x["applied"],'%Y-%m-%dT%H:%M:%S.%fZ') >= (datetime.today() - timedelta(days=180)) \
...
def fifty_percent_principal_paid(_, payload):
    ...
    if len(payload["loc_record"]["draw"]) > 0:
        balance_threshold = payload["loc_record"]["draw"][0]["principal_post_draw"] * 0.5
...
def two_payment_reductions_in_90_days(_, payload):
    ...
    payment_reductions_list = [x for x in payload["loc_record"]["modification"] \
                               if datetime.strptime(x["applied"],'%Y-%m-%dT%H:%M:%S.%fZ') >= (datetime.today() - timedelta(days=90)) \
                               if x["reduction_perc"] > 0]
...
def seventy_five_percent_principal_paid(_, payload):
    ...
    balance_threshold = 0
    if len(payload["loc_record"]["draw"]) > 0:
        balance_threshold = payload["loc_record"]["draw"][0]["principal_post_draw"] * 0.25
...
def on_payment_holiday(_, payload):
    ...
    payment_holidays = [x for x in payload["loc_record"]["modification"] if x["instalment_amount"] == 0 \
                        if datetime.strptime(x["applied"],'%Y-%m-%dT%H:%M:%S.%fZ') <= datetime.today() \
                        if datetime.strptime(x["suspended"] or '2070-01-01T00:00:00.000Z', '%Y-%m-%dT%H:%M:%S.%fZ') >= datetime.today()]
...
def had_payment_holiday(_, payload):
    ...
    return len([x for x in payload["loc_record"]["modification"] if x["instalment_amount"] == 0 \
            if datetime.strptime(x["applied"], '%Y-%m-%dT%H:%M:%S.%fZ') < datetime.today() \
            if x["suspended"] is not None \
            if datetime.strptime(x["suspended"] or '2070-01-01T00:00:00.000Z',
                                 '%Y-%m-%dT%H:%M:%S.%fZ') < datetime.today()]) > 0

@rule(rule_id="PR-004-U1", name="Three Consecutive Payments Since Payment Holiday", run_if=had_payment_holiday)
