...
#创建储存结果文件夹
    ...
    dir_gepian_baogao=dir_+'/'+name+'_轮轨力计算结果/'+datetime.datetime.now().strftime("%Y-%m-%d")+'_1各片结果_报告版'    
    ...
    list_dir=[dir_gepian,dir_gepian_baogao,dir_zuqiao,dir_zhengti,dir_sudu]
    for i in list_dir:  
        pass          
...
##画图完全展示版，内部自己分析趋势用
    ...
    for ii in ls_:
        ...
        j=ls_[ii]['j']
        name=ls_[ii]['name']
        dire=ls_[ii]['direction']
        res=cal_index(
                    time_=force[i]['时间_'+dire],
                  speed=force[i]['速度_'+dire],
                  mileage=force[i]['里程_'+dire],
                  force_left_chui=force[i]['垂_'+dire],
                  force_left_heng=force[i]['横_'+dire],
                  force_right_chui=force[j]['垂_'+dire],
                  force_right_heng=force[j]['横_'+dire],
                  dir_=dir_gepian,
                  name=name)
        ...
        huizong[name]['速度_脱轨系数']=round(res.loc[np.argmax(ls),'速度'],1)
        huizong[name]['里程_脱轨系数']=round(res.loc[np.argmax(ls),'里程'],3)    
        ls=res['轮重减载率']
        huizong[name]['轮重减载率']=round(ls.max(),2)
        huizong[name]['速度_减载率']=round(res.loc[np.argmax(ls),'速度'],1)
        huizong[name]['里程_减载率']=round(res.loc[np.argmax(ls),'里程'],3)   
        ls=res['轮轴横向力']
        huizong[name]['轮轴横向力']=round(ls.max(),1)
        huizong[name]['速度_横向力']=round(res.loc[np.argmax(ls),'速度'],1)
        huizong[name]['里程_横向力']=round(res.loc[np.argmax(ls),'里程'],3)
        
        plot_wheel_rail_force(res,dir_gepian,name)
        plot_wheel_rail_force_baogao(res,dir_gepian_baogao,name)
  ...
  #画速度图      
        ...
        plt.savefig(dir_sudu+'/'+name+'_速度.png', dpi=200)              
...
##报告用画图
def plot_wheel_rail_force_baogao(res,dir_,name):
    ...
    plt.grid(b='True',linestyle="dashed",linewidth=1)     
    plt.axhline(0.65,color = "r",linewidth = '2')
    plt.ylim(0,0.8) 
...
def cal_index(time_,speed,mileage,force_left_chui,force_left_heng,force_right_chui,force_right_heng,dir_,name):               
    res={}
