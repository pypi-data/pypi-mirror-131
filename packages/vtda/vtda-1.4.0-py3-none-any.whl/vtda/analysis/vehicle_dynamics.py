...
def discontinuous_wheel_rail_force(data, 
    ...
    calibration_factor={}
    for i in info_biaoding:
        pass
        calibration_factor[i]=np.linalg.inv(info_biaoding[i])
        
    speed=lvbo_low(data[info_tongdao['速度']],fq=3,fs=sample_rate,n=1)
    mileage=data[info_tongdao['里程']]
    yingbian={}
    list_tongdao=['左轮垂1','左轮横1','右轮垂1','右轮横1','左轮垂2','左轮横2','右轮垂2','右轮横2']
    fq=50     #50Hz低通滤波
    n=3   #董工数采应变有个3倍的关系，此处除以3    
    for i in info_tongdao:
        pass
        if i in list_tongdao:
            yingbian[i]=lvbo_low(data[info_tongdao[i]],fq=fq,fs=sample_rate)[speed>5]/n  
    ...
    #每个轮理论上可以出4个应力，考虑到后期可能有某个位置数值不可用，
    #所以采用解耦的方式分别进行求解，然后再叠加，这样方便剔除不可用数据  
    #一组桥比如0度、90度一般是一正一副的标定系数，正值对应的是上沿的应变，负值对应下沿的应变
    res_force={}
    info=
...
#90°片 
    ...
    data_chui=data['1']['8']
    data_heng=data['1']['9']
    info_biaoding=np.array([[0.714,-0.427],[-0.042,2.322]])
...
def dcon_wheel_rail_force_single_deta_time(data_chui, 
                                           data_heng,
                                    sample_rate=5000,
                                    info_biaoding=np.array([[0.714,-0.427],[-0.042,2.322]]),
                                    ):
    ...
    按照时间提取轮轨力，1秒提取一个值
    '''
    #data_chui=yingbian_left_chui_1
    #data_heng=yingbian_left_heng_1
    nn=0.2
    len_=sample_rate*nn    
    ...
    for i in np.arange(n_zong):
        ...
        yingbian_chui.append(y_.max())
        loc.append(int(i*len_+np.argmax(y_))) 
    
    yingbian_chui=np.array(yingbian_chui) 
    loc=np.array(loc)     
    yingbian_heng=data_heng[loc]
    #得出相应力 
    force=np.dot(info_biaoding,np.array([yingbian_chui,yingbian_heng])).T
    
    return loc,force[:,0],force[:,1]
...
def find_min_max_location(y):
    ...
    h=np.percentile(res,85)-np.percentile(res,1)
