...
class Figure_Card(v.Card):
    ...
    def create_figure(self):
        ...
        figure_tabs = v.Tabs(children=[train_tab, validation_tab, train_item, validation_item], fixed_tabs=True, color='#1d376c')
    ...
    def plot(self, train_df: DataFrame, validation_df: DataFrame):
        ...
        for tag in train_df.columns:
            self.train_fig.add_trace(go.Scatter(x=train_df.index, y=train_df[tag], name=tag, visible=False, line=dict(color=self.colors[tag_id%10])))
            tag_id += 1

        tag_id = 0
        for tag in validation_df.columns:
            self.validation_fig.add_trace(go.Scatter(x=validation_df.index, y=validation_df[tag], name=tag, visible=False, line=dict(color=self.colors[tag_id%10]))) 
            tag_id += 1
