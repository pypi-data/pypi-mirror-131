...
def find_matching_structure(data, invariant, chiral=False, minimal=False, external_dictionary=''):
    if not data:
        return data
    if invariant == 'Writhe':
        return data
    if type(data) is str or type(data) is Poly:
        ...
        return find_matching_knot(data, invariant, chiral=chiral, minimal=False,
...
def get_number_of_crossings(knot_name):
    ...
    for components in knot_name.split('#'):
        firstnum = components.split('_')[0].split('^')[0]
        no_of_crossings = int(firstnum.rstrip('n').rstrip('a').lstrip('+').lstrip('-'))
        crossings += no_of_crossings
    return crossings


def find_matching_knot(data, invariant, chiral=False, minimal=False, external_dictionary=''):
    data = str(data)
    inverted = -1
...
def find_point_matching(data, invariant, chiral=False, minimal=False, external_dictionary=''):
    result = {}
...
class Invariant(Graph):
    ...
    def calculate(self, invariant, closure=Closure.TWO_POINTS, tries=200, direction=0, reduce_method=ReduceMethod.KMT,
                  poly_reduce=True, translate=False, external_dictionary='', chiral=False, minimal=False,
...
class AlexanderGraph(Invariant):
    ...
    def translate_cuda_matrix(self, matrix, chiral=False, minimal=False, translate='piu'):
        new_matrix = {}
