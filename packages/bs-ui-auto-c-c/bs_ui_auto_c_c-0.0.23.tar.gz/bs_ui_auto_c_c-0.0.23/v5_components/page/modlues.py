...
def login_interface(environment, username, password):
    ...
    login_data = {
        ...
        "IsShowValCode": "false",
        "ValCodeKey": ""}
    login_url = get_environment_data(environment).get('italent_url')
...
def unfinished_transactions(driver, transaction_type, transaction_name):
    ...
    session = requests.session()
    italent_url = os.environ.get('environment').get('italent_url')
    ...
    cloud_url = os.environ.get('environment').get('cloud_url')
    driver.add_cookie(
