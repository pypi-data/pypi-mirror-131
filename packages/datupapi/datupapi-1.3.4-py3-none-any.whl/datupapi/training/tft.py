...
class Tft(Config):
    ...
    def add_dates(self, Qprep, data1, predict, suffix):
        data_range=pd.date_range(start=Qprep.timestamp.min(), periods=Qprep.timestamp.nunique()+self.forecast_horizon, freq=self.dataset_frequency+suffix) if self.dataset_frequency=="M" else pd.date_range(start=Qprep.timestamp.min(), end=Qprep.timestamp.max().date()+relativedelta(weeks=self.forecast_horizon), freq=self.dataset_frequency+suffix)
    ...
    def create_trainer(self, callbacks, logger):
        trainer = pl.Trainer(
            ...
            limit_train_batches=400,  # coment in for training, running valiation every 30 batches
