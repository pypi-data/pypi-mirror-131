...
def train_test_split(data, num_train=None, num_val=None, split_file=None, stratify_partitions=False, num_per_partition=False,):
    """
    ...
    Returns:
        nnpackage.data.AtomsData: training dataset
        nnpackage.data.AtomsData: validation dataset
        nnpackage.data.AtomsData: test dataset
    ...
    if split_file is not None and os.path.exists(split_file):
        ...
        assert num_train + num_val <= len(data), "Dataset is smaller than num_train + num_val!"
        ...
        if split_file is not None: np.savez(split_file, train_idx=train_idx, val_idx=val_idx, test_idx=test_idx)
...
def get_subset(data, num_samples=None, split_file=None, stratify_partitions=False, num_per_partition=False,):
    """
    Args:
    ...
    Returns:
        nnpackage.data.AtomsData: training dataset
        list : actual idxs of the training dataset

    """
    if split_file is not None and os.path.exists(split_file):
        S = np.load(split_file)
        sample_idx = S["sample_idx"].tolist()

    else:
        if num_samples is None:
            # need to take all data
            sample_idx = []
            idx = np.random.permutation(len(data))
            sample_idx = idx[:].tolist()

        else:
            assert num_samples <= len(data), "Dataset is smaller than num_samples!"

            num_samples = num_samples if num_samples > 1 else num_samples * len(data)
            num_samples = int(num_samples)

            if stratify_partitions:
                partitions   = data.get_metadata("partitions")
                n_partitions = len(partitions)
                if num_per_partition: num_samples_part = num_samples
                else:                 num_samples_part = num_samples // n_partitions

                sample_idx = []
                for start, stop in partitions.values():
                    idx         = np.random.permutation(np.arange(start, stop))
                    sample_idx += idx[:num_samples_part].tolist()
            else:

                idx        = np.random.permutation(len(data))
                sample_idx = idx[:num_samples].tolist()

            if split_file is not None: np.savez(split_file, sample_idx=sample_idx,)
    
    return data.create_subset(sample_idx), sample_idx
