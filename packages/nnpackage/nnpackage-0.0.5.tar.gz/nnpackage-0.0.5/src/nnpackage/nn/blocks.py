...
from nnpackage.nn import shifted_softplus, Dense
...
class ElementalGate(nn.Module):
    ...
    def __init__(self, elements, onehot=True, trainable=False):
        ...
        if onehot:
            ...
            for idx, Z in enumerate(elements): weights[Z, idx] = 1.0
            self.gate.weight.data = weights

        # Set trainable flag
        if not trainable: self.gate.weight.requires_grad = False
...
class GatedNetwork(nn.Module):
    ...
    def forward(self, inputs):
        """
        Args:
            inputs (dict of torch.Tensor): nnpackage format dictionary of input tensors.

        Returns:
            torch.Tensor: Output of the gated network.
        """
        # At this point, inputs should be the general nnpackage container
