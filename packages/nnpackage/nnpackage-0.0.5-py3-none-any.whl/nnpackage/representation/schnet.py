...
from nnpackage.nn.base        import Dense
from nnpackage                import Properties
from nnpackage.nn.cfconv      import CFConv
from nnpackage.nn.cutoff      import CosineCutoff
from nnpackage.nn.acsf        import GaussianSmearing
from nnpackage.nn.neighbors   import AtomDistances
from nnpackage.nn.activations import shifted_softplus
...
class SchNet(nn.Module):
    ...
    def __init__(
        ...
        if distance_expansion is None: self.distance_expansion = GaussianSmearing(0.0, cutoff, n_gaussians, trainable=trainable_gaussians)
        else:                          self.distance_expansion = distance_expansion
    ...
    def forward(self, inputs):
        ...
        Args:
            inputs (dict of torch.Tensor): nnpackage dictionary of input tensors.
        ...
        if self.return_intermediate: xs = [x]
        # compute interaction block to update atomic embeddings
        for interaction in self.interactions:
            v = interaction(x, r_ij, neighbors, neighbor_mask, f_ij=f_ij)
            x = x + v
            if self.return_intermediate: xs.append(x)

        if self.return_intermediate: return x, xs
        return x
