...
from peltak.core import context, log, shell
...
def extract_from_files(list_name: str, files: List[str]) -> List[Todo]:
    todos: List[Todo] = []
    list_name = f" <92>{list_name}<32>" if list_name else ''

    if context.get('verbose') >= 1:
        log.info(f"Checking <33>{len(files)}<32>{list_name} files")
