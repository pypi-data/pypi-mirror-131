...
def experiment(experiment_names: List[str]):
    ...
    for name in experiment_names:
        ...
        try:
            ...
            if not exp.exists:
                raise RuntimeError(f"Experiment {name} does not exist.")
            if not exp.delete():
                raise RuntimeError(f"Experiment {name} could not be deleted.")
            spinner.ok("✔")
    ...
    for name in run_names:
        ...
        try:
            ...
            if not run_obj.exists:
                raise RuntimeError(f"Run {name} does not exist.")
            if not run_obj.delete():
                raise RuntimeError(f"Run {name} could not be deleted.")
            spinner.ok("✔")
            styled_name = click.style(name, fg='blue')
...
@click.option(
    ...
    is_flag=True,
    help=
    'Force delete cluster from grid system. This does NOT delete any resources created by the cluster, just cleaning up the entry from the grid system. You should not use this under normal circumstances',
)
def cluster(cluster: str, force: bool = False):
