...
class GpsDaemon(Daemon):
    ...
    def startup(self):
        ...
        sleep_duration = self._get_config_float('GPSD', 'Sleep', 0.1)
        if sleep_duration <= 0:
            log.warning('You have disabled sleeping between frames which may result in a high CPU core use!')
    ...
    def _build_messages(self, strm: agps3.DataStream, raw: str) -> dict:
        return {
            ...
            KEY_ALTITUDE: self._get_float(strm.alt),
