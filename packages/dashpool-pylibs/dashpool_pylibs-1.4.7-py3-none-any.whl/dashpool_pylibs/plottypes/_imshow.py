...
def _get(inputDataFrame, plotConfigData):
    ...
    if "x" in plotConfigData["params"] and "y" in plotConfigData["params"] and "color" in plotConfigData["params"]:
        # extrat params
        ...
        if (len(extraGroupParams) > 0):
            ...
            for group, allValues in inputDataFrame.groupby(extraGroupParams):
                data_dict[group] = _get_MAP_array(allValues)
            ...
            for name, z in data_dict.items():
                fig.add_heatmap(
                    ...
                    z=z,
                    name=name,
                    hovertemplate=f"{x}: %{{x}}<br>{y}: %{{y}}<br>{color}: %{{z}}<br>",
                    row=rowid,
                    col=colid,
                    coloraxis="coloraxis",
            ...
            fig.add_heatmap(
                x0=all_min[0], dx=1,
                y0=all_min[1], dy=1,
                z=_get_MAP_array(inputDataFrame),
                hovertemplate=f"{x}: %{{x}}<br>{y}: %{{y}}<br>{color}: %{{z}}<br>"
            )
