...
@click.command()
@click.option("--input","-i",default= './',help="Path onde ler os arquivos para conversão",type=str)
@click.option("--output","-o",default= './',help="Path onde irá ser convertido os arquivos salvos",type=str)
@click.option("--delimiter", "-d",default= ';',help="Separador usedado para separar os arquivos",type=str)
@click.option("--prefix","-p",prompt=True,prompt_required = False,default='file',
    help = (
        "Prefixo usado para prefixar o nome do arquivo convertido salvo em disco. "
        "O sufixo irá ser o ínicio de um número de origem: 0. ge: file_0.json"
    )
)    
def converter(input : str = './', output: str = './', delimiter: str = ',', prefix: str = None):
    """Converter um arquivo ou uma lista de csv para Json."""
    ...
    for p in[input_path, output_path]:
        if not(p.is_file() or p.is_dir()):
            raise TypeError("O PATH ou o nome do arquivo não são válidos.")
...
def parse_csv_to_json(data) -> list:
    """Tranformar um arquivo ou um dicionário em Json"""
    header = data[0]
    body = data[1:]
...
def write_json_data(data: list,source: path, prefix: str):
    ...
    logger.info("Saving file %s in folder %s", file_name, source)
...
def read_File_path(source:Path, delimiter:str, name) -> list:
    """Ler um arquivo de origem de um PATH"""
...
def read_csv_file(source: Path, delimiter: str):
    """Carregar um arquivo csv na memória.
    Args:
    ...
    """
    if source.is_file():
        logger.info("Lendo somente um arquivo %s", source)
        data = read_File_single_arch(source, delimiter)
        return data
    logger.info("Lendo todos os arquivos para determinado PATH %s", source)
...
@click.option("--prefix","-p",prompt=True,prompt_required = False,default='file',
    help = (
        "Prefixo usado para prefixar o nome do arquivo convertido salvo em disco. "
        "O sufixo irá ser o ínicio de um número de origem: 0. ge: file_0.csv"
    )
)  
def converter_2(input : str = './', output: str = './', prefix: str = None):
    """Converter um arquivo ou uma lista de arquivos em Json para csv."""
    input_path = Path(input)
    output_path = Path(output)
    logger.info("Input Path: %s", input_path)
    logger.info("Output Path: %s", output_path)
    for p in[input_path, output_path]:
        if not(p.is_file() or p.is_dir()):
            raise TypeError("O PATH ou o nome(s) do(s) arquivo(s) não são válidos.")
    data = read_json_files(source = input_path)
    json_data = parse_json_to_csvs(data)
    write_csv_datas(json_data, output_path, prefix)

def write_csv_datas(data: list,source: path, prefix: str):
    file_name = source.joinpath(f"{prefix}.csv")
    logger.info("Salvando um arquivo %s em uma pasta %s", file_name, source)
    string = ""
    string_body = ""
    with open(file_name, "w") as outfile:
...
def parse_json_to_csvs(data) -> list:
    list_header = []
    list_header_body = []
    string = ""
...
def read_json_File_single_arch(source:Path) -> list:
    """Lendo somente um arquivo em csv."""
    with open(source,'r') as file:
        data = file.readlines()
    return data
    
def read_json_File_path(source:Path, name) -> list:
    """Lendo arquivos de um Path"""
    with open(name,"r") as file:
      data = file.readlines()
    return data    

def read_json_files(source: Path):
    """Carregar arquivos de csv na memória.
    Args:
        source (Path): Path of a single csv file or a directory containing csvs to be parsed;
        delimiter (str): Separator for columns in csv.
    Return:
    ...
    if source.is_file():
        ...
        return data_json
    logger.info("Lendo todos arquivos para determinado PATH %s", source)
    for name in source.iterdir():
