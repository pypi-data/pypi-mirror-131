...
from .ParameterStructure.Instances import SimInstancePlacementGeometry
...
from .utils import sort_references, sort_component_list, parse_slot
...
class SimultanObject(object, metaclass=MetaMock):
    ...
    def referenced_components(self):
        if self._referenced_components is None:
            if self._wrapped_obj is not None:
                self._referenced_components = [self._template_parser.create_python_object(x.Reference) for x in self._wrapped_obj.ReferencedComponents.Items]
    ...
    def get_slot_components(self):
        ...
        for comp in self._wrapped_obj.ReferencedComponents.Items:
            c_slot = comp.ReferenceFunction.SlotBase.Base
            c_slot_extension = comp.ReferenceFunction.SlotExtension

            sc_df = sc_df.append(
                pd.DataFrame(data={'comp': [self._template_parser.create_python_object(comp.Reference)],
                                   'slot': [c_slot],
                                   'slot_extension': c_slot_extension,
                                   'comp_type': 1}),
                ignore_index=True)
    ...
    def replace(self, key: str, new_component: Union[SimComponentType, SimultanObject]):
        ...
        # old component is not None:
            ...
            for ref_comp in set(component._wrapped_obj.ReferencedBy):
                ref = next((x for x in ref_comp.ReferencedComponents.Items if x.Reference == component._wrapped_obj), None)
            ...
            # old component is reference
                ...
                ref = next((x for x in self._wrapped_obj.ReferencedComponents.Items
                            if ((x.ReferenceFunction.SlotBase.Base == slot_name) & (
                        x.ReferenceFunction.SlotExtension == slot_extension))), None)
                if isinstance(new_component, SimultanObject):
                    new_sim_component = new_component._wrapped_obj
                    ref.Reference = new_component._wrapped_obj
                elif isinstance(new_component, SimComponent):
            ...
            for ref in references:
                ref.Reference = new_sim_component
...
class BuildInWallConstruction(BaseBuildInConstruction):
    ...
    def layers(self):
        ...
        slot_extensions = [int(x.Slot.SlotExtension) for x in self._wrapped_obj.Components.Items]
        # test with rotated
