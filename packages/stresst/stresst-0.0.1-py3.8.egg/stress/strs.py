...
def main():
	...
	wa = False
	if os.path.exists(os.path.join(os.path.abspath(os.getcwd()), str(args.file[0]))) and os.path.exists(os.path.join(os.path.abspath(os.getcwd()), str(args.test[0]))):
			...
			for i in f:
				...
				if pth.endswith(".in"):			
					with open(pth, 'r', encoding='utf-8') as fin:
						...
						p = subprocess.Popen(['python', str(args.file[0])], stdin=fin, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
