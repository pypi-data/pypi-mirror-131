...
def _resolve_test_plan(workspace_id: str, testplan_ref: str, authz_header: str) -> Plan:
    ...
    mapped_folder_name = local.map_folder_to_docker(folder_name=testplan_ref)
    if mapped_folder_name:
        plan = local.get_plan(mapped_folder_name, workspace_id=workspace_id)
    ...
    raise LevoCustomError(
        ...
        " Please specify the absolute path if using a folder."
    )
    # End
