...
from motus import analisis_regresion_lineal as arl
...
class Experimento:
    ...
    def __init__(self, nombre_archivo, dimension_caja=[100, 100], cantidad_filas=10, cantidad_columnas=10,
                 ...
                 tamanio_intervalo_para_promedio=100, genera_matriz_recurrencia=False, regiones_preferidas=None,
    ...
    def calcula_todos_histogramas(self):
        self.calcula_histograma_velocidades()
        self.calcula_histograma_objeto_1()
        self.calcula_histograma_objeto_2()
        self.calcula_histograma_objeto_3()
        self.calcula_histograma_objeto_4()

    def calcula_histograma_velocidades(self):
        self.valores_histograma_velocidad, bins, bars = plt.hist(self.serie_velocidad)

    def calcula_histograma_objeto_1(self):
        self.valores_histograma_objeto_1, bins, bars = plt.hist(self.dist_objeto_1)

    def calcula_histograma_objeto_2(self):
        self.valores_histograma_objeto_2, bins, bars = plt.hist(self.dist_objeto_2)

    def calcula_histograma_objeto_3(self):
        self.valores_histograma_objeto_3, bins, bars = plt.hist(self.dist_objeto_3)

    def calcula_histograma_objeto_4(self):
        self.valores_histograma_objeto_4, bins, bars = plt.hist(self.dist_objeto_4)
    ...
    def genera_matriz_cuadriculada_con_regiones_preferidas(self):
        matriz_cuadriculada_plana = np.flipud(self.matriz_cuadriculada).flatten()
        self.visitas_regiones_preferidas = matriz_cuadriculada_plana[self.regiones_preferidas]
        self.visitas_regiones_no_preferidas = np.delete(matriz_cuadriculada_plana, self.regiones_preferidas)

        resultado_visitas_regiones_preferidas = sum(self.visitas_regiones_preferidas)
        resultado_visitas_regiones_no_preferidas = sum(self.visitas_regiones_no_preferidas)

        arreglo_regiones_preferidas = resultado_visitas_regiones_no_preferidas * np.ones(len(matriz_cuadriculada_plana))
        arreglo_regiones_preferidas[self.regiones_preferidas] = resultado_visitas_regiones_preferidas

        self.matriz_cuadriculada_regiones_preferidas = \
            np.reshape(arreglo_regiones_preferidas, (self.cantidad_filas, self.cantidad_columnas))

        self.matriz_cuadriculada_regiones_preferidas = np.flipud(self.matriz_cuadriculada_regiones_preferidas)

    def genera_matriz_cuadriculada_solo_regiones_preferidas(self):
        matriz_cuadriculada_plana = np.flipud(self.matriz_cuadriculada).flatten()
        self.visitas_regiones_preferidas = matriz_cuadriculada_plana[self.regiones_preferidas]

        resultado_visitas_regiones_preferidas = sum(self.visitas_regiones_preferidas)

        arreglo_regiones_preferidas = np.zeros(len(matriz_cuadriculada_plana))
        arreglo_regiones_preferidas[self.regiones_preferidas] = resultado_visitas_regiones_preferidas

        self.matriz_cuadriculada_solo_regiones_preferidas = \
            np.reshape(arreglo_regiones_preferidas, (self.cantidad_filas, self.cantidad_columnas))

        self.matriz_cuadriculada_solo_regiones_preferidas = np.flipud(self.matriz_cuadriculada_solo_regiones_preferidas)

        print("Matriz cuadriculada, solo regiones preferidas", self.matriz_cuadriculada_solo_regiones_preferidas)

    def genera_matriz_cuadriculada_solo_regiones_no_preferidas(self):
        matriz_cuadriculada_plana = np.flipud(self.matriz_cuadriculada).flatten()
        self.visitas_regiones_preferidas = matriz_cuadriculada_plana[self.regiones_preferidas]
        self.visitas_regiones_no_preferidas = np.delete(matriz_cuadriculada_plana, self.regiones_preferidas)
        ...
        arreglo_regiones_no_preferidas = resultado_visitas_regiones_no_preferidas * np.ones(len(matriz_cuadriculada_plana))
        arreglo_regiones_no_preferidas[self.regiones_preferidas] = 0

        self.matriz_cuadriculada_solo_regiones_no_preferidas = \
            np.reshape(arreglo_regiones_no_preferidas, (self.cantidad_filas, self.cantidad_columnas))

        self.matriz_cuadriculada_solo_regiones_no_preferidas = np.flipud(self.matriz_cuadriculada_solo_regiones_no_preferidas)

        print("Matriz cuadriculada, solo regiones no preferidas", self.matriz_cuadriculada_solo_regiones_no_preferidas)
    ...
    def calcula_alturas_volumenes_circulares_objetos_relevantes(self, tamanio=24, divisiones=2, tipo="cilindros"):
        ...
        for i in range(0, longitud_movimientos):
            ...
            dist_4 = fa.calcula_distancia_entre_puntos(self.objeto_relevante_4[0], x_pos, self.objeto_relevante_4[1],
                                                       y_pos)
            if dist_1 < tamanio:
                cantidad_posiciones_volumen_1 = fa.modificar_lista_para_cilindros(cantidad_posiciones_volumen_1,
                                                                                  radios_figuras,
                                                                                  dist_1,
                                                                                  tipo=tipo)
            if dist_2 < tamanio:
                cantidad_posiciones_volumen_2 = fa.modificar_lista_para_cilindros(cantidad_posiciones_volumen_2,
                                                                                  radios_figuras,
                                                                                  tipo=tipo)
            if dist_3 < tamanio:
                cantidad_posiciones_volumen_3 = fa.modificar_lista_para_cilindros(cantidad_posiciones_volumen_3,
                                                                                  radios_figuras,
                                                                                  tipo=tipo)
            if dist_4 < tamanio:
                cantidad_posiciones_volumen_4 = fa.modificar_lista_para_cilindros(cantidad_posiciones_volumen_4,
                                                                                  radios_figuras,
                                                                                  tipo=tipo)
        ...
        self.cantidad_posiciones_volumen_1 = cantidad_posiciones_volumen_1
        self.cantidad_posiciones_volumen_2 = cantidad_posiciones_volumen_2
        self.cantidad_posiciones_volumen_3 = cantidad_posiciones_volumen_3
        self.cantidad_posiciones_volumen_4 = cantidad_posiciones_volumen_4
        self.radios_volumenes = radios_figuras
    ...
    def grafica(self, grafica, lim_inf_x=None, lim_sup_x=None, lim_inf_y=None, lim_sup_y=None,
                ...
                tamanio_volumenes=24, div_volumenes=2, tipo_grafica_volumen="cilindros",
                arreglo_series_tiempo=None, brincos_series_tiempo=1, longitud_series_tiempo=1,
                arreglo_grafica_distancias=[True, True, True, True],
        ...
        #        plt.imshow(np.flipud(self.matriz_cuadriculada)))
            ...
            indices_tiempo, series_tiempo = arl.generar_series_tiempo(lista_a_procesar= arreglo_series_tiempo,
                                                                     brinco_temporal=brincos_series_tiempo,
                                                                     longitud_ventana_tiempo=longitud_series_tiempo)
            ...
            self.calcula_alturas_volumenes_circulares_objetos_relevantes(tamanio=tamanio_volumenes,
        ...
        elif grafica == 'region_cuadricula_3d_regiones_preferidas':
        ...
        elif grafica == 'region_cuadricula_3d_solo_regiones_preferidas':
        ...
        elif grafica == 'region_cuadricula_3d_solo_regiones_no_preferidas':
