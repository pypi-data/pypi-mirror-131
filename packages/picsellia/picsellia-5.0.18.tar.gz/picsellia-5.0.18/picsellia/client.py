...
import subprocess
import threading
from pxl_requests import pxl_requests
...
class Client:
    ...
    class Datalake:
        ...
        class Dataset:
            ...
            @exception_handler
            @beartype
            def get_annotations(self, picture_name: str=None, limit: int=None, offset: int=None):
                ...
                if picture_name is None:
                    if limit is not None and offset is not None:
                        ...
                        annotations = []
                        annot_page = self._get_annotation_page(100, 0)
                        annotations += annot_page["annotations"]
                        nb_pages = annot_page["nb_pages"]
                        for page in range(1,nb_pages-1):
                            annot_page = self._get_annotation_page(100, 100*page)
                            annotations += annot_page["annotations"]
            ...
            def _get_annotation_page(self, limit, offset):
                params = {
                    'limit': limit,
                    'offset': offset
                }
