...
@LastEditTime: 2021-12-14 10:04:48
...
class FrameBaseHandler(BaseApiHandler):
    ...
    def _convert_request_params(self):
        """
        :Description: 转换请求参数 post请求：Content-type必须为application/json，前端必须对对象进行序列化转成json字符串，不能直接传对象,否则无法接收参数
        :last_editors: HuangJianYi
        """
        ...
        if is_encrypt == True and ("/server/" in self.request.uri and server_encrypt_type == 1) or ("/client/" in self.request.uri and client_encrypt_type == 1):
            ...
            #兼容淘宝小部件 当有参数orderId的时候取lc_app_id
            if self.get_argument("orderId", "", strip=True):
                app_id = self.get_argument("lc_app_id", "", strip=True)
            else:
    ...
    def get_source_app_id(self):
        ...
        """
        #兼容淘宝小部件 当有参数orderId的时候取lc_app_id
        if self.get_param("orderId"):
            source_app_id = self.get_param("lc_app_id")
