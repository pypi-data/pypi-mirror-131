...
class RDSInstance:
    def __init__(self, client, region_id, instance_id, intranet_download):
        self.client = client
        self.region_id = region_id
        self.instance_id = instance_id
        self.intranet_download = intranet_download
    ...
    def get_fullbackup_files(self, start_time, end_time=None, top=0, dummy=False):
        ...
        while True:
            ...
            for bkp in response['Items']['Backup']:
                download_url = bkp["BackupIntranetDownloadURL"] if self.intranet_download else bkp["BackupDownloadURL"]
                ...
                file_end_time = datetime.strptime(bkp["BackupEndTime"],
                                                  "%Y-%m-%dT%H:%M:%SZ")
                file = DBFile(
                    download_url, self.intranet_download, host_id,
        ...
        while True:
            ...
            for binlog in response['Items']['BinLogFile']:
                if binlog['HostInstanceID'] == host_id:
                    download_url = binlog['IntranetDownloadLink'] if self.intranet_download \
                        else binlog['DownloadLink']
                    ...
                    file_end_time = datetime.strptime(
                        binlog['LogEndTime'],
                        "%Y-%m-%dT%H:%M:%SZ")
                    file = DBFile(download_url, self.intranet_download, host_id,
                                  self.region_id, self.instance_id,
                                  file_start_time, file_end_time,
                                  file_type='binlog',
                                  file_size=file_size,
