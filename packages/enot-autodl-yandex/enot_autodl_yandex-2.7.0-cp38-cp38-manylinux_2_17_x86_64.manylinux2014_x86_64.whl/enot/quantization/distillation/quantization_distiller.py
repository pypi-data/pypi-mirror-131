...
from enot.utils.data.dataloaders import to_device
...
class QuantizationDistiller(DistillerInterface):
    ...
    def __init__(
            self,
            quantized_model: TrtFakeQuantizedModel,
    ...
    def _base_distill(
            self,
            quantized_model: TrtFakeQuantizedModel,
            regular_model: TrtFakeQuantizedModel,
    ) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch in tqdm_iterator:
                ...
                to_device(model_args, device=self.device, ignore_non_tensors=True)
                to_device(model_kwargs, device=self.device, ignore_non_tensors=True)
...
class DefaultQuantizationDistiller(SequentialDistiller):
    ...
    def __init__(
        ...
        self.quantized_model: TrtFakeQuantizedModel = quantized_model
        ...
        threshold_distiller = QuantizationDistiller(
        ...
        weight_distiller = QuantizationDistiller(
            quantized_model=quantized_model,
            dataloader=dataloader,
            optimizer=weight_optimizer,
        ...
        super().__init__(threshold_distiller, weight_distiller)
    ...
    def distill(self) -> None:
        ...
        if self.verbose >= 2:
            _LOGGER.info('Fine-tuning model weights and quantization thresholds.')
        super().distill()
