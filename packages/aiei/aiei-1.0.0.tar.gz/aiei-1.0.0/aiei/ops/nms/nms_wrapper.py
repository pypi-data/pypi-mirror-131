# Copy from https://github.com/open-mmlab/mmdetection/tree/master/mmdet/ops
# Modified by Lei Zhao (zlcnup.com)
...
def batched_nms(boxes, scores, inds, iou_thr):
    ...
    Arguments:
        ...
        inds (torch.Tensor): each index value correspond to a bbox cluster,
            ...
            shape (N, ).
        iou_thr (float): same nms
    Returns:
        tuple: kept bboxes and indice.
    """
    max_coordinate = boxes.max()
    offsets = inds.to(boxes) * (max_coordinate + 1)
    bboxes_for_nms = boxes + offsets[:, None]
    dets, keep = nms(torch.cat([bboxes_for_nms, scores[:, None]], -1), iou_thr)
    boxes = boxes[keep]
    scores = dets[:, -1]
    return torch.cat([boxes, scores[:, None]], -1), keep


def batched_soft_nms(boxes, scores, inds, iou_thr, method='linear', sigma=0.5, min_score=1e-3):
    max_coordinate = boxes.max()
    offsets = inds.to(boxes) * (max_coordinate + 1)
    bboxes_for_nms = boxes + offsets[:, None]
    dets, keep = soft_nms(torch.cat([bboxes_for_nms, scores[:, None]], -1), iou_thr, method, sigma, min_score)
    boxes = boxes[keep]
    scores = dets[:, -1]
    return torch.cat([boxes, scores[:, None]], -1), keep
