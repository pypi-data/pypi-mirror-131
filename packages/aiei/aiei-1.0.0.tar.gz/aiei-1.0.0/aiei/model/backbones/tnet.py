...
MODEL_DIR = f'{cfg.AIEI.TORCH_MODEL_HOME}/tnet'
...
class BasicLx(nn.Module):
    def __init__(self, ic=64, lx=2):
        ...
        if True:
            ...
            pre_state_dict = torch.load(f"{MODEL_DIR}/TNet_D2W{chs[0]}.pth", map_location=lambda storage, loc: storage)
            count = 0
...
class TNetD2(nn.Module):
    def __init__(self, ic=64, dx=2):
        ...
        if True:
            state_dict = self.state_dict()
            pre_state_dict = torch.load(f"{MODEL_DIR}/TNet_D{self.dx}W{chs[0]}.pth", map_location=lambda storage, loc: storage)
            count = 0
            for k, v in pre_state_dict.items():
                if k not in state_dict:
                    continue
