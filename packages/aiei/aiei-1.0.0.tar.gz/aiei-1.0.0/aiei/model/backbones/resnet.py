...
class FPN(nn.Module):
    def __init__(self, resnet_depth=50, **kwargs):
        super().__init__()
        self.res = ResNet(f'resnet{resnet_depth}', pretrained=True, **kwargs)
        self.fpn = FeaturePyramidNetwork([256, 512, 1024, 2048] if resnet_depth >= 50 else [64, 128, 256, 512],
...
class DeconvLayer(nn.Module):
    def __init__(
            self, in_planes, out_planes, deconv_kernel, deconv_stride=2, deconv_pad=1, deconv_out_pad=0, modulate_deform=True):
