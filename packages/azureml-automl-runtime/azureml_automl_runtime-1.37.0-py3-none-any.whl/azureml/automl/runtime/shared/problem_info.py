...
class ProblemInfo:
    ...
    def __repr__(self) -> str:
        ...
        new_params = {
            k: params[k] for k in params if not (k.startswith("_") or
                                                 k == 'runtime_constraints' or
                                                 (k in default_values and
                                                  np.all(default_values[k] == params[k])))
        }
    ...
    def get_default_parameters_and_values(cls):
        ...
        default_param_values = {
            ...
            if (
                init_sig.parameters[parameter].default != inspect.Parameter.empty and
                init_sig.parameters[parameter].kind
                in [
    ...
    def clean_attrs(self, useful_attrs: List[str]) -> 'ProblemInfo':
        ...
        default_values = ProblemInfo.get_default_parameters_and_values()
        for k in params:
            if (k in default_values and
                np.any(default_values[k] != params[k]) and
                k != 'runtime_constraints' and
                    k not in useful_attrs):
