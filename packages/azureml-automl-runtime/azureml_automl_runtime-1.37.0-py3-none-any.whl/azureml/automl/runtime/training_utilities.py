...
def _init_classic_experiment_store(
    ...
    expr_store.data.materialized.set_train(
        transformed_data_context.X, transformed_data_context.y, transformed_data_context.sample_weight
    )
    ...
    if task_type == constants.Tasks.REGRESSION:
        if transformed_data_context.cv_splits:
            bin_info = get_dataset_bins(
                transformed_data_context.cv_splits, transformed_data_context.X, transformed_data_context.y
            )
        else:
            bin_info = make_dataset_bins(transformed_data_context.X_valid.shape[0], transformed_data_context.y_valid)
