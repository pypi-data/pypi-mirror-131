...
class ExperimentData(_CacheableStoreBase):
    ...
    class MaterializedData(_CacheableStoreBase):
        ...
        def get_test(self):
            ...
            """
            ret = self._cache.get([Keys.FULL_TEST])  # type: Dict[str, pd.DataFrame]
            if ret is not None:
                merged_df = ret.get(Keys.FULL_TEST)  # type: pd.DataFrame
                if merged_df is not None:
                    return get_x_y_sample_weight_from_merged_df(merged_df)
                else:
                    separate_keys = [Keys.X_TEST, Keys.Y_TEST, Keys.SW_TEST]
        ...
        def set_test(
            ...
            if self._read_only:
                ...
                if merged_df is None:
                    self._cache.set(Keys.X_TEST, X)
                    self._cache.set(Keys.Y_TEST, y)
                    self._cache.set(Keys.SW_TEST, sample_weight)
                else:
                    self._cache.set(Keys.FULL_TEST, merged_df)
        ...
        def get_train(self):
            ...
            """
            ret = self._cache.get([Keys.FULL_TRAIN])  # type: Dict[str, Any]
            if ret is not None:
                merged_df = ret.get(Keys.FULL_TRAIN, None)  # type: pd.DataFrame
                if merged_df is not None:
                    return get_x_y_sample_weight_from_merged_df(merged_df)
                else:
                    separate_keys = [Keys.X_TRAIN, Keys.Y_TRAIN, Keys.SW_TRAIN]
                    ret = self._cache.get(separate_keys)
                    return ret[separate_keys[0]], ret[separate_keys[1]], ret[separate_keys[2]]
        ...
        def set_train(
            ...
            if self._read_only:
                ...
                if merged_df is None:
                    self._cache.set(Keys.X_TRAIN, X)
                    self._cache.set(Keys.Y_TRAIN, y)
                    self._cache.set(Keys.SW_TRAIN, sample_weight)
                else:
                    self._cache.set(Keys.FULL_TRAIN, merged_df)
        ...
        def set_raw(self, X, y, X_valid, y_valid):
            if self._read_only:
                ...
                self._cache.set(Keys.X_RAW, X)
                self._cache.set(Keys.Y_RAW, y)
                self._cache.set(Keys.X_RAW_VALID, X_valid)
                self._cache.set(Keys.Y_RAW_VALID, y_valid)
        ...
        def get_valid(self):
            ...
            """
            ret = self._cache.get([Keys.FULL_VALIDATION])  # type: Dict[str, pd.DataFrame]
            if ret is not None:
                merged_df = ret[Keys.FULL_VALIDATION]  # type: pd.DataFrame
                if merged_df is not None:
                    return get_x_y_sample_weight_from_merged_df(merged_df)
                else:
                    separate_keys = [Keys.X_VALID, Keys.Y_VALID, Keys.SW_VALID]
                    ret = self._cache.get(separate_keys)
                    return ret[separate_keys[0]], ret[separate_keys[1]], ret[separate_keys[2]]
        ...
        def set_valid(self, X, y, sample_weight):
            if self._read_only:
                ...
                if merged_df is None:
                    self._cache.set(Keys.X_VALID, X)
                    self._cache.set(Keys.Y_VALID, y)
                    self._cache.set(Keys.SW_VALID, sample_weight)
                else:
                    self._cache.set(Keys.FULL_VALIDATION, merged_df)
...
class ExperimentTansformers(_CacheableStoreBase):
    ...
    def _load(self):
        cache = self._cache
        read_only = self._read_only
        experiment_transformers_bytes = cast(
            bytes, self._cache.get(["ExperimentTransformers"])["ExperimentTransformers"]
        )
        self.__dict__ = pickle.loads(experiment_transformers_bytes).__dict__
        self._cache = cache
        self._read_only = read_only
