...
class DelUserWindow(QDialog):
    ...
    def __init__(self, database, server):
        ...
        self.label = QLabel(
            'Выберите пользователя для\nудаления из контактов', self)
        self.label.move(5, 5)

        self.selector = QComboBox(self)
        self.selector.setGeometry(5, 45, 170, 20)

        self.del_btn = QPushButton('Удалить', self)
        self.del_btn.setGeometry(180, 45, 100, 30)
        self.del_btn.clicked.connect(self.delete_user)

        self.cancel_btn = QPushButton('Отмена', self)
        self.cancel_btn.setGeometry(180, 80, 100, 30)
