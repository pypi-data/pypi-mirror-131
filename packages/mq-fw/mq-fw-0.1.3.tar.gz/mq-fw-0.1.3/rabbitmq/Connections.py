...
class Connection(object):
    ...
    def create_queue(self, exchange, routing_key):
        """
        创建 消息队列
        :param exchange:
        :param routing_key:
    ...
    def send(self, message_list: list, exchange, routing_key):
        """
        发送数据
        :param message_list:
        :param exchange:
    ...
    def receive(self, exchange, routing_key, task, thread_count=None):
        """
        消费者消费
        :param exchange:
        :param routing_key:
        ...
        :return:
        """
        self.create_queue(exchange, routing_key)
        if thread_count:
    ...
    def service(self, task, consumer_exchange, consumer_routing_key,
                producer_exchange, producer_routing_key, thread_count=None):
        """
        rabbitmq 服务
        1. 订阅rabbitmq
        2. 处理消费的数据
        ...
        def callback(body):
            result = task(body)
            if result:
                self.send(result, producer_exchange, producer_routing_key)
        self.receive(consumer_exchange, consumer_routing_key, callback, thread_count)
