...
class Acquisition():
    ...
    Every acquisition driver class should derive from this parent, so that the 
    automatic device class discovery can function.
    """
    def __init__(self, **kwargs):
        ...
        #: Set of functions to call on to indicate updates have been made to the data set.
        self._update_callbacks = set()

        #: Set of functions to call when acquisition is completed.
    ...
    def register_update_callback(self, callback: callable) -> None:
        """
        Register a function to receive notifications for updates to the raw data set.

        The callback method should take the form of ``update_callback(scans_i, times_i, wls_i)``,
        where ``scans_i``, ``times_i`` and ``wls_i`` are lists or slices indicating the array
        indicies from the `"raw/data"` element in :data:`datastorage.d`.
        If any of the parameters are ``None``, it indicates all the data in that array axis has
        potentially changed.

        :param callback: Function to call when raw data is updated.
        """
        if callable(callback):
            self._update_callbacks.add(callback)
        else:
            raise RuntimeError("Attempted to register non-callable function for update callbacks.")
    ...
    def unregister_update_callback(self, callback: callable) -> None:
        ...
        if callback in self._update_callbacks:
            ...
            raise RuntimeWarning("Attempted to unregister an unknown function for update callbacks.")
    ...
    def register_completion_callback(self, callback: callable) -> None:
        ...
        The callback method should take the form of ``completion_callback(error=None)``, with
        ``error`` being an :class:`Exception` describing any error code. The exception may be of
        type :class:`AcquisitionException` to indicate the error was due to something identifiable
        (for example, a device error), but a generic exception may be returned if the acquisition
        thread detects some other unknown error condition.  ``None`` indicates acquisition was
    ...
    def unregister_completion_callback(self, callback: callable) -> None:
        """
        Unregister a function previously registered to receive acquisition completion callbacks using
        :meth:`register_completion_callback`.
        ...
        if callback in self._completion_callbacks:
            ...
            raise RuntimeWarning("Attempted to unregister an unknown function for acquisition completion callbacks.")
    ...
    def start(self) -> None:
        ...
        should be obtained in whatever manner is appropriate.
        For example, the data range and number of scans can be extracted from the :class:`DataPanel`
        user interface.
        """
        pass
