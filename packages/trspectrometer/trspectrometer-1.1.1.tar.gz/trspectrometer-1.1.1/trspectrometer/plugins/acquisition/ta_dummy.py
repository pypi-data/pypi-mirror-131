...
from utils import AcquisitionError, AcquisitionAbortedWarning, si_unit_factor, status_message, now_string
...
class TA_DummySteppedAcquisition(Acquisition):
    ...
    def start(self) -> None:
        ...
        ds.signals.data_changed.emit(False)  # Don't update UI
    ...
    def _start_acq(self, t_start=0.0, scan_count=None):
        ...
        try:
            ...
            while s_i < (scan_count if scan_count else config.mainwindow.dataPanel.countSpinBox.value()):
                ...
                while t_i < ds.d["raw/time"].shape[0]:
                    ...
                    for cb in self._update_callbacks.copy():
                        cb([s_i], [t_i], None)
                    # And trigger the Qt data update signal
                    ds.signals.data_updated.emit([s_i], [t_i], None)
        ...
        # Iterate over copy of callbacks set so it can be modified if needed
        for cb in self._completion_callbacks.copy():
            cb(error)
        self._acq_thread = None
