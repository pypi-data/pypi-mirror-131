...
class DataPanel(QtWidgets.QWidget, loadUiType(__file__.split(".py")[0] + ".ui")[0]):
    ...
    #: Signal to respond to the end of acquisition.
    acquisition_completed = Signal(Exception)
    ...
    def __init__(self, parent=None):
        ...
        ds.signals.data_updated.connect(self._data_updated)
        ds.signals.raw_selection_changed.connect(self._raw_selection_changed)
        self.acquisition_completed.connect(self._acquisition_complete)
    ...
    def _start_clicked(self):
        ...
        if self._acquisition:
            ...
            self._acquisition = d
            self._acquisition.register_completion_callback(self._emit_acquisition_completed)
    ...
    def _emit_acquisition_completed(self, error=None):
        """
        Wrap the Qt Signal emit() so the acquisition thread because:
          1. Other threads shouldn't mess with things inside the Qt thread.
          2. The emit function signature doesn't stay constant so is unable to be removed from acquisition callback lists.
        
        The signal is connected to :meth:`_acquisition_complete`.
        """
        self.acquisition_completed.emit(error)


    def _acquisition_complete(self, error=None):
        """
        Handle the signal for completion of acquisition.
        """
        self._acquisition.unregister_completion_callback(self._emit_acquisition_completed)
        self._acquisition = None
