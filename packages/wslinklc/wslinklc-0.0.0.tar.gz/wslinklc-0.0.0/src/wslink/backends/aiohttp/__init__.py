...
from .progress_frame import encode_progress_frame
...
def create_webserver(server_config: wslinktypes.ServerConfig) -> AiohttpWslinkServer:
    ...
    logging.info("create_webserver with config {}".format(server_config))
...
class WslinkHandler(object):
    def __init__(
        self, protocol: Optional[wslinktypes.ServerProtocol] = None, web_app=None
    ) -> None:
        self.serverProtocol = protocol
        self.web_app = web_app
        self.functionMap: Dict[
            str, Tuple[wslinktypes.LinkProtocol, Callable[..., Any]]
        ] = {}
    ...
    async def handleWsRequest(self, request) -> Any:
        aiohttp_app = request.app

        client_id = str(uuid.uuid4()).replace("-", "")
        current_ws = aiohttp_web.WebSocketResponse(
            max_msg_size=MAX_MSG_SIZE, heartbeat=30
        )
    ...
    async def onConnect(self, request: wslinktypes.HTTPRequest, client_id: str) -> None:
        ...
        for linkProtocol in self.serverProtocol.getLinkProtocols():
            if getattr(linkProtocol, "onConnect"):
    ...
    async def onClose(self, client_id: str):
        ...
        for linkProtocol in self.serverProtocol.getLinkProtocols():
            if getattr(linkProtocol, "onClose"):
    ...
    def sendMessageSync(
        self, rpcid: str, content: Any, client_id: Optional[str] = None
    ):
        wrapper = {
            "wslink": "1.0",
            "id": rpcid,
        ...
        frame = encode_progress_frame(encMsg)
        for ws in self.connections.values():
            ws._writer._write(frame)
    ...
    def publish(self, topic: str, data: bytes, client_id: Optional[str] = None):
        if topic == "luminarycloud.progress":
            self.sendMessageSync(
                rpcid="publish:{0}:99999".format(topic),
                content=data,
                client_id=client_id,
            )
            return
