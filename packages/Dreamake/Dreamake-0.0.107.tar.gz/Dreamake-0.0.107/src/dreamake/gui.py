import pygame as pg
from dreamake.codekit import clamp, pad_rect
...
class Panel:
    def __init__(self, position=(0, 0), caption="Panel", width=300, height=250, index=0):
        ...
        self.lbl_caption.position = pg.Vector2(10, self.title_height/2 - self.lbl_caption.height/2)
        ...
        # Content properties
        self.content_rect = pg.Rect(0, 0, self.width-2, self.height - self.title_height-1)
    ...
    def resize(self, xdiff, ydiff):
        self.width += xdiff
        self.height += ydiff
        self.base_surface = pg.Surface((self.width-2, self.height - self.title_height - 1))
        self.content_surface = pg.Surface(self.content_rect.size)
    ...
    def update(self, dt):
        ...
        if self.caption_alignment == "center":
            self.lbl_caption.position.x = self.get_rect().w/2 - self.lbl_caption.width/2
    ...
    def draw(self, dest):
        if not self.hiding:
            ...
            pg.draw.rect(self.surface, self.border_color, (0, 0, self._width, self.title_height), 0, self.border_radius)
            pg.draw.rect(self.surface, self.border_color, (0, self.title_height-self.border_radius, self._width, self.border_radius), 0)

            # Draw the border
            pg.draw.rect(self.surface, self.border_color, (0, self.title_height, self._width, self._height-self.title_height), 1, 0)
            ...
            self.content_surface.fill(self.fill_color)
...
class TextEntry(Element):
    def __init__(self, position, width):
        ...
        self.input_rect = pg.Rect(position, (width, 20))
        self.input_label = Label("", position=(1, 0))
        self._index_label = Label("", position=self.input_label.position)

        self.active_color = COLORS["bisque"]
        self.inactive_color = COLORS["bisque"]
        self.surface = pg.Surface(self.input_rect.size)
        ...
        for event in events:
            ...
            if self.active:
                if event.type == pg.TEXTINPUT:
                    if not self.input_label.width > self.input_rect.width-10:

                        self.input_label.text = self.input_label.text[0:self.cursor_index] + event.text + self.input_label.text[self.cursor_index:]
                        self.cursor_index += 1

                if event.type == pg.KEYDOWN:
                    if event.key == pg.K_BACKSPACE:
                        if self.cursor_index > 0:
                            str_list = list(self.input_label.text)
                            str_list.pop(self.cursor_index-1)
                            s = ""
                            for char in str_list:
                                s += char
                            self.cursor_index -= 1
                            self.input_label.text = s
                        ...
                        if self.cursor_index < len(list(self.input_label.text)):
                            self.cursor_index += 1

        self._index_label.text = self.input_label.text[0:self.cursor_index]

    def update(self, dt):
        self.blink_elapsed = pg.time.get_ticks() - self.blink_start
        if self.blink_elapsed > self.blink_rate:
    ...
    def draw(self, dest, offset=(0, 0)):
        ...
        self.input_label.draw(self.surface)

        if self.active and self.blink_state:
            bx = self._index_label.position.x + self._index_label.width

            pg.draw.aaline(self.surface, COLORS["black"],
                           (bx, 2),
                           (bx, self._index_label.height-3))

        dest.blit(self.surface, self.position)
        pg.draw.rect(dest, COLORS["blue"], self.input_rect, 1)
...
class UIManager:
    ...
    def update(self, dt):
        for i in self.focus_indices:
            ...
            if self.panels[i].position.y < self.boundary_rect.top + self.menubar.height:
                self.panels[i].position.y = self.boundary_rect.top + self.menubar.height
