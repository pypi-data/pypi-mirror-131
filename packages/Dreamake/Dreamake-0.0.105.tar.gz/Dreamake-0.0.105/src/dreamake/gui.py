...
class TextEntry(Element):
    def __init__(self, position, width):
        ...
        self.active_color = COLORS["lightskyblue3"]
        self.inactive_color = COLORS["chartreuse4"]
        ...
        for event in events:
            ...
            if self.active:
                if event.type == pg.TEXTINPUT:
                    self.input_label.text += event.text

                if event.type == pg.KEYDOWN:
                    if event.key == pg.K_BACKSPACE:
                        self.input_label.text = self.input_label.text[0:-1]
    ...
    def draw(self, dest, offset=(0, 0)):
        ...
        if self.active:
            pg.draw.aaline(self.surface, COLORS["black"],
                           (self.input_label.width+2, 1),
                           (self.input_label.width+2, self.input_rect.height-2))
        dest.blit(self.surface, self.position)
