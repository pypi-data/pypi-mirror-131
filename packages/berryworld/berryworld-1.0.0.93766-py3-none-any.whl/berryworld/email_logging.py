...
class EmailLogging:
    ...
    def __init__(self, project_name, pipeline, ip_address, request_url, section=None, sql_con=None, timeout=30*60):
    ...
    def start_threading(self):
        ...
        if self.flag_submitted == 0:
            self.on_failure(error_message=f'The pipeline failed to succeed after running '
                                          f'for {self.timeout/60:.2f} minutes')

    def set_submitted(self, value=1):
        """ Set submitted value
        :param value: Value to set up the submitted flag
        """
        self.flag_submitted = value

    def on_successful(self):
        """ Update log on success
        """
        if self.flag_submitted == 0:
            self.flag_submitted = 1
            ...
            self.sql_con.insert(self.log_df, 'ETL', 'PythonEmailLogs')
