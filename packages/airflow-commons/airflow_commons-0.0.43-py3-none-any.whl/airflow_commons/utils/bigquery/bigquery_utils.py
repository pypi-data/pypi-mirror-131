...
from airflow_commons.glossary import COMMA, SQL_QUOTATION
...
SELECT_RETURN_TYPES = ["dict", "json", "dataframe", "pyarrow"]
...
def query(
    ...
    if query_job.state == "DONE":
        LOGGER.info(
            f"Query Job ID: {query_job.job_id}, Elapsed Time: {round((timer_end - timer_start).total_seconds())}, Total MBytes Billed: {query_job.total_bytes_billed / MEGABYTES_BILLED_DENOMINATOR}, Number of Rows Affected: {query_job.num_dml_affected_rows}"
        )
        return result
    else:
        LOGGER.error(
            f"Query Job ID: {query_job.job_id}, Elapsed Time: {round((timer_end - timer_start).total_seconds())}"
        )
...
def select(
    ...
    :param return_type: dataframe, json, json string, dictionary or pyarrow; default is dataframe
    :param client: Client needed for API request
    :param sql: query sql
...
def get_primitive_column_list(
    ...
    primitive_columns_list = query_information_schema(
        ...
        table_name=table_id,
        primitive_data_types=COMMA.join('"{}"'.format(i) for i in PRIMITIVE_DATA_TYPES),
    )
