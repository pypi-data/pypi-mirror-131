...
class COPASISolver(WhiteBoxSolver):
    ...
    def _change_log_path_for_restart(self, opt_task, index: int, log_restart_folder: str, log_report):
        opt_task.getReport().setReportDefinition(log_report)
        opt_task.getReport().setTarget(f"{log_restart_folder}/restart_{index}.log")
    ...
    def solve(self, model: Model, H: float, sim_engine: SimulationEngine,
        ...
        if self.n_restarts > 0:
            ...
            for i in range(self.n_restarts):
                ...
                self._change_log_path_for_restart(opt_task, f"restart_{i+1}.log", log_report)
                print(f"\t\tStart optimisation restart {i+1}..")
                result = self._run_and_get_output(model, i+1, opt_task, opt_problem)
