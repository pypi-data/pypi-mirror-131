...
from ...models.personal_access_token_revoke_request_model import PersonalAccessTokenRevokeRequestModel
...
def _get_kwargs(
    ...
    *,
    client: Client,
    json_body: PersonalAccessTokenRevokeRequestModel,
) -> Dict[str, Any]:
    url = "{}/v3/personal-access-tokens/{id}".format(client.base_url, id=id)
...
def _parse_response(*, response: httpx.Response) -> Optional[Union[HttpErrorResponseDto, PersonalAccessTokenDto]]:
    if response.status_code == 200:
        response_200 = PersonalAccessTokenDto.from_dict(response.json())
...
def _build_response(*, response: httpx.Response) -> Response[Union[HttpErrorResponseDto, PersonalAccessTokenDto]]:
    return Response(
...
def sync_detailed(
    ...
    *,
    client: Client,
    json_body: PersonalAccessTokenRevokeRequestModel,
) -> Response[Union[HttpErrorResponseDto, PersonalAccessTokenDto]]:
    kwargs = _get_kwargs(
        id=id,
        client=client,
        json_body=json_body,
...
def sync(
    ...
    *,
    client: Client,
    json_body: PersonalAccessTokenRevokeRequestModel,
) -> Optional[Union[HttpErrorResponseDto, PersonalAccessTokenDto]]:
