...
def _get_kwargs(
    ...
    *,
    ...
    url = "{}/v3/me/licenses/{id}".format(client.base_url, id=id)
...
def _parse_response(*, response: httpx.Response) -> Optional[Union[HttpErrorResponseDto, UserLicenseDto]]:
    if response.status_code == 200:
        response_200 = UserLicenseDto.from_dict(response.json())
...
def _build_response(*, response: httpx.Response) -> Response[Union[HttpErrorResponseDto, UserLicenseDto]]:
    return Response(
...
def sync_detailed(
    ...
    *,
    client: Client,
) -> Response[Union[HttpErrorResponseDto, UserLicenseDto]]:
    kwargs = _get_kwargs(
        id=id,
        client=client,
    )
...
def sync(
    ...
    *,
    client: Client,
) -> Optional[Union[HttpErrorResponseDto, UserLicenseDto]]:
