...
from ...models.license_policy_request_model import LicensePolicyRequestModel
...
def _get_kwargs(
    ...
    *,
    client: Client,
    json_body: LicensePolicyRequestModel,
) -> Dict[str, Any]:
    url = "{}/v3/license-policies/{id}".format(client.base_url, id=id)
...
def _parse_response(*, response: httpx.Response) -> Optional[Union[HttpErrorResponseDto, LicensePolicyDto]]:
    if response.status_code == 200:
        response_200 = LicensePolicyDto.from_dict(response.json())
...
def sync(
    ...
    *,
    ...
    """Updates the specified license policy by setting the values of the parameters passed. Any parameters not provided will be left unchanged."""
