...
from ...models.role_request_model import RoleRequestModel
...
def _get_kwargs(
    ...
    *,
    client: Client,
    json_body: RoleRequestModel,
) -> Dict[str, Any]:
    url = "{}/v3/roles/{id}".format(client.base_url, id=id)
...
def _parse_response(*, response: httpx.Response) -> Optional[Union[HttpErrorResponseDto, RoleDto]]:
    if response.status_code == 200:
        response_200 = RoleDto.from_dict(response.json())
