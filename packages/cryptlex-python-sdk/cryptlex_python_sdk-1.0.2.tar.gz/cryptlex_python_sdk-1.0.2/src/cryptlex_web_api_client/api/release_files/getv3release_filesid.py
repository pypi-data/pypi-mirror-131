...
def _get_kwargs(
    ...
    *,
    ...
    url = "{}/v3/release-files/{id}".format(client.base_url, id=id)
...
def _parse_response(*, response: httpx.Response) -> Optional[Union[HttpErrorResponseDto, ReleaseFileDto]]:
    if response.status_code == 200:
        response_200 = ReleaseFileDto.from_dict(response.json())
...
def _build_response(*, response: httpx.Response) -> Response[Union[HttpErrorResponseDto, ReleaseFileDto]]:
    return Response(
...
def sync_detailed(
    ...
    *,
    client: Client,
) -> Response[Union[HttpErrorResponseDto, ReleaseFileDto]]:
    kwargs = _get_kwargs(
        id=id,
        client=client,
    )
...
def sync(
    ...
    *,
    client: Client,
) -> Optional[Union[HttpErrorResponseDto, ReleaseFileDto]]:
