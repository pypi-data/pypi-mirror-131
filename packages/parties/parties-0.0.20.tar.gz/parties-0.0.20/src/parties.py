...
class simulation:
    ...
    def run(self, folder='.'):
        ...
        with open(self.log_name,'w') as log_file:
            if self.proc_num > 1:
                self.parties = subprocess.Popen([self.run_prefix,self.run_command,'-np', str(self.proc_num), './parties'],stdout=log_file,cwd=folder)
            if self.proc_num == 1:
    ...
    def check_status(self):
        ...
        if self.parties.poll() == 1: return 0
        else: return 0
...
class multiple_simulations:
    ...
    def run_parallel(self,out_func=None,stop_func=None):
        ...
        for idx, rows in self.vars.iterrows():
            s = simulation(proc_num=self.n_pps,
        ...
        while True:
            ...
            for item in self.sim_list:
                ...
                if item['status'] == 'Running':
                    ...
                    continue
                if self.work_proc < self.n_pps: continue
                #Run simulation
                item['simulation'].run(folder=(item['postfix']+'/'),run_prefix=self.run_prefix)
                item['status'] = 'Running'
                self.work_proc -= self.n_pps
            ########################## Stop ############################
