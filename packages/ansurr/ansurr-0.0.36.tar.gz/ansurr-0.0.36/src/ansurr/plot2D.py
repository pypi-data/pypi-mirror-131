		...
		if line[:4] == 'PDB:':
			...
			if line[9] != 'nan':
				...
				if shifts not in data[pdb]:
					if cyrange:
						data[pdb][shifts] = {'model':[model],'rmsd':[float(line[13])],'corr':[float(line[9])],'rmsd_welldefined':[float(line[21])],'corr_welldefined':[float(line[17])],'shift_perc':[shift_perc]}
					else:
						data[pdb][shifts] = {'model':[model],'rmsd':[float(line[13])],'corr':[float(line[9])],'shift_perc':[shift_perc]}

				else:
					data[pdb][shifts]['model'].append(model)
					data[pdb][shifts]['rmsd'].append(float(line[13]))
					data[pdb][shifts]['corr'].append(float(line[9]))
					data[pdb][shifts]['shift_perc'].append(shift_perc)
					if cyrange:
						data[pdb][shifts]['rmsd_welldefined'].append(float(line[21]))
						data[pdb][shifts]['corr_welldefined'].append(float(line[17]))
