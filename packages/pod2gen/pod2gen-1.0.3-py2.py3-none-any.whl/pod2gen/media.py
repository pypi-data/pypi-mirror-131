...
class Media(object):
    ...
    .. note::
        ...
        A warning called :class:`~pod2gen.warnings.NotSupportedByItunesWarning`
    ...
    * Document
    ...
    def __init__(self, url, size=0, type=None, duration=None, requests_session=None):
        self._url = None
        self._size = None
        self._type = None
        self._duration = None
        ...
        session with pod2gen as User-Agent.
    ...
    def url(self, url):
        ...
        if file_extension not in self.file_types:
            warnings.warn(
                "File extension %s is not supported by iTunes." % file_extension,
    ...
    def size(self, size):
        try:
            ...
            if size < 0:
                raise ValueError(
                    "File size must be 0 if unknown, or a positive" " integer."
                )
            self._size = size
            if self.size == 0:
                warnings.warn(
                    "Size is set to 0. This should ONLY be done when "
    ...
    def type(self):
        ...
        .. note::
            ...
            type will be auto-detected from the :attr:`~pod2gen.Media.url`
    ...
    def type(self, type):
        ...
        if type not in self.file_types.values():
            warnings.warn(
                "Media type %s is not supported by iTunes." % type,
                NotSupportedByItunesWarning,
                stacklevel=2,
            )
        self._type = type
    ...
    def duration_str(self):
        ...
        if self.duration is None:
            ...
            hours = self.duration.days * 24 + self.duration.seconds // 3600
    ...
    def create_from_server_response(
        cls, url, size=None, type=None, duration=None, requests_=None
    ):
        ...
        if not (size and type):
            ...
            if not size:
                try:
                    ...
                    raise RuntimeError(
                        "Content-Length not returned by server "
            ...
            if not type:
                try:
                    ...
                    raise RuntimeError(
                        "Content-Type header not returned by "
                        "server when sending HEAD request to %s" % url
                    )
