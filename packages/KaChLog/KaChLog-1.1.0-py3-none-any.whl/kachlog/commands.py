...
class AliasedGroup(click.Group):
    def get_command(self, ctx, cmd_name):
        ...
        if not matches:
            ...
            colored_matches = ', '.join(click.style(i, fg="bright_green", bold=True) for i in sorted(matches))
...
@click.command(cls=AliasedGroup, epilog="It is possible to call commands in shorter way"
...
@click.option('--sections', 'sections', is_flag=True, help="Add empty sections in [Unreleased]")
...
@click.option('--sections', 'sections', is_flag=True, help="Add empty sections in [Unreleased]")
def release(release_type: str, auto_confirm: bool,
            version: str, sections: bool) -> None:
    CL = ChangelogUtils()
    try:
