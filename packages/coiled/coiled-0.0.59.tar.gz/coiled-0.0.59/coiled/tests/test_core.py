...
def test_set_backend_options_aws_vm_customer_hosted(cloud, sample_user, mocker):
    ...
    options = {
        ...
        # These are added at the account level regarless
        # so lets add them here
        "enable_public_http": True,
        "enable_public_ssh": True,
        "disable_public_ingress": False,
    }
    ...
    async with coiled.Cloud(asynchronous=True) as cloud:
        cluster = await cloud.create_cluster(
            ...
            backend_options={
                "firewall": {
                    "ports": [8786, 8787],
                    # arbitrary CIDR block. This is one we're likely to receive, but we just need to validate the
                    # scheduler phone home. Not bad if the workers don't connect.
                    "cidr": "10.0.0.0/16",
                }
            },
        )
...
def test_set_backend_options_azure_vm_missing_params(
    ...
    with pytest.raises(AzureCredentialsParameterError) as e_info:
        coiled.set_backend_options(backend="azure", customer_hosted=True)
