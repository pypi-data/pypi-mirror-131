...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _list_instance_types(
        ...
        backend_options = await self.get_backend_options(self.default_account)
        instance_types = get_instance_types(backend_options)
        ...
        if backend_options.get("backend") == "vm":
            price_info = "customer hosted backends"
        else:
            price_info = "Coiled hosted backends"
        print(
            f"Note: The cost shown in the table is showing the cost for {price_info}."
        )
...
def list_instance_types(backend: str = "", json: bool = False) -> Optional[Dict]:
    ...
    backend
        ...
        the list will show all instances for AWS and GCP.
    json
