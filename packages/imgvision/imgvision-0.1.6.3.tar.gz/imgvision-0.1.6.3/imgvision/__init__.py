...
class spectra_metric():
    ...
    def __init__(self,x1,x2,scale =6):
        self.scale = scale
        self.info = x1.shape
        if len(self.info) == 3:
    ...
    def PSNR(self,mode=''):
        _PSNR = 10*np.log10(np.power(np.max(self.x2_,axis=0),2)/self.MSE('mat'))
        if mode=='mat':
            return _PSNR
        else:
            return np.mean(_PSNR)

    def SSIM(self, k1=0.01, k2=0.03, l=1, mode=''):
    ...
    def Evaluation(self,idx=0,l=1,k1=0.01,k2=0.03):
        print(f'{idx}\t{self.PSNR()}\t{self.SAM()}\t{self.ERGAS()}\t{self.SSIM(k1=k1,k2=k2,l=l)}')
