...
class CertClient:
    ...
    def __init__(self, host: str, port: int = 443, initialize: bool = True):
        """
        :param host:
        :param port:
        """
        ...
        if initialize:
            try:
                ...
                print(f'Invalid host name ({self.host}). Do not use HTTP(S), WWW or "/" in the end.', file=sys.stderr)
    ...
    def is_valid(self) -> bool:
        """Auxiliary method to validate "now" is more then valid_from and less then valid_to"""
    ...
    def get_alt_name(self) -> list:
        ...
        try:
            ...
            return [name.value for name in extension.value]
