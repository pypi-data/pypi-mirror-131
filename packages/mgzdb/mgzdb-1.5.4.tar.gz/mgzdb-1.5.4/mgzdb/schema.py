...
class Player(BASE):
    ...
    rank_snapshot = Column(Integer)
    random = Column(Boolean)
    full_random = Column(Boolean)
    mirror = Column(Boolean)
    vod = Column(String, index=True)
...
class MatchmakingLog(BASE):
    __tablename__ = 'matchmaking_log'
    id = Column(Integer, primary_key=True)
...
class MatchmakingPlayerLog(BASE):
    """Log special matches."""
    __tablename__ = 'matchmaking_player_log'
    match_id = Column(Integer, ForeignKey('matchmaking_log.id', ondelete='cascade'), primary_key=True)
