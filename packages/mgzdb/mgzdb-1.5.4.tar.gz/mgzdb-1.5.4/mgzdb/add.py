...
class AddFile:
    ...
    def _update_match_dashboard(self, match, user_data):
        if match.platform_id != 'de':
            return
        if not user_data:
            return
        lookup = {str(u['user_id']):u for u in user_data if 'user_id' in u}
        for player in match.players:
            ref = lookup.get(player.user_id)
            if not ref:
                continue
            player.random = ref.get('random')
            player.full_random = ref.get('full_random')
            player.mirror = ref.get('mirror')
            player.rate_snapshot = ref.get('rating')
            player.rank_snapshot = ref.get('rank')
            player.vod = ref.get('vod')


    def _update_match_hd(self, match):
        if match.platform_id != 'hd':
            return
        if match.rated:
            if match.type_id == 0:
                match.ladder_id = 0
            elif match.type_id == 2:
                match.ladder_id = 1
    ...
    def _update_match_users(self, platform_id, match_id, user_data):
        ...
        for user in user_data:
            ...
            player.random = user.get('random')
            player.full_random = user.get('full_random')
            player.mirror = user.get('mirror')
