...
if os.path.isfile('parse_file.py'):
    ...
    from plot_basics import PlotUlgBasics, PandasTools
else:
    from toopazo_ulg.parse_file import UlgParser
    from toopazo_ulg.plot_basics import PlotUlgBasics, PandasTools


class PlotUlgMixer(PlotUlgBasics):
    """
    ...
    The important function is
        ...
        PlotUlgMixer.least_square_fit(xvect, yvect)
    ...
    def check_data(data, vmin, vmax):
        ...
        if len(shape_tuple) == 1:
            PlotUlgMixer.check_data1d(data, vmin, vmax)
        else:
            PlotUlgMixer.check_data2d(data, vmin, vmax)
    ...
    def check_data2d(data2d, vmin, vmax):
        ...
        for ith_variable in range(0, ldim0):
            ...
            PlotUlgMixer.check_data1d(data1d, vmin, vmax)
    ...
    def mixer_input_output(self, ulgfile, closefig):
        ...
        # x = PlotUlgMixer.timestamp_to_datetime(x)
        ...
        if config_lsq_fit:
            ...
            [lsq_matrix, lsq_bias] = \
                PlotUlgMixer.get_stored_lsq_fit(
                    config_vehicle, config_output_units)

        [xvect, yvect, eval_yvect, eval_rms_error] = \
            PlotUlgMixer.least_square_eval(xvect, yvect, lsq_matrix, lsq_bias)
        _ = eval_rms_error
    ...
    def least_square_eval(xvect, yvect, lsq_matrix, lsq_bias):
        # arg = """
        # # PlotUlgMixer.least_square_eval(xvect, yvect)
        # #   xvect = [x0, x1, x2, x3]
    ...
    def least_square_fit(xvect, yvect):
        ...
        PlotUlgMixer.check_data(lsq_phi, -10000, 10000)
        print('[least_square_fit] check_data(lsq_yvect, -10000, 10000)')
        PlotUlgMixer.check_data(lsq_yvect, -10000, 10000)
    ...
    def ctrl_alloc_model(self, ulgfile, twin):
        ...
        yvect = yvect - 1500
        yvect = yvect / 500
        [lsq_matrix, lsq_bias, lsq_error] = PlotUlgMixer.least_square_fit(xvect, yvect)
        _ = lsq_error
    ...
    def ctrl_alloc_model_plot(self, ulgfile, df_out, xvect, yvect, lsq_matrix, lsq_bias):
        ...
        PlotUlgBasics.save_fig(fig, jpgfilename)
