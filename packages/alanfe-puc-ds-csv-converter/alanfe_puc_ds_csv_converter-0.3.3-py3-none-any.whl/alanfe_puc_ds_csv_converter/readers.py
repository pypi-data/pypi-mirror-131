...
def read_csv(file_path : Path, delimiter :str, encoding :str) -> Matrix:
    ...
    with open(file_path, "r") as stream:
        ...
        rtn = stream.readline().decode(encoding).replace('\n', "")
        while(rtn != ''):
...
def read_json(file_path : Path, encoding: str) -> Matrix: 
    ...
    with open(file_path, "r") as stream:
        whole_data = stream.read().decode(encoding)
