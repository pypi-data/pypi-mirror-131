...
class FrameInfo(object):
    def __init__(self, frame):
        ...
        self.is_ipython_cell = (
            code.co_name == "<module>"
            and code.co_filename.startswith("<ipython-input-")
        )
    ...
    def update_variables(self, watch, watch_extras, event, whitelist):
        ...
        self.local_reprs = OrderedDict(
            ...
            for source, value in self.get_local_reprs(watch, watch_extras, whitelist)
        )
    ...
    def get_local_reprs(self, watch, watch_extras, whitelist):
        ...
        vars_order = (
            code.co_varnames + code.co_cellvars + code.co_freevars + tuple(var_names)
        )
        var_names.sort(key=vars_order.index)
        result_items = [(key, frame.f_locals[key]) for key in var_names]
...
def set_json_file_path(json_file_path):
    global debug_json_file_path
    debug_json_file_path = json_file_path
...
class Tracer(object):
    ...
    def trace(self, frame, event, arg):
        if not self._is_traced_frame(frame):
            if (
                self.depth == 1 or self._is_internal_frame(frame)
                ...
                while True:
                    ...
                    if (
                        ...
                        or self._is_internal_frame(candidate)
                    ):
                        return None
        ...
        if event in ("call", "enter"):
            ...
            trace_event = Event(
                frame_info, event, arg, thread_local.depth, line_no=line_no
            )
        ...
        if not (
            frame.f_code.co_name == "<genexpr>" and event not in ("return", "exception")
        ):
        ...
        if self.code_line != "" and self.line_num != 0:
            self.list_of_lines.append(
                {
                    # -8 to offset the line number
                    "line_num": self.line_num - 8,
                    "code_on_line": self.code_line,
                    "value_of_variable": trace_event.variables,
                }
            )

            if trace_event.opname == "RETURN_VALUE":
                import json

                # with open("some.json", "w") as f:
                with open(debug_json_file_path, "w") as f:
                    json.dump(self.list_of_lines[1:-1], f)

        self.code_line = trace_event.source_line
        self.line_num = trace_event.line_no
...
class Spy(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        try:
            ...
            raise Exception(
                "You must install birdseye separately to use spy: pip install birdseye"
            )
