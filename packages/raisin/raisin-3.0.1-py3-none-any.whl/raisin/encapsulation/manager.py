...
def make_tasks(functions, arguments):
    ...
    ** Generates the tasks to be performed. **
    ...
    new: [Argument(1), Argument(2), Func(f)], task: Task(3689584644591251701, [12402937695876512259, 15584102817361292984])
    new: [Argument(3)], task: Task(3689584644591251701, [15584102817361292984, 15229981472126021560])
    >>>
    """
...
class TasksSharer(threading.Thread):
    ...
    ** Communicate packets with the local server. **
    ...
    >>> with Handler(SocketConn(soc1)) as hand:
    ...     hand.start()
    ...     tasks_generator = make_tasks((f, f), ([2], [3]))
    ...     with TasksSharer(tasks_generator, SocketConn(soc2)) as tasks_sharer:
    ...         tasks_sharer.start()
