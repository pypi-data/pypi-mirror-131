...
class AggStringNUnique : public Aggregator {
  public:
    ...
    AggStringNUnique(Grid<IndexType> *grid, bool dropmissing, bool dropnan)
        : grid(grid), grid_data(nullptr), string_sequence(nullptr), data_mask_ptr(nullptr), selection_mask_ptr(nullptr), dropmissing(dropmissing), dropnan(dropnan) {
    ...
    virtual void reduce(std::vector<Aggregator *> others) {
        ...
        for (size_t i = 0; i < this->grid->length1d; i++) {
            ...
            if (dropmissing)
                grid_data[i] = counters[i].count() - counters[i].null_count;
            else
    ...
    void set_data(StringSequence *string_sequence, size_t index) { this->string_sequence = string_sequence; }
...
void add_agg(Module m, Base &base, const char *class_name) {
    py::class_<Agg>(m, class_name, py::buffer_protocol(), base)
        ...
        .def_buffer([](Agg &agg) -> py::buffer_info {
            ...
            return py::buffer_info(agg.grid_data,                                            /* Pointer to buffer */
