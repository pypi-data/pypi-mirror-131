...
class MenuItem(MPTTModel):
    ...
    icon = models.CharField(max_length=50, null=True, blank=True, help_text=_("Icon"))
    slug = models.SlugField(unique=True, max_length=80)
    description = models.TextField(null=True, blank=True, verbose_name=_("Description"))
    classnames = models.CharField(max_length=255, null=True, blank=True, verbose_name=_("class names"))
...
class Menu(models.Model):
    ...
    def full_clean(self, *args, **kwargs):
        if self.template_name:
            try:
                ...
                raise ValidationError({"template": "%s does not exist." % self.template_name})
    ...
    def get_template(self):
        ...
        if not (self.template_string and self.template_name):
            default_templates = ["menu/%s.html" % self.slug, "menu.html"]
        if self.template_string:
            template = Template(template_string=self.template_string)
            return template
        template_name = [self.template_name] if self.template_name else default_templates
