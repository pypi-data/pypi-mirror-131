...
class FileProvider(ConfigProvider):
    ...
    def extract_config(self) -> Optional[Dict[str, str]]:
        ...
        - S3 secret key not provided
        """
        config_file = Path(f"{settings.ROOT_DIR}/keys.json")
        possible_configs: List[Path] = []
        if not config_file.is_file():
            home = Path.home()
            possible_configs.extend(
                _dir
                for _dir in home.rglob("canalyst/keys.json")
                if not re.search("Trash", str(_dir))
            )
        ...
        try:
            ...
            raise ConfigException("Configuration file unreadable")

        if not config.get("canalyst_api_key"):
            return None

        if not config.get("canalyst_s3_id"):
            raise ConfigException("S3 access key ID was not provided")

        if not config.get("canalyst_s3_key"):
            raise ConfigException("S3 secret key was not provided")

        return {
            "canalyst_api_key": config.get("canalyst_api_key"),
            "s3_access_key_id": config.get("canalyst_s3_id"),
            "s3_secret_key": config.get("canalyst_s3_key"),
