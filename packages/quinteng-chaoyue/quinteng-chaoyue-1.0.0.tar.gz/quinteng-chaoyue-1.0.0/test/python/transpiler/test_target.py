...
from quinteng.test.mock.fake_backend_v2 import FakeBackendV2


class TestTarget(QuintengTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_coupling_map_3q_gate(self):
        ...
        with self.assertLogs("quinteng.transpiler.target", level="WARN") as log:
            cmap = fake_target.build_coupling_map()
        self.assertEqual(
            log.output,
            [
                "WARNING:quinteng.transpiler.target:"
...
class TestPulseTarget(QuintengTestCase):
    def setUp(self):
        super().setUp()
...
class TestInstructionProperties(QuintengTestCase):
