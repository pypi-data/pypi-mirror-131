...
from quinteng import QuantumRegister, ClassicalRegister, QuantumCircuit, schedule, transpile, assemble
from quinteng.pulse import Schedule
from quinteng.qobj import PulseQobj
from quinteng.test import QuintengTestCase
from quinteng.test.mock.utils import ConfigurableFakeBackend
from quinteng.test.mock import FakeAthens

from quinteng.test.mock.fake_backend import HAS_AER
...
class GeneratedFakeBackendsTest(QuintengTestCase):
    """Generated fake backends test."""

    def setUp(self) -> None:
        self.backend = ConfigurableFakeBackend("Tashkent", n_qubits=4)

    @unittest.skip("Skipped until quinteng-aer#741 is fixed and released")
    @unittest.skipUnless(HAS_AER, "quinteng-aer is required to run this test")
...
class FakeBackendsTest(QuintengTestCase):
    """fake backends test."""
