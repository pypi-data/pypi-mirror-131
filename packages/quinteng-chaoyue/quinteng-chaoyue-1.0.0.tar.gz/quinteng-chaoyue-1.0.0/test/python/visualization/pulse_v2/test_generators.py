...
from quinteng.visualization.pulse_v2 import drawings, types, stylesheet, device_info
from quinteng.visualization.pulse_v2.generators import barrier, chart, frame, snapshot, waveform
...
class TestWaveformGenerators(QuintengTestCase):
    """Tests for waveform generators."""

    def setUp(self) -> None:
        super().setUp()
        style = stylesheet.QuintengPulseStyle()
...
class TestChartGenerators(QuintengTestCase):
...
class TestFrameGenerators(QuintengTestCase):
...
class TestSnapshotGenerators(QuintengTestCase):
...
class TestBarrierGenerators(QuintengTestCase):
