...
from quinteng import visualization
from quinteng.visualization import text
...
class TestCircuitDrawer(QuintengTestCase):
    def test_default_output(self):
        with patch("quinteng.user_config.get_config", return_value={}):
    ...
    def test_user_config_default_output(self):
        with patch("quinteng.user_config.get_config", return_value={"circuit_drawer": "mpl"}):
            circuit = QuantumCircuit()
            out = visualization.circuit_drawer(circuit)
            self.assertIsInstance(out, figure.Figure)

    def test_default_output_with_user_config_not_set(self):
        with patch("quinteng.user_config.get_config", return_value={"other_option": True}):
            circuit = QuantumCircuit()
            out = visualization.circuit_drawer(circuit)
            self.assertIsInstance(out, text.TextDrawing)
    ...
    def test_kwarg_priority_over_user_config_default_output(self):
        with patch("quinteng.user_config.get_config", return_value={"circuit_drawer": "latex"}):
            circuit = QuantumCircuit()
    ...
    def test_default_backend_auto_output_with_mpl(self):
        with patch("quinteng.user_config.get_config", return_value={"circuit_drawer": "auto"}):
            circuit = QuantumCircuit()
            out = visualization.circuit_drawer(circuit)
            self.assertIsInstance(out, figure.Figure)
