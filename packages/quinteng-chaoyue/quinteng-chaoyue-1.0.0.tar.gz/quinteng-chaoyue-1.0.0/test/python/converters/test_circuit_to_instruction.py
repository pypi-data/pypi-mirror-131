...
from quinteng.converters import circuit_to_instruction
...
class TestCircuitToInstruction(QuintengTestCase):
    ...
    def test_underspecified_parameter_map_raises(self):
        ...
        self.assertRaises(QuintengError, circuit_to_instruction, qc, {theta: gamma})

        # Raise if provided more parameters than present in the circuit
        delta = Parameter("delta")
        self.assertRaises(
            QuintengError, circuit_to_instruction, qc, {theta: gamma, phi: phi, delta: delta}
        )
