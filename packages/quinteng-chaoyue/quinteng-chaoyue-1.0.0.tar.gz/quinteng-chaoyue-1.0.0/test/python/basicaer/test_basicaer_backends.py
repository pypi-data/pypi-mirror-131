...
from quinteng.providers.basicaer import BasicAerProvider
from quinteng.providers.exceptions import QuintengBackendNotFoundError
from quinteng.test import providers


class TestBasicAerBackends(providers.ProviderTestCase):
    """Quinteng BasicAer Backends (Object) Tests."""
    ...
    def test_deprecated(self):
        ...
        def _get_first_available_backend(provider, backend_names):
            ...
            for backend_name in backend_names:
                ...
                except QuintengBackendNotFoundError:
                    pass
            return None
        ...
        for oldname, newname in deprecated_names.items():
            expected = (
                "WARNING:quinteng.providers.providerutils:Backend '%s' is deprecated. "
                "Use '%s'." % (oldname, newname)
            )
            with self.subTest(oldname=oldname, newname=newname):
                with self.assertLogs("quinteng.providers.providerutils", level="WARNING") as context:
    ...
    def test_aliases_fail(self):
        ...
        self.assertRaises(QuintengBackendNotFoundError, BasicAer.get_backend, "bad_name")
