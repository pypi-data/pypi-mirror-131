...
from quinteng.pulse import LoConfig, Kernel, Discriminator
...
from quinteng.pulse.library import Waveform, Gaussian, GaussianSquare, Constant, Drag
from quinteng.pulse.schedule import Schedule
from quinteng.qobj import (
...
class TestInstructionToQobjConverter(QuintengTestCase):
    ...
    def test_gaussian_square_pulse_instruction(self):
        ...
        converter = InstructionToQobjConverter(PulseQobjInstruction, meas_level=2)
        instruction = Play(
            GaussianSquare(duration=1500, sigma=15, amp=-0.5 + 0.2j, width=1300), MeasureChannel(1)
        )
...
class TestQobjToInstructionConverter(QuintengTestCase):
    """Pulse converter tests."""
...
class TestLoConverter(QuintengTestCase):
    """LO converter tests."""
