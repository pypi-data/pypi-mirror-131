...
from quinteng.test.mock import FakeProvider, FakeLegacyProvider
...
@ddt
class TestFakeBackends(QuintengTestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_circuit_on_fake_backend(self, backend, optimization_level):
        if not HAS_AER and backend.configuration().num_qubits > 20:
            self.skipTest(
                "Unable to run fake_backend %s without quinteng-aer"
