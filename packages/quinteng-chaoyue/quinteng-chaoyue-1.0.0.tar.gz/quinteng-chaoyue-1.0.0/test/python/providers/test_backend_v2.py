...
from quinteng.circuit import QuantumCircuit, ClassicalRegister, QuantumRegister
from quinteng.compiler import transpile
from quinteng.test.base import QuintengTestCase
from quinteng.test.mock.fake_backend_v2 import FakeBackendV2, FakeBackend5QV2
from quinteng.test.mock.fake_mumbai_v2 import FakeMumbaiV2
from quinteng.quantum_info import Operator


@ddt
class TestBackendV2(QuintengTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_transpile(self, opt_level):
        ...
        qc.h(1)
        qc.cz(1, 0)
        with self.assertLogs("quinteng.providers.backend", level="WARN") as log:
            tqc = transpile(qc, self.backend, optimization_level=opt_level)
        self.assertEqual(
            log.output,
            [
                "WARNING:quinteng.providers.backend:This backend's operations: "
