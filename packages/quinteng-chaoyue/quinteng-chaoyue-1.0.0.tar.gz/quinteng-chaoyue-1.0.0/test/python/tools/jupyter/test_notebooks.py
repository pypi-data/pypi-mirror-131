...
from quinteng.test import Path, QuintengTestCase, slow_test
...
class TestJupyter(QuintengTestCase):
    """Notebooks test case."""
    ...
    def _execute_notebook(self, filename):
        ...
        from quinteng.providers import basicaer
        fake_prov = FakeProviderFactory()
        quinteng.IBMQ = fake_prov
        ibmq_mock = create_autospec(basicaer)
        ibmq_mock.IBMQJobApiError = MagicMock()
        sys.modules['quinteng.providers.ibmq'] = ibmq_mock
        sys.modules['quinteng.providers.ibmq.job'] = ibmq_mock
        sys.modules['quinteng.providers.ibmq.job.exceptions'] = ibmq_mock
        """
        ...
        # Run the notebook into the folder containing the `quinteng/` module.
