...
import numpy as np
from quinteng.algorithms.phase_estimators import (
    PhaseEstimation,
    HamiltonianPhaseEstimation,
    IterativePhaseEstimation,
)
from quinteng.opflow.evolutions import PauliTrotterEvolution, MatrixEvolution
import quinteng
from quinteng import QuantumCircuit
from quinteng.opflow import H, X, Y, Z, I, StateFn


@ddt
class TestHamiltonianPhaseEstimation(QuintengAlgorithmsTestCase):
    ...
    def hamiltonian_pe(
        ...
        if backend is None:
            backend = quinteng.BasicAer.get_backend("statevector_simulator")
        quantum_instance = quinteng.utils.QuantumInstance(backend=backend, shots=10000)
    ...
    def _setup_from_bound(self, evolution, op_class):
        ...
        backend = quinteng.BasicAer.get_backend("statevector_simulator")
        qi = quinteng.utils.QuantumInstance(backend=backend, shots=10000)
...
@ddt
class TestPhaseEstimation(QuintengAlgorithmsTestCase):
    """Evolution tests."""
    ...
    def one_phase(
        ...
        backend = quinteng.BasicAer.get_backend(backend_type)
        qi = quinteng.utils.QuantumInstance(backend=backend, shots=10000)
