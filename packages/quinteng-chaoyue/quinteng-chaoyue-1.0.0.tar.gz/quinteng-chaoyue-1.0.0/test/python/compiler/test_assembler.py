...
from quinteng.circuit import Instruction, Gate, Parameter, ParameterVector
from quinteng.circuit import QuantumRegister, ClassicalRegister, QuantumCircuit
from quinteng.compiler.assembler import assemble
from quinteng.exceptions import QuintengError
from quinteng.pulse import Schedule, Acquire, Play
from quinteng.pulse.channels import MemorySlot, AcquireChannel, DriveChannel, MeasureChannel
from quinteng.pulse.configuration import Kernel, Discriminator
from quinteng.pulse.library import gaussian
from quinteng.qobj import QasmQobj, PulseQobj
from quinteng.qobj.utils import MeasLevel, MeasReturnType
from quinteng.pulse.macros import measure
from quinteng.test import QuintengTestCase
from quinteng.test.mock import FakeOpenPulse2Q, FakeOpenPulse3Q, FakeYorktown, FakeAlmaden
...
class TestCircuitAssembler(QuintengTestCase):
    ...
    def test_shots_greater_than_max_shots(self):
        ...
        self.assertRaises(QuintengError, assemble, self.backend, shots=1024000)

    def test_shots_not_of_type_int(self):
        """Test assembling with shots having type other than int"""
        self.assertRaises(QuintengError, assemble, self.backend, shots="1024")
    ...
    def test_assemble_user_rep_time_delay(self):
        ...
        with self.assertRaises(QuintengError):
    ...
    def test_assemble_circuits_raises_for_bind_circuit_mismatch(self):
        ...
        self.assertRaises(QuintengError, assemble, full_bound_circ, **partial_bind_args)

        # Raise when no parameters passed for parametric circuit
        self.assertRaises(QuintengError, assemble, partial_param_circ)
        self.assertRaises(QuintengError, assemble, full_param_circ)

        # Raise when circuit has more parameters than run_config
        self.assertRaises(QuintengError, assemble, full_param_circ, **partial_bind_args)

        # Raise when not all circuits have all parameters
        self.assertRaises(
            QuintengError, assemble, [full_param_circ, partial_param_circ], **full_bind_args
        )

        # Raise when not all binds have all circuit params
        self.assertRaises(QuintengError, assemble, full_param_circ, **inconsistent_bind_args)
    ...
    def test_assemble_circuits_rases_for_bind_mismatch_over_expressions(self):
        ...
        # Raise when no parameters passed for parametric circuit
        self.assertRaises(QuintengError, assemble, expr_circ)

        # Raise when circuit has more parameters than run_config
        self.assertRaises(QuintengError, assemble, expr_circ, **partial_bind_args)
...
class TestPulseAssembler(QuintengTestCase):
...
class TestPulseAssemblerMissingKwargs(QuintengTestCase):
...
class TestLogAssembler(QuintengTestCase):
