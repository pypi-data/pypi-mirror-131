...
from quinteng.opflow import StateFn, Zero, One, H, X, I, Z, Plus, Minus, CircuitSampler, ListOp
from quinteng.opflow.exceptions import OpflowError


@ddt
class TestStateOpMeasEvals(QuintengOpflowTestCase):
    ...
    def test_quantum_instance_with_backend_shots(self):
        ...
        try:
            from quinteng.providers.aer import AerSimulator
        except Exception as ex:  # pylint: disable=broad-except
            self.skipTest(f"Aer doesn't appear to be installed. Error: '{str(ex)}'")
