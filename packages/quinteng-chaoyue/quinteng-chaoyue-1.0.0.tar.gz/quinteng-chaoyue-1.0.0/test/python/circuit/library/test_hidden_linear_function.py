...
from quinteng.circuit.library import HiddenLinearFunction
from quinteng.quantum_info import Operator


class TestHiddenLinearFunctionLibrary(QuintengTestCase):
    """Test library of Hidden Linear Function circuits."""
    ...
    def assertHLFIsCorrect(self, hidden_function, hlf):
        ...
        for i in range(2 ** num_qubits):
            i_quinteng = int(bin(i)[2:].zfill(num_qubits)[::-1], 2)
            x_vec = np.asarray(list(map(int, bin(i)[2:].zfill(num_qubits)[::-1])))
            expected[i_quinteng, i_quinteng] = 1j ** (
