...
"""Test Quinteng's controlled gate operation."""
...
from quinteng import QuantumRegister, QuantumCircuit, execute, BasicAer, QuintengError
from quinteng.test import QuintengTestCase
from quinteng.circuit import ControlledGate, Parameter
from quinteng.circuit.exceptions import CircuitError
from quinteng.quantum_info.operators.predicates import matrix_equal, is_unitary_matrix
from quinteng.quantum_info.random import random_unitary
from quinteng.quantum_info.states import Statevector
import quinteng.circuit.add_control as ac
...
from quinteng.circuit._utils import _compute_control_matrix
import quinteng.circuit.library.standard_gates as allGates
from quinteng.extensions import UnitaryGate

from .gate_utils import _get_free_params


@ddt
class TestControlledGate(QuintengTestCase):
        ...
        for special_case_gate, n_controls in tests:
            with self.subTest(gate=special_case_gate.name):
                ...
                if not isinstance(special_case_gate, CXGate):
                    # CX is treated like a primitive within ChaoYue, and doesn't have a definition.
    ...
    def test_multi_control_u3(self):
        ...
        from quinteng.circuit.library.standard_gates import u3
    ...
    def test_multi_control_u1(self):
        ...
        from quinteng.circuit.library.standard_gates import u1
    ...
    def test_multi_controlled_u1_matrix(self, num_controls):
        ...
        https://github.com/Quinteng/quinteng-aqua/blob/769ca8f/test/aqua/test_mcu1.py
        """
    ...
    def test_multi_control_toffoli_matrix_clean_ancillas(self, num_controls):
        ...
        Based on the test moved here from Aqua:
        https://github.com/Quinteng/quinteng-aqua/blob/769ca8f/test/aqua/test_mct.py
        """
        # set up circuit
        q_controls = QuantumRegister(num_controls)
        q_target = QuantumRegister(1)
    ...
    def test_multi_controlled_rotation_gate_matrices(
        ...
        Based on the test moved here from Aqua:
        https://github.com/Quinteng/quinteng-aqua/blob/769ca8f/test/aqua/test_mcr.py
        """
        q_controls = QuantumRegister(num_controls)
        q_target = QuantumRegister(1)
    ...
    def test_ccx_ctrl_state_consistency(self):
        ...
        See issue: https://github.com/Quinteng/quinteng-chaoyue/issues/6465
        """
        qreg = QuantumRegister(3)
        qc = QuantumCircuit(qreg)
...
@ddt
class TestOpenControlledToMatrix(QuintengTestCase):
...
@ddt
class TestSingleControlledRotationGates(QuintengTestCase):
    """Test the controlled rotation gates controlled on one qubit."""

    from quinteng.circuit.library.standard_gates import u1, rx, ry, rz
...
@ddt
class TestControlledStandardGates(QuintengTestCase):
    ...
    def test_controlled_standard_gates(self, num_ctrl_qubits, gate_class):
        ...
        for ctrl_state in (ctrl_state_ones, ctrl_state_zeros, ctrl_state_mixed):
            with self.subTest(i=f"{gate_class.__name__}, ctrl_state={ctrl_state}"):
                ...
                except (AttributeError, QuintengError):
...
@ddt
class TestParameterCtrlState(QuintengTestCase):
    ...
    def test_ctrl_state_one(self, gate, controlled_gate):
        ...
        See https://github.com/Quinteng/quinteng-chaoyue/pull/4025
        """
        self.assertEqual(
...
@ddt
class TestControlledGateLabel(QuintengTestCase):
