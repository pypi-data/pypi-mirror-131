...
"""
Tests for quinteng-chaoyue/quinteng/quantum_info/synthesis/xx_decompose/circuits.py .
"""
...
import quinteng.quantum_info.operators
from quinteng.quantum_info.synthesis.weyl import weyl_coordinates
from quinteng.quantum_info.synthesis.xx_decompose.circuits import (
...
class TestMonodromyCircuits(unittest.TestCase):
    ...
    def test_xx_circuit_step(self):
        ...
        for _ in range(100):
            ...
            source_embodiment = quinteng.QuantumCircuit(2)
            source_embodiment.append(
                quinteng.extensions.UnitaryGate(canonical_matrix(*source_coordinate)), [0, 1]
            )
            interaction_embodiment = quinteng.QuantumCircuit(2)
            interaction_embodiment.append(
                quinteng.extensions.UnitaryGate(canonical_matrix(*interaction)), [0, 1]
            )
            ...
            self.assertTrue(
                np.all(
                    np.abs(
                        quinteng.quantum_info.operators.Operator(target_embodiment).data
