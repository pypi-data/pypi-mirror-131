...
from quinteng import transpile
from quinteng.circuit.library import HGate, QFT
from quinteng.providers.basicaer import QasmSimulatorPy

from quinteng.quantum_info.random import random_unitary, random_statevector, random_pauli
...
from quinteng.visualization.state_visualization import numbers_to_latex_terms, state_to_latex

logger = logging.getLogger(__name__)


@ddt
class TestStatevector(QuintengTestCase):
    ...
    def test_init_array_except(self):
        ...
        vec = self.rand_vec(4)
        self.assertRaises(QuintengError, Statevector, vec, dims=[4, 2])
        self.assertRaises(QuintengError, Statevector, vec, dims=[2, 4])
        self.assertRaises(QuintengError, Statevector, vec, dims=5)
    ...
    def test_getitem_except(self):
        ...
        for i in range(1, 4):
            ...
            self.assertRaises(QuintengError, state.__getitem__, 2 ** i)
            self.assertRaises(QuintengError, state.__getitem__, -1)
    ...
    def test_inner_except(self):
        ...
        self.assertRaises(QuintengError, vec0.inner, vec1)
        vec0 = Statevector(self.rand_vec(6), dims=(2, 3))
    ...
    def test_add_except(self):
        ...
        self.assertRaises(QuintengError, state1.__add__, state2)
