...
from quinteng.utils.mitigation._filters import MeasurementFilter
from quinteng.utils.mitigation.circuits import count_keys

try:
    from quinteng.providers.aer import Aer
    from quinteng.providers.aer.noise import NoiseModel
    from quinteng.providers.aer.noise.errors.standard_errors import pauli_error
...
def meas_calib_circ_creation():
    ...
    ghz = quinteng.QuantumCircuit(total_number_of_qubit, len(qubit_list))
    ghz.h(qubit_1)
...
def tensored_calib_circ_creation():
    ...
    mit_pattern = [[2], [4, 1]]
    meas_layout = [2, 4, 1]
    qr = quinteng.QuantumRegister(5)
    # Generate the calibration circuits
    meas_calibs, mit_pattern = tensored_meas_cal(mit_pattern, qr=qr)

    cr = quinteng.ClassicalRegister(3)
    ghz_circ = quinteng.QuantumCircuit(qr, cr)
...
def meas_calibration_circ_execution(shots: int, seed: int):
    ...
    backend = quinteng.Aer.get_backend("qasm_simulator")
    cal_results = quinteng.execute(
...
@unittest.skipUnless(HAS_AER, "Quinteng aer is required to run these tests")
class TestMeasCal(QuintengTestCase):
    """The test class."""
    ...
    def test_ideal_meas_cal(self):
        ...
        for nq in self.nq_list:
            for pattern_type in range(1, 2 ** nq):
                ...
                job = quinteng.execute(meas_calibs, backend=backend, shots=self.shots)
    ...
    def test_ideal_tensored_meas_cal(self):
        ...
        # Perform an ideal execution on the generated circuits
        backend = Aer.get_backend("qasm_simulator")
        cal_results = quinteng.execute(meas_calibs, backend=backend, shots=self.shots).result()
