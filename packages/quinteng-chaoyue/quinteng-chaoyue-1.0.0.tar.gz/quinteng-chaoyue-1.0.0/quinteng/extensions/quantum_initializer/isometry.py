...
from quinteng.quantum_info.operators.predicates import is_isometry
from quinteng.extensions.quantum_initializer.uc import UCGate
from quinteng.extensions.quantum_initializer.mcg_up_to_diagonal import MCGupDiag
...
class Isometry(Instruction):
    ...
    # finally, we convert the labels back to the qubit numbering used in Quinteng
...
# Methods for applying gates to matrices (should be moved to Quinteng AER)
...
def _apply_ucg(m, k, single_qubit_gates):
    ...
    # ToDo: this method should be implemented by the state vector simulator in Quinteng AER.
...
def _apply_multi_controlled_gate(m, control_labels, target_label, gate):
    # ToDo: This method should be integrated into the state vector simulator in Quinteng AER.
    num_qubits = int(np.log2(m.shape[0]))
    num_cols = m.shape[1]
    control_labels.sort()
...
# be compatible with the standard convention of Quinteng.
...
def iso(
    ...
    Raises:
        QuintengError: if the array is not an isometry of the correct size corresponding to
