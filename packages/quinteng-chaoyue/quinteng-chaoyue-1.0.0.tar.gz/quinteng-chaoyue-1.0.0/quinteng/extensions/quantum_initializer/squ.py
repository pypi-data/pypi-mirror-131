...
from quinteng.circuit import QuantumRegister, Qubit, QuantumCircuit
...
class SingleQubitUnitary(Gate):
    ...
    def __init__(self, unitary_matrix, mode="ZYZ", up_to_diagonal=False):
        ...
        if mode not in ["ZYZ"]:
            raise QuintengError("The decomposition mode is not known.")
        # Check if the matrix u has the right dimensions and if it is a unitary
        if unitary_matrix.shape != (2, 2):
            raise QuintengError("The dimension of the input matrix is not equal to (2,2).")
        if not is_unitary_matrix(unitary_matrix):
            raise QuintengError("The 2*2 matrix is not unitary.")
...
def squ(
    ...
    Raises:
        QuintengError: if the format is wrong; if the array u is not unitary
    """
    ...
    if not isinstance(qubit, Qubit):
        raise QuintengError("The target qubit is not a single qubit from a QuantumRegister.")
