...
from quinteng.circuit.library.standard_gates.x import CXGate, XGate
from quinteng.circuit.library.standard_gates.h import HGate
from quinteng.circuit.library.standard_gates.s import SGate, SdgGate
    ...
    def __init__(self, params, num_qubits=None):
        ...
        params (str, list, int or Statevector):
          ...
          * string: labels of basis states of the Pauli eigenstates Z, X, Y. See
               :meth:`~quinteng.quantum_info.states.statevector.Statevector.from_label`.
        ...
        if isinstance(params, str):
            ...
            if num_qubits == 0 or not num_qubits.is_integer():
                raise QuintengError("Desired statevector length not a positive power of 2.")

            # Check if probabilities (amplitudes squared) sum to 1
            if not math.isclose(sum(np.absolute(params) ** 2), 1.0, abs_tol=_EPS):
                raise QuintengError("Sum of amplitudes-squared does not equal one.")
...
def initialize(self, params, qubits=None):
    ...
    Returns:
        quinteng.circuit.Instruction: a handle to the instruction that was just initialized
