...
from quinteng.circuit.quantumcircuit import QuantumCircuit, QuantumRegister
...
class DiagonalGate(Gate):
    ...
    def __init__(self, diag):
        ...
        if not isinstance(diag, list):
            raise QuintengError("The diagonal entries are not provided in a list.")
        ...
        if num_action_qubits < 1 or not num_action_qubits.is_integer():
            raise QuintengError("The number of diagonal entries is not a positive power of 2.")
        for z in diag:
            try:
                complex(z)
            ...
            if not np.abs(z) - 1 < _EPS:
                raise QuintengError("A diagonal entry has not absolute value one.")
        # Create new gate.
...
def diagonal(self, diag, qubit):
    ...
    Raises:
        QuintengError: if the list of the diagonal entries or the qubit list is in bad format;
