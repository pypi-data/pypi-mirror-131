...
from quinteng.circuit.quantumcircuit import QuantumRegister, QuantumCircuit
    ...
    def __init__(self, gate, num_controls, num_ancillas_zero, num_ancillas_dirty):
        ...
        Args:
            ...
            num_ancillas_dirty (int): number of ancilla qubits that are allowed to start in an
                arbitrary state
        Raises:
            QuintengError: if the input format is wrong; if the array gate is not unitary
        """
        ...
        if not gate.shape == (2, 2):
            raise QuintengError("The dimension of the controlled gate is not equal to (2,2).")
        # Check if the single-qubit gate is unitary
        if not is_isometry(gate, _EPS):
            raise QuintengError("The controlled gate is not unitary.")
        # Create new gate.
