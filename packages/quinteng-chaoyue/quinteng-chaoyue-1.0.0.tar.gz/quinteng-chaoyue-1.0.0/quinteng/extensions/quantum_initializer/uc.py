...
from quinteng.quantum_info.synthesis import OneQubitEulerDecomposer
...
class UCGate(Gate):
    ...
    def __init__(self, gate_list, up_to_diagonal=False):
        ...
        Raises:
            QuintengError: in case of bad input to the constructor
        """
        # check input format
        if not isinstance(gate_list, list):
            raise QuintengError("The single-qubit unitaries are not provided in a list.")
        for gate in gate_list:
            if not gate.shape == (2, 2):
                raise QuintengError("The dimension of a controlled gate is not equal to (2,2).")
        if not gate_list:
            raise QuintengError("The gate list cannot be empty.")
        ...
        for gate in gate_list:
            if not is_unitary_matrix(gate, _EPS):
                raise QuintengError("A controlled gate is not unitary.")
...
def uc(self, gate_list, q_controls, q_target, up_to_diagonal=False):
    ...
    Raises:
        QuintengError: if the list number of control qubits does not correspond to the provided
