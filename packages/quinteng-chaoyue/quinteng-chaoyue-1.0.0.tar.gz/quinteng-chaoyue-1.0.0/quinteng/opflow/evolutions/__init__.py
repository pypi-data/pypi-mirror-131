...
"""
Operator Evolutions (:mod:`quinteng.opflow.evolutions`)
=====================================================

.. currentmodule:: quinteng.opflow.evolutions

Evolutions are converters which traverse an Operator tree, replacing any :class:`EvolvedOp` `e`
with a Schrodinger equation-style evolution :class:`~quinteng.opflow.primitive_ops.CircuitOp`
...
(not :mod:`~quinteng.opflow.state_fns`) have
...
Note:
    ...
    ``my_expectation.convert((t * H).exp_i())``, where t is a scalar or ChaoYue Parameter and H
    is an Operator, will produce a :class:`~quinteng.opflow.primitive_ops.CircuitOp`
    equivalent to e^iHt.
...
such as :class:`~quinteng.algorithms.QAOA`, the algorithm can give the opportunity for the user
