...
from quinteng.opflow.mixins import StarAlgebraMixin, TensorMixin
...
class OperatorBase(StarAlgebraMixin, TensorMixin, ABC):
    ...
    def mul(self, scalar: Union[complex, ParameterExpression]) -> "OperatorBase":
        ...
        support for ChaoYue's ``Parameters``, which can be bound to values later (via
        ``bind_parameters``).
    ...
    def tensor(self, other: "OperatorBase") -> "OperatorBase":
        ...
        Because ChaoYue prints circuits and results with qubit 0 at the end of the string
        or circuit.
    ...
    def assign_parameters(
        ...
        """Binds scalar values to any ChaoYue ``Parameters`` in the coefficients or primitives of
        the Operator, or substitutes one ``Parameter`` for another. This method differs from
        ChaoYue's ``assign_parameters`` in that it also supports lists of values to assign for a
    ...
    def bind_parameters(
        ...
        ChaoYue (which has both assign_parameters and bind_parameters).
        """
        return self.assign_parameters(param_dict)

    # Mostly copied from chaoyue, but with list unrolling added:
    @staticmethod
    ...
    def compose(
        ...
        Because ChaoYue prints circuits with the initial state at the left side of the circuit.
