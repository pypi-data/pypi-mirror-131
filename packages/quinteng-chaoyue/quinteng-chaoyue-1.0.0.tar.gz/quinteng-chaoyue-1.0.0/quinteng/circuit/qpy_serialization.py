...
"""
###########################################################
QPY serialization (:mod:`quinteng.circuit.qpy_serialization`)
###########################################################

.. currentmodule:: quinteng.circuit.qpy_serialization
...
2 user facing functions: :func:`quinteng.circuit.qpy_serialization.dump` and
:func:`quinteng.circuit.qpy_serialization.load` which are used to dump QPY data
...
For example::
    ...
    from quinteng.circuit import qpy_serialization
...
you should be able to load a QPY with any newer Quinteng version than the one
that generated it. However, loading a QPY file with an older Quinteng version is
not supported and may not work.

For example, if you generated a QPY file using quinteng-chaoyue 0.18.1 you could
load that QPY file with quinteng-chaoyue 0.19.0 and a hypothetical quinteng-chaoyue
...
serialization format for :class:`~quinteng.circuit.QuantumCircuit` objects in Quinteng. The basic
file format is as follows:

A QPY file (or memory object) always starts with the following 7
byte UTF8 string: ``QUINTENG`` which is immediately followed by the overall
...
.. code-block:: c
    ...
    struct {
        uint8_t qpy_version;
        uint8_t quinteng_major_version;
        uint8_t quinteng_minor_version;
        uint8_t quinteng_patch_version;
...
size of a ``double``. If it's ``'p'`` defines a :class:`~quinteng.circuit.Parameter`
object  which is represented by a PARAM struct (see below), ``e`` defines a
:class:`~quinteng.circuit.ParameterExpression` object (that's not a
:class:`~quinteng.circuit.Parameter`) which is represented by a PARAM_EXPR struct
(see below).
...
for the ``name``. ``name`` here is the Quinteng class name for the Instruction
class if it's defined in Quinteng. Otherwise it falls back to the custom
...
``'p'`` defines a :class:`~quinteng.circuit.Parameter` object  which is
...
A PARAMETER represents a :class:`~quinteng.circuit.Parameter` object the data for
...
A PARAMETER_EXPR represents a :class:`~quinteng.circuit.ParameterExpression`
...
``type`` is ``p`` then it represents a :class:`~quinteng.circuit.Parameter` and
...
from quinteng.circuit.quantumregister import QuantumRegister, Qubit
...
from quinteng.circuit import library
from quinteng import circuit as circuit_mod
from quinteng import extensions
from quinteng.extensions import quantum_initializer
from quinteng.version import __version__
...
def dump(circuits, file_obj):
    ...
    loaded with future versions of Quinteng.
...
def load(file_obj):
    ...
    :class:`~quinteng.circuit.QuantumCircuit` objects from its contents.
    For example:
    ...
    :class:`~quinteng.circuit.QuantumCircuit` objects from the file.
    ...
    if (
        ...
        warnings.warn(
            "The quinteng version used to generate the provided QPY "
            "file, %s, is newer than the current quinteng version %s. "
            "This may result in an error if the QPY file uses "
            "instructions not present in this current quinteng "
                ...
                if standalone:
                    ...
                    for index in indices:
                        ...
                        if index in register_bits:
                            raise QuintengError("Duplicate register bits found")
