...
class Gate(Instruction):
    ...
    def power(self, exponent: float):
        ...
        Returns:
            quinteng.extensions.UnitaryGate: To which `to_matrix` is self.to_matrix^exponent.

        Raises:
            CircuitError: If Gate is not unitary
        """
        from quinteng.quantum_info.operators import Operator  # pylint: disable=cyclic-import
        from quinteng.extensions.unitary import UnitaryGate  # pylint: disable=cyclic-import
    ...
    def control(
        ...
        Returns:
            quinteng.circuit.ControlledGate: Controlled version of gate. This default algorithm
            uses num_ctrl_qubits-1 ancillae qubits so returns a gate of size
            num_qubits + 2*num_ctrl_qubits - 1.

        Raises:
            QuintengError: unrecognized mode or invalid ctrl_state
        """
