...
def get_measured_qubits(
    ...
    Raises:
        QuintengError: invalid qubit mapping
    """
    qubit_index = None
    qubit_mappings = {}
...
def build_measurement_error_mitigation_circuits(
    ...
    Raises:
        QuintengError: when the fitter_cls is not recognizable.
    """
    circlabel = "mcal"

    if not qubit_list:
        raise QuintengError("The measured qubit list can not be [].")
    ...
    if not run:
        try:
            ...
            raise QuintengError(f"Unknown fitter {fitter_cls}") from ex
        if fitter_cls == CompleteMeasFitter_IG:
            ...
            raise QuintengError(f"Unknown fitter {fitter_cls}")
...
def build_measurement_error_mitigation_qobj(
    ...
    Raises:
        QuintengError: when the fitter_cls is not recognizable.
        MissingOptionalLibraryError: Quinteng-Ignis not installed
    """
