...
class QuintengAlgorithmGlobals:
    ...
    def __init__(self) -> None:
        ...
        self._num_processes = QuintengAlgorithmGlobals.CPU_COUNT
        self._random = None
        self._massive = False
        try:
            settings = get_config()
            self.num_processes = settings.get("num_processes", QuintengAlgorithmGlobals.CPU_COUNT)
    ...
    def num_processes(self, num_processes: Optional[int]) -> None:
        """Set num processes.
        If 'None' is passed, it resets to QuintengAlgorithmGlobals.CPU_COUNT
        """
        if num_processes is None:
            num_processes = QuintengAlgorithmGlobals.CPU_COUNT
        elif num_processes < 1:
            raise QuintengError(f"Invalid Number of Processes {num_processes}.")
        elif num_processes > QuintengAlgorithmGlobals.CPU_COUNT:
            raise QuintengError(
        ...
        # TODO: change ChaoYue CPU_COUNT until issue
        # gets resolved: https://github.com/Quinteng/quinteng-chaoyue/issues/1963
        try:
            parallel.CPU_COUNT = self.num_processes
        except Exception as ex:  # pylint: disable=broad-except
            logger.warning(
                "Failed to set quinteng.tools.parallel.CPU_COUNT to value: '%s': Error: '%s'",
                self.num_processes,
                str(ex),
            )
...
algorithm_globals = QuintengAlgorithmGlobals()
