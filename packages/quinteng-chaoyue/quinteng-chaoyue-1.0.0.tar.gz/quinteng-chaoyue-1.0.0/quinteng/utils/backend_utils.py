...
def has_ibmq():
    ...
    if not _PROVIDER_CHECK.checked_ibmq:
        try:
            from quinteng.providers.ibmq import IBMQFactory
            from quinteng.providers.ibmq.accountprovider import AccountProvider
...
def has_aer():
    ...
    if not _PROVIDER_CHECK.checked_aer:
        try:
            from quinteng.providers.aer import AerProvider
...
def is_aer_provider(backend):
    ...
    if has_aer():
        ...
        if isinstance(backend.provider(), AerProvider):
            return True
        from quinteng.providers.aer.backends.aerbackend import AerBackend
...
def is_statevector_backend(backend):
    ...
    """
    if has_aer():
        from quinteng.providers.aer.backends import AerSimulator, StatevectorSimulator
