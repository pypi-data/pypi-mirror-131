...
from quinteng.utils.mitigation._filters import MeasurementFilter, TensoredFilter
...
class CompleteMeasFitter:
    ...
    def __init__(
        ...
        .. warning::
            ...
            :class:`~quinteng.utils.QuantumInstance` class's constructor (as
    ...
    def add_data(self, new_results, rebuild_cal_matrix=True):
        ...
        Args:
            new_results (list or quinteng.result.Result): a single result or list
                of result objects.
    ...
    def subset_fitter(self, qubit_sublist=None):
        ...
        Raises:
            QuintengError: If the calibration matrix is not initialized
        """

        if self._tens_fitt.cal_matrices is None:
            raise QuintengError("Calibration matrix is not initialized")

        if qubit_sublist is None:
            raise QuintengError("Qubit sublist must be specified")

        for qubit in qubit_sublist:
            if qubit not in self._qubit_list:
                raise QuintengError("Qubit not in the original set of qubits")
...
class TensoredMeasFitter:
    ...
    def readout_fidelity(self, cal_index=0, label_list=None):
        ...
        Raises:
            QuintengError: If the calibration matrix has not been set for the
                object.
        ...
        if self._cal_matrices is None:
            raise QuintengError("Cal matrix has not been set")
