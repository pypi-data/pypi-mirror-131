...
def complete_meas_cal(
    ...
    Raises:
        QuintengError: if both `qubit_list` and `qr` are `None`.
    ...
    from quinteng.circuit.exceptions import QuintengError

    if qubit_list is None and qr is None:
        raise QuintengError("Must give one of a qubit_list or a qr")
...
def tensored_meas_cal(
    ...
    Raises:
        QuintengError: if both `mit_pattern` and `qr` are None.
        QuintengError: if a qubit appears more than once in `mit_pattern`.
    ...
    if mit_pattern is None and qr is None:
        raise QuintengError("Must give one of mit_pattern or qr")
