...
import numpy as np
from quinteng import QuantumCircuit, QuantumRegister, ClassicalRegister
from quinteng.providers import Backend, BaseBackend, JobStatus, JobError, BaseJob
from quinteng.providers.jobstatus import JOB_FINAL_STATES
from quinteng.result import Result
from quinteng.qobj import QasmQobj
from ..exceptions import QuintengError, MissingOptionalLibraryError
...
MAX_CIRCUITS_PER_JOB = os.environ.get("QUINTENG_AQUA_MAX_CIRCUITS_PER_JOB", None)
MAX_GATES_PER_JOB = os.environ.get("QUINTENG_AQUA_MAX_GATES_PER_JOB", None)
...
def _combine_result_objects(results: List[Result]) -> Result:
    ...
    TODO:
        This function would be removed after ChaoYue supports job with infinite circuits.
    """
    if len(results) == 1:
        return results[0]
...
def _split_qobj_to_qobjs(qobj: QasmQobj, chunk_size: int) -> List[QasmQobj]:
    ...
    if num_chunks == 1:
        ...
        if isinstance(qobj, QasmQobj):
            ...
            raise QuintengError("Only support QasmQobj now.")
    ...
    for _ in range(max_job_retries):
        try:
            ...
            if is_ibmq_provider(backend):
                try:
                    from quinteng.providers.ibmq import IBMQBackendJobLimitError
                except ImportError as ex1:
                    raise MissingOptionalLibraryError(
                        libname="quinteng-ibmq-provider",
                        name="_safe_submit_qobj",
                        pip_install="pip install quinteng-ibmq-provider",
                    ) from ex1
        ...
        raise QuintengError("Max retry limit reached. Could not submit the qobj correctly")
...
def _safe_get_job_status(job: BaseJob, job_id: str, max_job_retries: int) -> JobStatus:
    for _ in range(max_job_retries):
        try:
            ...
            logger.warning(
                "FAILURE: job id: %s, status: 'FAIL_TO_GET_STATUS' ChaoYue job error: %s",
                job_id,
                ex,
            )
            time.sleep(5)
        ...
        raise QuintengError(f"Max retry limit reached. Failed to get status for job with id {job_id}")
...
def run_qobj(
    ...
    """
    An execution wrapper with Quinteng-ChaoYue, with job auto recover capability.
    ...
    Raises:
        ...
        QuintengError: Any error except for JobError raised by Quinteng ChaoYue
    """
        ...
        if result.status == "PARTIAL COMPLETED":
            ...
            for res in result.results:
                if not res.success:
                    msg += ", " + res.status
                    break
        raise QuintengError(f"Circuit execution failed: {msg}")
...
def _safe_submit_circuits(
    ...
    for _ in range(max_job_retries):
        ...
        raise QuintengError("Max retry limit reached. Failed to submit the qobj correctly")
