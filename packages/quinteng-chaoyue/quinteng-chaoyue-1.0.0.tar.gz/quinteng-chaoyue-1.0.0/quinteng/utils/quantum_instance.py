...
from quinteng.qobj import Qobj
from quinteng.utils import circuit_utils
from quinteng.exceptions import QuintengError
from quinteng.utils.backend_utils import (
    is_ibmq_provider,
...
from quinteng.utils.mitigation import (
    CompleteMeasFitter,
    TensoredMeasFitter,
)
...
class _MeasFitterType(Enum):
    ...
    def type_from_class(meas_class):
        ...
        try:
            from quinteng.ignis.mitigation.measurement import (
        ...
        if meas_class == CompleteMeasFitter_IG:
            warnings.warn(
                "The use of quinteng-ignis for measurement mitigation is "
                "deprecated and will be removed in a future release. Instead "
                "use the CompleteMeasFitter class from quinteng.utils.mitigation",
                DeprecationWarning,
                stacklevel=3,
            )
            return _MeasFitterType.COMPLETE_MEAS_FITTER
            ...
            warnings.warn(
                "The use of quinteng-ignis for measurement mitigation is "
                "deprecated and will be removed in a future release. Instead "
                "use the TensoredMeasFitter class from quinteng.utils.mitigation",
                DeprecationWarning,
                stacklevel=3,
            )
            return _MeasFitterType.TENSORED_MEAS_FITTER
        else:
            raise QuintengError(f"Unknown fitter {meas_class}")
    ...
    def type_from_instance(meas_instance):
        ...
        if isinstance(meas_instance, CompleteMeasFitter_IG):
            ...
            raise QuintengError(f"Unknown fitter {meas_instance}")
...
class QuantumInstance:
    ...
    # https://github.com/Quinteng/quinteng-aer/blob/master/quinteng/providers/aer/backends/qasm_simulator.py
    ...
    def __init__(
        ...
        """
        Quantum Instance holds a Quinteng ChaoYue backend as well as configuration for circuit
        ...
        Args:
            ...
            pass_manager (Optional['PassManager']): Pass manager to handle how to compile the circuits.
                ...
                :meth:`~quinteng.utils.QuantumInstance.transpile` method with the argument
            ...
            measurement_error_mitigation_cls: The approach to mitigate
                measurement errors. The classes :class:`~quinteng.utils.mitigation.CompleteMeasFitter`
                or :class:`~quinteng.utils.mitigation.TensoredMeasFitter` from the
                :mod:`quinteng.utils.mitigation` module can be used here as exact values, not
        ...
        Raises:
            QuintengError: the shots exceeds the maximum number of shots
            QuintengError: set noise model but the backend does not support that
            QuintengError: set backend_options but the backend does not support that
        """
        ...
        if shots is None:
            from quinteng.providers.basebackend import BaseBackend  # pylint: disable=cyclic-import
            from quinteng.providers.backend import BackendV1  # pylint: disable=cyclic-import
    ...
    def __str__(self) -> str:
        ...
        """
        from quinteng import __version__ as chaoyue_version

        info = f"\nQuinteng ChaoYue version: {chaoyue_version}\n"
    ...
    def transpile(self, circuits, pass_manager=None):
        ...
        """
        # pylint: disable=cyclic-import
        from quinteng import compiler
        from quinteng.transpiler import PassManager
    ...
    def execute(self, circuits, had_transpiled: bool = False):
        ...
        Raises:
            QuintengError: Invalid error mitigation fitter class
            QuintengError: TensoredMeasFitter class doesn't support subset fitter
        ...
        """
        from quinteng.utils.run_circuits import run_qobj, run_circuits
        from quinteng.utils.measurement_error_mitigation import (
