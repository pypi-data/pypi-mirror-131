...
from quinteng.pulse import instructions, transforms, library, schedule, channels
from quinteng.qobj import utils as qobj_utils, converters
from quinteng.qobj.converters.pulse_instruction import ParametricPulseShapes
...
def assemble_schedules(
    ...
    Raises:
        QuintengError: when frequency settings are not supplied.
    """
    if not hasattr(run_config, "qubit_lo_freq"):
        raise QuintengError("qubit_lo_freq must be supplied.")
    if not hasattr(run_config, "meas_lo_freq"):
        raise QuintengError("meas_lo_freq must be supplied.")
...
def _assemble_experiments(
    ...
    Raises:
        QuintengError: when frequency settings are not compatible with the experiments.
    """
...
def _validate_meas_map(
    ...
    Raises:
        QuintengError: If the instructions do not satisfy the measurement map.
    """
