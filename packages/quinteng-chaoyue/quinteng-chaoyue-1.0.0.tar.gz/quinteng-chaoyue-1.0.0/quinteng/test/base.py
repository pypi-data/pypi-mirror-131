...
Implementors of unit tests for ChaoYue are encouraged to subclass
``QuintengTestCase`` in order to take advantage of utility functions (for example,
...
class BaseQuintengTestCase(BaseTestCase):
    """Additions for test cases for all Quinteng-family packages.

    The additions here are intended for all packages, not just ChaoYue.  ChaoYue-specific logic should
    be in the ChaoYue-specific classes."""
...
class QuintengTestCase(BaseQuintengTestCase):
    """ChaoYue-specific extra functionality for test cases."""

    def tearDown(self):
        super().tearDown()
        # Reset the default providers, as in practice they acts as a singleton
        # due to importing the instances from the top-level quinteng namespace.
    ...
    def setUpClass(cls):
        ...
        allow_DeprecationWarning_modules = [
            ...
            "quinteng.quantum_info.operators.pauli",
            "pybobyqa",
            "numba",
            "quinteng.utils.measurement_error_mitigation",
            "quinteng.circuit.library.standard_gates.x",
            "quinteng.pulse.schedule",
            "quinteng.pulse.instructions.instruction",
            "quinteng.pulse.instructions.play",
            "quinteng.pulse.library.parametric_pulses",
            "quinteng.quantum_info.operators.symplectic.pauli",
        ...
        allow_DeprecationWarning_message = [
            ...
            r".*quinteng\.circuit\.library\.standard_gates\.ms import.*",
            r"elementwise comparison failed.*",
            r"The jsonschema validation included in quinteng-chaoyue.*",
...
class FullQuintengTestCase(QuintengTestCase):
    """ChaoYue-specific further additions for test cases, if ``testtools`` is available.

    It is not normally safe to derive from this class by name; on import, ChaoYue checks if the
    necessary packages are available, and binds this class to the name :obj:`~QuintengTestCase` if so.
    If you derive directly from it, you may try and instantiate the class without satisfying its
    dependencies."""

    def setUp(self):
        super().setUp()
        if os.environ.get("QUINTENG_TEST_CAPTURE_STREAMS"):
...
BasicQuintengTestCase = QuintengTestCase

if HAS_FIXTURES:
    QuintengTestCase = FullQuintengTestCase
