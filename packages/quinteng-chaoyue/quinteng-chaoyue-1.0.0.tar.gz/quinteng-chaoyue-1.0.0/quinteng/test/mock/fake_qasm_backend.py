...
from quinteng.providers.models import BackendProperties, QasmBackendConfiguration
from quinteng.test.mock.fake_backend import FakeBackend, FakeLegacyBackend
from quinteng.test.mock.utils.json_decoder import (
...
class FakeQasmBackend(FakeBackend):
    ...
    def _get_conf_from_json(self):
        if not self.conf_filename:
            raise QuintengError("No configuration file has been defined")
    ...
    def _set_props_from_json(self):
        if not self.props_filename:
            raise QuintengError("No properties file has been defined")
