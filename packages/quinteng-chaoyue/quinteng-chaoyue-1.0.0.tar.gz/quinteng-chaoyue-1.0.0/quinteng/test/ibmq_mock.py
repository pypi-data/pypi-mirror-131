...
"""Mock functions for quinteng.IBMQ."""

from unittest.mock import MagicMock
import quinteng
from quinteng.test import mock as backend_mocks


def mock_get_backend(backend):
    """Replace quinteng.IBMQ with a mock that returns a single backend.

    Note this will set the value of quinteng.IBMQ to a MagicMock object. It is
    ...
    context be aware that you will have to manually restore quinteng.IBMQ the
    value to quinteng.providers.ibmq.IBMQ after you finish using your mock.
    ...
    if not hasattr(backend_mocks, backend):
        raise NameError("The specified backend name is not a valid mock from quinteng.test.mock")
