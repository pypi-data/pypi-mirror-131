...
"""Decorator for using with Quinteng unit tests."""
...
def is_aer_provider_available():
    ...
    if sys.platform == "darwin":
        return False
    try:
        import quinteng.providers.aer  # pylint: disable=unused-import
    except ImportError:
        return False
    return True
...
def requires_aer_provider(test_item):
    """Decorator that skips test if quinteng aer provider is not available
...
def _get_credentials():
    ...
    """
    try:
        from quinteng.providers.ibmq.credentials import Credentials, discover_credentials
    except ImportError as ex:
        raise unittest.SkipTest(
            "quinteng-ibmq-provider could not be found, "
            "and is required for executing online tests. "
            'To install, run "pip install quinteng-ibmq-provider" '
    ...
    elif os.getenv("QUINTENG_TESTS_USE_CREDENTIALS_FILE"):
...
def online_test(func):
    ...
    It involves:
        ...
        * if the test is not skipped, it reads `qe_token` and `qe_url` from
            `Qconfig.py`, environment variables or quintengrc.
    ...
    def _wrapper(self, *args, **kwargs):
        ...
        if HAS_NET_CONNECTION is None:
            HAS_NET_CONNECTION = _has_connection("quinteng.org", 443)
