...
"""Utils for using with Quinteng unit tests."""
...
from quinteng import __path__ as quinteng_path


class Path(Enum):
    """Helper with paths commonly used during the tests."""

    # Main SDK path:    quinteng/
    SDK = quinteng_path[0]
    # test.python path: quinteng/test/python/
    TEST = os.path.normpath(os.path.join(SDK, "..", "test", "python"))
    # Examples path:    examples/
    EXAMPLES = os.path.normpath(os.path.join(SDK, "..", "examples"))
    # Schemas path:     quinteng/schemas
    SCHEMAS = os.path.normpath(os.path.join(SDK, "schemas"))
    # Sample QASMs path: quinteng/test/python/qasm
