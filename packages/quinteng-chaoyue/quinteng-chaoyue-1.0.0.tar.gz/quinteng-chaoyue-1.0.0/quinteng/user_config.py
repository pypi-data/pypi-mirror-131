...
DEFAULT_FILENAME = os.path.join(os.path.expanduser("~"), ".quinteng", "settings.conf")
...
class UserConfig:
    ...
    circuit_mpl_style_path = ~/.quinteng:<default location>
    ...
    def __init__(self, filename=None):
        ...
        Args:
            filename (str): The path to the user config file. If one isn't
                specified, ~/.quinteng/settings.conf is used.
        """
        if filename is None:
        ...
        if "default" in self.config_parser.sections():
            ...
            if circuit_drawer:
                if circuit_drawer not in ["text", "mpl", "latex", "latex_source", "auto"]:
                    raise exceptions.QuintengUserConfigError(
                ...
                for path in cpath_list:
                    if not os.path.exists(os.path.expanduser(path)):
                        warn(
                            ...
                            " Correct the path in ~/.quinteng/settings.conf." % path,
                            UserWarning,
                            2,
                        )
...
def set_config(key, value, section=None, file_path=None):
    ...
    Args:
        ...
        file_path (str, optional): the file to which config is added.
            ...
            if set, the value of `QUINTENG_SETTINGS` env variable.

    Raises:
        QuintengUserConfigError: if the config is invalid
    """
    filename = file_path or os.getenv("QUINTENG_SETTINGS", DEFAULT_FILENAME)
    section = "default" if section is None else section

    if not isinstance(key, str):
        raise exceptions.QuintengUserConfigError("Key must be string type")
    ...
    if section in [None, "default"]:
        if key not in valid_config:
            raise exceptions.QuintengUserConfigError(f"{key} is not a valid user config.")
...
def get_config():
    ...
    ~/.quinteng/settings.conf or if set the value of the QUINTENG_SETTINGS env var.

    It will return the parsed settings dict from the parsed config file.
    Returns:
        dict: The settings dict from the parsed config file.
    """
    filename = os.getenv("QUINTENG_SETTINGS", DEFAULT_FILENAME)
