...
The stylesheet `QuintengTimelineStyle` is initialized with the hard-corded default values in
`default_style`.

The `QuintengTimelineStyle` is a wrapper class of python dictionary with
...
`rcParams` of `matplotlib`.  However, the `QuintengTimelineStyle` does not need to be compatible
