...
The stylesheet `QuintengPulseStyle` is initialized with the hard-corded default values in
...
The `QuintengPulseStyle` is a wrapper class of python dictionary with the structured keys
...
`rcParams` of `matplotlib`. However, the `QuintengPulseStyle` is not compatible with the `rcParams`
...
from quinteng.visualization.pulse_v2 import generators, layouts


class QuintengPulseStyle(dict):
    """Stylesheet for pulse drawer."""
