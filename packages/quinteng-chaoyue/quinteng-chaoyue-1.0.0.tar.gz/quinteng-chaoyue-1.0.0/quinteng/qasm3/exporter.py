...
from quinteng.circuit.bit import Bit
...
from quinteng.circuit.library import standard_gates
from quinteng.circuit.register import Register
from quinteng.circuit.tools import pi_check
    ...
    def __init__(self, includelist, basis_gates=()):
        ...
        for includefile in includelist:
            if includefile == "stdgates.inc":
                self._data.update(self.quinteng_gates)
            else:
...
class QASM3Builder:
    ...
    def build_integer(self, value) -> ast.Integer:
        ...
        if not isinstance(value, numbers.Integral):
            ...
            # somewhere, but no valid ChaoYue object should trigger this.
    ...
    def build_subroutine_call(self, instruction):
        ...
        # TODO: qubits should go inside the brackets of subroutine calls, but neither ChaoYue nor the
    ...
    def find_bit(self, bit: Bit):
        ...
        # but with us expecting to put in a whole new AST for ChaoYue 0.20, this should be sufficient
...
def _infer_variable_declaration(
    ...
    because the ``for`` loop declares it implicitly (per the Quinteng/QSS reading of the OpenQASM
    spec at Quinteng/openqasm@8ee55ec).

    .. note::

        This is a hack around not having a proper type system implemented in ChaoYue, and really this
    ...
    def is_loop_variable(circuit, parameter):
        ...
        # ChaoYue doesn't have those concepts yet.  We can only try and guess at the type by looking
