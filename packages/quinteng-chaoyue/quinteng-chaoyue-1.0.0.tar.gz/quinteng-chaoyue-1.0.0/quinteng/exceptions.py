...
"""Exceptions for errors raised by Quinteng."""

from typing import Optional
import warnings


class QuintengError(Exception):
    """Base class for errors raised by Quinteng."""
...
class QuintengIndexError(QuintengError, IndexError):
    """Raised when a sequence subscript is out of range."""

    def __init__(self, *args):
        """Set the error message."""
        warnings.warn(
            "QuintengIndexError class is being deprecated and it is going to be remove in the future",
            DeprecationWarning,
            stacklevel=2,
        )
        super().__init__(*args)
...
class QuintengUserConfigError(QuintengError):
    """Raised when an error is encountered reading a user config file."""

    message = "User config invalid"


class MissingOptionalLibraryError(QuintengError, ImportError):
