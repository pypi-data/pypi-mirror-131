...
from quinteng.quantum_info import SparsePauliOp, Pauli
...
def evolve_pauli(
    ...
    # same for two qubits, use Quinteng's native rotations
...
def _two_qubit_evolution(pauli, time, cx_structure):
    ...
    # go through all cases we have implemented in Quinteng
    if all(labels == "X"):  # RXX
    ...
    else:  # all the others are not native in Quinteng, so use default the decomposition
        definition = _multi_qubit_evolution(pauli, time, cx_structure)
