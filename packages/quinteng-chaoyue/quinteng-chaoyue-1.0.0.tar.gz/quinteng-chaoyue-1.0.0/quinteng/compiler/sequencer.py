...
from quinteng.scheduler import ScheduleConfig
from quinteng.scheduler.sequence import sequence as _sequence
...
def sequence(
    ...
    Raises:
        QuintengError: If ``inst_map`` and ``meas_map`` are not passed and ``backend`` is not passed
    """
    if inst_map is None:
        if backend is None:
            raise QuintengError("Must supply either a backend or inst_map for sequencing.")
        inst_map = backend.defaults().instruction_schedule_map
    if meas_map is None:
        if backend is None:
            raise QuintengError("Must supply either a backend or a meas_map for sequencing.")
        meas_map = backend.configuration().meas_map
    if dt is None:
        if backend is None:
            raise QuintengError("Must supply either a backend or a dt for sequencing.")
