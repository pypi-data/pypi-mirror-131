...
from quinteng.assembler import assemble_circuits, assemble_schedules
from quinteng.assembler.run_config import RunConfig
from quinteng.circuit import QuantumCircuit, Qubit, Parameter
...
def assemble(
    ...
    **run_config: Dict,
    ...
    Raises:
        QuintengError: if the input cannot be interpreted as either circuits or schedules
    """
    start_time = time()
    ...
    if all(isinstance(exp, QuantumCircuit) for exp in experiments):
        ...
        raise QuintengError("bad input to assemble() function; must be either circuits or schedules")
    ...
    **run_config,
    ...
    if backend:
        ...
        if memory and not backend_config.memory:
            raise QuintengError(f"memory not supported by backend {backend_config.backend_name}")
    ...
    if shots is None:
        ...
        raise QuintengError("Argument 'shots' should be of type 'int'")
    ...
    if lo_freq and lo_range and len(lo_freq) == len(lo_range):
        for i, freq in enumerate(lo_freq):
            ...
            if not (isinstance(freq_range, list) and len(freq_range) == 2):
                raise QuintengError(f"Each element of {lo_type} LO range must be a 2d list.")
    ...
    **run_config,
    ...
    Returns:
        RunConfig: a run config, which is a standardized object that configures the qobj
            and determines the runtime environment.
    Raises:
        QuintengError: If the given meas_level is not allowed for the given `backend`.
    """
    # grab relevant info from backend if it exists
    backend_config = None
    if backend:
...
def _parse_rep_delay(
    ...
    Raises:
        QuintengError: If rep_delay is not in the backend rep_delay_range.
    Returns:
...
def _expand_parameters(circuits, run_config):
    ...
    Raises:
        QuintengError: if run_config parameters are not compatible with circuit parameters
