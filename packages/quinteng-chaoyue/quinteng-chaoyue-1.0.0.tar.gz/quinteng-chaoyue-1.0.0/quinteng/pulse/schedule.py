...
class Schedule:
    ...
    Pulse program representation for the original Quinteng Pulse model [1].
    ...
    To parametrize an instruction's duration, the :class:`~quinteng.pulse.ScheduleBlock`
        ...
        Args:
            *schedules: Child Schedules of this parent Schedule. May either be passed as
            ...
            metadata: Arbitrary key value metadata to associate with the schedule. This gets
                ...
                :attr:`~quinteng.pulse.Schedule.metadata` attribute. It will not be directly
                used in the schedule.
        Raises:
            TypeError: if metadata is not a dict.
        """
        from quinteng.pulse.parameter_manager import ParameterManager
    ...
    def initialize_from(cls, other_program: Any, name: Optional[str] = None) -> "Schedule":
        ...
        Args:
            other_program: Quinteng program that provides metadata to new object.
    ...
    def _instructions(self, time: int = 0):
        ...
        Yields:
            Iterable[Tuple[int, Instruction]]: Tuple containing the time each
                :class:`~quinteng.pulse.Instruction`
                starts at and the flattened :class:`~quinteng.pulse.Instruction` s.
        """
    ...
    def filter(
        ...
        *filter_funcs: Callable,
        ...
        """
        from quinteng.pulse.filters import composite_filter, filter_instructions
...
def _require_schedule_conversion(function: Callable) -> Callable:
    ...
    def wrapper(self, *args, **kwargs):
        from quinteng.pulse.transforms import block_to_schedule
...
class ScheduleBlock:
    ...
    - :class:`~quinteng.pulse.transforms.AlignLeft`: Align instructions in the
      `as-soon-as-possible` manner. Instructions are scheduled at the earliest
      possible time on the channel.

    - :class:`~quinteng.pulse.transforms.AlignRight`: Align instructions in the
      `as-late-as-possible` manner. Instructions are scheduled at the latest
      possible time on the channel.

    - :class:`~quinteng.pulse.transforms.AlignSequential`: Align instructions sequentially
      even though they are allocated in different channels.

    - :class:`~quinteng.pulse.transforms.AlignEquispaced`: Align instructions with
      equal interval within a specified duration. Instructions on different channels
      are aligned sequentially.

    - :class:`~quinteng.pulse.transforms.AlignFunc`: Align instructions with
    ...
    Note that ``ScheduleBlock`` can contain :class:`~quinteng.pulse.instructions.Instruction`
    and other ``ScheduleBlock`` to build an experimental program, but ``Schedule`` is not
    supported. This should be added as a :class:`~quinteng.pulse.instructions.Call` instruction.
    ...
    def __init__(
        ...
        Args:
            ...
            metadata: Arbitrary key value metadata to associate with the schedule. This gets
                stored as free-form data in a dict in the
                :attr:`~quinteng.pulse.ScheduleBlock.metadata` attribute. It will not be directly
                used in the schedule.
        ...
        """
        from quinteng.pulse.parameter_manager import ParameterManager
        from quinteng.pulse.transforms import AlignLeft
    ...
    def filter(
        ...
        *filter_funcs: List[Callable],
        ...
        raise PulseError(
            ...
            "time. Apply ``quinteng.pulse.transforms.block_to_schedule`` function to "
    ...
    def __eq__(self, other: "ScheduleBlock") -> bool:
        ...
        from quinteng.pulse.transforms import block_to_dag
