...
from quinteng.pulse.schedule import Schedule, ScheduleBlock, ParameterizedSchedule
...
class InstructionScheduleMap:
    """Mapping from :py:class:`~quinteng.circuit.QuantumCircuit`
    :py:class:`quinteng.circuit.Instruction` names and qubits to
    :py:class:`~quinteng.pulse.Schedule` s. In particular, the mapping is formatted as type::
    ...
    def __init__(self):
        ...
        # This crashes quinteng parallel. Note that parallel framework passes args as
    ...
    def has_custom_gate(self) -> bool:
        ...
        for qubit_inst in self._map.values():
            for generator in qubit_inst.values():
                ...
                publisher = metadata.get("publisher", CalibrationPublisher.QUINTENG)
    ...
    def get(
        ...
        **kwparams: Union[complex, ParameterExpression],
        ...
        """Return the defined :py:class:`~quinteng.pulse.Schedule` or
        :py:class:`~quinteng.pulse.ScheduleBlock` for the given instruction on the given qubits.
    ...
    def add(
        ...
        if hasattr(schedule, "metadata") and "publisher" not in schedule.metadata:
            schedule.metadata["publisher"] = CalibrationPublisher.QUINTENG
