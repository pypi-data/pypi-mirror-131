...
"""
``Instruction`` s are single operations within a :py:class:`~quinteng.pulse.Schedule`, and can be
used the same way as :py:class:`~quinteng.pulse.Schedule` s.
...
class Instruction(ABC):
    ...
    def insert(self, start_time: int, schedule, name: Optional[str] = None):
        """Return a new :class:`~quinteng.pulse.Schedule` with ``schedule`` inserted within
        ``self`` at ``start_time``.
    ...
    def append(self, schedule, name: Optional[str] = None):
        """Return a new :class:`~quinteng.pulse.Schedule` with ``schedule`` inserted at the
