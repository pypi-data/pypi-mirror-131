...
from quinteng.pulse import channels, instructions, library
from quinteng.pulse.configuration import Kernel, Discriminator
from quinteng.pulse.exceptions import QuintengError
from quinteng.pulse.parser import parse_string_expr
from quinteng.pulse.schedule import Schedule
from quinteng.qobj import QobjMeasurementOption
from quinteng.qobj.utils import MeasLevel
from quinteng.circuit import Parameter, ParameterExpression
...
class ParametricPulseShapes(Enum):
    ...
    value is its mapping to the OpenPulse Command in Quinteng.
    """
...
class ConversionMethodBinder:
    ...
    def get_bound_method(self, bound):
        ...
        try:
            ...
            raise QuintengError(f"Bound method for {bound} is not found.") from ex
...
class InstructionToQobjConverter:
    ...
    def convert_bundled_acquires(
        ...
        Returns:
            dict: Dictionary of required parameters.
        Raises:
            QuintengError: If ``instructions`` is empty.
        """
        if not instructions_:
            raise QuintengError('"instructions" may not be empty.')
...
class QobjToInstructionConverter:
    ...
    def get_channel(self, channel: str) -> channels.PulseChannel:
        ...
        Raises:
            QuintengError: Is raised if valid channel is not matched
        """
        ...
        raise QuintengError("Channel %s is not valid" % channel)
    ...
    def disassemble_value(value_expr: Union[float, str]) -> Union[float, ParameterExpression]:
        ...
        input string and generates Quinteng ParameterExpression object.
