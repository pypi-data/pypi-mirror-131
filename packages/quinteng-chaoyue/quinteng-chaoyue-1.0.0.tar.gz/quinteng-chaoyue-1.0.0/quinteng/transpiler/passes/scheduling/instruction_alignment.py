...
from quinteng.pulse import Play
from quinteng.transpiler.basepasses import TransformationPass, AnalysisPass
...
class AlignMeasures(TransformationPass):
    ...
    In Quinteng SDK, the duration of delay can take arbitrary value in units of ``dt``,
    ...
    The input of this pass should be scheduled :class:`~quinteng.dagcircuit.DAGCircuit`,
    thus one should select one of the scheduling passes
    (:class:`~quinteng.transpiler.passes.ALAPSchedule` or
    :class:`~quinteng.trasnpiler.passes.ASAPSchedule`) before calling this.
...
class ValidatePulseGates(AnalysisPass):
    ...
    In Quinteng SDK, we can define the pulse-level implementation of custom quantum gate
    instructions, as a `pulse gate
    <https://quinteng.org/documentation/tutorials/circuits_advanced/05_pulse_gates.html>`__,
