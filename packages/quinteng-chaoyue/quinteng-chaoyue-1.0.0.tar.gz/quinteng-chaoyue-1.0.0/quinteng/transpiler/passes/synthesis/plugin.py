...
"""
====================================================================
Synthesis Plugins (:mod:`quinteng.transpiler.passes.synthesis.plugin`)
====================================================================

.. currentmodule:: quinteng.transpiler.passes.synthesis.plugin

This module defines the plugin interfaces for the synthesis transpiler passes
in Quinteng. These provide a hook point for external python packages to implement
their own synthesis techniques and have them seamlessly exposed as opt-in
options to users when they run :func:`~quinteng.compiler.transpile`.
...
which enable packages external to quinteng to advertise they include a synthesis
plugin.
...
:class:`~quinteng.transpiler.passes.synthesis.plugin.UnitarySynthesisPlugin`.
The plugin class defines the interface and contract for unitary synthesis
plugins. The primary method is
:meth:`~quinteng.transpiler.passes.synthesis.plugin.UnitarySynthesisPlugin.run`
which takes in a single positional argument, a unitary matrix as a numpy array,
and is expected to return a :class:`~quinteng.dagcircuit.DAGCircuit` object
representing the synthesized circuit from that unitary matrix. Then to inform
the Quinteng transpiler about what information is necessary for the pass there
...
something like::
    ...
    from quinteng.transpiler.passes.synthesis import plugin
    from quinteng_plugin_pkg.synthesis import generate_dag_circuit_from_matrix
...
:class:`~quinteng.transpiler.passes.synthesis.plugin.UnitarySynthesisPlugin` as
...
``quinteng.unitary_synthesis`` namespace. For example::

    entry_points = {
        'quinteng.unitary_synthesis': [
            'special = quinteng_plugin_pkg.module.plugin:SpecialUnitarySynthesis',
        ]
    },
...
expose multiple plugins if necessary. The name ``default`` is used by Quinteng
...
synthesis plugin. Then Quinteng will automatically discover the installed
plugins and expose them as valid options for the appropriate
:func:`~quinteng.compiler.transpile` kwargs and pass constructors. If there are
...
:func:`quinteng.transpiler.passes.synthesis.plugin.unitary_synthesis_plugin_names`
function.
...
class UnitarySynthesisPlugin(abc.ABC):
    ...
    def supports_coupling_map(self):
        ...
        :class:`~quinteng.transpiler.CouplingMap` object representing the qubit
    ...
    def supports_natural_direction(self):
        ...
        Refer to the documentation for :class:`~quinteng.transpiler.passes.UnitarySynthesis`
    ...
    def run(self, unitary, **options):
        ...
        Args:
            unitary (numpy.ndarray): The unitary matrix to synthesize to a
                :class:`~quinteng.dagcircuit.DAGCircuit` object
        ...
        Returns:
            ...
            :meth:`quinteng.dagcircuit.DAGCircuit.substitute_node_with_dag`. If you return a tuple
            and ``wires`` is ``None`` this will behave just as if only a
            :class:`~quinteng.dagcircuit.DAGCircuit` was returned. Additionally if this returns
...
class UnitarySynthesisPluginManager:
    ...
    def __init__(self):
        self.ext_plugins = stevedore.ExtensionManager(
            "quinteng.unitary_synthesis", invoke_on_load=True, propagate_map_exceptions=True
        )
...
def unitary_synthesis_plugin_names():
    ...
    Returns:
        ...
        values for the :func:`~quinteng.compiler.transpile` kwarg ``unitary_synthesis_method``.
    """
