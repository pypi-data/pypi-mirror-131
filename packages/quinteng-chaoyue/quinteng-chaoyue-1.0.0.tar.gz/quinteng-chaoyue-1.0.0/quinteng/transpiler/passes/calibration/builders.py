...
from quinteng.circuit.library.standard_gates import RZXGate
...
from quinteng.pulse.instructions.instruction import Instruction as PulseInst
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        for node in dag.gate_nodes():
            ...
            if self.supported(node.op, qubits) and not dag.has_calibration_for(node):
                ...
                publisher = schedule.metadata.get("publisher", CalibrationPublisher.QUINTENG)
...
class RZXCalibrationBuilder(CalibrationBuilder):
    ...
    def __init__(
        ...
        Raises:
            QuintengError: if open pulse is not supported by the backend.
        """
        super().__init__()
        if backend is not None:
            ...
            if instruction_schedule_map is None or qubit_channel_mapping is None:
                raise QuintengError("Calibrations can only be added to Pulse-enabled backends")
    ...
    def rescale_cr_inst(instruction: Play, theta: float, sample_mult: int = 16) -> Play:
        ...
        Returns:
            quinteng.pulse.Play: The play instruction with the stretched compressed
                GaussianSquare pulse.

        Raises:
            QuintengError: if the pulses are not GaussianSquare.
        """
        ...
        if isinstance(pulse_, GaussianSquare):
            ...
            raise QuintengError("RZXCalibrationBuilder only stretches/compresses GaussianSquare.")
    ...
    def get_calibration(self, node_op: CircuitInst, qubits: List) -> Union[Schedule, ScheduleBlock]:
        ...
        Raises:
            QuintengError: if the control and target qubits cannot be identified or the backend
        ...
        rzx_theta.metadata["publisher"] = CalibrationPublisher.QUINTENG
        ...
        if control is None:
            raise QuintengError("Control qubit is None.")
        if target is None:
            raise QuintengError("Target qubit is None.")
...
class RZXCalibrationBuilderNoEcho(RZXCalibrationBuilder):
    ...
    :class:`~quinteng.transpiler.passes.RZXCalibrationBuilder` pass
