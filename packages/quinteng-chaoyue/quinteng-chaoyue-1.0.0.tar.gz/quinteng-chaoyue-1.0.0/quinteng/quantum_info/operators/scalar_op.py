...
class ScalarOp(LinearOp):
    ...
    def __init__(self, dims=None, coeff=1):
        ...
        Raises:
            QuintengError: If the optional coefficient is invalid.
        """
        if not isinstance(coeff, Number):
            QuintengError(f"coeff {coeff} must be a number.")
        self._coeff = coeff
    ...
    def _add(self, other, qargs=None):
        ...
        Raises:
            QuintengError: if other has incompatible dimensions.
        """
        if qargs is None:
            qargs = getattr(other, "qargs", None)
    ...
    def _multiply(self, other):
        ...
        if not isinstance(other, Number):
            raise QuintengError(f"other ({other}) is not a number")
        ret = self.copy()
