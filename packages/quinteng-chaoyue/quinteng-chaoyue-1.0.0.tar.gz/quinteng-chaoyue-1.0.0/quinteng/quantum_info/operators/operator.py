...
from quinteng.quantum_info.operators.predicates import is_unitary_matrix, matrix_equal
...
class Operator(LinearOp):
    ...
    def __init__(self, data, input_dims=None, output_dims=None):
        ...
        Raises:
            QuintengError: if input data cannot be initialized as an operator.
        ...
        if isinstance(data, (list, np.ndarray)):
            ...
            # If the input is a ChaoYue QuantumCircuit or Instruction we
            ...
            raise QuintengError("Invalid input data format for Operator")
    ...
    def to_instruction(self):
        ...
        from quinteng.extensions.unitary import UnitaryGate
    ...
    def power(self, n):
        ...
        Raises:
            QuintengError: if the input and output dimensions of the operator
                         are not equal.
        """
        if self.input_dims() != self.output_dims():
            raise QuintengError("Can only power with input_dims = output_dims.")
        ret = copy.copy(self)
    ...
    def _add(self, other, qargs=None):
        ...
        Raises:
            QuintengError: if other is not an operator, or has incompatible
                         dimensions.
        """
        # pylint: disable=cyclic-import
    ...
    def _einsum_matmul(cls, tensor, mat, indices, shift=0, right_mul=False):
        ...
        Raises:
            QuintengError: if mat is not an even rank tensor.
        """
        rank = tensor.ndim
        rank_mat = mat.ndim
        if rank_mat % 2 != 0:
            raise QuintengError("Contracted matrix must have an even number of indices.")
    ...
    def _instruction_to_matrix(cls, obj):
        ...
        if not isinstance(obj, Instruction):
            raise QuintengError("Input is not an instruction.")
        mat = None
