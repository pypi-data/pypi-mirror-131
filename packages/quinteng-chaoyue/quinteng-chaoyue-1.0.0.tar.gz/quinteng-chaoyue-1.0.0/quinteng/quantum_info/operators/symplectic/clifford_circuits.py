...
def _append_circuit(clifford, circuit, qargs=None):
    ...
    if name in non_clifford:
        raise QuintengError(f"Cannot update Clifford with non-Clifford gate {name}")
    if name in basis_1q:
        if len(qargs) != 1:
    ...
    for instr, qregs, cregs in gate.definition:
        if cregs:
            raise QuintengError(f"Cannot apply Instruction with classical registers: {instr.name}")
