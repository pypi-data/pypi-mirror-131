...
from quinteng.quantum_info.operators.mixins import AdjointMixin, MultiplyMixin
...
class BasePauli(BaseOperator, AdjointMixin, MultiplyMixin):
    ...
    def compose(self, other, qargs=None, front=False, inplace=False):
        ...
        Raises:
            QuintengError: if number of qubits of other does not match qargs.
        """.format(
            cls=type(self).__name__
        )
        # Validation
        if qargs is None and other.num_qubits != self.num_qubits:
            raise QuintengError(f"other {type(self).__name__} must be on the same number of qubits.")
    ...
    def _multiply(self, other):
        ...
        Raises:
            QuintengError: if the phase is not in the set ``[1, -1j, -1, 1j]``.
        """.format(
            cls=type(self).__name__
        )
    ...
    def _phase_from_complex(coeff):
        ...
        if np.isclose(coeff, 1j):
            return 3
        raise QuintengError("Pauli can only be multiplied by 1, -1j, -1, 1j.")
    ...
    def _from_array(z, x, phase=0):
        ...
        if base_z.ndim == 1:
            ...
            raise QuintengError("Invalid Pauli z vector shape.")
        ...
        if base_x.ndim == 1:
            ...
            raise QuintengError("Invalid Pauli x vector shape.")

        if base_z.shape != base_x.shape:
            raise QuintengError("z and x vectors are different size.")
    ...
    def _append_circuit(self, circuit, qargs=None):
        ...
        Raises:
            QuintengError: if input gate cannot be decomposed into Clifford gates.
        """
        ...
        if isinstance(gate, str):
            ...
            if gate not in basis_1q and gate not in basis_2q:
                raise QuintengError(f"Invalid Clifford gate name string {gate}")
            name = gate
        else:
        ...
        if name in non_clifford:
            raise QuintengError(f"Cannot update Pauli with non-Clifford gate {name}")
        if name in basis_1q:
            if len(qargs) != 1:
                raise QuintengError("Invalid qubits for 1-qubit gate.")
            return basis_1q[name](self, qargs[0])
        if name in basis_2q:
            if len(qargs) != 2:
                raise QuintengError("Invalid qubits for 2-qubit gate.")
        ...
        if gate.definition is None:
            raise QuintengError(f"Cannot apply Instruction: {gate.name}")
