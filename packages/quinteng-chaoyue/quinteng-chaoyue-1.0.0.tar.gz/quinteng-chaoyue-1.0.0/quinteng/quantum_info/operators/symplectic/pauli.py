...
from quinteng.circuit.library.standard_gates import IGate, XGate, YGate, ZGate
from quinteng.exceptions import QuintengError
from quinteng.quantum_info.operators.mixins import generate_apidocs
from quinteng.quantum_info.operators.scalar_op import ScalarOp
from quinteng.quantum_info.operators.symplectic.base_pauli import BasePauli
...
class Pauli(BasePauli):
    ...
    **Initialization**
    ...
    A Pauli object can be initialized in several ways:
        ...
        ``Pauli(obj)``
            ...
            :class:`~quinteng.quantum_info.ScalarOp` operator, or a Pauli
            gate or :class:`~quinteng.QuantumCircuit` containing only
            Pauli gates.
    ...
    **Matrix Operator Representation**
    ...
    :class:`~quinteng.quantum_info.Operator` using the :meth:`to_operator` method,
    ...
    **Data Access**
    ...
    def __init__(self, data=None, x=None, *, z=None, label=None):
        ...
        Raises:
            QuintengError: if input array is invalid shape.
        """
        if isinstance(data, BasePauli):
            ...
            raise QuintengError("Invalid input data for Pauli.")

        # Initialize BasePauli
        if base_z.shape[0] != 1:
            raise QuintengError("Input is not a single Pauli")
    ...
    def delete(self, qubits):
        ...
        Raises:
            QuintengError: if ind is out of bounds for the array size or
                         number of qubits.
        """
        if isinstance(qubits, (int, np.integer)):
            qubits = [qubits]
        ...
        if len(qubits) == self.num_qubits:
            raise QuintengError("Cannot delete all qubits of Pauli")
    ...
    def insert(self, qubits, value):
        ...
        Raises:
            QuintengError: if the insertion qubits are invalid.
        """
        if not isinstance(value, Pauli):
            value = Pauli(value)
    ...
    def compose(self, other, qargs=None, front=False, inplace=False):
        ...
        Raises:
            QuintengError: if other cannot be converted to an operator, or has
    ...
    def evolve(self, other, qargs=None):
        ...
        Raises:
            QuintengError: if the Clifford number of qubits and qargs don't match.
        """
        if qargs is None:
            qargs = getattr(other, "qargs", None)

        # pylint: disable=cyclic-import
        from quinteng.quantum_info.operators.symplectic.clifford import Clifford
    ...
    def _from_label(label):
        ...
        Raises:
            QuintengError: if Pauli string is not valid.
        """
        if Pauli._VALID_LABEL_PATTERN.match(label) is None:
            raise QuintengError(f'Pauli string label "{label}" is not valid.')
    ...
    def _from_scalar_op(cls, op):
        ...
        if op.num_qubits is None:
            raise QuintengError(f"{op} is not an N-qubit identity")
    ...
    def _from_pauli_instruction(cls, instr):
        ...
        raise QuintengError("Invalid Pauli instruction.")
    ...
    def _from_circuit(cls, instr):
        ...
        if isinstance(instr, Instruction):
            ...
            if instr.definition is None:
                raise QuintengError(f"Cannot apply Instruction: {instr.name}")
            # Convert to circuit
    ...
    def from_label(label):
        ...
        Raises:
            QuintengError: If the input list is empty or contains invalid
            Pauli strings.
        """
    ...
    @deprecate_function(
        ...
        "3 months after the release date of Quinteng ChaoYue 0.17.0. "
    ...
    def update_z(self, z, indices=None):
        ...
        Raises:
            QuintengError: when updating whole z, the number of qubits must be the same.
        """
        phase = self.phase
    ...
    def update_x(self, x, indices=None):
        ...
        Raises:
            QuintengError: when updating whole x, the number of qubits must be the same.
        """
        phase = self.phase
    ...
    def insert_paulis(self, indices=None, paulis=None, pauli_labels=None):
        ...
        Raises:
            QuintengError: provide both `paulis` and `pauli_labels` at the same time
        """
        if pauli_labels is not None:
            if paulis is not None:
                raise QuintengError("Please only provide either `paulis` or `pauli_labels`")
    ...
    @deprecate_function(
        ...
        "3 months after the release date. "
        "Use `quinteng.quantum_info.random_pauli` instead"
    )
    def random(cls, num_qubits, seed=None):
        """DEPRECATED: Return a random Pauli on number of qubits.

        This function is deprecated use
        :func:`~quinteng.quantum_info.random_pauli` instead.
        ...
        """
        # pylint: disable=cyclic-import
        from quinteng.quantum_info.operators.symplectic.random import (
            random_pauli,
        )
...
def _phase_from_label(label):
    ...
    if label not in phases:
        raise QuintengError(f"Invalid Pauli phase label '{label}'")
    return phases[label]
