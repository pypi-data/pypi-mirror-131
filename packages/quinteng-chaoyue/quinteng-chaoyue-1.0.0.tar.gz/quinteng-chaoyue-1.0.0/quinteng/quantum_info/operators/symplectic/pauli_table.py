    ...
    **Iteration**
    ...
    def __init__(self, data):
        ...
        if isinstance(data, (np.ndarray, list)):
            ...
            if data.num_qubits is None:
                raise QuintengError(f"{data} is not an N-qubit identity")
            self._array = np.zeros((1, 2 * data.num_qubits), dtype=bool)
        else:
            raise QuintengError("Invalid input data for PauliTable.")
        ...
        if self._array.ndim != 2 or self._array.shape[1] % 2 != 0:
            raise QuintengError("Invalid shape for PauliTable.")
    ...
    def sort(self, weight=False):
        ...
        **Example**
        ...
        .. jupyter-execute::
            ...
            from quinteng.quantum_info.operators import PauliTable
    ...
    def tensor(self, other):
        ...
        **Example**
        ...
        Raises:
            QuintengError: if other cannot be converted to a PauliTable.
        """
    ...
    def compose(self, other, qargs=None, front=True):
        ...
        **Example**
        ...
        if qargs is None and other.num_qubits != self.num_qubits:
            raise QuintengError("other PauliTable must be on the same number of qubits.")
        if qargs and other.num_qubits != len(qargs):
            raise QuintengError("Number of qubits in the other PauliTable does not match qargs.")
    ...
    def commutes(self, pauli):
        ...
        Raises:
            QuintengError: if input is not a single Pauli row.
        """
