...
from quinteng.quantum_info.operators.symplectic.pauli import BasePauli
from quinteng.quantum_info.operators.symplectic.pauli_list import PauliList
from quinteng.quantum_info.operators.symplectic.pauli_table import PauliTable
from quinteng.quantum_info.operators.symplectic.pauli_utils import pauli_basis
...
class SparsePauliOp(LinearOp):
    ...
    :class:`~quinteng.quantum_info.Operator` in terms of N-qubit
    :class:`~quinteng.quantum_info.PauliList` and complex coefficients.
    ...
    :class:`~quinteng.quantum_info.PauliList` object and can be accessed
    ...
    def __init__(self, data, coeffs=None, *, ignore_pauli_phase=False, copy=True):
        ...
        Raises:
            QuintengError: If the input data or coeffs are invalid.
        """
        if ignore_pauli_phase and not isinstance(data, PauliList):
            raise QuintengError("ignore_pauli_list=True is only valid with PauliList data")
    ...
    @property
    @deprecate_function(
        "The SparsePauliOp.table method is deprecated as of Quinteng ChaoYue 0.19.0 "
    ...
    def sum(ops):
        ...
        Raises:
            QuintengError: if the input list is empty.
            QuintengError: if the input list includes an object that is not SparsePauliOp.
            QuintengError: if the numbers of qubits of the objects in the input list do not match.
        """
        if len(ops) == 0:
            raise QuintengError("Input list is empty")
        if not all(isinstance(op, SparsePauliOp) for op in ops):
            raise QuintengError("Input list includes an object that is not SparsePauliOp")
        for other in ops[1:]:
    ...
    def from_operator(obj, atol=None, rtol=None):
        ...
        Raises:
            QuintengError: if the input operator is not an N-qubit operator.
        """
        # Get default atol and rtol
        if atol is None:
        ...
        if num_qubits is None:
            raise QuintengError("Input Operator is not an N-qubit operator.")
        data = obj.data
