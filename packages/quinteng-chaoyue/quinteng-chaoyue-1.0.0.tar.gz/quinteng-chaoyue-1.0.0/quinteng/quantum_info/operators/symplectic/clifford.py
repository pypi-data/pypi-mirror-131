...
from quinteng.quantum_info.synthesis.clifford_decompose import decompose_clifford
from quinteng.quantum_info.operators.mixins import generate_apidocs, AdjointMixin
...
class Clifford(BaseOperator, AdjointMixin):
    ...
    **Representation**
    ...
    :class:`~quinteng.quantum_info.StabilizerTable` using the convention
    from reference [1].

    * Rows 0 to *N-1* are the *destabilizer* group generators
    * Rows *N* to *2N-1* are the *stabilizer* group generators.

    The internal :class:`~quinteng.quantum_info.StabilizerTable` for the Clifford
    ...
    .. jupyter-execute::
        ...
        from quinteng.quantum_info import Clifford
    ...
    **Circuit Conversion**
    ...
    following Clifford gates: :class:`~quinteng.circuit.library.IGate`,
    :class:`~quinteng.circuit.library.XGate`, :class:`~quinteng.circuit.library.YGate`,
    :class:`~quinteng.circuit.library.ZGate`, :class:`~quinteng.circuit.library.HGate`,
    :class:`~quinteng.circuit.library.SGate`, :class:`~quinteng.circuit.library.SdgGate`,
    :class:`~quinteng.circuit.library.CXGate`, :class:`~quinteng.circuit.library.CZGate`,
    :class:`~quinteng.circuit.library.SwapGate`.
    They can be converted back into a :class:`~quinteng.circuit.QuantumCircuit`,
    or :class:`~quinteng.circuit.Gate` object using the :meth:`~Clifford.to_circuit`
    ...
    .. note::
        ...
        :class:`~quinteng.circuit.library.SGate` gate and *either* the
        :class:`~quinteng.circuit.library.CXGate` or
        :class:`~quinteng.circuit.library.CZGate` two-qubit gate.

    Clifford operators can also be converted to
    :class:`~quinteng.quantum_info.Operator` objects using the
    ...
    def __init__(self, data, validate=True):
        ...
        # Initialize from ScalarOp as N-qubit identity discarding any global phase
            ...
            if not data.num_qubits or not data.is_unitary():
                raise QuintengError("Can only initialize from N-qubit identity ScalarOp.")
    ...
    def from_circuit(circuit):
        ...
        Args:
            circuit (QuantumCircuit or ~quinteng.circuit.Instruction):
        ...
        Raises:
            QuintengError: if the input instruction is non-Clifford or contains
                         classical register instruction.
        """
        if not isinstance(circuit, (QuantumCircuit, Instruction)):
            raise QuintengError("Input must be a QuantumCircuit or Instruction")
    ...
    def from_label(label):
        ...
        Raises:
            QuintengError: if the label contains invalid characters.
