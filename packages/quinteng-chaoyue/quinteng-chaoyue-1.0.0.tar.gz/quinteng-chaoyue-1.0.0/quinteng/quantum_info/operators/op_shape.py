...
class OpShape:
    ...
    def _validate(self, shape, raise_exception=False):
        ...
        if ndim > 2:
            if raise_exception:
                raise QuintengError(f"Input shape is not 1 or 2-dimensional (shape = {shape})")
            return False
        ...
        if self._dims_l:
            ...
            if raise_exception:
                raise QuintengError("Number of left qubits does not match matrix shape")
            return False
        ...
        if ndim == 2:
            if self._dims_r:
                ...
                if raise_exception:
                    raise QuintengError("Number of right qubits does not match matrix shape")
                return False
        elif self._dims_r or self._num_qargs_r:
            if raise_exception:
                raise QuintengError("Input dimension should be empty for vector shape.")
            return False
    ...
    def auto(
        ...
        if dims and (dims_l or dims_r):
            raise QuintengError("`dims` kwarg cannot be used with `dims_l` or `dims_r`")
    ...
    def subset(self, qargs=None, qargs_l=None, qargs_r=None):
        ...
        if qargs:
            ...
            if qargs_l or qargs_r:
                raise QuintengError("qargs cannot be specified with qargs_l or qargs_r")
            if self._num_qargs_l:
                qargs_l = qargs
        ...
        if qargs_l and max(qargs_l) >= self._num_qargs_l:
            raise QuintengError("Max qargs_l is larger than number of left qargs")

        if qargs_r and max(qargs_r) >= self._num_qargs_r:
            raise QuintengError("Max qargs_r is larger than number of right qargs")
