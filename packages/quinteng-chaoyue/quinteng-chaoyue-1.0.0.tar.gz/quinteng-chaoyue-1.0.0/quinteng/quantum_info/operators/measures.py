...
from quinteng.quantum_info.operators.channel import Choi, SuperOp
from quinteng.quantum_info.states.densitymatrix import DensityMatrix
from quinteng.quantum_info.states.measures import state_fidelity
...
def process_fidelity(channel, target=None, require_cp=True, require_tp=True):
    ...
    where :math:`F` is the :func:`~quinteng.quantum_info.state_fidelity`,
    :math:`\rho_{\mathcal{E}} = \Lambda_{\mathcal{E}} / d` is the
    normalized :class:`~quinteng.quantum_info.Choi` matrix for the channel
    ...
    :class:`~quinteng.quantum_info.SuperOp` matrices for the *input* quantum
    ...
    Raises:
        QuintengError: if the channel and target do not have the same dimensions.
    """
    # Format inputs
...
def average_gate_fidelity(channel, target=None, require_cp=True, require_tp=False):
    ...
    :meth:`~quinteng.quantum_info.process_fidelity` of the input quantum
    ...
    if target is not None:
        try:
            ...
            raise QuintengError(
                ...
                "`quinteng.quantum_info.process_fidelity` function instead."
            ) from ex
    dim, _ = channel.dim
...
def gate_error(channel, target=None, require_cp=True, require_tp=False):
    ...
    :meth:`~quinteng.quantum_info.average_gate_fidelity` of the input
...
def diamond_norm(choi, **kwargs):
    ...
    Raises:
        QuintengError: if CVXPY package cannot be found.
