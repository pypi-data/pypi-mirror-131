...
def _append_circuit(elem, circuit, qargs=None):
    ...
    Raises:
        QuintengError: if input gates cannot be decomposed into CNOTDihedral gates.
    """
    ...
    if gate.name == "cx":
        if len(qargs) != 2:
            raise QuintengError("Invalid qubits for 2-qubit gate cx.")
        elem._append_cx(qargs[0], qargs[1])
        return elem

    elif gate.name == "cz":
        if len(qargs) != 2:
            raise QuintengError("Invalid qubits for 2-qubit gate cz.")
    ...
    if gate.name == "id":
        if len(qargs) != 1:
            raise QuintengError("Invalid qubits for 1-qubit gate id.")
        return elem
    ...
    for instr, qregs, _ in gate.definition:
        ...
        if instr.name == "x" or gate.name == "x":
            ...
            if len(new_qubits) != 2:
                raise QuintengError("Invalid qubits for 2-qubit gate swap.")
            elem._append_cx(new_qubits[0], new_qubits[1])
            elem._append_cx(new_qubits[1], new_qubits[0])
            elem._append_cx(new_qubits[0], new_qubits[1])
            ...
            raise QuintengError(f"Not a CNOT-Dihedral gate: {instr.name}")
