...
class SpecialPolynomial:
    ...
    def __init__(self, n_vars):
        ...
        if n_vars < 1:
            raise QuintengError("n_vars for SpecialPolynomial is too small.")
        self.n_vars = n_vars
    ...
    def mul_monomial(self, indices):
        ...
        length = len(indices)
        if length >= 4:
            raise QuintengError("There is no term with on more than 3 indices.")
        indices_arr = np.array(indices)
        if (indices_arr < 0).any() and (indices_arr > self.n_vars).any():
            raise QuintengError("Indices are out of bounds.")
        if length > 1 and (np.diff(indices_arr) <= 0).any():
            raise QuintengError("Indices are non-increasing!")
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, int):
            ...
            if self.n_vars != other.n_vars:
                raise QuintengError("Multiplication on different n_vars.")
            terms0 = [[]]
            terms1 = list(combinations(range(self.n_vars), r=1))
            terms2 = list(combinations(range(self.n_vars), r=2))
            terms3 = list(combinations(range(self.n_vars), r=3))
    ...
    def __add__(self, other):
        """Add two polynomials."""
        if not isinstance(other, SpecialPolynomial):
            raise QuintengError("Element to add is not a SpecialPolynomial.")
        if self.n_vars != other.n_vars:
            raise QuintengError("Addition on different n_vars.")
        result = SpecialPolynomial(self.n_vars)
    ...
    def evaluate(self, xval):
        ...
        if len(xval) != self.n_vars:
            raise QuintengError("Evaluate on wrong number of variables.")
        ...
        if False in check_int and False in check_poly:
            raise QuintengError("Evaluate on a wrong type.")
        is_int = False not in check_int
        if not is_int:
            if False in [i.n_vars == self.n_vars for i in xval]:
                raise QuintengError("Evaluate on incompatible polynomials.")
        else:
            xval = xval % 2
