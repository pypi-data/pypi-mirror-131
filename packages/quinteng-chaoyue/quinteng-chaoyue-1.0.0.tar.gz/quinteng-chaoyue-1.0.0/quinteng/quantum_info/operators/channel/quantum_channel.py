...
from quinteng.quantum_info.operators.channel.transformations import _transform_rep
from quinteng.quantum_info.operators.channel.transformations import _to_choi
from quinteng.quantum_info.operators.channel.transformations import _to_kraus
from quinteng.quantum_info.operators.channel.transformations import _to_operator
...
class QuantumChannel(LinearOp):
    ...
    def __init__(self, data, num_qubits=None, op_shape=None):
        ...
        Raises:
            QuintengError: if arguments are invalid.
        """
        self._data = data
    ...
    def conjugate(self):
        ...
        .. note::
            ...
            :class:`~quinteng.quantum_info.SuperOp` representation
    ...
    def power(self, n):
        ...
        Raises:
            QuintengError: if the input and output dimensions of the
                         CLASS are not equal.
        ...
        if self._input_dim != self._output_dim:
            raise QuintengError("Can only take power with input_dim = output_dim.")
    ...
    def to_instruction(self):
        ...
        Returns:
            quinteng.circuit.Instruction: A kraus instruction for the channel.

        Raises:
            QuintengError: if input data is not an N-qubit CPTP quantum channel.
        """
        ...
        if not self.is_cptp():
            raise QuintengError("Cannot convert QuantumChannel to Instruction: channel is not CPTP.")
    ...
    def _format_state(self, state, density_matrix=False):
        ...
        ndim = state.ndim
        if ndim > 2:
            raise QuintengError("Input state is not a vector or matrix.")
    ...
    def _evolve(self, state, qargs=None):
        ...
        Raises:
            QuintengError: if the quantum channel dimension does not match the
