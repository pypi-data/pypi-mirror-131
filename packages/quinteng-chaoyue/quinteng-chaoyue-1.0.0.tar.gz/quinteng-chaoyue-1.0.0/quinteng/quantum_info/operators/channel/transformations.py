...
from quinteng.quantum_info.operators.predicates import ATOL_DEFAULT
...
def _transform_rep(input_rep, output_rep, data, input_dim, output_dim):
    ...
    raise QuintengError(f"Invalid QuantumChannel {output_rep}")
...
def _to_choi(rep, data, input_dim, output_dim):
    ...
    raise QuintengError(f"Invalid QuantumChannel {rep}")
...
def _kraus_to_operator(data):
    ...
    if data[1] is not None or len(data[0]) > 1:
        raise QuintengError("Channel cannot be converted to Operator representation")
    return data[0][0]
...
def _bipartite_tensor(mat1, mat2, shape1=None, shape2=None):
    ...
    Raises:
        QuintengError: if input matrices are wrong shape.
    """
...
def _check_nqubit_dim(input_dim, output_dim):
    ...
    if 2 ** num_qubits != input_dim:
        raise QuintengError("Not an n-qubit channel: input_dim != 2 ** n")
