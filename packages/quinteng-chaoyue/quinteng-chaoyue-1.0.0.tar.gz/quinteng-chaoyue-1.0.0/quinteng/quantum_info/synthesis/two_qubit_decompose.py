...
from quinteng.circuit.quantumcircuit import QuantumCircuit, Gate
from quinteng.circuit.library.standard_gates import CXGate, RXGate, RYGate, RZGate
from quinteng.exceptions import QuintengError
from quinteng.quantum_info.operators import Operator
from quinteng.quantum_info.synthesis.weyl import weyl_coordinates, transform_to_magic_basis
from quinteng.quantum_info.synthesis.one_qubit_decompose import (
...
def decompose_two_qubit_product_gate(special_unitary_matrix):
    ...
    Throws QuintengError if this isn't possible.
    """
    ...
    if abs(detR) < 0.1:
        ...
        detR = R[0, 0] * R[1, 1] - R[0, 1] * R[1, 0]
    if abs(detR) < 0.1:
        raise QuintengError("decompose_two_qubit_product_gate: unable to decompose: detR < 0.1")
    R /= np.sqrt(detR)
    ...
    if abs(detL) < 0.9:
        raise QuintengError("decompose_two_qubit_product_gate: unable to decompose: detL < 0.9")
    L /= np.sqrt(detL)
...
class TwoQubitWeylDecomposition:
    ...
    Passing non-None fidelity to specializations is treated as an assertion, raising QuintengError if
    ...
    def __new__(cls, unitary_matrix, *, fidelity=(1.0 - 1.0e-9)):
        ...
        for _ in range(100):  # FIXME: this randomized algorithm is horrendous
            ...
            raise QuintengError(
                ...
                " Please report this at https://github.com/Quinteng/quinteng-chaoyue/issues/4159."
                f" Input: {U.tolist()}"
            )
...
class TwoQubitControlledUDecomposer:
    ...
    def __init__(self, rxx_equivalent_gate: Type[Gate]):
        ...
        Raises:
            QuintengError: If the gate is not locally equivalent to an RXXGate.
        """
        atol = DEFAULT_ATOL
        ...
        for test_angle in test_angles:
            ...
            try:
                ...
                raise QuintengError("Equivalent gate needs to take exactly 1 angle parameter.") from _
    ...
    def _to_rxx_gate(self, angle: float):
        ...
        Raises:
            QuintengError: If the circuit is not equivalent to an RXXGate.
        """
...
class TwoQubitBasisDecomposer:
    ...
    def _pulse_optimal_chooser(self, best_nbasis, decomposition, target_decomposed):
        ...
        Raises:
            QuintengError: Decomposition for selected basis not implemented.
        """
        circuit = None
        ...
        if self._decomposer1q.basis in {"ZSX", "ZSXX"}:
            if isinstance(self.gate, CXGate):
                ...
                raise QuintengError("pulse_optimizer currently only works with CNOT entangling gate")
        else:
            raise QuintengError(
    ...
    def _get_sx_vz_3cx_efficient_euler(self, decomposition, target_decomposed):
        ...
        if x12_isHalfPi:
            ...
            if self.pulse_optimize is None:
                ...
                raise QuintengError("possible non-pulse-optimal decomposition encountered")
