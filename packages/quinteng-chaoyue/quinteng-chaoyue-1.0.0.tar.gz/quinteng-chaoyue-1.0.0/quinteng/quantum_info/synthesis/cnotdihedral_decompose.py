...
def decompose_cnotdihedral_2_qubits(elem):
    ...
    Raises:
        QuintengError: if the element in not 1 or 2-qubit CNOTDihedral.
    ...
    if elem.num_qubits == 1:
        if elem.poly.weight_0 != 0 or elem.linear != [[1]]:
            raise QuintengError("1-qubit element in not CNOT-Dihedral .")
    ...
    if elem.poly.weight_0 != 0:
        raise QuintengError("2-qubit element in not CNOT-Dihedral .")
...
def decompose_cnotdihedral_general(elem):
    ...
    Raises:
        QuintengError: if the element could not be decomposed into a circuit.
    ...
    if not np.allclose((np.linalg.det(elem_cpy.linear) % 2), 1):
        raise QuintengError("Linear part is not invertible.")
    ...
    if (
        ...
        raise QuintengError("Cannot do Gauss elimination on linear part.")
    ...
    if elem.poly != new_elem.poly:
        raise QuintengError("Could not recover phase polynomial.")
