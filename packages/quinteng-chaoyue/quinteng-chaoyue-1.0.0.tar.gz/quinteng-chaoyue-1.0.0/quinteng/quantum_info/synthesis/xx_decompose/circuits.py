...
from quinteng.circuit.library.standard_gates import RXXGate, RYYGate, RZGate
...
def xx_circuit_step(source, strength, target, embodiment):
    ...
    pi/4); and `embodiment` is a Quinteng circuit which enacts the canonical gate of the prescribed
...
def canonical_xx_circuit(target, strength_sequence, basis_embodiments):
    """
    Assembles a Quinteng circuit from a specified `strength_sequence` of XX-type interactions which
