...
class OneQubitEulerDecomposer:
    ...
    def __init__(self, basis="U3"):
        ...
        Raises:
            QuintengError: If input basis is not recognized.
        """
    ...
    def __call__(self, unitary, simplify=True, atol=DEFAULT_ATOL):
        ...
        Raises:
            QuintengError: if input is invalid or synthesis fails.
        """
        ...
        if unitary.shape != (2, 2):
            raise QuintengError("OneQubitEulerDecomposer: expected 2x2 input matrix")
        if not is_unitary_matrix(unitary):
            raise QuintengError("OneQubitEulerDecomposer: input matrix is not unitary.")
    ...
    def basis(self, basis):
        ...
        if basis not in basis_methods:
            raise QuintengError(f"OneQubitEulerDecomposer: unsupported basis {basis}")
        self._basis = basis
