...
def _reduce_cost(clifford, inv_circuit, cost):
    ...
    raise QuintengError("Failed to reduce Clifford CX cost.")
...
def decompose_clifford_greedy(clifford):
    ...
    Raises:
        QuintengError: if symplectic Gaussian elimination fails.
    """
...
def _compute_greedy_cost(list_pairs):
    ...
    if (A_num % 2) == 0:
        raise QuintengError("Symplectic Gaussian elimination fails.")
