...
def state_fidelity(state1, state2, validate=True):
    ...
    Raises:
        QuintengError: if ``validate=True`` and the inputs are invalid quantum states.
    """
...
def purity(state, validate=True):
    ...
    Raises:
        QuintengError: if the input isn't a valid quantum state.
    """
...
def entropy(state, base=2):
    ...
    Raises:
        QuintengError: if the input state is not a valid QuantumState.
    """
...
def mutual_information(state, base=2):
    ...
    Raises:
        QuintengError: if the input state is not a valid QuantumState.
        QuintengError: if input is not a bipartite QuantumState.
    """
    state = _format_state(state, validate=True)
    if len(state.dims()) != 2:
        raise QuintengError("Input must be a bipartite quantum state.")
...
def concurrence(state):
    ...
    :class:`~quinteng.quantum_info.Statevector` :math:`|\psi\rangle` is
    given by
    ...
    :func:`~quinteng.quantum_info.partial_trace` of the input state.
    ...
    Raises:
        QuintengError: if the input state is not a valid QuantumState.
        QuintengError: if input is not a bipartite QuantumState.
        QuintengError: if density matrix input is not a 2-qubit state.
    """
    ...
    if isinstance(state, Statevector):
        ...
        dims = state.dims()
        if len(dims) != 2:
            raise QuintengError("Input is not a bipartite quantum state.")
    ...
    if state.dim != 4:
        raise QuintengError("Input density matrix must be a 2-qubit state.")
