...
def partial_trace(state, qargs):
    ...
    :meth:`~quinteng.quantum_info.DensityMatrix.trace` of the
    input state.
    ...
    Raises:
        QuintengError: if input state is invalid.
    """
...
def _format_state(state, validate=True):
    ...
    if not isinstance(state, (Statevector, DensityMatrix)):
        raise QuintengError("Input is not a quantum state")
    if validate and not state.is_valid():
        raise QuintengError("Input quantum state is not a valid")
    return state
