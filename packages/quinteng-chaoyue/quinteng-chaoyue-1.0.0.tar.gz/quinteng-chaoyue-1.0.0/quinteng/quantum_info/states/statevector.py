...
from quinteng.quantum_info.operators.mixins.tolerances import TolerancesMixin
from quinteng.quantum_info.operators.operator import Operator
from quinteng.quantum_info.operators.symplectic import Pauli, SparsePauliOp
...
class Statevector(QuantumState, TolerancesMixin):
    ...
    def __init__(self, data, dims=None):
        ...
        Raises:
            QuintengError: if input data is not valid.
        ...
        if isinstance(data, (list, np.ndarray)):
            ...
            if input_dim != 1:
                raise QuintengError("Input Operator is not a column-vector.")
            self._data = np.ravel(data.data)
        elif isinstance(data, (QuantumCircuit, Instruction)):
            self._data = Statevector.from_instruction(data).data
        else:
            raise QuintengError("Invalid input data format for Statevector")
        ...
        if ndim != 1:
            if ndim == 2 and shape[1] == 1:
                ...
                raise QuintengError("Invalid input: not a vector or column-vector.")
        ...
        **paulivec**: Matplotlib figure, rendering of statevector using `plot_state_paulivec()`.
        ...
        Args:
            output (str): Select the output method to use for drawing the
                ...
                ``~/.quinteng/settings.conf`` under ``[default]``.
            drawer_args: Arguments to be passed directly to the relevant drawing
                ...
                See the relevant function under `quinteng.visualization` for that function's
                documentation.
        ...
        from quinteng.visualization.state_visualization import state_drawer
    ...
    def __getitem__(self, key):
        ...
        Raises:
            QuintengError: if key is not valid.
        """
        if isinstance(key, str):
            try:
                key = int(key, 2)
            except ValueError:
                raise QuintengError(f"Key '{key}' is not a valid binary string.") from None
        if isinstance(key, int):
            if key >= self.dim:
                raise QuintengError(f"Key {key} is greater than Statevector dimension {self.dim}.")
            if key < 0:
                raise QuintengError(f"Key {key} is not a valid positive value.")
            return self._data[key]
        else:
            raise QuintengError("Key must be int or a valid binary string.")
    ...
    def tensor(self, other):
        ...
        Raises:
            QuintengError: if other is not a quantum state.
        """
    ...
    def inner(self, other):
        ...
        Raises:
            QuintengError: if other is not a quantum state or has different dimension.
        """
        if not isinstance(other, Statevector):
            other = Statevector(other)
    ...
    def _add(self, other):
        ...
        Raises:
            QuintengError: if other is not a quantum state, or has
    ...
    def _multiply(self, other):
        ...
        Raises:
            QuintengError: if other is not a valid complex number.
        """
        if not isinstance(other, Number):
            raise QuintengError("other is not a number")
        ret = copy.copy(self)
    ...
    def evolve(self, other, qargs=None):
        ...
        Raises:
            QuintengError: if the operator dimension does not match the
        ...
        if isinstance(other, Instruction):
            if self.num_qubits is None:
                raise QuintengError("Cannot apply QuantumCircuit to non-qubit Statevector.")
    ...
    def from_label(cls, label):
        ...
        Raises:
            QuintengError: if the label contains invalid characters, or the
                         length of the label is larger than an explicitly
                         specified num_qubits.
        """
        # Check label is valid
        if re.match(r"^[01rl\-+]+$", label) is None:
            raise QuintengError("Label contains invalid characters.")
    ...
    def from_instruction(cls, instruction):
        ...
        Args:
            instruction (quinteng.circuit.Instruction or QuantumCircuit): instruction or circuit

        Returns:
            Statevector: The final statevector.

        Raises:
            QuintengError: if the instruction contains invalid instructions for
    ...
    def _evolve_instruction(statevec, obj, qargs=None):
        ...
        if obj.definition is None:
            raise QuintengError(f"Cannot apply Instruction: {obj.name}")
