...
from quinteng.quantum_info.operators.predicates import is_positive_semidefinite_matrix
from quinteng.quantum_info.operators.channel.quantum_channel import QuantumChannel
from quinteng.quantum_info.operators.channel.superop import SuperOp
...
class DensityMatrix(QuantumState, TolerancesMixin):
    ...
    def __init__(self, data, dims=None):
        ...
        if isinstance(data, (list, np.ndarray)):
            ...
            raise QuintengError("Invalid input data format for DensityMatrix")
        ...
        if ndim == 2 and shape[0] == shape[1]:
            ...
            raise QuintengError("Invalid DensityMatrix input: not a square matrix.")
    ...
    def reset(self, qargs=None):
        ...
        Additional Information:
            ...
            :class:`~quinteng.quantum_info.SuperOp` of the reset subsystems.
        """
        if qargs is None:
    ...
    def from_label(cls, label):
        ...
        Raises:
            QuintengError: if the label contains invalid characters, or the length
    ...
    def _append_instruction(self, other, qargs=None):
        ...
        if other.definition is None:
            raise QuintengError(f"Cannot apply Instruction: {other.name}")
    ...
    def to_statevector(self, atol=None, rtol=None):
        ...
        Raises:
            QuintengError: if the state is not pure.
        """
        from quinteng.quantum_info.states.statevector import Statevector
        ...
        if not is_hermitian_matrix(self._data, atol=atol, rtol=rtol):
            raise QuintengError("Not a valid density matrix (non-hermitian).")
        ...
        if len(nonzero_evals) != 1 or not np.isclose(nonzero_evals[0], 1, atol=atol, rtol=rtol):
            raise QuintengError("Density matrix is not a pure state")
