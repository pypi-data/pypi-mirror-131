...
from quinteng.quantum_info.operators.symplectic import Clifford, Pauli
from quinteng.quantum_info.operators.symplectic.clifford_circuits import _append_x


class StabilizerState(QuantumState):
    """StabilizerState class.
    Stabilizer simulator using the convention from reference [1].
    Based on the internal class :class:`~quinteng.quantum_info.Clifford`.

    .. jupyter-execute::

        from quinteng import QuantumCircuit
        from quinteng.quantum_info import StabilizerState, Pauli
    ...
    def trace(self):
        ...
        Raises:
            QuintengError: if input is not a StabilizerState.
        """
        if not self.is_valid():
            raise QuintengError("StabilizerState is not a valid quantum state.")
        return 1.0
    ...
    def tensor(self, other):
        ...
        Raises:
            QuintengError: if other is not a StabilizerState.
        """
    ...
    def evolve(self, other, qargs=None):
        ...
        Args:
            other (Clifford or QuantumCircuit or quinteng.circuit.Instruction):
    ...
    def _phase_exponent(x1, z1, x2, z2):
        ...
        if phase == 2:
            raise QuintengError("Invalid rowsum phase exponent in measurement calculation.")
        return phase
    ...
    def _rowsum(accum_pauli, accum_phase, row_pauli, row_phase):
        ...
        newr %= 4
        if (newr != 0) & (newr != 2):
            raise QuintengError("Invalid rowsum in measurement calculation.")
