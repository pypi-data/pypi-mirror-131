...
class UchannelLO:
    ...
    def __init__(self, q, scale):
        ...
        Raises:
            QuintengError: If q is < 0
        """
        if q < 0:
            raise QuintengError("q must be >=0")
        self.q = q
        self.scale = scale
...
class QasmBackendConfiguration:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # fit within the Qasm / Pulse backend partitioning in Quinteng
        if dt is not None:
            self.dt = dt * 1e-9
        if dtm is not None:
            self.dtm = dtm * 1e-9
    ...
    def num_qubits(self):
        ...
        throughout Quinteng. Until this is properly refactored, this property serves as intermediate
        solution.
        """
