...
"""
=============================================
Providers Interface (:mod:`quinteng.providers`)
=============================================

.. currentmodule:: quinteng.providers

This module contains the classes used to build external providers for ChaoYue. A
provider is anything that provides an external service to ChaoYue. The typical
example of this is a Backend provider which provides
:class:`~quinteng.providers.Backend` objects which can be used for executing
:class:`~quinteng.circuit.QuantumCircuit` and/or :class:`~quinteng.pulse.Schedule`
objects. This module contains the abstract classes which are used to define the
interface between a provider and chaoyue.
...
Each minor version release of quinteng-chaoyue **may** increment the version of any
...
ChaoYue will support support multiple versions of each class at once. Given the
nature of one version per release the version deprecation policy is a bit
more conservative than the standard deprecation policy. ChaoYue will support a
...
It's worth pointing out that ChaoYue's version support policy doesn't mean
...
just for ChaoYue's supported versions. Part of this lengthy window prior to
...
Aer could update its :class:`~quinteng.providers.aer.aerbackend.AerBackend` class
to use version 34 they'd need to deprecate the old signature prior to switching
over. The changeover for Aer is not guaranteed to be lockstep with ChaoYue so we
...
Quinteng you will need to write a provider. A provider will provide ChaoYue with a
method to get available :class:`~quinteng.providers.BackendV2` objects. The
:class:`~quinteng.providers.BackendV2` object provides both information describing
a backend and its operation for the :mod:`~quinteng.transpiler` so that circuits
can be compiled to something that is optimized and can execute on the
backend. It also provides the :meth:`~quinteng.providers.BackendV2.run` method which can
run the :class:`~quinteng.circuit.QuantumCircuit` objects and/or
:class:`~quinteng.pulse.Schedule` objects. This enables users and other Quinteng
APIs, such as :func:`~quinteng.execute_function.execute` and higher level algorithms in
:mod:`quinteng.algorithms`, to get results from executing circuits on devices in a standard
fashion regardless of how the backend is implemented. At a high level the basic
steps for writing a provider are:

 * Implement a :class:`~quinteng.providers.ProviderV1` subclass that handles
   access to the backend(s).
 * Implement a :class:`~quinteng.providers.BackendV2` subclass and its
   :meth:`~~quinteng.providers.BackendV2.run` method.

   * Add any custom gates for the backend's basis to the session
     :class:`~quinteng.circuit.EquivalenceLibrary` instance.

 * Implement a :class:`~quinteng.providers.JobV1` subclass that handles
   interacting with a running job.

For a simple example of a provider, see the
`quinteng-aqt-provider <https://github.com/Quinteng-Partners/quinteng-aqt-provider>`__
...
provide the interface between Quinteng and the hardware or simulator that will
...
:attr:`~quinteng.providers.BackendV2.target` property to define the model of the
backend for the compiler, a :attr:`~quinteng.providers.BackendV2.max_circuits`
...
:meth:`~quinteng.providers.BackendV2.run` method to accept job submissions, and
a :obj:`~quinteng.providers.BackendV2._default_options` method to define the
user configurable options and their default values. For example, a minimum working
example would be something like::

    from quinteng.providers import BackendV2 as Backend
    from quinteng.transpiler import Target
    from quinteng.providers import Options
    from quinteng.circuit import Parameter, Measure
    from quinteng.circuit.library import PhaseGate, SXGate, UGate, CXGate, IGate
    ...
    class Mybackend(Backend):
        ...
        def __init__(self):
            ...
            # a quinteng.transpiler.InstructionProperties object to define properties
            # for an instruction.
            lam = Parameter("λ")
...
The key piece of the :class:`~quinteng.providers.Backend` object is how it describes itself to the
compiler. This is handled with the :class:`~quinteng.transpiler.Target` class which defines
a model of a backend for the transpiler. A backend object will need to return
a :class:`~quinteng.transpiler.Target` object from the :attr:`~quinteng.providers.BackendV2.target`
attribute which the :func:`~quinteng.compiler.transpile` function will use as
its model of a backend target for compilation.

Custom Basis Gates
^^^^^^^^^^^^^^^^^^

1. If your backend doesn't use gates in the Quinteng circuit library
   (:mod:`quinteng.circuit.library`) you can integrate support for this into your
   provider. The basic method for doing this is first to define a
   :class:`~quinteng.circuit.Gate` subclass for each custom gate in the basis
   set. For example::
...
2. Add the custom gate to the target for your backend. This can be done with
   ...
   :class:`~quinteng.providers.BackendV2` implementation for your backend::

       from quinteng.transpiler import InstructionProperties
...
3. After you've defined the custom gates to use for the backend's basis set
   ...
   so that the :func:`~quinteng.compiler.transpile` function and
   :mod:`~quinteng.transpiler` module can convert an arbitrary circuit using the
   ...
   :class:`~quinteng.circuit.library.CXGate` (if your basis doesn't include a
   standard 2 qubit gate) and some commonly used single qubit rotation gates like
   the :class:`~quinteng.circuit.library.HGate` and
   :class:`~quinteng.circuit.library.UGate` that should be sufficient for the
   ...
   we could define the :class:`~quinteng.circuit.library.U2Gate` and
   :class:`~quinteng.circuit.library.HGate`::

       from quinteng.circuit.equivalence_library import SessionEquivalenceLibrary
       from quinteng.circuit.library import HGate
       from quinteng.circuit.library import ZGate
       from quinteng.circuit.library import RZGate
       from quinteng.circuit.library import U2Gate


       # H => Z SY
       q = quinteng.QuantumRegister(1, "q")
       def_sy_h = quinteng.QuantumCircuit(q)
       ...
       # u2 => Z SY Z
       phi = quinteng.circuit.Parameter('phi')
       lam = quinteng.circuit.Parameter('lambda')
       q = quinteng.QuantumRegister(1, "q")
       def_sy_u2 = quinteng.QuantumCircuit(q)
   ...
   :class:`~quinteng.transpiler.passes.BasisTranslator` pass is run with the
   ...
   :class:`~quinteng.transpiler.passes.Optimize1qGatesDecomposition`, may not
   be able to operate with your custom basis. For our ``SYGate`` example, the
   :class:`~quinteng.transpiler.passes.Optimize1qGatesDecomposition` will not be
   able to simplify runs of single qubit gates into the SY basis. This is because
   the :class:`~quinteng.quantum_info.OneQubitEulerDecomposer` class does not
   know how to work in the SY basis. To solve this the ``SYGate`` class would
   need to be added to Quinteng and
   :class:`~quinteng.quantum_info.OneQubitEulerDecomposer` updated to support
   ...
   transpiler will ensure that it continues to be well supported by Quinteng
   moving forward.

Run Method
----------

Of key importance is the :meth:`~quinteng.providers.BackendV2.run` method, which
...
:class:`~quinteng.providers.Job` object. Depending on the type of backend this
typically involves serializing the circuit object into the API format used by a
backend. For example, on IBMQ backends from the ``quinteng-ibmq-provider``
...
expected that the backend's :obj:`~quinteng.providers.BackendV2.run` method will
...
backend are defined using an :class:`~quinteng.providers.Options` object. This
object is initially created by the
:obj:`~quinteng.providers.BackendV2._default_options` method of a Backend class.
The default options returns an initialized :class:`~quinteng.providers.Options`
...
:obj:`~quinteng.providers.BackendV2._default_options` method would look like::

    @classmethod
    def _default_options(cls):
        return Options(shots=1024)

You can also set validators on an :class:`~quinteng.providers.Options` object to
...
you can refer to the :meth:`~quinteng.providers.Options.set_validator` documentation
...
The output from the :obj:`~quinteng.providers.BackendV2.run` method is a :class:`~quinteng.providers.JobV1`
...
:obj:`~quinteng.providers.JobV1.result` is the primary user facing method
which will block until the execution is complete and then will return a
:class:`~quinteng.result.Result` object with results of the job.
...
:obj:`~quinteng.providers.BackendV1.run` method on the backend will block until a
:class:`~quinteng.result.Result` object is generated and the sync job will return
with that inner :class:`~quinteng.result.Result` object.

An example job class for an async API based backend would look something like::

    from quinteng.providers import JobV1 as Job
    from quinteng.providers import JobError
    from quinteng.providers import JobTimeoutError
    from quinteng.providers.jobstatus import JobStatus
...
backend to make them work with native Quinteng data structures and have flatter
...
:class:`~quinteng.transpiler.Target` object and the backend's attributes often query
its :attr:`~quinteng.providers.BackendV2.target`
...
returns a :class:`~quinteng.transpiler.CouplingMap` constructed from the
:class:`~quinteng.transpiler.Target` accesible in the
:attr:`~quinteng.providers.BackendV2.target` attribute, however the target may contain
instructions that operate on more than two qubits (which can't be represented in a
:class:`~quinteng.transpiler.CouplingMap`) or has instructions that only operate on
...
:attr:`~quinteng.providers.BackendV2.coupling_map`. So depending on your use case
...
.. list-table:: Migrate from :obj:`~BackendV1` to :obj:`~BackendV2`
   ...
   * - ``backend.configuration().coupling_map``
     ...
     - The return from :obj:`~BackendV2` is a :class:`~quinteng.transpiler.CouplingMap` object.
       ...
       information contained in :class:`~quinteng.transpiler.Target` object.
   * - ``backend.configuration().backend_name``
     - ``backend.name``
     -
   * - ``backend.configuration().backend_version``
     - ``backend.backend_version``
     - The :attr:`~quinteng.providers.BackendV2.version` attribute represents
       the version of the abstract :class:`~quinteng.providers.Backend` interface
       the object implements while :attr:`~quinteng.providers.BackendV2.backend_version`
   ...
   * - ``backend.configuration().basis_gates``
     ...
     - The :obj:`~BackendV2` return is a list of operation names contained in the
       ``backend.target`` attribute. The :class:`~quinteng.transpiler.Target` may contain more
   ...
   * - ``backend.properties().readout_error(0)``
     ...
     - In :obj:`~BackendV2` the error rate for the :class:`~quinteng.circuit.Measure`
       ...
       separately in a :class:`~quinteng.transpiler.Target`.
   * - ``backend.properties().readout_length(0)``
     - ``backend.target["measure"][(0,)].duration``
     - In :obj:`~BackendV2` the duration for the :class:`~quinteng.circuit.Measure`
...
================================================================
Legacy Provider Interface Base Objects (:mod:`quinteng.providers`)
================================================================
...
from quinteng.providers.backend import QubitProperties
from quinteng.providers.options import Options
from quinteng.providers.job import Job
from quinteng.providers.job import JobV1

# Legacy providers interface
from quinteng.providers.basebackend import BaseBackend
from quinteng.providers.baseprovider import BaseProvider
from quinteng.providers.basejob import BaseJob
