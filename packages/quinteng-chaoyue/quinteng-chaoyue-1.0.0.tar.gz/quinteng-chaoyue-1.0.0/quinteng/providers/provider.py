...
class ProviderV1(Provider, ABC):
    ...
    def get_backend(self, name=None, **kwargs):
        ...
        Raises:
            QuintengBackendNotFoundError: if no backend could be found or
                more than one backend matches the filtering criteria.
        """
        backends = self.backends(name, **kwargs)
        if len(backends) > 1:
            raise QuintengBackendNotFoundError("More than one backend matches the criteria")
        if not backends:
            raise QuintengBackendNotFoundError("No backend matches the criteria")
