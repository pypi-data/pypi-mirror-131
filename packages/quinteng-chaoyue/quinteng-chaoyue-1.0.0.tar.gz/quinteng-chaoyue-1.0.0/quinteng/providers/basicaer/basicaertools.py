...
import quinteng.circuit.library.standard_gates as gates
...
def single_gate_matrix(gate: str, params: Optional[List[float]] = None):
    ...
    Raises:
        QuintengError: If a gate outside the supported set is passed in for the
            ``Gate`` argument.
    """
    if params is None:
        params = []
    ...
    if gate == "U":
        ...
        raise QuintengError("Gate is not a valid basis gate for this simulator: %s" % gate)
...
def _einsum_matmul_index_helper(gate_indices, number_of_qubits):
    ...
    Raises:
        QuintengError: if the total number of qubits plus the number of
    ...
    if len(gate_indices) + number_of_qubits > 26:
        raise QuintengError("Total number of free indexes limited to 26")
