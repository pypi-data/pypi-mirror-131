...
class DAGNode:
    ...
    def type(self, dag_type):
        ...
        if dag_type not in ["op", "in", "out"]:
            raise QuintengError("The DAGNode type must be 'op', 'in', or 'out'")
        self._type = dag_type
    ...
    def op(self):
        ...
        if not self.type or self.type != "op":
            raise QuintengError("The node %s is not an op node" % (str(self)))
        return self._op
    ...
    def wire(self):
        ...
        if self.type not in ["in", "out"]:
            raise QuintengError("The node %s is not an input/output node" % str(self))
        return self._wire
