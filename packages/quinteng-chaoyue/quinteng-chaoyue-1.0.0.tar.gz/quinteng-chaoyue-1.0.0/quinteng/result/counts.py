...
class Counts(dict):
    ...
    def __init__(self, data, time_taken=None, creg_sizes=None, memory_slots=None):
        ...
        Raises:
            ...
            QuintengError: If a dit string key is input with creg_sizes and/or
                memory_slots
        """
        bin_data = None
        data = dict(data)
                    ...
                    if not creg_sizes and not memory_slots:
                        ...
                        for bitstring, value in data.items():
                            if not self.bitstring_regex.search(bitstring):
                                raise exceptions.QuintengError(
    ...
    def most_frequent(self):
        ...
        Raises:
            QuintengError: when there is >1 count with the same max counts, or
                an empty object.
        """
        if not self:
            raise exceptions.QuintengError("Can not return a most frequent count on an empty object")
    ...
    def hex_outcomes(self):
        ...
        Returns:
            dict: A dictionary with the keys as hexadecimal strings instead of
                bitstrings
        Raises:
            QuintengError: If the Counts object contains counts for dit strings
        """
        if self.hex_raw:
