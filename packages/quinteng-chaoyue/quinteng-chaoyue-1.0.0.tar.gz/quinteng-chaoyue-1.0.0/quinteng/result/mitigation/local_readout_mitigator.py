...
class LocalReadoutMitigator(BaseReadoutMitigator):
    ...
    The mitigator should either be calibrated using quinteng experiments,
    ...
    def __init__(
        ...
        Raises:
            QuintengError: matrices sizes do not agree with number of qubits
        """
        if amats is None:
    ...
    def quasi_probabilities(
        ...
        Raises:
            QuintengError: if qubit and clbit kwargs are not valid.
        """
        if qubits is None:
            qubits = self._qubits
    ...
    def _from_backend(self, backend, qubits):
        ...
        if qubits is not None:
            if any(qubit >= len(backend_qubits) for qubit in qubits):
                raise QuintengError("The chosen backend does not contain the specified qubits.")
