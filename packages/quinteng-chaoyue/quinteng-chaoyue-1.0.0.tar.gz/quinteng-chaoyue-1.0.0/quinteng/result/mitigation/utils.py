...
def str2diag(string):
    ...
    for i in string:
        if i not in chars:
            raise QuintengError(f"Invalid diagonal string character {i}")
...
def remap_qubits(
    ...
    if qubits is not None:
        if len(qubits) != num_qubits:
            raise QuintengError("Num qubits does not match vector length.")
...
def counts_probability_vector(
    ...
    Raises:
        QuintengError: if qubits and clbits kwargs are not valid.
