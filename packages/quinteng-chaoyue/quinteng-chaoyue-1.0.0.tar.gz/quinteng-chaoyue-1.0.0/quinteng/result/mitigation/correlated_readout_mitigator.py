...
class CorrelatedReadoutMitigator(BaseReadoutMitigator):
    ...
    The mitigation_matrix should be calibrated using quinteng experiments.
    ...
    def __init__(self, amat: np.ndarray, qubits: Optional[Iterable[int]] = None):
        ...
        Raises:
            QuintengError: matrix size does not agree with number of qubits
        """
        if np.any(amat < 0) or not np.allclose(np.sum(amat, axis=0), 1):
            raise QuintengError("Assignment matrix columns must be valid probability distributions")
