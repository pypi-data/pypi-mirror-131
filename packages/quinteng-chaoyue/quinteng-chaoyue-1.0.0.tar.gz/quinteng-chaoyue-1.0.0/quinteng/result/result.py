...
from quinteng.result.counts import Counts
from quinteng.qobj.utils import MeasLevel
from quinteng.qobj import QobjHeader
...
class Result:
    ...
    def data(self, experiment=None):
        ...
        Raises:
            QuintengError: if data for the experiment could not be retrieved.
        """
        try:
            return self._get_experiment(experiment).data.to_dict()
        except (KeyError, TypeError) as ex:
            raise QuintengError(f'No data for experiment "{repr(experiment)}"') from ex
    ...
    def get_memory(self, experiment=None):
        ...
        Raises:
            QuintengError: if there is no memory data for the circuit.
        """
        ...
        try:
            ...
            if meas_level == MeasLevel.CLASSIFIED:
                ...
                raise QuintengError(f"Measurement level {meas_level} is not supported")
    ...
    def get_counts(self, experiment=None):
        ...
        Raises:
            QuintengError: if there are no counts for the experiment.
        """
        ...
        for key in exp_keys:
            ...
            except (AttributeError, QuintengError):  # header is not available
                header = None
            ...
            if "counts" in self.data(key).keys():
                ...
                raise QuintengError(f'No counts for experiment "{repr(key)}"')
    ...
    def get_statevector(self, experiment=None, decimals=None):
        ...
        Raises:
            QuintengError: if there is no statevector for the experiment.
        """
        try:
            ...
            raise QuintengError(f'No statevector for experiment "{repr(experiment)}"') from ex
    ...
    def get_unitary(self, experiment=None, decimals=None):
        ...
        Raises:
            QuintengError: if there is no unitary for the experiment.
        """
        try:
            return postprocess.format_unitary(self.data(experiment)["unitary"], decimals=decimals)
        except KeyError as ex:
            raise QuintengError(f'No unitary for experiment "{repr(experiment)}"') from ex
    ...
    def _get_experiment(self, key=None):
        ...
        Raises:
            QuintengError: if there is no data for the experiment, or an unhandled
        ...
        if isinstance(key, int):
            try:
                ...
                raise QuintengError(f'Result for experiment "{key}" could not be found.') from ex
        else:
            ...
            if len(exp) == 0:
                raise QuintengError('Data for experiment "%s" could not be found.' % key)
            if len(exp) == 1:
                exp = exp[0]
            else:
                warnings.warn(
        ...
        raise QuintengError(result_status, ", ", exp_status)
