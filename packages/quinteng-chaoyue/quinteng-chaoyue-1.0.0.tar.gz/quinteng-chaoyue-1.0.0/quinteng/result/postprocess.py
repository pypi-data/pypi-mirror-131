...
def _list_to_complex_array(complex_list):
    ...
    Raises:
        QuintengError: If inner most array of input nested list is not of length 2.
    """
    arr = np.asarray(complex_list, dtype=np.complex_)
    if not arr.shape[-1] == 2:
        raise QuintengError("Inner most nested list is not of length 2.")
...
def format_level_0_memory(memory):
    ...
    Raises:
        QuintengError: If the returned numpy array does not have 2 (avg) or 3 (single)
            indices.
    """
    ...
    if not 2 <= len(formatted_memory.shape) <= 3:
        raise QuintengError("Level zero memory is not of correct shape.")
...
def format_level_1_memory(memory):
    ...
    Raises:
        QuintengError: If the returned numpy array does not have 1 (avg) or 2 (single)
            indices.
    """
    formatted_memory = _list_to_complex_array(memory)
    # infer meas_return from shape of returned data.
    if not 1 <= len(formatted_memory.shape) <= 2:
        raise QuintengError("Level one memory is not of correct shape.")
    return formatted_memory
...
def format_statevector(vec, decimals=None):
    """Format statevector coming from the backend to present to the Quinteng user.
...
def format_unitary(mat, decimals=None):
    """Format unitary coming from the backend to present to the Quinteng user.
