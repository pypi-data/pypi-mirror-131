    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            header (quinteng.qobj.QobjExperimentHeader): A free form dictionary
                header for the experiment
            kwargs: Arbitrary extra fields

        Raises:
            QuintengError: If meas_return or meas_level are not valid values
        """
        self._metadata = {}
        self.shots = shots
        ...
        if meas_return is not None:
            if meas_return not in list(MeasReturnType):
                raise QuintengError("%s not a valid meas_return value")
