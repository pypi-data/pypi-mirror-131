...
def get_unique_backends():
    ...
    Raises:
        ...
        MissingOptionalLibraryError: If quinteng-ibmq-provider is not installed
    """
    try:
        from quinteng.providers.ibmq import IBMQ
    ...
    if not unique_hardware_backends:
        raise QuintengError("No backends available.")
...
def backend_monitor(backend):
    ...
    Raises:
        QuintengError: Input is not a IBMQ backend.
        MissingOptionalLibraryError: If quinteng-ibmq-provider is not installed
    """
    try:
        from quinteng.providers.ibmq import IBMQBackend
    except ImportError as ex:
        raise MissingOptionalLibraryError(
            libname="quinteng-ibmq-provider",
    ...
    if not isinstance(backend, IBMQBackend):
        raise QuintengError("Input variable is not of type IBMQBackend.")
