...
from quinteng.tools.events.progressbar import TextProgressBar
...
class StatusMagic(Magics):
    ...
    def quinteng_job_status(self, line="", cell=None):
        """A Jupyter magic function to check the status of a Quinteng job instance."""
        args = magic_arguments.parse_argstring(self.quinteng_job_status, line)
            ...
            if "#" not in var:
                ...
                if iter_var:
                    for item in self.shell.user_ns[var]:
                        if isinstance(item, quinteng.providers.basejob.BaseJob):
                            jobs.append(item)
                else:
                    if isinstance(self.shell.user_ns[var], quinteng.providers.basejob.BaseJob):
...
class ProgressBarMagic(Magics):
    ...
    def quinteng_progress_bar(self, line="", cell=None):  # pylint: disable=unused-argument
        """A Jupyter magic function to generate progressbar."""
        args = magic_arguments.parse_argstring(self.quinteng_progress_bar, line)
        if args.type == "html":
            pbar = HTMLProgressBar()
        elif args.type == "text":
            pbar = TextProgressBar()
        else:
            raise quinteng.QuintengError("Invalid progress bar type.")
...
if HAS_MATPLOTLIB and get_ipython():
    ...
    @register_line_magic
    def circuit_library_info(circuit: quinteng.QuantumCircuit) -> None:
