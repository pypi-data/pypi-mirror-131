...
class _Broker:
    ...
    Event format:
        "chaoyue.<component>.<method>.<action>"

    Examples:
        "chaoyue.transpiler.compile.start"
        "chaoyue.job.status.changed"
        "chaoyue.backend.run.start"
        "chaoyue.job.result.received"
    """
    ...
    def subscribe(self, event, callback):
        ...
        if not callable(callback):
            raise QuintengError("Callback is not a callable!")
