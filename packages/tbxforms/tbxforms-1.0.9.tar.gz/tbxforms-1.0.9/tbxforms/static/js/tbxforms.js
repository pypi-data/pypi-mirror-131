...
class TbxForms {
    ...
    getOrCreateId(node) {
        if (!node.id) {
            // If the element doesn't have an ID, generate one that's likely to be unique.
            node.id = `tbxforms-autoid-${Math.random().toString(12)}`;
        }
        return node.id;
    }
