...
class Downloader:
    ...
    def _insert_logs(self) -> None:
        """Pop all logs from the queue, and bulk-insert them into the
        database

        Does nothing if the log queue is empty.
        """
        with self._log_lock:
        ...
        self.logger.debug('Inserting logs into DB: %s', log_queue)
        db_util.insert_logs(self.job_id, log_queue)

    def _insert_progress(self) -> None:
        """Pop all progress entries from the queue, and bulk-insert them
        into the database

        Does nothing if the progress queue is empty.
        """
        with self._progress_lock:
        ...
        # Don't log the actual progress entries because they're
        # extremely long when using yt-dlp
        self.logger.debug(
            'Inserting %d progress entries into DB', len(progress_queue)
        )
        db_util.upsert_progress(self.job_id, progress_queue)
