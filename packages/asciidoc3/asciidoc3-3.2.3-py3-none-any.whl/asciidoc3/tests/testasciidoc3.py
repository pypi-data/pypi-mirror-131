...
(c) Stuart Rackham (c) 2020 Berthold Gehrke <berthold.gehrke@gmail.com>.
...
__version__ = '3.2.3'
__copyright__ = 'Copyright (C) 2009 Stuart Rackham, (c) 2020 Berthold Gehrke'

# Default backends:
BACKENDS = ('html4', 'xhtml11', 'docbook45', 'docbook51', 'html5')
BACKEND_EXT = {'html4': '.html', 'xhtml11': '.html', 'docbook45': '.xml',
               'docbook51': '.xml', 'slidy': '.html', 'html5': '.html'}
...
class AsciiDoc3Test(object):
    ...
    def parse(self, lines, confdir, datadir):
        ...
        lines = Lines(lines)
        while not lines.eol():
            some_lines = lines.read_until(r'^%')
            if some_lines:
                if not some_lines[0].startswith('%'):
                    if some_lines[0][0] == '!':
                        self.disabled = True
                        self.title = some_lines[0][1:]
                    else:
                        self.title = some_lines[0]
                    self.description = some_lines[1:]
                    continue
                reo = re.match(r'^%\s*(?P<directive>[\w_-]+)', some_lines[0])
                if not reo:
                    raise ValueError
                directive = reo.groupdict()['directive']
                data = normalize_data(some_lines[1:])
                if directive == 'source':
                    if data:
    ...
    def generate_expected(self, backend):
        ...
        # this is the db51 workaround handling <abstract>
        if backend == 'docbook51':
            # add default ('--doctype', 'article') if missing
            options_values_list = [item for item in asciidoc3.options.values]
            options_values_list = [item[0] for item in options_values_list]
            if '--doctype' not in options_values_list:
                asciidoc3.options.values.append(('--doctype', 'article'))
            if ('--doctype', 'article') in asciidoc3.options.values:
                input_so_far = outfile.getvalue()
                # abstract without preamble
                input_so_far, counter = re.subn(
                    r'</info>\s*(<abstract.*?</abstract>)',
                    r'\g<1>\n</info>', input_so_far, 1, flags=re.DOTALL)
                ...
                return input_so_far.splitlines()
            if ('--doctype', 'book') in asciidoc3.options.values:
                input_so_far = outfile.getvalue()
                input_so_far, counter = re.subn(
...
class AsciiDoc3Tests(object):
    ...
    def update(self, number=None, backend=None, force=False):
        ...
        for test in self.tests:
            if (not test.disabled or number) and \
               (not number or number == test.number):
    ...
    def list(self):
        ...
        """
        for test in self.tests:
            print('%d: %s%s' % (test.number, iif(test.disabled, '!'),
                                test.title))
...
class Lines(list):
    ...
    def eol(self):
        """
        Return 'True' if 'self.pos' is equal or greater than len(list).
        """
...
if __name__ == '__main__':
    ...
    if number is not None and \
       number not in list(range(1, len(tests.tests)+1)):
