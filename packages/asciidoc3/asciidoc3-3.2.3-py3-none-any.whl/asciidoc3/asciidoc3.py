...
VERSION = '3.2.3'           # See CHANGELOG file for version history.
MIN_PYTHON_VERSION = '3.7'  # Require this version of Python or better.
...
class Message:
    ...
    def stderr(self, msg=''):
        ...
        # __name__ == 'asciidoc3' if dockerized
    ...
    def format(self, msg, prefix='', linenos=True, cursor=None, offset=0):
        ...
        if self.linenos is not False and ((linenos or self.linenos)
                                          and reader.cursor):
            if cursor is None:
                cursor = reader.cursor
            prefix += '%s: line %d: ' % (os.path.basename(cursor[0]),
                                         cursor[1]+offset)
        return prefix + msg
...
def parse_options(options, allowed, errmsg):
    ...
    'errmsg' is an errormessage prefix if an illegal option error is thrown."""
    result = []
    if options:
...
def subs_quotes(text):
    ...
    for q in keys:
        ...
        if tag[0] == '#':
            ...
            # The text within constrained quotes must be bounded by whitespace.
...
def subs_tag(tag, dict={}):
    ...
    if not s:
        message.warning('tag \'%s\' dropped: contains undefined attribute'
                        % tag)
        return [None, None]
    result = s.split('|')
    if len(result) == 1:
...
def filter_lines(filter_cmd, lines, attrs={}):
    ...
    try:
        ...
        raise EAsciiDoc('filter error: %s: %s' %
                        (filter_cmd, sys.exc_info()[1]))
    if output:
...
def system(name, args, is_macro=False, attrs=None):
    ...
    if name not in ('eval', 'eval3', 'sys', 'sys2', 'sys3', 'include',
                    'include1', 'counter', 'counter2', 'set', 'set2',
                    'template'):
        if is_macro:
    ...
    if is_macro:
        s = subs_attrs(args)
        if s is None:
            message.warning('skipped %s: undefined attribute in: %s'
                            % (name, args))
            return None
        args = s
    if name != 'include1':
    ...
    if name in ('eval', 'eval3'):
        ...
        try:
            ...
            if os.name == 'nt':
                ...
                if path_to_executable.group(0) and \
                   r" " in path_to_executable.group(0):
                    cmd = re.sub(r'".*python3?\.exe', r'"python3',
                                 cmd, 1, re.IGNORECASE)
                # End discard space in 'path to executable'.
                # Adjust quoting for 'sys' in 'image-inlinemacro'
                # and 'image-blockmacro'.
                if 'ad3mtype' in cmd:
                    cmd = re.sub(r'ad3data\)" <', r'ad3data)" "<',
                                 cmd, 1, re.IGNORECASE)
                # Remove redundant quoting -- this is not just
                # cosmetic, unnecessary quoting appears to cause
                # command line truncation.
    ...
    for line in lines:
                ...
                if reo == reo2:
                    ...
                    for n in names:
                        if not re.match(r'^[^\\\W][-\w]*$', n):
                            message.error('illegal attribute syntax: %s'
                                          % attr)
                    if sep == OR:
                ...
                if lval is None:
                    ...
                    if op == '=':
                        ...
                        if len(v) not in (2, 3):
                            ...
                            message.error('illegal attribute regexp: %s'
                                          % attr)
                            s = ''
                        else:
        ...
        for reo in reos:
            ...
            while True:
                ...
                if dictionary is not None and action in (
                        'counter', 'counter2', 'set', 'set2'):
...
class Ad3Codec():
    """New in AsciiDoc3. Class Ad3Codec is not used directly, but provides
       ...
       Python3's muddle concerning encoding regarding the users
       input, stdin/out, output, and command line options happens here."""
    ...
    def check_errors(self, error_name):
        ...
        if error_name.rstrip().lower() not in ('strict', 'ignore', 'replace'):
            raise EAsciiDoc(
                'errors has to be "strict" or "ignore" or "replace!"')
...
class Ad3In(Ad3Codec):
    """Exactly one instance in AsciiDoc3, handles the attributes
       'input-encoding' and 'input-errors' and their corresponding
       'private' variables '_ie' and '_ierr'."""
    ...
    def update_input_encoding(self, fname, conf_attrs, cmd_attrs):
        """Input-encoding '_ie' is updated and then fixed, find this point
           in time at approx. line 5370 of the source code.
           (Search for "update_attrs(self.conf_attrs, d)" and add three
           lines downwards). The user can not set _ie directly: she (or he)
           sets the attribute "input-encoding" in command line (ranks first)
           or conf-files. Just before the output file is opened, '_ie' is
           fixed and inalterable later on. This implies, _ie is not affected
           by ':input-encoding: xyz' somewhere in the input-file, such a
           statement is ignored. "input-encoding" has to be set in the
           command-line or *.conf, '_ie' is initialized as 'utf-8'."""
        if self.update_ie_allowed:
            message.verbose('Entering "update_input-encoding()" '
                            'with file: {filename}'.format(filename=fname))
            # 'stdin' here seems quite impossible
            # if-branch added for 'security reasons' only.
            if fname == 'stdin'.lower():
                """Stdin found, no more encoding updates any more!
                   No change is made to _ie."""
                self.block_updating()
                message.verbose('stdin as input, encodings are fixed:'
                ...
                if cmd_attrs and ('input-encoding' in cmd_attrs):
                    ...
                    message.verbose('input-encoding is fixed as found in '
                                    'command-line: '
                                    '{iencoding}'.format(iencoding=self._ie))
                # _ie as set in conf-files by user
                elif conf_attrs and ('input-encoding' in conf_attrs):
                    # conf_attrs['input-encoding'] may be 'None' or missing
                    self._ie = conf_attrs['input-encoding']
                    message.verbose('input-encoding updated as in {filename}: '
                                    '{iencoding}'.format(
                                        filename=fname, iencoding=self._ie))
                else:
                    pass
            else:
    ...
    def get_input_encoding(self, fname):
        """get the input-encoding: conf/css/py are utf-8, 'stdin' is possibly
        'special', asciidoc3-input-file is opened with _ie. 'stdin' and
        'input-file' block any further altering of '_ie'."""
    ...
    def update_input_errors(self, fname, conf_attrs, cmd_attrs):
        """Input-errors is updated and fixed undergoing the same procedures
           as input-encoding. _ierr has to be set in *.conf or command-line,
           _ierr is initialized as 'strict'."""
        if self.update_ierr_allowed:
            message.verbose('Entering "update/fix input-errors": '
                            '{filename}'.format(filename=fname))
            # 'stdin' here seems quite impossible
            # this if-branch is added for 'security reasons'.
            if fname == 'stdin'.lower():
                """stdin found, no more encoding updates any more!
                   No change is made to _ierr."""
                self.block_updating()
    ...
    def get_input_errors(self, fname):
        ...
        # AsciiDoc3 opens 'infile' and/or a file not ending with conf/css/py
            ...
            raise EAsciiDoc("""\n
                            AsciiDoc3 cannot open a file that is not
...
class Ad3Out(Ad3Codec):
    ...
    def update_output_encoding(self, fname, conf_attrs, cmd_attrs):
        """Output-encoding '_oe' is updated and then fixed, find this point
           in time at approx. line 5370 of the source code.
           (Search for "update_attrs(self.conf_attrs, d)" and add three
           lines downwards). The user can not set _oe directly: she (or he)
           sets the attribute "output-encoding" in command line (ranks first)
           or conf-files. Just before the output file is opened, '_oe' is
           fixed and inalterable later on.
           This implies, _oe is not affected by ':output-encoding: xyz'
           somewhere in the input-file, such a statement is ignored.
           "output-encoding" has to be set in the command-line or
           *.conf, '_oe' is initialized as 'utf-8'."""

        if self.update_oe_allowed:
            message.verbose('Entering "update/fix output-encoding" '
                            'with input-file: {filename}'.format(
                                filename=fname))
            # 'stdin' here seems quite impossible
            # if-branch added for 'security reasons'.
            if fname == 'stdin'.lower():
                """Stdin found, no more encoding updates any more!
                   No change is made to _oe."""
                self.block_updating()
                message.verbose('stdin found, encodings are fixed: '
                                '{filename}'.format(filename=fname))
            elif fname.rstrip().endswith(".conf"):
                ...
                if cmd_attrs and 'output-encoding' in cmd_attrs:
                    ...
                    message.verbose('output-encoding is fixed as found '
                                    'in command-line: {oencoding}'.format(
                                        oencoding=self._oe))
                # as set in conf-files from user
                elif conf_attrs and ('output-encoding' in conf_attrs):
                    # conf_attrs['output-encoding'] may be None or missing
                    self._oe = conf_attrs['output-encoding']
                    message.verbose(
                        'output-encoding updated as in {filename}: '
                        '{oencoding}'.format(
                            filename=fname, oencoding=self._oe))
                else:
                    pass
            else:
                """fname is not == stdin and does not end with '.conf':
    ...
    def update_output_errors(self, fname, conf_attrs, cmd_attrs):
        """Output-errors (_oerr) is fixed;
           _oerr has to be set in *.conf or command-line."""
        if self.update_oerr_allowed:
            message.verbose('Entering "update/fix output-errors" '
                            'with input-file: {filename}'.format(
                                filename=fname))
            # 'stdin' here seems quite impossible
            # if-branch added for 'security reasons'.
...
class Document():
    ...
    def __init__(self):
        ...
        self.has_errors = False    # Set true if process. errors were flagged.
    ...
    def consume_attributes_and_comments(self,
                                        comments_only=False, noblanks=False):
        """
    ...
    def translate(self, has_header):
        if self.doctype == 'manpage':
            ...
            if Lex.nxt() is not Title:
                ...
                s = ' '.join(lines)
                mo = re.match(r'^(?P<manname>.*?)\s+-\s+(?P<manpurpose>.*)$',
                              s)
                if not mo:
                    message.error('malformed name section body')
                ...
                names = [s.strip()
                         for s in self.attributes['manname'].split(',')]
                if len(names) > 9:
        ...
        if has_header:
            ...
            self.attributes['doctitle'] = Title.dosubs(
                self.attributes['doctitle'])
...
class Header:
    ...
    REV_LINE_RE = \
        r'^(\D*(?P<revnumber>.*?),)?(?P<revdate>.*?)(:\s*(?P<revremark>.*))?$'
    RCS_ID_RE = \
        r'^\$Id: \S+ (?P<revnumber>\S+) (?P<revdate>\S+) ' \
        r'\S+ (?P<author>\S+) (\S+ )?\$$'
...
class AttributeEntry:
    ...
    def isnext():
        ...
        if not AttributeEntry.pattern:
            ...
            if not pat:
                message.error(
                    "[attributes] missing 'attributeentry-pattern' entry")
    ...
    def translate():
        ...
        if attr.name2 is not None:
            ...
            if attr.name2 != '':
                ...
                if attr.name in ('attributes', 'miscellaneous') \
                   and attr.name2[-1] == '!':
                    section[attr.name] = [attr.name2]
                else:
                    section[attr.name] = ['%s=%s'
                                          % (attr.name2, attr.value)]
            ...
            if attr.value is not None:
                mo = re.match(r'^pass:(?P<attrs>.*)\[(?P<value>.*)\]$',
                              attr.value)
                if mo:
                    ...
                    attr.subs = document.attributes.get(
                        'attributeentry-subs',
...
class Title:
    ...
    def getnumber(level):
        ...
        number = ''
        for level_length in range(len(Title.section_numbers)):
            n = Title.section_numbers[level_length]
            if not level_length:
                continue
            elif level_length < level:
                number = '%s%d.' % (number, n)
            elif level_length == level:
                number = '%s%d.' % (number, n + 1)
                Title.section_numbers[level_length] = n + 1
            elif level_length > level:
                # Reset unprocessed section levels.
                Title.section_numbers[level_length] = 0
        return number
    ...
    def set_id():
        if document.attributes.get('sectids') is not None \
           and 'id' not in AttributeList.attrs:
            ...
            AttributeList.attrs['id'] = Section.gen_id(
    ...
    def translate():
        ...
        if Title.level > document.level \
           ...
           and prev_sectname in ('colophon', 'abstract',
            ...
            message.error('%s section cannot contain sub-sections'
                          % prev_sectname)
...
class AbstractBlock:
    # blocknames[]: Global stack of names for
    # push_blockname() and pop_blockname().
    blocknames = []

    def __init__(self):
        # Configuration parameter names common to all blocks.
        self.CONF_ENTRIES = ('delimiter', 'options', 'subs', 'presubs',
                             'postsubs', 'posattrs', 'style', '.*-style',
                             'template', 'filter')
        self.start = None      # File reader cursor at start delimiter.
        self.defname = None    # Config. file block definition section name.
        ...
        # Next line: Each entry is a styles dictionary.
        ...
        self.PARAM_NAMES = ('template', 'options',
                            'presubs', 'postsubs', 'filter')
    ...
    def dump(self):
        ...
        def write_dump(s):
            return sys.stdout.write('%s%s' % (s, writer.newline))
        write_dump('['+self.defname+']')
        if self.is_conf_entry('delimiter'):
            write_dump('delimiter='+self.delimiter)
        if self.template:
            write_dump('template='+self.template)
        if self.options:
            write_dump('options='+','.join(self.options))
        if self.presubs:
            if self.postsubs:
                write_dump('presubs='+','.join(self.presubs))
            else:
                write_dump('subs='+','.join(self.presubs))
        if self.postsubs:
            write_dump('postsubs='+','.join(self.postsubs))
        if self.filter:
            write_dump('filter='+self.filter)
        if self.posattrs:
            write_dump('posattrs='+','.join(self.posattrs))
        if self.style:
            write_dump('style='+self.style)
        if self.styles:
            for style, d in list(self.styles.items()):
                s = ''
                for k, v in list(d.items()):
                    s += '%s=%r,' % (k, v)
                write_dump('%s-style=%s' % (style, s[:-1]))
    ...
    def validate(self):
        ...
        if self.is_conf_entry('template') and 'skip' not in self.options:
            if self.template:
                ...
                if not isinstance(self, List):  # Lists don't have templates.
                    message.warning('missing styles templates: [%s]'
                                    % self.defname)
    ...
    def merge_attributes(self, attrs, params=[]):
        ...
        if style:
            ...
            # Lists have implicit styles and do their own style checks.
                ...
                message.warning('missing style: [%s]: %s'
                                % (self.defname, style))
                style = self.style
...
class Paragraph(AbstractBlock):
    ...
    def translate(self):
        ...
        stag = config.section2tags(template, self.attributes,
                                   skipend=True)[0]
        if self.parameters.filter:
            body = filter_lines(self.parameters.filter, body,
                                self.attributes)
        body = Lex.subs(body, postsubs)
        etag = config.section2tags(template, self.attributes,
                                   skipstart=True)[1]
...
class List(AbstractBlock):
    NUMBER_STYLES = ('arabic', 'loweralpha', 'upperalpha',
    ...
    def __init__(self):
        ...
        self.index = None     # Matched delim. 'index' group (numbered lists).
    ...
    def dump(self):
        ...
        def write_dump(s):
            return sys.stdout.write('%s%s' % (s, writer.newline))
        write_dump('type='+self.type)
        write_dump('tags='+self.tags)
        write_dump('')
    ...
    def translate_item(self):
        ...
        if text:
            writer.write_tag(self.tag.text, text, self.presubs,
                             self.attributes, trace='list text')
        ...
        while True:
            ...
            if nxt in lists.open:
                ...
                if nxt is Title:
                    message.error('section title not allowed in list item',
                                  halt=True)
                nxt.translate()
            else:
                break
    ...
    def translate(self):
        ...
        if self.type in ('numbered', 'callout'):
            ...
            if self.number_style not in self.NUMBER_STYLES:
                message.error('illegal numbered list style: %s'
                              % self.number_style)
...
class Lists(AbstractBlocks):
    ...
    def __init__(self):
        ...
        self.tags = {}           # Tags dictionary. Entries are tags AttrDict.
        self.terminators = None  # List of compiled re's.
    ...
    def translate(self):
        ...
        if 'skip' in options:
            ...
            if 'sectionbody' in options:
                ...
                if self.parameters.filter:
                    body = filter_lines(
                        self.parameters.filter, body, self.attributes)
                body = Lex.subs(body, postsubs)
                # Write start tag, content, end tag.
                etag = config.section2tags(
                    template, self.attributes, skipstart=True)[1]
...
class Table(AbstractBlock):
    ...
    SEPARATORS = dict(
        ...
        psv=r'((?<!\S)((?P<span>[\d.]+)(?P<op>[*+]))?' \
            r'(?P<align>[<\^>.]{,3})?(?P<style>[a-z])?)?\|'
    )
    ...
    def parse_cols(self, cols, halign, valign):
        ...
        for col in self.columns:
            col.halign = col.halign or halign or \
                document.attributes.get('halign') or 'left'
            col.valign = col.valign or valign or \
                document.attributes.get('valign') or 'top'
        ...
        for col in self.columns:
            ...
            if config.pageunits in ('cm', 'mm', 'in', 'em'):
                ...
                col.abswidth = '%d' % Decimal(str(
                    col.abswidth)).quantize(Decimal('1'),
    ...
    def build_colspecs(self):
        ...
        for col in self.columns:
            ...
            if colspec:
                ...
                if not s:
                    message.warning(
                        'colspec dropped: contains undefined attribute')
                else:
                    cols.append(s)
            i += 1
        if cols:
    ...
    def parse_rows(self, text):
        ...
        for ri, row in enumerate(self.rows):
            ...
            for cell in row:
                if not cell.reserved:
                    ...
                    break
            if empty:
                message.warning('table row %d: empty spanned row'
                                % (ri+1))
        ...
        for ri, row in enumerate(self.rows):
            ...
            if row_span < header_span:
                message.warning('table row %d: does not span all columns'
                                % (ri+1))
            if row_span > header_span:
                message.warning('table row %d: exceeds columns span'
                                % (ri+1))
...
class Tables(AbstractBlocks):
    ...
    def validate(self):
        ...
        # Next line: Mandatory default tags
...
class Macros:
        ...
        for entry in entries:
            ...
            if m.name is None:
                ...
                for i, m2 in enumerate(self.macros):
                    ...
                    message.warning('unable to delete missing macro: %s'
                                    % m.pattern)
            else:
                # Check for duplicates.
                for m2 in self.macros:
                    if m2.pattern == m.pattern:
                        message.verbose('macro redefinition: %s%s'
                                        % (m.prefix, m.name))
                        break
                else:
                    self.macros.append(m)
    ...
    def dump(self):
        ...
        for m in self.macros[1:]:
            # Escape = in pattern.
            macro = '%s=%s%s' % (m.pattern.replace('=', r'\='),
                                 m.prefix, m.name)
    ...
    def subs(self, text, prefix='', callouts=False):
        ...
        # if False then all non-callout macros are processed.
        result = text
        for m in self.macros:
            if m.prefix == prefix:
    ...
    def subs(self, text):
        def subs_func(mo):
            ...
            if self.name:
                ...
                if 'name' not in d:
                    message.warning('missing macro name group: %s'
                                    % mo.re.pattern)
                    return ''
                name = d['name']
    ...
    def subs_passthroughs(self, text, passthroughs):
        ...
        def subs_func(mo):
            ...
            if 'passtext' not in d:
                message.warning(
                    'passthrough macro %s: missing passtext group' %
                    d.get('name', ''))
                return mo.group()
            passtext = d['passtext']
            ...
            if d.get('subslist'):
                if d['subslist'].startswith(':'):
                    message.error('block macro cannot occur here: %s'
                                  % mo.group(), halt=True)
                subslist = parse_options(
                    d['subslist'], SUBS_OPTIONS,
...
class Reader1:
    ...
    def open(self, fname):
        """Depending on file is stdin or not:
           set proper encoding, check BOM (ignored in v3)."""
        self.fname = fname
        ...
        if Reader1.read(self):
            if self.cursor[2].startswith(str(UTF8_BOM, encoding='utf-8')):
                self.cursor[2] = self.cursor[2][len(str(
                    UTF8_BOM, encoding='utf-8')):]
                message.verbose('BOM utf-8 was erased, no output with BOM')
                # replaces "self.bom = UTF8_BOM" -> BOM is cut and BOM == False
                self.bom = None
    ...
    def read(self, skip=False):
        ...
        if self.nxt:
            ...
            if mo and not skip:
                ...
                # Don't process include macro once
                ...
                if self.fname != '<stdin>':
                    ...
                    if not os.path.isfile(fname):
                        if warnings:
                            message.warning('include file not found: %s'
                                            % fname)
                        return Reader1.read(self)   # Return next input line.
                    if mo.group('name') == 'include1':
                        if not config.dumping:
                            if fname not in config.include1:
                                message.verbose('include1: '
                                                + fname, linenos=False)
                                # Store the include file in memory
                                # for later; retrieval by the {include1:}
                                # system attribute.
                                f = open(fname)
                                try:
                            ...
                            # just pass the macro call through.
                            return result
                ...
                if 'depth' in attrs:
                    try:
                        ...
                        raise EAsciiDoc(
                            "include macro: illegal 'depth' argument")
...
class Reader(Reader1):
    ...
    def read(self):
        ...
        if result is None:
            return None
        while self.skip:
            mo = macros.match('+', r'ifdef|ifndef|ifeval|endif',
                              result)
            if mo:
                name = mo.group('name')
                ...
                if name == 'endif':
                    self.depth -= 1
                    if self.depth < 0:
                        raise EAsciiDoc('mismatched macro: %s'
                                        % result)
                    ...
                    if name in ('ifdef', 'ifndef'):
                        if not target:
                            raise EAsciiDoc('missing macro target: %s'
                                            % result)
                        if not attrlist:
                            self.depth += 1
                    elif name == 'ifeval':
                        if not attrlist:
                            raise EAsciiDoc('missing ifeval condition: %s'
                                            % result)
                        self.depth += 1
            result = self.read_super()
            if result is None:
...
class Writer:
    ...
    def __init__(self):
        ...
        # The next three lines are for handling <abstract> / DocBook5.1.
        self.handle_article_abstract = False
        self.handle_book_abstract = False
        self.wait_for_chapter = False
    ...
    def open(self, fname, bom=None):
        ...
        if fname == '<stdout>':
            ...
            ad3out.block_updating()
            self.f = sys.stdout
            if 'backend' in document.attributes and \
               document.attributes['backend'] == 'docbook51':
                message.verbose(
                    """\n
                    !!! stdout and DocBook51 may have limitations:
                    Do not use 'abstract' or change to DocBook4.5 !!!\n """)
        else:
            self.f = open(fname, 'w+',
                          encoding=ad3out.get_output_encoding(fname),
                          errors=ad3out.get_output_errors(fname))
            try:
                if document.attributes['backend'] == 'docbook51' and \
                   document.attributes['doctype'] == 'article':
                    self.handle_article_abstract = True
            except KeyError:
                pass
            try:
                if document.attributes['backend'] == 'docbook51' and \
                   document.attributes['doctype'] == 'book':
                    self.handle_book_abstract = True
            except KeyError:
                pass
    ...
    def write_line(self, line=None):
        if not (self.skip_blank_lines and (not line or not line.strip())):
            ...
            self.lines_out += 1
            # This first of all check makes it much more faster:
            if self.handle_article_abstract:
                if r'</abstract>' in line:
                    self.db51_adjustments()
                    self.handle_article_abstract = False
            elif self.handle_book_abstract:
                if r'</abstract>' in line:
                    self.wait_for_chapter = True
                elif r'<chapter' in line and self.wait_for_chapter:
                    self.db51_adjustments()
                    self.handle_book_abstract = False
                    self.wait_for_chapter = False

    def db51_adjustments(self):
        """DocBook5.1 xml output."""
        self.f.flush()
        self.f.seek(0)
        input_so_far = self.f.read()
        if document.attributes['doctype'] == 'article':
            # abstract without preamble
            input_so_far, counter = re.subn(
                r'</info>\s*(<abstract.*?</abstract>)',
                r'\g<1>\n</info>',
                input_so_far, 1, flags=re.DOTALL)
            # print("counter", counter)  # debugging
            if not counter:
                # abstract with preamble
                input_so_far, counter = re.subn(
                    r'</info>\s*(<simpara>.*?)\s?(<abstract.*?</abstract>)',
                    r'\g<2>\n</info>\n\g<1>',
                    input_so_far, 1, flags=re.DOTALL)
        elif document.attributes['doctype'] == 'book':
            input_so_far, counter = re.subn(
                r'</info>(.*?)(<abstract.*?</abstract>)\s?(.*?)<chapter',
                r'\g<2>\n</info>\g<1>\g<3><chapter',
                input_so_far, 1, flags=re.DOTALL)
            # print(counter)  # uncomment for debugging
        # backend 'manpage' is ignored, since there is nothing to do anyway
        self.f.seek(0)
        self.f.write(input_so_far)
...
def _subs_specialwords(mo):
    ...
    args = {}
    # The full match string is argument 'words':
    args['words'] = mo.group()
    # Add other named match groups to the arguments:
...
class Config:
    ...
    ENTRIES_SECTIONS = ('tags', 'miscellaneous', 'attributes',
                        'specialcharacters', 'specialwords', 'macros',
                        'replacements', 'quotes', 'titles', r'paradef-.+',
                        r'listdef-.+', r'blockdef-.+', r'tabledef-.+',
                        r'tabletags-.+', r'listtags-.+', 'replacements[23]',
                        r'old_tabledef-.+')

    def __init__(self):
        # Keyed by section name containing lists of section lines.
        ...
        self.quotes = collections.OrderedDict()
    ...
    def find_in_dirs(self, filename, dirs=None):
        ...
        result = [item for item in result
                  if not item.endswith('asciidoc.conf')]
        return result
    ...
    def validate(self):
        ...
        message.linenos = False     # Disable document line numbers.
        # Heuristic to validate that at least one conf file was loaded.
        ...
        for q in list(self.quotes.keys())[:]:
            ...
            if not tag:
                ...
                if tag not in self.tags:
                    message.warning('[quotes] %s missing tag definition: %s'
                                    % (q, tag))
    ...
    def parse_specialsections(self):
        ...
        d = {}
        parse_entries(self.sections.get('specialsections',
                                        ()), d, unquote=True)
...
class Tables_OLD(AbstractBlocks):
    ...
    def validate(self):
        ...
        for b in self.blocks:
            ...
            if config.verbose:
                if b.check_msg:
                    message.warning('[%s] table definition: %s'
                                    % (b.defname, b.check_msg))
        ...
        for zi in zipo.infolist():
            ...
            if not outfile.endswith('/'):
                ...
                if sys.platform == 'win32':
                    flags |= os.O_BINARY
                if not perms:
                    # Zip files created under Windows
        ...
        if os.path.isfile(src):
            ...
            for root, dirs, files in os.walk(srcdir):
                ...
                if skip_hidden:
                    for d in dirs[:]:
                        if d.startswith('.'):
                            message.verbose('skipping: %s'
                                            % os.path.join(arcroot, d))
...
class Plugin:
    ...
    def remove(args):
        ...
        args[1] is optional plugin directory (defaults to
        ~/.asciidoc3/<plugin_name>).
        """
        if len(args) not in (1, 2):
    ...
    def list(args):
        ...
        """
        for d in [os.path.join(d, Plugin.type+'s')
                  for d in config.get_load_dirs()]:
            if os.path.isdir(d):
                # TODO pylint3 says: Generator 'generator' has no
                # 'next' member (no-member)
...
def asciidoc3(backend, doctype, confiles, infile, outfile, options):
    ...
    try:
        ...
        if os.name == 'nt':
            ...
            # But, to run filter with 'sys' we need
            # '...python3.exe' (see line ~840). TODO
            document.attributes['python3'], \
                                            n = re.subn(r'pythonw\.exe',
                                                        r'python3.exe',
                                                        document.attributes[
                                                            'python3'],
                                                        1, re.IGNORECASE)
            if n:
                message.verbose("""sys.executable: pythonw.exe
                                   replaced with python3.exe""")
        ...
        if '-e' not in options:
            ...
            if not config.load_from_dirs('asciidoc3.conf',
                                         include=['attributes']):
...
def show_help(topic, f=None):
    ...
    if not n:
        if topic != 'topics':
            message.stderr('help topic not found: [%s] in %s'
                           % (topic, help_file))
        message.stderr('available help topics: %s'
                       % ', '.join(list(config.sections.keys())))
        sys.exit(1)
    elif n > 1:
...
def execute(cmd, opts, args):
    ...
    for o, v in opts:
        ...
        if o == '--unsafe':
            message.verbose('unsafe option is ignored!')
        if o == '--safe':
            document.safe = True
        if o == '--version':
        ...
        if o in ('-f', '--conf-file') and not \
           v.strip().lower().endswith('asciidoc.conf'):
            confiles.append(v)
        if o == '--filter':
