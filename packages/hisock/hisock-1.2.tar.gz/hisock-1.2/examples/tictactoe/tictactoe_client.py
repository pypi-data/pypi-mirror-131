...
def run():
    ...
    def game_started(opponent: str):
        ...
        if goes_first == b"First":
            ...
            while (
                move_input == ""
                or not move_input.isdigit()
                or (move_input.isdigit() and not 1 <= int(move_input) <= 9)
            ):
                move_input = input(
                    "Enter your move (1-9, left to right, top to bottom): "
                )
    ...
    def player_turn(move: int):
        ...
        while (
            ...
            or not move_input.isdigit()
            or (move_input.isdigit() and not 1 <= int(move_input) <= 9)
            or (move_input.isdigit() and data.board[int(move_input) - 1] != " ")
        ):
            move_input = input("Enter your move (1-9, left to right, top to bottom): ")
        move_input = int(move_input)
