...
class OpeningLeadSuit():
    ...
    def _select_suit_for_preempt_nt_contract(self):
        ...
        return log(inspect.stack(), SelectedSuit(self.hand.longest_suit.name, '021'))
    ...
    def _select_suit_for_informed_nt_contract(self):
        ...
        if ((self.hand.shape[0] >= 5 or max_sequence_length >= 4) and
                self.hand.longest_suit in long_sequences):
    ...
    def _select_long_suit_for_blind_nt_contract(self):
        ...
        if self.hand.shape[0] >= 5 and long_suit_honours >= 2:
            return log(inspect.stack(), SelectedSuit(self.hand.longest_suit.name, selection_reason='001'))
        ...
        if self.contract.level >= 6:
            if Call('D') in self.partners_bids:
                ...
                for call in calls:
                    if double:
                        if call.is_suit_call:
                            return log(inspect.stack(), SelectedSuit(call.denomination.name, '031'))
                        else:
                            for dummys_call in self.dummys_bids:
                                if dummys_call.is_suit_call:
                                    return log(inspect.stack(), SelectedSuit(dummys_call.denomination.name, '032'))
                    if call == Call('D'):
                        double = True
            if self.hand.hcp >= 7:
                for suit in SUITS:
                    if suit != self.contract.denomination.name:
                        if Card('K' + suit) in self.hand.cards:
                            return log(inspect.stack(), SelectedSuit(suit, '030'))
            return None
        return None
    ...
    def _trump_after_double(self):
        ...
        if Call('D') in self.leaders_bids and partner_passed:
            return log(inspect.stack(), SelectedSuit(self.contract.denomination.name, '028'))
    ...
    def _suit_with_AK(self):
        ...
        if len(candidate_suits) > 1:
            ...
            return log(inspect.stack(), SelectedSuit(candidate_suits[0], '007'))
        return None
        ...
        for suit_name, suit in SUITS.items():
            if suit != self.denomination:
                ...
                if len(cards) == 1:
                    candidate_suits.append(suit)
        if candidate_suits:
            return log(inspect.stack(), SelectedSuit(candidate_suits[0].name, '008'))
        return None
    ...
    def _suit_bid_by_partner(self):
        ...
        for call in self.partners_bids:
            if call.is_suit_call:
                return log(inspect.stack(), SelectedSuit(call.denomination.name, '020'))
        return None
    ...
    def _suit_if_not_deprecated(self):
        ...
        if len(candidate_suits) == 1:
            return log(inspect.stack(), SelectedSuit(candidate_suits[0], '014'))
    ...
    def _use_trumps(self):
        ...
        if len(cards) >= 2 and not cards[0].high_card_points:
            return log(inspect.stack(), SelectedSuit(self.denomination.name, '028'))
        return None
    ...
    def _best_of_poor_suits(self):
        ...
        for suit in suits:
            ...
            if Card('K'+ suit.name) in cards:
                return log(inspect.stack(), SelectedSuit(suit, '023'))
            if Card('Q'+ suit.name) in cards:
                return log(inspect.stack(), SelectedSuit(suit, '024'))
            if Card('J'+ suit.name) in cards:
                return log(inspect.stack(), SelectedSuit(suit, '025'))
