...
class HuaweiRedisClient(HuaweiClient):
    ...
    def get_redis_info(self, params=None):
        ...
        if params:
            ...
            redis_list.extend(self.redis_client.list_instances(request).instances)
        else:
            offset = 0
            limit = 100
            i = 0
            while True:
                ...
                res = self.redis_client.list_instances(request)
                redis_list.extend(res.instances)
                offset = (i + 1) * limit
                if int(offset) >= int(res.instance_num):
                    break
                i += 1
        return redis_list
