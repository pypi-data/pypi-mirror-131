...
class ASTInvalidToken(Exception):
    """
    NML AST Eval Error: Invalid token
    """
    ...
    def __init__(self, token: Token, *args):
        ...
        super(ASTInvalidToken).__init__(*args)
    ...
    def _create(self) -> list:
        ...
        for token in self._tokenized:
            ...
            if token.type() == Token.ListBegin:
                ...
                return [[Token(Token.Identifier, 'identity'),
                         Token(Token.NException,
                               (ASTInvalidToken(token), token.position()))]]
