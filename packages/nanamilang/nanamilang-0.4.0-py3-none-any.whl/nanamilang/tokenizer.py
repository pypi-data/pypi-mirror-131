...
class Tokenizer:
    ...
    def _literal_or_an_invalid_token(self, literal: str) -> Token:
        ...
        # Try to match an IntegerNumber literal (2-base)
        if re.match(self._binary_number_pattern, literal):
            return Token(
                Token.IntegerNumber,
                int(literal, base=2), _position=position)
        # Try to match an IntegerNumber literal (10-base)
    ...
    def _get_next_token_list(self) -> List[Token]:
        ...
        #       Move pointer next && increment line-character-number counter
        #       <return> -----------------------> [Token.ListBegin, Token.Identifier 'make-set']
