""" Модуль клиентской транспортной подсистемы"""
import socket
import time
import logging
import json
...
class ClientTransport(threading.Thread, QObject):
    ...
    def connection_init(self, port, ip):
        ...
        logger.debug('Запуск диалога авторизации')
        ...
        logger.debug(f'Хеш пароля готов: {passwd_hash_string}')
        ...
        with socket_lock:
            ...
            logger.debug(f"Сформировано сообщение о присутствии = {presense}")
            # Отправляем серверу приветственное сообщение.
            try:
                send_message(self.transport, presense)
                ans = get_message(self.transport)
                logger.debug(f'Получен ответ сервера = {ans}.')
                ...
                logger.debug('Ошибка соединения', exc_info=err)
