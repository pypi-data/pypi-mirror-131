...
class Container(DataclassDictMixIn):
    """A single application container that you want to run within a pod.
      ...
      * **terminationMessagePolicy** ``str`` - *(optional)* Indicate how the termination message should be populated. File will use the
        ...
        container logs for the container status message when the container exits with
        an error and the terminationMessagePath has no contents.
         - `"File"` is the default behavior and will set the container status message
        to the contents of the container's terminationMessagePath when the container
        exits.
...
class NodeAddress(DataclassDictMixIn):
    """NodeAddress contains information for the node's address.
      ...
      * **type** ``str`` - Node address type, one of Hostname, ExternalIP or InternalIP.
        ...
        be more usable from outside the cluster then an internal IP, though no
        specific semantics are defined. It may be a globally routable IP, though it is
        not required to be. External IPs may be assigned directly to an interface on
        the node, like a NodeInternalIP, or alternatively, packets sent to the
        external IP may be NAT'ed to an internal node IP rather than being delivered
        directly (making the IP less efficient for node-to-node traffic than a
        NodeInternalIP).
         - `"Hostname"` identifies a name of the node. Although every node can be
        assumed to have a NodeAddress of this type, its exact syntax and semantics are
        not defined, and are not consistent between different clusters.
        ...
        not be a listed NodeInternalIP address.
         - `"InternalIP"` identifies an IP address which is assigned to one of the
        node's network interfaces. Every node should have at least one address of this
        type. An internal IP is normally expected to be reachable from every other
        node, but may not be visible to hosts outside the cluster. By default it is
        assumed that kube-apiserver can reach node internal IPs, though it is possible
        to configure clusters where this is not the case. NodeInternalIP is the
        default type of node IP, and does not necessarily imply that the IP is ONLY
        reachable internally. If a node has multiple internal IPs, no specific
...
class PersistentVolumeClaimStatus(DataclassDictMixIn):
    """PersistentVolumeClaimStatus is the current status of a persistent volume
      ...
      * **phase** ``str`` - *(optional)* Phase represents the current phase of PersistentVolumeClaim.
        ...
        PersistentVolume. The claim was bound to a PersistentVolume and this volume
        does not exist any longer and all data on it was lost.
         - `"Pending"` used for PersistentVolumeClaims that are not yet bound
      * **resizeStatus** ``str`` - *(optional)* ResizeStatus stores status of resize operation. ResizeStatus is not set by
    ...
    """PersistentVolumeStatus is the current status of a persistent volume.
      ...
      * **phase** ``str`` - *(optional)* Phase indicates if a volume is available, bound to a claim, or released by a
        ...
        volumes are held by the binder and matched to PersistentVolumeClaims
         ...
         - `"Failed"` used for PersistentVolumes that failed to be correctly recycled
        or deleted after being released from a claim
         - `"Pending"` used for PersistentVolumes that are not available
         - `"Released"` used for PersistentVolumes where the bound
        PersistentVolumeClaim was deleted released volumes must be recycled before
        becoming available again this phase is used by the persistent volume claim
    ...
    """PodStatus represents information about the status of a pod. Status may trail
      ...
      * **phase** ``str`` - *(optional)* The phase of a Pod is a simple, high-level summary of where the Pod is in its
        ...
        code or was stopped by the system).
         - `"Pending"` means the pod has been accepted by the system, but one or more
        of the containers has not been started. This includes time before being bound
        ...
        containers have been started. At least one container is still running or is in
...
class Probe(DataclassDictMixIn):
    """Probe describes a health check to be performed against a container to
      ...
      * **grpc** ``GRPCAction`` - *(optional)* GRPC specifies an action involving a GRPC port. This is an alpha field and
...
class ScopedResourceSelectorRequirement(DataclassDictMixIn):
    """A scoped-resource selector requirement is a selector that contains values, a
      ...
      * **scopeName** ``str`` - The name of the scope that the selector applies to.
        ...
        cross-namespace pod (anti)affinity mentioned. This is a beta feature enabled
        by the PodAffinityNamespaceSelector feature flag.
...
class Taint(DataclassDictMixIn):
    """The node this Taint is attached to has the "effect" on any pod that does not
      ...
      * **effect** ``str`` - Required. The effect of the taint on pods that do not tolerate the taint.
        ...
        not to schedule new pods onto the node, rather than prohibiting new pods from
