...
class CertificateSigningRequest(DataclassDictMixIn):
    """CertificateSigningRequest objects provide a mechanism to obtain x509
      ...
      * **status** ``CertificateSigningRequestStatus`` - *(optional)* status contains information about whether the request is approved or denied,
    ...
    """CertificateSigningRequestCondition describes a condition of a
      ...
      * **type** ``str`` - type of the condition. Known conditions are "Approved", "Denied", and
        ...
        issued by the signer.
         - `"Denied"` Denied indicates the request was denied and should not be issued
        by the signer.
      ...
      * **lastUpdateTime** ``meta_v1.Time`` - *(optional)* lastUpdateTime is the time of the last update to this condition
      * **message** ``str`` - *(optional)* message contains a human readable message with details about the request state
      * **reason** ``str`` - *(optional)* reason indicates a brief reason for the request state
    """
    status: 'str'
    type: 'str'
...
class CertificateSigningRequestSpec(DataclassDictMixIn):
    """CertificateSigningRequestSpec contains the certificate request.
      ...
      * **expirationSeconds** ``int`` - *(optional)* expirationSeconds is the requested duration of validity of the issued
        ...
        gate.
      * **extra** ``dict`` - *(optional)* extra contains extra attributes of the user that created the
        CertificateSigningRequest. Populated by the API server on creation and
        immutable.
      * **groups** ``List[str]`` - *(optional)* groups contains group membership of the user that created the
        CertificateSigningRequest. Populated by the API server on creation and
        immutable.
      * **uid** ``str`` - *(optional)* uid contains the uid of the user that created the CertificateSigningRequest.
        Populated by the API server on creation and immutable.
      * **usages** ``List[str]`` - *(optional)* usages specifies a set of key usages requested in the issued certificate.
      ...
      * **username** ``str`` - *(optional)* username contains the name of the user that created the
        CertificateSigningRequest. Populated by the API server on creation and
        immutable.
    """
    request: 'str'
...
class CertificateSigningRequestStatus(DataclassDictMixIn):
    """CertificateSigningRequestStatus contains conditions used to indicate
      ...
      * **certificate** ``str`` - *(optional)* certificate is populated with an issued certificate by the signer after an
      ...
      * **conditions** ``List[CertificateSigningRequestCondition]`` - *(optional)* conditions applied to the request. Known conditions are "Approved", "Denied",
        and "Failed".
    """
