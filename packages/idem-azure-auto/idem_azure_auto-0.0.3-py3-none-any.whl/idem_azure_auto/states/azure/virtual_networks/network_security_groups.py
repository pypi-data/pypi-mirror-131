...
async def present(
    ...
    **Autogenerated function**
    ...
    response_get = await hub.exec.request.json.get(
        ctx,
        url=f"https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Network/networkSecurityGroups/{network_security_group_name}?api-version=2021-03-01",
        success_codes=[200],
    ...
    if force_update:
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment="Would force to update azure.virtual_networks.network_security_groups",
            )
        ...
        if response_force_put["result"]:
            ...
            hub.log.debug(
                f"Could not force to update Network Security Groups {response_force_put['comment']} {response_force_put['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
    ...
    if not response_get["result"]:
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment="Would create azure.virtual_networks.network_security_groups",
            )
        ...
        if response_get["status"] == 404:
            ...
            hub.log.debug(
                f"Could not get Network Security Groups {response_get['comment']} {response_get['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
        ...
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment=f"Would update azure.virtual_networks.network_security_groups with parameters: {new_parameters}",
            )
        ...
        if not response_patch["result"]:
            hub.log.debug(
                f"Could not update Network Security Groups {response_patch['comment']} {response_patch['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
...
async def absent(
    hub, ctx, name: str, resource_group_name: str, network_security_group_name: str
) -> StateReturn:
    r"""
    **Autogenerated function**
    ...
    Examples:
        ...
        .. code-block:: sls
            ...
            resource_is_absent:
              azure.virtual_networks.network_security_groups.absent:
                - name: value
                - resource_group_name: value
                - network_security_group_name: value
    """
    ...
    if response_get["result"]:
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment="Would delete azure.virtual_networks.network_security_groups",
            )
        ...
        if not response_delete["result"]:
            hub.log.debug(
                f"Could not delete Network Security Groups {response_delete['comment']} {response_delete['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
...
async def describe(hub, ctx) -> Dict[str, Dict[str, Any]]:
    ...
    **Autogenerated function**
    ...
    List all Network Security Groups under the same subscription
    ...
    Examples:
        ...
        .. code-block:: bash
            ...
            $ idem describe azure.virtual_networks.network_security_groups
    """
        ...
        async for page_result in hub.tool.azure.request.paginate(
            ...
            if resource_list:
                for resource in resource_list:
                    ...
                    result[resource["id"]] = {
                        f"azure.virtual_networks.network_security_groups.present": uri_parameter_values
                        + [{"parameters": resource}]
                    }
    except ValueError as e:
        raise DescribeError(f"Error on describing Network Security Groups: {str(e)}")
    return result
