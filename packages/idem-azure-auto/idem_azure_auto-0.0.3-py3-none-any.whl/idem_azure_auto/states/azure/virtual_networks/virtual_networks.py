...
async def present(
    ...
    **Autogenerated function**
    ...
    response_get = await hub.exec.request.json.get(
        ctx,
        url=f"https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Network/virtualNetworks/{virtual_network_name}?api-version=2021-03-01",
        success_codes=[200],
    ...
    if force_update:
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment="Would force to update azure.virtual_networks.virtual_networks",
            )
        ...
        if response_force_put["result"]:
            ...
            hub.log.debug(
                f"Could not force to update Virtual Networks {response_force_put['comment']} {response_force_put['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
    ...
    if not response_get["result"]:
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment="Would create azure.virtual_networks.virtual_networks",
            )
        ...
        if response_get["status"] == 404:
            ...
            if not response_put["result"]:
                hub.log.debug(
                    f"Could not create Virtual Networks {response_put['comment']} {response_put['ret']}"
                )
                return StateReturn(
                    name=name,
                    result=False,
            ...
            hub.log.debug(
                f"Could not get Virtual Networks {response_get['comment']} {response_get['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
        ...
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment=f"Would update azure.virtual_networks.virtual_networks with parameters: {new_parameters}",
            )
        ...
        if not response_patch["result"]:
            hub.log.debug(
                f"Could not update Virtual Networks {response_patch['comment']} {response_patch['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
    ...
    if response_get["result"]:
        if ctx.get("test", False):
            return StateReturn(
                name=name,
                result=True,
                comment="Would delete azure.virtual_networks.virtual_networks",
            )
        ...
        if not response_delete["result"]:
            hub.log.debug(
                f"Could not delete Virtual Networks {response_delete['comment']} {response_delete['ret']}"
            )
            return StateReturn(
                name=name,
                result=False,
        ...
        async for page_result in hub.tool.azure.request.paginate(
            ...
            if resource_list:
                for resource in resource_list:
                    ...
                    result[resource["id"]] = {
                        f"azure.virtual_networks.virtual_networks.present": uri_parameter_values
                        + [{"parameters": resource}]
                    }
    except ValueError as e:
        raise DescribeError(f"Error on describing Virtual Networks: {str(e)}")
    return result
