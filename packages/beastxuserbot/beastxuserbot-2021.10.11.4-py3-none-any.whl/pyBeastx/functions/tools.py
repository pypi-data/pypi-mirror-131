...
async def metadata(file):
    ...
    try:
        ...
        data["performer"] = (
            info[0].get("Performer") or udB.get("artist") or beastx_bot.me.first_name
        )
        if len(info) >= 2:
...
async def get_chatbot_reply(message):
    ...
    req_link = chatbot_base.format(
        beastx_bot.me.first_name or "ultroid user",
        message,
    )
    try:
...
def telegraph_client():
    ...
    gd_name = get_display_name(beastx_bot.me)
    short_name = gd_name if len(gd_name) < 32 else "Ultroid"
    profile_url = (
        f"https://t.me/{beastx_bot.me.username}" if beastx_bot.me.username else None
    )
    try:
