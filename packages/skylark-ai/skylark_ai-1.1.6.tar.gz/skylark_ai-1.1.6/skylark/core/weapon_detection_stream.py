...
class WeaponDetectionStreamClient(StreamClient):
	def __init__(self, fps, batch_size, sampling_rate, token):
		...
		self.token = token
		self.client = RealTimeClient(Service.WEAPON_DETECTION, self.on_receive, batch_size, self.on_network_status_change, token)
		self.response_jsons = []
		self.createTasks()
						...
						for i in range(self.sample_length):
							...
							if "weapons" in result:
								for weapon in result["weapons"]:
									start_point = (int(weapon[0]), int(weapon[1]))
									end_point = (int(weapon[2]), int(weapon[3]))
									color = (0, 0, 255)
									thickness = 1
