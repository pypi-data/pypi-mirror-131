...
class SekkayBot(commands.Bot):
    ...
    async def remove_if_offline(self) -> None:
        pause_time = 10
        day_amount = 21
        while True:
            for f in self.friends:
                friend = self.get_friend(f)
                day = int(datetime_.now().strftime("%d"))
                month = int(datetime_.now().strftime("%m"))
                year = int(datetime_.now().strftime("%y"))
                dates = str(friend.last_logout).split(' ')[0].split('-')
                offline_year = int(dates[0])
                offline_month = int(date[1])
                offline_day = int(dates[2])
                last_online = date(
                    offline_year,
                    offline_month, 
                    offline_day
                ...
                if numOfDays(now, last_online) > day_amount:
                    await friend.remove()
                else:
                    continue
            await asyncio.sleep(pause_time*60)
    ...
    async def event_ready(self) -> None:
        ...
        self.loop.create_task(self.remove_if_offline())
