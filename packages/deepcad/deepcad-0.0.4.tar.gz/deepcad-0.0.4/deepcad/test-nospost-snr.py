...
parser.add_argument('--patch_x', type=int, default=150, help="the width of 3D patches (patch size in x, also in y and t)")
...
#############################################################################################################################################################
# clean_im = tiff.imread('/home/bbnclxy/extdisk/deepcad-1122/datasets/clean-img/clean_30Hz_6000frames.tif')
clean_im = tiff.imread('/home/aa/LXY/deepcad-noaug/datasets/clean-img/clean_30Hz_6000frames.tif')
##############################################################################################################################################################
...
for pth_index in range(len(model_list)):
    ...
    if '.pth' in aaa:
        ...
        for N in range(len(img_list)):
            ...
            ############################### calculate and store snr for test image###########################################
            clean_im=clean_im.astype(np.float32)
            denoise_im=output_img
            snrr=cal_snr(denoise_im[100:500,:,:],clean_im[100:500,:,:])
            # snrr=cal_snr(denoise_im,clean_im)
            print(snrr)
            with open(output_path1+ '//result_' + current_time + '.txt', 'a', encoding='utf-8') as f:
              f.write(str(snrr) + '\n')
