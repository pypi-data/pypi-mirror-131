...
def solve_fast_{id_}(np.ndarray y0,
                     np.ndarray tvec,
                     rtol,
                     atol,
                     int max_iter=200000,
...
{merge_function_src}
    ...
    cdef double t_switch = {switch_time_function_name}(tvec)
    ...
    if t_switch > tvec[0]:
        result_0, meta_0 = {solver_0}(y0, tvec_0, rtol, atol, max_iter, max_order,
                                    enable_fast_solver, enable_sparse_lu_solver)
        y0 = {switch_function_name}(t_switch, result_0)
    else:
    ...
    return {merge_function_name}(tvec_0, t_switch, tvec_1, result_0, result_1), meta
