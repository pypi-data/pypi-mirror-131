...
def view(struct, words, annotate):
    ngram_dictionary={}
...
def freq(word_distribution,top_n=None, words=None, graph=True):
    ...
    for a in word_distribution:
        ...
        total_news=total_news+word_distribution["total_news"]
    if top_n==None and words ==None:
        ...
        top=concept_freq_total.most_common(3)
        if graph:
        ...
        plt.bar(words,y)
        plt.tittle("term frecuency bar graph")
