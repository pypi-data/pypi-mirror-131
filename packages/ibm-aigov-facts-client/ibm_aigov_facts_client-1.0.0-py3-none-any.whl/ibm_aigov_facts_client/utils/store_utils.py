...
def check_if_auth_used(authenticator):
    is_auth_used = False
    if type(authenticator) in [CloudPakForDataAuthenticator, IAMAuthenticator, BearerTokenAuthenticator]:
        if authenticator.token_manager.get_token() or authenticator.bearer_token:
            is_auth_used = True
    return is_auth_used
