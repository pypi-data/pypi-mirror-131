...
class Prop:
    def __init__(self, name=None, framework_name=None, autolog=None, version=None,manual_log=None, trigger_methods=None, training_metrics=None, post_training_metrics=None, parameters=None, tags=None, artifacts=None, search_estimator=None):
        self.name = name
...
class SupportBase:
    ...
    def _generate_doc_table(self):
        return self._generate_table('Name', 'Framework Name', 'Autolog', 'Version','Manual Log', 'Trigger Methods', 'Metrics', 'Parameters', 'Tags', 'Artifacts', 'Search Estimators', format='grid')
...
""".format(resource_name, SupportBase(self._props_definitions)._generate_doc_framework_table())
    ...
    def _generate_table(self, name_label='NAME', framework_label='FRAMEWORK_NAME',
                        autolog_label='AUTOLOG',version_label='VERSION', manual_log_label='MANUAL_LOG', 
        ...
        header = [name_label, framework_label,
                  autolog_label, version_label]

        table_content = []

        for prop in self._props_definitions:
            row = [prop.name, prop.framework_name, u'Y' if prop.autolog else u'N', prop.version]
...
class FrameworkSupportOptions(SupportBase):
    _props_definitions = [
        Prop('scikit', 'sklearn', True,
             '0.20.3 <= scikit-learn <= 0.24.2'),
        Prop('Tensorflow', 'tensorflow', True,
             '1.15.4 <= tensorflow <= 2.6.0'),
        Prop('Keras', 'keras', True,
             '2.2.4 <= keras <= 2.6.0'),
        Prop('PySpark', 'pyspark', True,
             '3.0.0 <= pyspark <= 3.1.2'),
        Prop('Xgboost', 'xgboost', True,
             '0.90 <= xgboost <= 1.4.2'),
        Prop('LightGBM', 'lightgbm', True,
             '2.3.1 <= lightgbm <= 3.2.1'),
        Prop('PyTorch', 'pytorch', True,
...
class FrameworkSupportXGB(SupportBase):
    _props_definitions_xgb = [
        Prop(framework_name='xgboost', trigger_methods='xgboost.train()', training_metrics='- Metrics at each iteration (if `evals` specified), \n- Metrics at best iteration (if `early_stopping_rounds` specified)',
             parameters='- params specified in `xgboost.train`', tags='', post_training_metrics='', search_estimator=''),
    ]
    limit = ''
    __doc__ = SupportBase(
