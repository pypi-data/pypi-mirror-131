        ...
        if (self._params.pk_norm_type == "A_s") or (
            ...
            for j, onek in enumerate(kk):
                ...
                if self._params.N_massive_nu != 0:
                    omega_nu_m = self._wrapper.omega_nu_m(a)
                    P_nu_m = self._wrapper.P_nu_m(a)
                    omega_m_tot += omega_nu_m
                    omega_plus_p_m_tot += omega_nu_m + P_nu_m
                    delta_nu_m = self._wrapper.delta_nu_m_ufunc(a, y[1:])
                    theta_nu_m = onek * self._wrapper.u_nu_m_ufunc(a, y[1:])
                    src_m[:, j] = (
                        delta_nu_m * omega_nu_m / omega_m_tot
                        + (3 * a * H / onek ** 2)
                        * theta_nu_m
                        * (omega_nu_m + P_nu_m)
                        / omega_plus_p_m_tot
