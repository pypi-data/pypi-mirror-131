...
def init_db(user, password, database, host='127.0.0.1', port=3306, use_unicode=True, **kwargs):
    from mysql.connector import connect
    global _DB_CTX
    if ('pool_size' in kwargs or 'pool_reset_session' in kwargs) and 'pool_name' not in kwargs:
        kwargs['pool_name'] = database
...
def named_execute(sql, **kwargs):
    sql, args = _get_named_sql_args(sql, **kwargs)
    return execute(sql, *args)


def named_get(sql, **kwargs):
    sql, args = _get_named_sql_args(sql, **kwargs)
    return get(sql, *args)


def named_select_one(sql, **kwargs):
    sql, args = _get_named_sql_args(sql, **kwargs)
    return select_one(sql, *args)


def named_select(sql, **kwargs):
    sql, args = _get_named_sql_args(sql, **kwargs)
