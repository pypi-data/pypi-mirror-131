...
class ClinicalQualityMeasure131v6Test(SDKBaseTest):
    ...
    def test_description(self):
        ...
        self.assertEqual(expected, ClinicalQualityMeasure131v6.description)

    def test_information(self):
        expected = 'https://ecqi.healthit.gov/ecqm/measures/cms131v6'
        self.assertEqual(expected, ClinicalQualityMeasure131v6.information)

    def test_revision(self):
        self.assertEqual('2019-08-02v1', ClinicalQualityMeasure131v6.revision())

    def test_change_types(self):
        result = ClinicalQualityMeasure131v6.compute_on_change_types
        expected = [
            'protocol_override',
            'condition',
            'patient',
    ...
    def test_compute_results(self):
        ...
        text = ('Dohav has diabetes and no documentation of retinal '
                'examination in the past 24 months.')
        ...
        for code in tests:
            ...
            if code == '721103006':
                text = (
                    ...
                    'Next examination is due 1/31/19.$')
                self.assertRegex(result.narrative, text)
                self.assertEqual([], result.recommendations)
                self.assertEqual('satisfied', result.status)
                self.assertEqual(160, result.due_in)
            else:
                text = (
        ...
        for code in tests:
            ...
            if code == '721103006':
                text = ('Dohav has diabetes and a retinal examination was done 8 months ago '
                        ...
                        'Next examination is due 7/31/19.')
                self.assertEqual(text, result.narrative)
                self.assertEqual([], result.recommendations)
                self.assertEqual('satisfied', result.status)
                self.assertEqual(120, result.due_in)
            else:
