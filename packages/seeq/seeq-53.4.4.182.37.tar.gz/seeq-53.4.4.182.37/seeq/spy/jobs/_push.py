...
def get_parameters(jobs_df, interactive_index, status):
    if _jobs_common.running_in_executor():
        ...
        if not (schedule_index in jobs_df.index or int_index in jobs_df.index):
            raise ValueError(f'Cannot execute job with index {schedule_index} because it is not in the '
        ...
        if interactive_index is None:
            ...
            raise ValueError(f'Cannot schedule job with interactive_index {interactive_index} because it is not in '
