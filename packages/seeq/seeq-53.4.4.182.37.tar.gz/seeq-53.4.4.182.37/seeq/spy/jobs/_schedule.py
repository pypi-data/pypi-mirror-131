...
def schedule_df(jobs_df=None, spread=None, datalab_notebook_url=None, label=None, user=None, suspend=False,
    ...
    if datalab_notebook_url:
        _verify_existing_and_accessible(datalab_notebook_url)

    try:
        user_identity = _login.find_user(user) if user is not None else None
        if len(indexed_cron_expressions) == 0:
            _call_unschedule_notebook_api(project_id, file_path, label)
        else:
            _call_schedule_notebook_api(indexed_cron_expressions, project_id, file_path, label, user_identity)
    ...
    if indexed_cron_expressions:
        ...
        try:
            pickle_path = _pickle_jobs_df(jobs_df, datalab_notebook_url, label)
            if pickle_path:
        ...
        status.update(f'Unscheduled all jobs for notebook <strong>{file_path}'
                      ...
                      f'unscheduling with a label of \'*\'', _common.Status.SUCCESS)
    else:
        pickle_path = _pickle_jobs_df(jobs_df, datalab_notebook_url, label)
...
def _verify_existing_and_accessible(datalab_notebook_url):
    datalab_base_url, project_id, file_path = retrieve_notebook_path(datalab_notebook_url)
    ...
    if is_referencing_this_project(datalab_notebook_url):
        ...
        resp = _jobs_common.requests_get(f'{contents_api_url}/{file_path}', params=params, cookies=cookies)
        if resp.status_code != 200:
            raise error


def _pickle_jobs_df(jobs_df, datalab_notebook_url=None, label=None):
    ...
    if is_referencing_this_project(datalab_notebook_url):
        ...
        resp = _jobs_common.requests_put(f'{contents_api_url}/{put_pickle_path}',
                                         data=json.dumps(content_model),
                                         cookies=cookies)
        if resp.status_code in [200, 201]:
            return put_pickle_path
        else:
            raise RuntimeError(f'Could not retrieve job due to error calling Jupyter Contents API: '
...
def get_or_create_folder(contents_api_url: str, folder_path: str):
    if is_referencing_this_project(contents_api_url):
        ...
        resp = _jobs_common.requests_get(f'{contents_api_url}/{folder_parent}', cookies=cookies)
        if folder_path_as_path.name in [item['name'] for item in resp.json()['content']]:
            return True
        else:
            new_folder_post_resp = _jobs_common.requests_post(contents_api_url,
                                                              data=json.dumps({'type': 'directory'}),
                                                              cookies=cookies)
            new_folder_path = pathlib.PurePosixPath(new_folder_post_resp.json()['path'])
            rename_folder_resp = _jobs_common.requests_patch(f'{contents_api_url}/{new_folder_path}',
...
def _call_schedule_notebook_api(cron_expressions, project_id, file_path, label, user_identity):
...
def _call_unschedule_notebook_api(project_id, file_path, label):
    if label is None:
...
def get_notebook_path():
    ...
    response = _jobs_common.requests_get(url, cookies=cookies)
