...
def test_push_signal_with_replace():
    ...
    push_df = spy.push(numeric_data_df)
...
def test_delete_samples_and_capsules():
    ...
    signal_push_df = spy.push(signal_data, workbook='delete_samples_capsules')
    ...
    condition_push_df = spy.push(condition_data, metadata=condition_metadata, workbook='delete_samples_capsules')
    ...
    spy.push(metadata=pd.concat([signal_push_df, condition_push_df]),
             workbook='delete_samples_capsules',
             replace={
                 'Start': pd.to_datetime('2019-01-01', utc=True),
...
def test_archive():
    ...
    with pytest.raises(ValueError, match='metadata must be provided'):
        spy.push(metadata=pd.DataFrame(), archive=True)
    ...
    push_two = spy.push(metadata=pd.DataFrame([{
        ...
        'Name': 'My Asset 3'
    }]), workbook='test_archive_by_path', archive=True)
...
def test_ignore_properties_on_repeated_push():
    ...
    spy.push(metadata=push_results)
