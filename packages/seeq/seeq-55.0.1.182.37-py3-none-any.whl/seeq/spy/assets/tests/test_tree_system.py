...
start_cleanup_names = {'test_tree_system', 'test_tree_system2', 'Example', 'test_tree_system4', 'test_tree_system5',
                       'test_tree_system6', 'test_tree_system7', 'test_tree_system8', 'root_only_tree', 'My Root',
                       'test_pull_calcs', 'test_tree_system9', 'test_tree_system10'}
end_cleanup_ids = set()
...
def test_double_push_handling_metadata():
    ...
    tree = Tree('test_tree_system10', workbook=workbook)
...
def test_insert_calculation_regex_several_asset_levels_and_regex_glob_combo():
    workbook = 'test_multiple_asset_levels_with_regex'
    tree = Tree('test_tree_system7', workbook=workbook)
    example_calc = spy.search({'Name': 'Area A_Temperature', 'Datasource ID': 'Example Data'})
    example_calc_id = example_calc.iloc[0]['ID']
    ...
    expected_result.append({
        'Name': 'Asset 1',
        'Path': 'test_tree_system7',
        'Type': 'Asset',
        'Push Result': 'Success'
    })
    expected_result.append({
        'Name': 'Asset 2',
        'Path': 'test_tree_system7 >> Asset 1',
        'Type': 'Asset',
        'Push Result': 'Success'
    })
    expected_result.append({
        'Name': 'Asset 3',
        'Path': 'test_tree_system7 >> Asset 1 >> Asset 2',
        'Type': 'Asset',
        'Push Result': 'Success'
    })
    expected_result.append({
        'Name': "Where's Waldo",
        'Path': 'test_tree_system7 >> Asset 1 >> Asset 2 >> Asset 3',
...
def test_insert_calculation_regex_failure_cases():
    workbook = 'failure_cases'
    tree = Tree('test_tree_system6', workbook=workbook)
    example_calc = spy.search({'Name': 'Area A_Temperature', 'Datasource ID': 'Example Data'})
    example_calc_id = example_calc.iloc[0]['ID']
    tree.insert('Buffer')
...
def test_insert_calculation_regex_glob_test():
    workbook = 'test_insert_regex'
    tree = Tree('test_tree_system5', workbook=workbook)
    example_calc = spy.search({'Name': 'Area A_Temperature', 'Datasource ID': 'Example Data'})
    example_calc_id = example_calc.iloc[0]['ID']
    ...
    expected_result.append({
        ...
        'Path': 'test_tree_system5',
        'Type': 'Asset',
        'Push Result': 'Success'
    })
    expected_result.append({
        'Name': 'Test Calculation',
        'Path': 'test_tree_system5 >> Test Insert Regex',
        'Type': 'CalculatedSignal',
        'Formula Parameters': [f'x={example_calc_id}'],
        'Formula': '$x',
        'Push Result': 'Success'
...
def test_insert_formulas_with_complex_formula_parameters():
    workbook = 'test_insert_custom_formula_based_children'
    tree = Tree('test_tree_system4', workbook=workbook)
    ...
    expected = list()
    expected.append({
        'Name': 'test_tree_system4',
        'Path': '',
        'Type': 'Asset'
    })
    expected.append({
        'Name': 'Test Asset',
        'Path': 'test_tree_system4',
        'Type': 'Asset'
    })
    expected.append({
        'Name': 'Test Asset Child',
        'Path': 'test_tree_system4 >> Test Asset',
        'Type': 'Asset'
    })
    expected.append({
        'Name': 'Insert By ID',
        'Path': 'test_tree_system4 >> Test Asset >> Test Asset Child',
        'Type': pd.NA,
...
def test_move_pushed_items():
    tree = Tree('Example', workbook='test_move_pushed_items')
    tree.push()
    ...
    for _, before_row in before_df.iterrows():
        if re.search(r'Area [A-Z]', _tree._get_full_path(before_row)):
...
def test_friendly_name_example_data():
    example_data = spy.search([{'Datasource ID': 'Example Data', 'Name': 'Example', 'Type': 'Asset'},
                               {'Datasource ID': 'Example Data', 'Path': 'Example'}], all_properties=True)
...
def test_csv_creates():
    ...
    csv_df = pd.DataFrame({'Path': [signal1['Path'], signal2['Path']],
                           'ID': [signal1_id, signal2_id],
                           'Name': [signal1['Name'], signal2['Name']]})
    csv_df.to_csv(os.path.join(csv_dir, filename), index=False)

    tree_id = Tree(os.path.join(csv_dir, filename))
...
def test_get_ids_by_name_from_user_input():
    ...
    status = spy.Status()
    results = spy.assets._tree._get_ids_by_name_from_user_input(search_df, status)
...
def test_process_csv_data():
    csv_dir = os.path.join(os.path.dirname(__file__), 'tree_csv_files')
    status = spy.Status()
    results = spy.assets._tree._process_csv_data(os.path.join(csv_dir, 'simplest.csv'), status)
...
def test_double_push():
    ...
    pd.testing.assert_series_equal(search_results.apply(_tree._determine_path, axis=1),
...
def _get_first_id_from_signal_name(name):
    return spy.search(
        pd.DataFrame.from_dict({'Name': [name], 'Type': ['Signal']}),
        order_by=["ID"])['ID'][0]
