...
from seeq.spy._common import Status, EMPTY_GUID
...
def push(data=None, *, metadata=None, replace=None, workbook=_common.DEFAULT_WORKBOOK_PATH,
    ...
    if metadata is not None and not metadata.empty:
        push_result_df = _metadata.push(metadata, workbook_id, datasource_output, sync_token, errors,
    ...
    if data is not None or replace is not None:
        ...
        if data is not None and item_type == 'Signal':
            for column in data:
                try:
                    ...
                    status.add_job(status_index,
                                   (_push_signal, column, signal_metadata, replace, data, signal_output,
            ...
            try:
                ...
                status.add_job(0,
                               (_push_condition, condition_metadata, replace, data, 0, status),
                               _on_success)
            ...
            for status_index, row in push_result_df.iterrows():
                try:
                    if 'StoredSignal' in _common.get(row, 'Type'):
                        ...
                        _put_item_defaults(signal_metadata)
                        status.add_job(status_index,
                                       (_push_signal, None, signal_metadata, replace, None, None,
                                        type_mismatches, status_index, status),
                                       _on_success)
                        ...
                        _put_item_defaults(condition_metadata)
                        status.add_job(status_index,
                                       (_push_condition, condition_metadata, replace, data, status_index, status))
                except BaseException as e:
...
def _push_signal(column, signal_metadata, replace, data, signal_output, type_mismatches, status_index,
                 status: Status):
    signals_api = SignalsApi(_login.client)
    ...
    if replace is not None:
        pd_start, pd_end = _login.validate_start_and_end(replace['Start'],
                                                         replace['End'])
    ...
    if data is not None:
        ...
        for index, sample_value in series.iteritems():
            ...
            if len(add_samples_input.samples) >= _config.options.push_page_size:
                if replace is not None:
...
def _push_condition(condition_metadata, replace, data, status_index, status: Status):
    ...
    if data is not None:
        for index, row in data.iterrows():
            ...
            pd_start, pd_end = _login.validate_start_and_end(row['Capsule Start'],
                                                             row['Capsule End'])
