...
class Report(Annotation):
    ...
    def pull(self, include_images=True):
        ...
        for content in contents.content_items:
            ...
            new_content_definition = {'Name': content.name,
                                      ...
                                      'Asset Selection ID': content.asset_selection.id if new_asset_selection is not
                                                                                          None else None}
    ...
    def _push_specific(self, item_map, datasource_output, label, new_annotation, existing_annotation,
        ...
        for asset_selection_id in existing_annotation.asset_selection_ids:
            existing_asset_selection = AssetSelection.pull(asset_selection_id)
        ...
        for _, asset_selection in self.asset_selections.items():  # type: (_, AssetSelection)
            try:
                asset_selection_output = asset_selection.push(item_map, existing_asset_selections)
                if asset_selection_output.id in asset_selection_ids_to_archive:
                    asset_selection_ids_to_archive.remove(asset_selection_output.id)
            except ApiException as e:
                if item_push_errors is not None and e.status == 400:
                    item_push_errors.add(f'Error processing Asset Selection: {asset_selection}\n{e.body}')
                continue
