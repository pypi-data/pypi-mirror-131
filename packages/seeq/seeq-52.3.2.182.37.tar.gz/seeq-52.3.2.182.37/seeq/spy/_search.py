...
def search(query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True,
    ...
    order_by : {str, list}, default None
        ...
        Fields on which results can be sorted are 'Guid', 'Name', and 'Description'.
...
def _search(query, *, all_properties=False, workbook=_common.DEFAULT_WORKBOOK_PATH, recursive=True,
    ...
    if estimate_sample_period is not None:
        ...
        pd_start, pd_end = _login.validate_start_and_end(estimate_sample_period['Start'],
    ...
    if order_by and not output_df.empty:
        id_order_by = ["ID" if i == "Guid" else i for i in order_by]
        output_df.sort_values(id_order_by, ignore_index=True, inplace=True)
...
def _validate_order_by(order_by):
    ...
    # validate order_by
    _order_fields = ['Guid', 'Name', 'Description']
