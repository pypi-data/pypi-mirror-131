...
from seeq.spy.workbooks._content import Content, DateRange
...
def test_push_repeatedly():
    ...
    push_corp1_df = spy.workbooks.push(workbooks, path=f'{spy.workbooks.CORPORATE} >> test_push_repeatedly_corporate')
    assert push_corp1_df.iloc[0]['ID'] == push_corp1_df.iloc[0]['Pushed Workbook ID'] == workbooks[0].id

    push_corp2_df = spy.workbooks.push(workbooks, path=f'{spy.workbooks.CORPORATE} >> test_push_repeatedly_corporate')
    assert push_corp2_df.iloc[0]['ID'] == push_corp2_df.iloc[0]['Pushed Workbook ID'] == workbooks[0].id

    push_corp3_df = spy.workbooks.push(workbooks, path=f'{spy.workbooks.CORPORATE} >> test_push_repeatedly_corporate')
...
def test_content_selector_not_stripped():
    class AssetForScorecardMetric(Asset):
        ...
        def my_display(self, metadata, analysis):
            analysis.definition['Name'] = 'test_content_display'
            worksheet = analysis.worksheet('test_content_selector_worksheet')
        ...
        def my_document(self, metadata, topic):
            topic.definition['Name'] = 'test_content_selector_document'
            document = topic.document('test_content_selector_document')
    ...
    def _topic_document_search():
        spy_search = spy.search(query={'Name': 'test_content_selector_document'})
    ...
    def _pull_workbooks_and_assert_selector_intact(wb_search_df):
        ...
        workbook = [wb for wb in wbs if wb['Name'] == 'test_content_selector_document'][0]
    ...
    spy.push(metadata=build_df, workbook='test_content_display', worksheet='test_content_selector_worksheet')
...
def test_date_range_condition_not_stripped():
    def _pull_workbook_and_assert_condition_intact(condition_id):
        workbooks_search = spy.workbooks.search(query={'Name': 'test_date_range_condition'})
        workbook = spy.workbooks.pull(workbooks_search)[0]
        worksheet = workbook.worksheets[0]
        assert len(worksheet.date_ranges.values()) == 1
        date_range = list(worksheet.date_ranges.values())[0]
        assert date_range.definition['Condition ID'] == condition_id
        return workbook
    ...
    topic = Topic('test_date_range_condition')
    doc = topic.document('test_daterange_condition_worksheet')
