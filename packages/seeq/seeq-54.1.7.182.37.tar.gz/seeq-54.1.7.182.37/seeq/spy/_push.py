    ...
    if archive:
        ...
        try:
            ...
            if 'does not have MANAGE access for the Datasource' in get_api_exception_message(e):
                status.warn(f'Warning: could not archive stale tree items because user <strong>{_login.user.name} '
                            f'({_login.user.email})</strong> does not have MANAGE permissions for datasource '
