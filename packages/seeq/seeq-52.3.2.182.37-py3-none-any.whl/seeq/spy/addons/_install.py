...
def install(tool, *, include_workbook_parameters=True, update_tool=False, update_permissions=False, errors='raise',
    ...
    try:
        return _install(tool=tool, include_workbook_parameters=include_workbook_parameters, update_tool=update_tool,
...
def _install(tool, *, include_workbook_parameters=True, update_tool=False, update_permissions=False, errors='raise',
    ...
    for status_idx, _tool in enumerate(tools):
        ...
        if update_tool or update_permissions:
            ...
            if tool_id is not None:
                tool_record = addons.search({"ID": tool_id}, quiet=True).to_dict('records')
                if not tool_record:
        ...
        if not update_tool and update_permissions and _tool.get('ID'):
            ...
            set_permissions(new_tool_config, _tool['ID'])
            status.df.at[status_idx, 'Result'] = 'Updated permissions'

        else:
            new_id = None
            try:
                new_id = _install_or_update_tool(new_tool_config, update_tool=update_tool, tool_id=_tool['ID'])
                if update_permissions or _tool['ID'] is None:
                    set_permissions(new_tool_config, new_id)
    ...
    _common.add_properties_to_df(install_result_df, **push_df_properties)

    return install_result_df


def _install_or_update_tool(new_tool_config, update_tool, tool_id):
    system_api = SystemApi(_login.client)
    ...
    if new_tool_config["name"] in tool_names:
        if not update_tool or tool_id is None:
            message = f'Add-on tool with name "{new_tool_config["name"]}" already exists. You can update the existing ' \
                      f'tool with `update_tool=True`'
...
def set_permissions(new_tool_config, tool_id):
    items_api = ItemsApi(_login.client)
    ...
    for group_name in groups:
        group = _login.find_group(group_name, exact_match=True)
        if group:
    ...
    for user_name in users:
        _user = _login.find_user(user_name, exact_match=True)
        if _user:
