...
"""
    if interactive_index is None and not all and not _jobs_common.running_in_executor():
        raise IOError(f'When not running in an executor, an interactive_index '
                      f'must be supplied for spy.jobs.pull unless all=True')
    ...
    if not scheduled_notebooks:
        ...
        raise RuntimeError('There should be only one scheduled notebook')
    ...
    if _schedule.is_referencing_this_project(datalab_notebook_url):
        ...
        resp = _jobs_common.requests_get(f'{data_lab_url}/{project_id}/api/contents/{get_pickle_path}',
                                         cookies=cookies)
...
def load_pickle(pickle_path):
    ...
    if not _schedule.path_exists(full_path):
        raise RuntimeError(f'Schedule DataFrame pickle file not found for path {pickle_path}.')
