...
def get_parameters(jobs_df, interactive_index, status):
    if _jobs_common.running_in_executor():
        ...
        if not (schedule_index in jobs_df.index or int_index in jobs_df.index):
            raise IndexError(f'Cannot execute job with index {schedule_index} because '
                             f'it is not in the index of the DataFrame for the job')
        ...
        if interactive_index is None:
            ...
            raise IndexError(f'Cannot execute job with index {interactive_index} because '
                             f'it is not in the index of the DataFrame for the job')
        else:
