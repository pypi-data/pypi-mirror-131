...
def test_order_by():
    ...
    id_results = spy.search(query, order_by=['Guid'])
    id_list = list(id_results['ID'])
    assert id_list == sorted(id_list)

    # test ID as a string
    id_str_results = spy.search(query, order_by='Guid')
    ...
    results = spy.search(query, order_by=['Guid', 'Name'])
    ...
    name_id_results = spy.search(query, order_by=['Name', 'Guid'])
...
def test_order_by_validation():
    ...
    message = re.escape("Invalid order_by fields: ['Type']. Search results can only be ordered on ['Guid', 'Name', "
                        "'Description'] fields.")
