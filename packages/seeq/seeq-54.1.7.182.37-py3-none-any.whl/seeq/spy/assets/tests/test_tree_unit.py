...
from seeq.spy.assets.tests.test_tree_system import assert_tree_equals_expected
...
def test_formula_params_to_dict_strip():
    ...
    param_id = _common.new_placeholder_guid()
    tree.insert(None, formula='$x', formula_parameters=f'  x    =    {param_id}  ', friendly_name='Spacey')
    expected = list()
    expected.append({
...
def test_formula_params_to_dict():
    pandas_na_input = _tree._formula_parameters_to_dict(pd.NA)
    assert pd.isnull(pandas_na_input)

    none_input = _tree._formula_parameters_to_dict(None)
    assert pd.isnull(none_input)

    empty_string_input = _tree._formula_parameters_to_dict('')
    assert empty_string_input == {}

    empty_list_input = _tree._formula_parameters_to_dict([])
    assert empty_list_input == {}

    empty_dict_input = _tree._formula_parameters_to_dict({})
    assert empty_dict_input == {}

    string_input = _tree._formula_parameters_to_dict('$x=sinusoid()')
    assert string_input == {'$x': 'sinusoid()'}

    series_data = ['$x=sinusoid()', '$y=cosinusoid()']
    series_index = [1, 2]
    series_input = _tree._formula_parameters_to_dict(pd.Series(data=series_data, index=series_index))
    assert series_input == {'$x': 'sinusoid()', '$y': 'cosinusoid()'}

    list_input = _tree._formula_parameters_to_dict(['$x = sinusoid()', '$y=cosinusoid()'])
...
def test_formula_insert():
    ...
    tree.insert(children=None, formula='$x', formula_parameters='x=A1C58783-5951-413F-A584-D73CED2C0191',
                friendly_name='Test Formula Parameters')
    tree.insert(children=None, friendly_name='Test No Formula Parameters', formula='sinusoid()')
    expected = list()
    expected.append({
    ...
    with pytest.raises(Exception, match='Friendly Name must be specified if no children argument is given.'):
        tree.insert(children=None, formula='$x')
...
def test_formula_parameters_to_dict():
    formula_parameters_empty = _tree._formula_parameters_to_dict(None)
    assert pd.isnull(formula_parameters_empty)

    formula_string = 'x=42'
    formula_parameters_string = _tree._formula_parameters_to_dict(formula_string)
    ...
    formula_parameters_list = _tree._formula_parameters_to_dict(formula_list)
    ...
    formula_parameters_empty = _tree._formula_parameters_to_dict(invalid_formula_syntax_empty)
    ...
    with pytest.raises(Exception, match='needs to be in the format \'paramName=inputItem\''):
        _tree._formula_parameters_to_dict(invalid_formula_syntax_no_equals)

    invalid_formula_syntax_double_equals = ['x=1', 'y==2', 'z=3', 'a=4']
    with pytest.raises(Exception, match='needs to be in the format \'paramName=inputItem\''):
        _tree._formula_parameters_to_dict(invalid_formula_syntax_double_equals)
...
def test_validate_properties():
    ...
    _, error_series = _tree._validate(df, stage='input')
    error_list = [
    ...
    assert len(error_list) == len(_tree._property_validations(dict(), 'input'))

    for i in range(len(error_list)):
        row_validation = _tree._property_validations(df.iloc[i], 'input')
    ...
    _, error_series = _tree._validate(df, stage='final')
    final_error_list = [
    ...
    assert len(final_error_list) == len(_tree._property_validations(dict(), 'final'))

    for i in range(len(final_error_list)):
        row_validation = _tree._property_validations(df.iloc[i], 'final')
        assert row_validation[i][1] in error_series.loc[i]
...
def test_validate_calculations():
    ...
    error_summaries, error_series = _tree._validate(df1, stage='final')
    assert len(error_summaries) == 1
    ...
    error_summaries, error_series = _tree._validate(df2, stage='final')
    assert len(error_summaries) == 1
    ...
    error_summaries, error_series = _tree._validate(df3, stage='final')
    assert len(error_summaries) == 2
    assert (msg + 'Root >> Sibling 1') in error_summaries[0]
    assert (msg + 'Root >> Sibling 2 >> Sibling Child') in error_summaries[1]
    assert (msg + 'Root >> Sibling 1') in error_series[1]
    assert (msg + 'Root >> Sibling 2 >> Sibling Child') in error_series[1]
...
def test_fill_column_values():
    ...
    for (re_query, glob_query), output in queries_to_expected_output.items():
        df['Parent'] = df.apply(_tree._fill_column_values, axis=1, query=re_query)
        assert df['Parent'].equals(output)
        df['Parent'] = df.apply(_tree._fill_column_values, axis=1, query=glob_query)
        assert df['Parent'].equals(output)
...
def test_is_column_value_query():
    assert _tree._is_column_value_query('{{Name}}') is True
    assert _tree._is_column_value_query('{{Name}(Area ?)_*}') is True
    assert _tree._is_column_value_query('{{Facility Type} Factory ([a-zA-Z]{1,20})}') is True
    assert _tree._is_column_value_query('{ {Name}(Area ?)_*}') is False
    assert _tree._is_column_value_query('{(Area ?)_*{Name}}') is False
    assert _tree._is_column_value_query('{Name}') is False
    assert _tree._is_column_value_query('Name') is False
...
def test_glob_capture_groups():
    ...
    match = re.match(_tree._glob_with_capture_groups_to_regex(query_asset), item)
    assert match is not None
    assert match[1] == 'Area A'

    match = re.match(_tree._glob_with_capture_groups_to_regex(query_name), item)
    assert match is not None
...
def test_make_paths_from_levels():
    ...
    _tree._make_paths_from_levels(both)
    ...
    with pytest.raises(ValueError):
        _tree._make_paths_from_levels(fail_df)
