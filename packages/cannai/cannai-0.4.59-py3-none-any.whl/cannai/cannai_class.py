...
class Cmodel(Cannaibase):
    ...
    def avr_two_models(self):
        sml_len = len(self.model_list)
        for ii in range(sml_len-1):
            for ii2 in range(ii + 1,sml_len):
                mod1 = self.model_list[ii]
                mod2 = self.model_list[ii2]
                n_mod_name = mod1[0] + "+" + mod2[0]
                mod_val = (mod1[1] + mod2[1]) / 2
                dat = self.set_date()
                new_mod_group = [n_mod_name,mod_val,dat,"No_model",None,None,None]
                if n_mod_name in self.model_name_list_additional:
                    mnlaa = self.model_name_list_additional.index(n_mod_name)
                    self.model_list_additional[mnlaa] = new_mod_group
                else:
                    self.model_list_additional.append(new_mod_group)
                    self.model_name_list_additional.append(n_mod_name)
