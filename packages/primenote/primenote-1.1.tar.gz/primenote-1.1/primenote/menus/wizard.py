...
def isRunning(name: str) -> bool:
    """ Verifies if a process is running (Linux) """
    if sys.platform.startswith("win"):
        return False
    process = QtCore.QProcess()
    process.start("ps -eo comm")
    process.waitForFinished()
    process = str(process.readAllStandardOutput())
    return bool(process.find(name) > -1)
...
class PageLinux(Page):
    def __init__(self, core):
        ...
        trigger = "click" if isRunning("xfce4-panel") else "right click"
        doneLabel = Label(10, wrap=True, text=f"All done ! You may now {trigger} on the tray icon to begin.")
...
class Wizard(QWizard):
    ...
    def _postInstall(self):
        self._legacyPrompt()
        if isRunning("xfce4-panel"):
            # Bind main menu popup to left click as right click is already used by xfce
            self.core.sdb["mouse events"]["tray"]["left"] = ["core", "menu"]
        else:
            tray = self.core.tray
            if tray.isSystemTrayAvailable() and tray.supportsMessages():
                tray.showMessage(APP_NAME, "You may right click on the\ntray icon to open its menu",
                                 tray.Information, 6000)
    ...
    def _legacyPrompt(self):
        ...
        if qtpad.is_dir() and not self.core.sdb["legacy"]:
            ...
            answer = dialog.question(self, "QtPad Notes Migration", prompt, dialog.No | dialog.Yes, dialog.Yes)
