...
@click.option("--depth", "-l", type=int, default=3,
...
@click.argument("path", type=str, required=True)
@add_options([_db_option, _user_rules_option])
def lint(path, database, flintrc):
...
@click.command()
@click.argument("language", type=click.Choice(['fortran', 'python', 'cpp']),
                nargs=-1, required=True)
def config(language):
    """Copy the default rule files locally.
    """
    import shutil
    from flinter.utils import MAP_DEFAULT_RULE_NAMES
    from flinter.utils import _get_default_rule_filename

    copied_filenames = []
    for language_ in language:
        rule_name = MAP_DEFAULT_RULE_NAMES[language_]
        filename = f'{rule_name}.yaml'
        shutil.copy2(
            _get_default_rule_filename(rule_name),
            filename,
        )
