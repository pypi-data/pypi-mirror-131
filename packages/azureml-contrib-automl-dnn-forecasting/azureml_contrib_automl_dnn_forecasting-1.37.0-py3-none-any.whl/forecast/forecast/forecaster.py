...
import logging
from typing import Callable, ContextManager, Dict, List, Mapping, Optional, Sequence, Tuple, Type, Union
...
class BatchTransform(abc.ABC):
    ...
    This transform:
    - transforms a batch of data before it is fed to the DNN for training
    - can reverse the transform on the predictions generated by the DNN
    """
    ...
    def undo_y(self, y: torch.Tensor) -> torch.Tensor:
        ...
        -----------
        y: torch.Tensor
            A y tensor. (For exmaple, this tensor could be a tensor of output predictions from the DNN.)
...
class Forecaster:
    ...
    def __init__(self,
                 ...
                 batch_transform_type: Optional[Type[BatchTransform]] = None):
        """An optional utility for driving a training run of a model.
        ...
        batch_transform_type: Type[BatchTransform], optional
            Type of the batch transform
        """
        ...
        self._batch_transform_type = batch_transform_type
    ...
    def predict(self, dataloader: DataLoader) -> List[np.ndarray]:
        ...
        with torch.no_grad():
            for batch_ind, batch in enumerate(dataloader):
                ...
                batch_transform, batch = self._do_batch_transform(batch)
                ...
                _cuda_synchronize()
                utf_preds = self._get_utf_predictions(batch, predictions, tf, batch_transform)
    ...
    def _train_epoch(self, dataloader: DataLoader, epoch: int) -> float:
        ...
        for batch_ind, batch in enumerate(tqdm(dataloader, desc=f'Epoch {epoch}: ', leave=False, dynamic_ncols=True)):
            ...
            if self._train_metrics:
                ...
                _cuda_synchronize()
                utf_preds = self._get_utf_predictions(batch, results.predictions, tf, batch_transform)
    ...
    def _get_utf_targets(self, batch: Mapping[str, torch.Tensor], tf: AbstractTransform) -> np.ndarray:
    ...
    def _get_utf_predictions(self,
                             batch: Mapping[str, torch.Tensor],
                             predictions: List[torch.Tensor],
                             tf: Optional[AbstractTransform],
                             batch_transform: Optional[BatchTransform]) -> List[np.ndarray]:
        ...
        if batch_transform is not None:
            predictions = [batch_transform.undo_y(pred) for pred in predictions]
    ...
    def train_batch(self,
        ...
        return TorchResults(loss=loss.detach().to(dtype=dtype).item(),
                            predictions=[p.detach().to(dtype=dtype) for p in predictions])
    ...
    def _do_batch_transform(
            self,
            batch: Mapping[str, torch.Tensor]) -> Tuple[Optional[BatchTransform], Dict[str, torch.Tensor]]:
        """Apply the batch transform.

        Parameters
        ----------
        batch: Mapping[str, torch.Tensor]
            A batch of untransformed training data

        Returns
        -------
        Tuple[Optional[BatchTransform], Dict[str, torch.Tensor]]
            The batch transform and the transformed batch

        """
        if self._batch_transform_type is None:
            return None, batch
        batch_transform = self._batch_transform_type()
        transformed_batch = batch_transform.do(batch)
        return batch_transform, transformed_batch
...
class AMPHandler:
    def __init__(self, device: torch.device, *args, **kwargs):
        ...
        if int(tv[0]) < 1 or (int(tv[0]) == 1 and int(tv[1]) < 6):
            ...
            self._enabled = False
                ...
                (device.type == 'cuda' and torch.cuda.get_device_capability(device)[0] < 7)  # 7.X+ have tensor cores
        ):
...
class _DummyGS:
    ...
    def step(self, opt: torch.optim.Optimizer) -> Optional[float]:
        return opt.step()
