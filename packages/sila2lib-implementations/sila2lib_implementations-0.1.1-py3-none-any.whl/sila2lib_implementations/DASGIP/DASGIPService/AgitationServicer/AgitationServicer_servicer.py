...
:file:    AgitationServicer_servicer.py
:authors: Lukas Bromig

:date: (creation)          2020-04-16T10:19:13.375905
:date: (last modification) 2020-04-16T10:19:13.375905
...
from .AgitationServicer_simulation import AgitationServicerSimulation
from .AgitationServicer_real import AgitationServicerReal


class AgitationServicer(AgitationServicer_pb2_grpc.AgitationServicerServicer):
    """
    This is a DASGIP Service
    """
    implementation: Union[AgitationServicerSimulation, AgitationServicerReal]
    ...
    def __init__(self, reactors, simulation_mode: bool = True):
        ...
        if simulation_mode:
            self._inject_implementation(AgitationServicerSimulation(self.reactors))
        else:
            self._inject_implementation(AgitationServicerReal(self.reactors))

    def _inject_implementation(self,
                               implementation: Union[AgitationServicerSimulation,
                                                     AgitationServicerReal]
                               ) -> bool:
        """
    ...
    def GetActuatorPwrPV(self, request, context: grpc.ServicerContext) \
        ...
        logging.debug(
            "GetActuatorPwrPV called in {current_mode} mode".format(
                current_mode=('simulation' if self.simulation_mode else 'real')
            )
        )
    
        try:
            return self.implementation.GetActuatorPwrPV(request, context)
        except SiLAError as err:
            err.raise_rpc_error(context=context)
    ...
    def GetActuatorTStirPV(self, request, context: grpc.ServicerContext) \
        ...
        logging.debug(
            "GetActuatorTStirPV called in {current_mode} mode".format(
                current_mode=('simulation' if self.simulation_mode else 'real')
            )
        )
    
        try:
            return self.implementation.GetActuatorTStirPV(request, context)
        except SiLAError as err:
            err.raise_rpc_error(context=context)
