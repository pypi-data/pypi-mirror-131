...
:date: (creation)          2020-04-16T10:18:59.306961
:date: (last modification) 2020-04-16T10:18:59.306961
        ...
        if not self.lock.locked():
            with acquire_timeout_lock(self.lock, 0.2):
                try:
                    ...
                    try:
                        self.UnitID = int(read[:1])
                        if " CO2_" in read:
                            self.CO2ID = int(read[read.find(" CO2_") + 5:read.find("CO2_") + 9])
                        else:
                            self.CO2ID = DeviceServicer_pb2.GetSensorID_Responses(
                                **default_dict['GetSensorID_Responses']['CO2ID'])

                        if " O2_" in read:
                            self.O2ID = int(read[read.find(" O2_") + 4:read.find(" O2_") + 9])
                        else:
                            self.O2ID = DeviceServicer_pb2.GetSensorID_Responses(
                                **default_dict['GetSensorID_Responses']['O2ID'])

                        if " HUM_" in read:
                            self.HUMID = int(read[read.find(" HUM_") + 5:read.find(" HUM_") + 10])
                        else:
                            self.HUMID = DeviceServicer_pb2.GetSensorID_Responses(
                                **default_dict['GetSensorID_Responses']['HUMID'])

                        if " PRES_" in read:
                            self.PRESID = int(read[read.find(" PRES_") + 6:read.find(" PRES_") + 11])
                        else:
                            self.PRESID = DeviceServicer_pb2.GetSensorID_Responses(
                                **default_dict['GetSensorID_Responses']['UnitID'])
                        par_dict = {
                            'UnitID': silaFW_pb2.Integer(value=self.UnitID),
                            'CO2ID': silaFW_pb2.Integer(value=self.CO2ID),
                            'O2ID': silaFW_pb2.Integer(value=self.O2ID),
                            'HUMID': silaFW_pb2.Integer(value=self.HUMID),
                            'PRESID': silaFW_pb2.Integer(value=self.PRESID)
                        }
                        return_value = DeviceServicer_pb2.GetSensorID_Responses(**par_dict)
                    except Exception:
            ...
            logging.warning('Lock is locked. Passing old values as return.')
            par_dict = {
                'UnitID': silaFW_pb2.Integer(value=self.UnitID),
                'CO2ID': silaFW_pb2.Integer(value=self.CO2ID),
                'O2ID': silaFW_pb2.Integer(value=self.O2ID),
        ...
        if not self.lock.locked():
            with acquire_timeout_lock(self.lock, 0.2):
                try:
                    self.ser.write(str.encode(command))
                    read = str(bytes.decode(self.ser.readline().rstrip()))
                    self.sensor_info = read
                    return_value = DeviceServicer_pb2.GetSensorInfo_Responses(
                        SensorInfo=silaFW_pb2.String(value=self.sensor_info))
                    time.sleep(0.2)
                except Exception:
                    logging.error("Error reading concentration data string")
                    logging.error('Return string from device is empty. Check device status and connection!')
