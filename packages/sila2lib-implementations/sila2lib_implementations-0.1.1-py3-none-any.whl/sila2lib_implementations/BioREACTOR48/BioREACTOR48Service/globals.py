...
def init():
    ...
    global log_buffer
    log_buffer = queue.Queue(maxsize=500)

def start_logging_worker(log_path):
    log_grabber = threading.Thread(target=fill_log_queue, name=('log_grabber_worker'), args=(log_path,),
                                   daemon=True)
    log_grabber.start()

def fill_log_queue(log_path):
    t = Tail(log_path)
    while True:
        line = t.nextline()

        # Check for queue size. If queue size close to queue max size, pop first elements to make space. Use:
        if log_buffer.qsize() >= log_buffer.maxsize - 100 or log_buffer.full():
            for i in range(0, 50, 1):
                if not log_buffer.empty():
                    log_buffer.get_nowait()
        log_buffer.put(line, block=True, timeout=30)
        print(line)

def get_log_from_queue():
    # Before popping from the queue, check if not empty. If empty, sleep for a while.
    print('########################################')
    if not log_buffer.empty():
        line = log_buffer.get_nowait()
        print(f'({log_buffer.qsize()}) Popped line {line}')
        return line
    else:
        sleep(5)
