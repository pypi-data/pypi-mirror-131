...
:date: (creation)          2020-04-16T10:19:35.461170
:date: (last modification) 2020-04-16T10:19:35.461170
    ...
    def GetPHlmax(self, request, context: grpc.ServicerContext) \
        ...
        try:
            for j in range(0, self.TotalBars, 1):
                ...
                par_dict['CurrentPHlmax'].append(silaFW_pb2.Real(value=CurrentPHImax))
            time.sleep(0.2)
            ...
            logging.debug('Executed command GetPHlmax in mode: {mode} with response: {response}'
                          .format(mode='Real', response=par_dict['CurrentPHlmax']))
        except ConnectionError:
            logging.exception(
                'Communication failed executing the command: {command} with bar: {Bar}'.format(command=command, Bar=j))
            time.sleep(0.2)
    ...
    def GetPHlmin(self, request, context: grpc.ServicerContext) \
        ...
        try:
            for j in range(0, self.TotalBars, 1):
                ...
                par_dict['CurrentPHlmin'].append(silaFW_pb2.Real(value=CurrentPHImin))
            time.sleep(0.2)
            ...
            logging.debug('Executed command GetPHlmin in mode: {mode} with response: {response}'
                          .format(mode='Real', response=par_dict['CurrentPHlmin']))
        except ConnectionError:
            logging.exception(
                'Communication failed executing the command: {command} with bar: {Bar}'.format(command=command, Bar=j))
            time.sleep(0.2)
