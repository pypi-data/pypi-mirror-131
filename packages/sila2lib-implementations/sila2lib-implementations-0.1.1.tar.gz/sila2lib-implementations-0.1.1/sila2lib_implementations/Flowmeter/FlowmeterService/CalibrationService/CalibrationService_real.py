...
:date: (creation)          2021-03-19T13:54:01.476087
:date: (last modification) 2021-03-19T13:54:01.476087
    ...
    def SetCalibrationDate(self, request, context: grpc.ServicerContext) \
        ...
        for i in range(0, 3, 1):
            try:
                ...
                logging.debug(f'Read-out: {read}')
                type_answer = default_dict['SetCalibrationDate_Responses']['Status'].value
                response = decode_response_write(protocol=self.protocol, message_str=read, type_answer=type_answer)
                logging.info(f'Response: {response}')
