...
class MySQLHelper(object):
    ...
    def get_table_fields(self, cnx: PooledMySQLConnection = None, table: str = None):
        ...
        feilds = [{'key': f[0], 'type': f[1], 'null': 'NULL' if f[2] == 'YES' else 'NOT NULL'} for f in
                  cursor.fetchall()]
        return feilds

    @auto_close
    def get_table_ddl(self, cnx: PooledMySQLConnection = None, table: str = None):
        """
        数据库小助手-获取表DDL\n
        :param cnx: 数据库连接，装饰器自动赋值
        :param table: 表名
...
if __name__ == '__main__':
    ...
    # print(helper.get_table_ddl(table='student'))
    ...
    # sql添加
    # querykw = {}
    # querykw.setdefault('sql', 'insert into student (name, age) values ("李四", 18)')
    ...
    # ql_list = []
    # ql = QueryDictList()
    # ql.append(QueryDict(key='name', value='王五'))
    # ql.append(QueryDict(key='age', value=20))
    # ql_list.append(ql)
    # ql = QueryDictList()
    # ql.append(QueryDict(key='name', value='赵六'))
    # ql.append(QueryDict(key='age', value=23))
    # ql_list.append(ql)
