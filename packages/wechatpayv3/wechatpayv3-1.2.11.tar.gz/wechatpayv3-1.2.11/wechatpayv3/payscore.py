...
def payscore_create(self, out_order_no, service_id, service_introduction, time_range,
    ...
    :param attach: 商户数据包，户数据包可存放本订单所需信息，需要先urlencode后传入。当商户数据包总长度超出256字符时，报错处理。示例值：'Easdfowealsdkjfnlaksjdlfkwqoi&wl3l2sald'
    ...
    if attach:
        ...
        raise Exception('attach is not assigned.')
    if post_payments:
        params.update({'post_payments': post_payments})
    if post_discounts:
        params.update({'post_discounts': post_discounts})
