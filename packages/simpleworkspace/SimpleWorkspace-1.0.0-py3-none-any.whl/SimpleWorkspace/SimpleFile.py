...
from SimpleWorkspace.SimpleFlow import levelprint, levelinput
from SimpleWorkspace.SimpleConversion import ByteConversion
import queue
import re


def RequireModules(modulesList):
    import sys
    import importlib
    import pkg_resources
    import subprocess

    required = modulesList
...
def GetAllFilesInDirectory(searchDir: str, callback: Callable[[str], None] = None, includeDirs=True, fileIncludeFilter=None, satisfiedCondition: Callable[[str], bool] = None):
        ...
        try:
            ...
            for filePath in currentFiles:
                ...
                if os.path.isfile(filePath):
                    if fileIncludeFilter != None and re.match(fileIncludeFilter, filePath):
                        continue
                    callback(filePath)
...
def Directory_ListFiles(searchDir: str, callback: Callable[[str], None] = None, includeDirs=True, fileIncludeFilter=None, satisfiedCondition: Callable[[str], bool] = None):
    if not os.path.exists(searchDir):
        return
...
def File_Read(filePath, callback: Callable[[str], None] = None, readSize=-1, readLimit=-1, getBytes=True):
    """
...
def Path_FindEmptySpot(filepath):
...
def File_Create(filepath, data=None):
    openMode = "wb"
    if type(data) is str:
        openMode = "w"
    with open(filepath, openMode) as file:
