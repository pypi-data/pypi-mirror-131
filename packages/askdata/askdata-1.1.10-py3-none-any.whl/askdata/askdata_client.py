...
class Askdata(SignUp):
    ...
    def load_dataset(self, workspace_slug, dataset_slug):
        all_fields = self.get_columns(workspace_slug, dataset_slug)["code"]
        limit = 1000
        df = self.query(workspace_slug, dataset_slug, all_fields, limit=limit)
        if(len(df.index) < 1000):
            return df
        else:
            final_df = df
            while(len(df.index) == 1000):
                df = self.query(workspace_slug, dataset_slug, all_fields, limit=(limit+1000), offset=limit)
    ...
    def query(self, workspace='', dataset='', fields:list=[], filters=None, sorted_by=None, limit=50, offset=0, pivot=[]):
