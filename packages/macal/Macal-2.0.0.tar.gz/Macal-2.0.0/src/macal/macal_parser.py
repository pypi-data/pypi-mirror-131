...
from .macal_parsernodes import (ast_Variable, ast_list_Index, ast_function_Param_list,
                              ast_Function_definition, ast_Assign, ast_Block, ast_Foreach,
                              ast_Halt, ast_Break, ast_Call_function, ast_Return)
...
class PParser:
    ...
    def _primary(self):
        result = None
        if self._match(BOOL, STRING, INT, FLOAT):
            result = Expr().literal(ExpressionItem(self._previous().value, self._previous().type))
        elif self._match(NIL):
            result = Expr().literal(ExpressionItem(None, NIL))
            ...
            result = Expr().literal(ExpressionItem([], ARRAY))
        if self._match_kw(KW_RECORD):
            result = Expr().literal(ExpressionItem(dict(), RECORD))
        if result is None:
    ...
    def _parse_variable_array_index(self, node: ast_Variable):
        while self._check(LINDEX):
            ...
            if self._check(RINDEX):
                expr = Expr().literal(ExpressionItem(NEW_ARRAY_INDEX, ARRAY))
            else:
                expr = self._expression()
