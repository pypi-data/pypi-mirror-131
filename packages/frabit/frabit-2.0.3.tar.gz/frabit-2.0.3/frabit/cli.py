...
def server_completer(prefix, parsed_args, **kwargs):
    ...
    for conf in frabit.__config__.servers():
...
@arg('--reuse-backup', nargs='?',
     choices=frabit.config.REUSE_BACKUP_VALUES,
...
def global_config(args):
    ...
    if hasattr(args, 'config'):
        ...
        try:
            ...
            filename = None
    config = frabit.config.Config(filename)
    frabit.__config__ = config
    ...
    configure_logging(config.log_file,
                      log_level or frabit.config.DEFAULT_LOG_LEVEL,
                      config.log_format)
    ...
    _logger.debug('Initialised Barman version %s (config: %s, args: %s)',
                  frabit.__version__, config.config_file, pretty_args(args))
