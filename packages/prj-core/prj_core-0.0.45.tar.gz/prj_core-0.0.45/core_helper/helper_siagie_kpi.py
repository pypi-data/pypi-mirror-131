...
def get_kpi_foraneo(df,anio_df=None, anio_h=None ,t_anios=1,modalidad="EBR"):
...
def generar_kpis_agg_servicios(df,anio_df=None , anio_h =None ,t_anios=1,modalidad="EBR"):

    ultimo_anio, num = gestionar_errores_filtro_kpi(anio_df,anio_h,t_anios)

    df_serv = df[["COD_MOD","ANEXO"]].copy()
    ...
    for anio in range(anio_h,ultimo_anio,-1):
        ...
        **dic_valores        
        ...
        df_agg_serv['RATIO_DESAPROBADOS'] = df_agg_serv['TOTAL_DESAPROBADO']/df_agg_serv['TOTAL_ESTUDIANTES']
        df_agg_serv['RATIO_RETIRADO'] = df_agg_serv['TOTAL_RETIRADO']/df_agg_serv['TOTAL_ESTUDIANTES']
...
def generar_kpis_traslado(df,anio_df=None , anio_h =None ,t_anios=1,modalidad="EBR"):
...
def generar_kpis_traslado_a_publico(df,anio_df=None, anio_h =None ,t_anios=1,df_servicios=None,modalidad="EBR"):
...
def generar_kpis_desercion(df,anio_df=None, anio_h=None ,t_anios=1,modalidad="EBR"):
